/**
 * Language server discovery for the built-in `lsp` tool (ported from the
 * Electron adapter to the standalone server runtime).
 */
...
function registerDefaultLanguageServers(): void {
  if (!registry.has("typescript")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("typescript-language-server", ["--stdio"]),
    ...
    registerLanguageServer({
      ...
      resolveCommand: async () => {
        ...
        for (const candidate of candidates) {
          ...
          if (found) return { command: candidate.binary, args: candidate.args };
        }
        return null;
      },
  ...
  if (!registry.has("rust")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("rust-analyzer"),
  ...
  if (!registry.has("go")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("gopls"),
  ...
  if (!registry.has("clangd")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("clangd"),
      languageIdForFile: (filePath) => (normalizeExtension(filePath).endsWith(".c") ? "c" : "cpp")
    });
  }
  ...
  if (!registry.has("json")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("vscode-json-language-server", ["--stdio"]),
      languageIdForFile: (filePath) => (normalizeExtension(filePath).endsWith(".jsonc") ? "jsonc" : "json")
    });
  }
  ...
  if (!registry.has("yaml")) {
    registerLanguageServer({
      ...
      resolveCommand: async () => resolvePathOnly("yaml-language-server", ["--stdio"]),
