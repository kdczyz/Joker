/**
 * Minimal LSP client for the built-in `lsp` AI tool. Ported from the Electron
 * adapter to the standalone server runtime (no LocalToolHost dependency).
 *
 * Speaks JSON-RPC 2.0 over stdio to a language server process. Query-only:
 * initialize, didOpen, and a handful of textDocument/ workspace/ requests.
 *
 * Sessions are keyed by workspace root + server key and reference-counted;
 * when the last caller releases, the server stays alive briefly
 * (CLEANUP_DELAY) before being killed, so back-to-back tool calls don't
 * respawn the server.
 */

import { spawn, type ChildProcess } from "node:child_process";
import { pathToFileURL } from "node:url";
import { getLanguageServer, resolveServerCommand } from "./servers.js";
...
/**
 * Map<workspaceRoot::serverKey, Promise<LspSession>>.
 * Storing the Promise (not the resolved session) prevents a race where two
 * concurrent acquireLspSession calls both spawn their own server.
 */
...
function configurationValueForSection(session: LspSession, section: string): Record<string, unknown> {
  switch (section) {
    ...
    case "python.analysis":
      return { diagnosticMode: "openFilesOnly", typeCheckingMode: "basic" };
    case "basedpyright":
      return { disableLanguageServices: false, disablePullDiagnostics: false };
  ...
  switch (method) {
    ...
    case "workspace/workspaceFolders": {
      sendMessage(session, {
        jsonrpc: "2.0",
        id,
        result: [{ uri: pathToFileURL(session.workspaceRoot).href, name: session.workspaceRoot }]
      });
      return;
    }
    case "client/registerCapability": {
      const registrations = msg.params && typeof msg.params === "object"
        ...
        : null;
      const diagnosticRegistration = registrations?.find((registration) => registration.method === "textDocument/diagnostic");
      ...
      sendMessage(session, { jsonrpc: "2.0", id, result: null });
      return;
    }
...
function processBuffer(session: LspSession): void {
  ...
  while (true) {
    ...
    if (session.stdoutBuffer.length < bodyStart + contentLength) return;
  ...
  session.initPromise = (async () => {
    await sendRequest(session, "initialize", {
      ...
      initializationOptions: session.serverKey === "python"
        ? { diagnosticMode: "openFilesOnly", disablePullDiagnostics: false }
        : undefined,
      workspaceFolders: [
        { uri: pathToFileURL(session.workspaceRoot).href, name: session.workspaceRoot }
      ]
    }).catch((err) => {
...
export async function acquireLspSession(workspaceRoot: string, serverKey: string): Promise<LspSession> {
  ...
  if (!server) throw new Error(`Unknown language server: ${serverKey}`);
  const key = sessionKey(workspaceRoot, serverKey);
...
/** Kill all active LSP sessions; call on process shutdown. */
...
export async function lspOpenDocument(
  ...
  sendNotification(session, "textDocument/didOpen", {
    textDocument: { uri: filePathToUri(filePath), languageId, version: 1, text: content }
  });
}
...
export async function lspDefinition(session: LspSession, filePath: string, line: number, character: number): Promise<unknown> {
...
export async function lspReferences(session: LspSession, filePath: string, line: number, character: number): Promise<unknown> {
...
export async function lspHover(session: LspSession, filePath: string, line: number, character: number): Promise<unknown> {
...
export async function lspImplementation(session: LspSession, filePath: string, line: number, character: number): Promise<unknown> {
...
export async function lspGetDiagnostics(session: LspSession, filePath: string): Promise<DiagnosticsFetchResult> {
  const cached = session.diagnostics.get(filePathToUri(filePath)) ?? [];
  if (cached.length > 0) {
    return { diagnostics: cached, source: "publishDiagnostics-cache" };
  }
  ...
  try {
    ...
    if (pulled.length > 0) {
      return { diagnostics: pulled, source: "textDocument/diagnostic" };
    }
  } catch {
  ...
  try {
    ...
    if (workspaceDiagnostics.length > 0) {
      return { diagnostics: workspaceDiagnostics, source: "textDocument/diagnostic" };
    }
  } catch {
...
/** Synchronous last-resort cleanup on process exit (SIGKILL, no grace period). */
