...
import {
  JokerRuntimeSupervisor,
  MAX_RESTART_DELAY_MS,
  RestartBudget,
  type JokerRuntimeStatus
} from './Joker-runtime-supervisor'
...
describe('JokerRuntimeSupervisor', () => {
  function harness(overrides: {
    healthy?: boolean
    restartError?: Error
    stopped?: boolean
  } = {}) {
    const statuses: JokerRuntimeStatus[] = []
    ...
    const deps = {
      ...
      publish: (status: JokerRuntimeStatus) => { statuses.push(status) },
      warn: () => undefined,
      error: () => undefined,
      sleep: async () => undefined
    }
    const supervisor = new JokerRuntimeSupervisor({
      deps,
