...
export function resolveJokerConfigPath(): string {
  return join(homedir(), '.Joker', 'config.toml')
}

export function resolveDeepseekConfigPath(): string {
  return resolveJokerConfigPath()
}

export function resolveJokerMcpJsonPath(): string {
  return join(homedir(), '.Joker', 'mcp.json')
}
...
async function readJsonFile(path: string): Promise<unknown | null> {
  ...
  try {
    ...
    throw new Error(`Failed to parse Joker MCP config at ${path}: ${message}`, { cause: error })
  }
}
...
export async function syncClawScheduleMcpConfig(
  ...
  const configTomlPath = paths.configTomlPath ?? resolveJokerConfigPath()
  const mcpJsonPath = paths.mcpJsonPath ?? resolveJokerMcpJsonPath()
