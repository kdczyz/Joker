...
function createSettings(binaryPath: string): AppSettingsV1 {
  return {
    ...
    agents: {
      Rcode: {
        ...defaultRcodeRuntimeSettings(testRcodePort),
        binaryPath,
        autoStart: true
      }
    },
...
async function readRcodeLog(): Promise<string> {
  if (!tempRoot) throw new Error('temp root not initialized')
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const logFile = readdirSync(tempRoot).find((entry) => entry.startsWith('Rcode-') && entry.endsWith('.log'))
    if (logFile) return readFileSync(join(tempRoot, logFile), 'utf8')
    await new Promise((resolve) => setTimeout(resolve, 25))
  }
  throw new Error('Expected a Rcode log file to be created')
}
...
beforeEach(async () => {
  tempRoot = mkdtempSync(join(tmpdir(), 'Rcode-process-'))
  testRcodePort = await allocateTestPort()
  configureLogger({ dir: tempRoot, enabled: true, retentionDays: 7 })
})

afterEach(async () => {
  const module = await import('./Rcode-process')
  await module.stopRcodeChildAndWait()
...
describe('startRcodeChild', () => {
  it('waits for the explicit Rcode ready marker before resolving', async () => {
    const script = writeScript(
      'ready-child.js',
      [
        "const http = require('node:http')",
        `const port = ${testRcodePort}`,
        "const server = http.createServer((req, res) => {",
        "  res.setHeader('content-type', 'application/json')",
        "  res.end(JSON.stringify({ service: 'Rcode', mode: 'serve', status: 'ok' }))",
        "})",
        "server.listen(port, '127.0.0.1', () => {",
        "  setTimeout(() => {",
        "    process.stdout.write('RCODE_READY ' + JSON.stringify({ service: 'Rcode', mode: 'serve', port }) + '\\n')",
        "  }, 50)",
        "})",
        "setInterval(() => {}, 1_000)"
      ].join('\n')
    ...
    const module = await import('./Rcode-process')
    await expect(module.startRcodeChild(createSettings(script))).resolves.toBeUndefined()
    expect(module.isRcodeChildRunning()).toBe(true)
    await module.stopRcodeChildAndWait()
    const logText = await readRcodeLog()
    expect(logText).toContain('RCODE_READY')
    expect(logText).toContain(`ready marker received on port ${testRcodePort}`)
  })
  ...
  it('does not settle on the ready marker until the /health endpoint responds', async () => {
    ...
    const module = await import('./Rcode-process')
    let resolved = false
    const start = module.startRcodeChild(createSettings(script)).then(() => {
      resolved = true
    })
  ...
  it('shares the startup promise while Rcode is spawned but not ready', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    ...
    const first = module.startRcodeChild(settings)

    for (let attempt = 0; attempt < 100 && !module.isRcodeChildRunning(); attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10))
    }
    expect(module.isRcodeChildRunning()).toBe(true)

    let secondResolved = false
    const second = module.startRcodeChild(settings).then(() => {
      secondResolved = true
    })
  ...
  it('rejects when the child exits before reporting ready', async () => {
    ...
    const module = await import('./Rcode-process')
    await expect(module.startRcodeChild(createSettings(script))).rejects.toThrow(
      /Rcode exited during startup with code 23[\s\S]*bind failed on port 18899/
    )
    expect(module.isRcodeChildRunning()).toBe(false)
    await module.stopRcodeChildAndWait()
    const logText = await readRcodeLog()
...
describe('resolveRcodeStartupTimeoutMs', () => {
  it('gives Windows the larger default and other platforms a smaller one', async () => {
    const { resolveRcodeStartupTimeoutMs } = await import('./Rcode-process')
    expect(resolveRcodeStartupTimeoutMs('win32', {})).toBe(90_000)
    expect(resolveRcodeStartupTimeoutMs('darwin', {})).toBe(60_000)
    expect(resolveRcodeStartupTimeoutMs('linux', {})).toBe(60_000)
  })

  it('honors a valid RCODE_STARTUP_TIMEOUT_MS override on every platform', async () => {
    const { resolveRcodeStartupTimeoutMs } = await import('./Rcode-process')
    expect(resolveRcodeStartupTimeoutMs('win32', { RCODE_STARTUP_TIMEOUT_MS: '120000' })).toBe(120_000)
    expect(resolveRcodeStartupTimeoutMs('linux', { RCODE_STARTUP_TIMEOUT_MS: ' 30000 ' })).toBe(30_000)
  })
  ...
  it('clamps an out-of-range override to the 15s–10min bounds', async () => {
    ...
    expect(resolveRcodeStartupTimeoutMs('linux', { RCODE_STARTUP_TIMEOUT_MS: '99999999' })).toBe(600_000)
  })

  it('falls back to the platform default when the override is not a finite number', async () => {
    const { resolveRcodeStartupTimeoutMs } = await import('./Rcode-process')
    expect(resolveRcodeStartupTimeoutMs('win32', { RCODE_STARTUP_TIMEOUT_MS: 'soon' })).toBe(90_000)
    expect(resolveRcodeStartupTimeoutMs('darwin', { RCODE_STARTUP_TIMEOUT_MS: '' })).toBe(60_000)
    expect(resolveRcodeStartupTimeoutMs('darwin', { RCODE_STARTUP_TIMEOUT_MS: '   ' })).toBe(60_000)
  })
})

describe('waitForRcodeStartupSettled', () => {
  it('resolves immediately when no launch is in flight', async () => {
    const module = await import('./Rcode-process')
    let resolved = false
    await Promise.race([
      module.waitForRcodeStartupSettled().then(() => {
        resolved = true
      }),
  ...
  it('does not resolve until an in-flight launch settles', async () => {
    ...
    const module = await import('./Rcode-process')
    const settings = createSettings(script)
    const start = module.startRcodeChild(settings)
    ...
    let settled = false
    const settledPromise = module.waitForRcodeStartupSettled().then(() => {
      settled = true
    })
    await new Promise((resolve) => setTimeout(resolve, 50))
...
describe('reclaimRcodePort', () => {
  it('reports a port as unavailable when another listener owns it', async () => {
    ...
    try {
      ...
      await expect(module.reclaimRcodePort(address.port)).resolves.toEqual({
        ok: false,
  ...
  it('allows non-positive ports so Rcode can request an ephemeral port', async () => {
    const module = await import('./Rcode-process')

    await expect(module.reclaimRcodePort(0)).resolves.toEqual({ ok: true })
  })
  ...
  it('resolves the next available fallback port when the preferred port is unavailable', async () => {
    ...
    try {
      ...
      const resolved = await module.resolveAvailableRcodePort(preferredPort)
      ...
      await expect(module.reclaimRcodePort(resolved.port)).resolves.toEqual({ ok: true })
    } finally {
      await new Promise<void>((resolve) => server.close(() => resolve()))
    }
  })

  it('keeps the configured endpoint when the currently managed Rcode child owns it', async () => {
    ...
    const module = await import('./Rcode-process')
    const settings = createSettings(script)
    settings.agents.Rcode.port = preferredPort

    await module.startRcodeChild(settings)
    const resolved = await module.resolveAvailableRcodePort(preferredPort)

    expect(resolved).toEqual({ port: preferredPort, changed: false })
    expect(module.isRcodeChildRunning()).toBe(true)
    expect(await readRcodeLog()).not.toContain(`killing stale Rcode process holding port ${preferredPort}`)
  })
})

describe('resolveRcodeDataDir', () => {
  it('expands Windows-style home-relative data directories', async () => {
    const module = await import('./Rcode-process')

    expect(module.resolveRcodeDataDir({ dataDir: '~\\deepseek\\Rcode' })).toBe(join(homedir(), 'deepseek', 'Rcode'))
  })

  it('does not expand non-home tilde prefixes', async () => {
    const module = await import('./Rcode-process')

    expect(module.resolveRcodeDataDir({ dataDir: '~other\\Rcode' })).toBe('~other\\Rcode')
  })
})
...
describe('parseListeningPidsFromNetstat', () => {
  it('extracts the listening TCP PIDs for the port across IPv4/IPv6, ignoring everything else', async () => {
    const { parseListeningPidsFromNetstat } = await import('./Rcode-process')
...
describe('syncGuiManagedRcodeConfig', () => {
  it('creates GUI-managed config with attachments enabled for image paste/upload', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Rcode-process')

    await module.syncGuiManagedRcodeConfig(tempRoot, defaultRcodeRuntimeSettings())
  ...
  it('exports per-model max output tokens into Rcode model profiles', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Rcode-process')

    await module.syncGuiManagedRcodeConfig(tempRoot, {
      ...defaultRcodeRuntimeSettings(),
      modelProfiles: {
        writer: {
    ...
    const parsed = JSON.parse(readFileSync(configPath, 'utf8')) as any
    expect(RcodeConfigSchema.safeParse(parsed).success).toBe(true)
  ...
  it('writes the selected provider endpoint into the default model client config', async () => {
    ...
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Rcode-process')
    const settings = createSettings('/tmp/fake-Rcode-child.js')
    ...
    await module.syncGuiManagedRcodeConfig(tempRoot, resolveRcodeRuntimeSettings(settings), {
      scheduleMcp: {
        settings,
        launch: {
  ...
  it('writes the image generation capability and omits cleared fields', async () => {
    ...
    await module.syncGuiManagedRcodeConfig(tempRoot, runtime)
  ...
  it('keeps the config stable across repeated syncs with imageGen configured', async () => {
    ...
    // every sync rewrites the file and restarts Rcode in a loop.
    await module.syncGuiManagedRcodeConfig(tempRoot, runtime)
  ...
  it('adds the built-in schedule MCP server to Rcode runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Rcode-process')
    const settings = createSettings('/tmp/fake-Rcode-child.js')
  ...
  it('adds GUI project and configured global skill roots to Rcode runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Rcode-process')
    const settings = createSettings('/tmp/fake-Rcode-child.js')
  ...
  it('writes GUI-managed MCP search settings without removing existing servers', async () => {
    ...
    await module.syncGuiManagedRcodeConfig(
      tempRoot,
      {
        ...defaultRcodeRuntimeSettings(),
        storage: {
          backend: 'hybrid',
          sqlitePath: '/tmp/Rcode-index.sqlite3'
        },
        contextCompaction: {
  ...
  it('does not auto-import repo-local .Rcode/mcp.json servers into the runtime', async () => {
    ...
    mkdirSync(join(repo, '.Rcode'), { recursive: true })
    writeFileSync(join(repo, '.Rcode', 'mcp.json'), JSON.stringify({
      servers: {
  ...
  it('replaces unparsable historical Rcode config with a valid GUI-managed config', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
  ...
  it('does not enable MCP when the capability is explicitly disabled', async () => {
    ...
    await module.syncGuiManagedRcodeConfig(tempRoot, defaultRcodeRuntimeSettings(), {
      scheduleMcp: {
        settings: createSettings('/tmp/fake-Rcode-child.js'),
        launch: {
          appPath: '/tmp/deepseek-gui-test-app',
          execPath: '/tmp/electron',
          isPackaged: false
