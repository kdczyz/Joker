...
describe('desktop behavior', () => {
  ...
  it('starts hidden only for Windows login launch with the hidden arg', async () => {
    ...
    expect(shouldStartHidden(startupSettings, 'win32', ['Joker', HIDDEN_START_ARG])).toBe(true)
    expect(shouldStartHidden(startupSettings, 'win32', ['Joker'])).toBe(false)
    expect(shouldStartHidden(startupSettings, 'darwin', ['Joker', HIDDEN_START_ARG])).toBe(false)
    expect(shouldStartHidden(settings({ openAtLogin: true }), 'win32', ['Joker', HIDDEN_START_ARG])).toBe(false)
  })
