...
import { JokerRuntimeAdapter } from './runtime/Joker-adapter'
import { JsonSettingsStore } from './settings-store'

describe('Joker single-agent regression', () => {
  it('seeds provider credentials and Joker port from legacy local HTTP settings', () => {
  ...
  it('does not carry legacy local-runtime binary paths into Joker', () => {
    const migrated = migrateLegacyAppSettings({
      version: 1,
  ...
  it('does not keep the legacy default local HTTP port for Joker', () => {
    const migrated = migrateLegacyAppSettings({
      version: 1,
      agentProvider: 'codewhale',
      agents: {
  ...
  it('seeds provider credentials and Joker model from legacy reasoning settings', () => {
    const migrated = migrateLegacyAppSettings({
      version: 1,
  ...
  it('Joker adapter reports base url and id', () => {
    ...
    chatContentMaxWidthPx: 896,
      ...
      agents: {
        Joker: defaultJokerRuntimeSettings(19000)
      },
      workspaceRoot: '/tmp',
    ...
    expect(JokerRuntimeAdapter.id).toBe('Joker')
    expect(JokerRuntimeAdapter.getBaseUrl(settings)).toBe('http://127.0.0.1:19000')
  })

  it('JsonSettingsStore saves only Joker after legacy settings migration', async () => {
    ...
    expect(loaded.agents).toEqual({
      Joker: expect.objectContaining({ port: 18787 })
    })
