/**
 * Crash-loop budget and status contract for the GUI-managed Joker
...
export type JokerRuntimeStatus = JokerRuntimeStatusPayload
...
export type JokerRuntimeSupervisorDeps<Settings> = {
  ...
  isStopped: () => boolean
  publish: (status: JokerRuntimeStatus) => void
...
export class JokerRuntimeSupervisor<Settings> {
  ...
  private readonly deps: JokerRuntimeSupervisorDeps<Settings>
  private watchdogTimer: ReturnType<typeof setInterval> | null = null
  private watchdogFailures = 0
  private watchdogTickInFlight = false
  private crashRecoveryInFlight = false
  private currentStatus: JokerRuntimeStatus | null = null

  constructor(options: {
    deps: JokerRuntimeSupervisorDeps<Settings>
  ...
  get lastStatus(): JokerRuntimeStatus | null {
  ...
  publish(status: Omit<JokerRuntimeStatus, 'at'>): void {
    const full: JokerRuntimeStatus = { ...status, at: new Date().toISOString() }
  ...
  handleUnexpectedExit(info: { code: number | null; signal: NodeJS.Signals | null; stderrTail: string }): void {
    void this.recoverFromCrash(info).catch((error: unknown) => {
      this.deps.error('Joker-supervisor', 'supervised restart crashed', {
  ...
  startWatchdog(): void {
    ...
    const timer = schedule(() => {
      void this.watchdogTick().catch((error: unknown) => {
        this.deps.warn('Joker-watchdog', 'watchdog tick failed', {
          message: error instanceof Error ? error.message : String(error)
        })
      })
  ...
  async watchdogTick(): Promise<void> {
    ...
    try {
      ...
      this.publish({
        state: 'restarting',
        source: 'watchdog',
        message: 'Joker stopped responding to health checks; restarting it.'
      })
      try {
        ...
        this.publish({
          state: 'failed',
          source: 'watchdog',
          message: `Joker is unresponsive and the automatic restart failed: ${
  ...
  private async recoverFromCrash(info: {
    ...
    this.publish({
      state: 'crashed',
      source: 'supervisor',
      message: `Joker exited unexpectedly (${exitLabel}).`,
    ...
    try {
      ...
      if (!this.deps.canAutoRestart(settings)) {
        this.publish({
          state: 'stopped',
          source: 'supervisor',
          message: 'Joker exited and automatic restart is unavailable (missing API key or auto-start disabled).'
        })
        return
      }
      let lastError = ''
      for (;;) {
        ...
        if (!verdict.allowed) {
          this.publish({
            ...
            source: 'supervisor',
            message: lastError
              ? `Joker keeps crashing; automatic restarts are paused. Last error: ${lastError}`
              : 'Joker keeps crashing; automatic restarts are paused. Check the runtime logs, then retry.',
            stderrTail: info.stderrTail
          })
          return
        }
        this.publish({
          ...
          maxAttempts: 3,
          message: `Restarting Joker automatically (attempt ${verdict.attempt}/3).`
        })
