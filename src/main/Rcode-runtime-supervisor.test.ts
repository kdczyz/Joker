...
import {
  RcodeRuntimeSupervisor,
  MAX_RESTART_DELAY_MS,
  RestartBudget,
  type RcodeRuntimeStatus
} from './Rcode-runtime-supervisor'
...
describe('RcodeRuntimeSupervisor', () => {
  function harness(overrides: {
    healthy?: boolean
    restartError?: Error
    stopped?: boolean
  } = {}) {
    const statuses: RcodeRuntimeStatus[] = []
    ...
    const deps = {
      ...
      publish: (status: RcodeRuntimeStatus) => { statuses.push(status) },
      warn: () => undefined,
      error: () => undefined,
      sleep: async () => undefined
    }
    const supervisor = new RcodeRuntimeSupervisor({
      deps,
