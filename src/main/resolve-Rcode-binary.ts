...
/**
 * Resolve the Rcode executable. Rcode ships as a TypeScript
 * package inside the Rcode workspace (`Rcode/`) and is
 ...
 * 2. Bundled `Rcode/dist/cli/serve-entry.js` (built by the root
 *    `build:Rcode` script before dev, build, install, and packaging).
 *
...
export function resolveRcodeExecutable(
  appRoot: string,
  userBinaryPath: string
): RcodeBinaryResolution {
...
/**
 * Build the full `Rcode serve` argv from resolved binary info
 * and Rcode runtime settings. The function is pure: no I/O, no
 * side effects, easy to test.
 */
export function buildRcodeServeArgs(input: {
  resolution: RcodeBinaryResolution
  host: string
  port: number
  dataDir: string
