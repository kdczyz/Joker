...
type NodeOutcome = {
  ...
  branch?: string
  /** For ai-agent nodes: the Joker thread created. */
  threadId?: string
}
...
export class WorkflowRuntime {
  ...
  private async handleWebhookRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (secret) {
        const rawHeader = req.headers['x-Joker-secret']
  ...
  private async handleInternalRequest(
    ...
    if (pathname === '/workflow/internal/hook-run') {
      // The hook payload (the Joker invocation) is the workflow input; nodes read it via {{json.*}}.
  ...
  /** Run a workflow on behalf of the Joker agent tool: resolve by id/name, await it, return its output. */
  ...
  /**
   * Run a workflow triggered by a Joker agent hook. Resolves by id (no callableByAgent
