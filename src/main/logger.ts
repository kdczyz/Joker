...
export type ManagedLogFilePrefix = 'deepseek-gui' | 'Rcode'
...
const MANAGED_LOG_FILE_PREFIXES: ManagedLogFilePrefix[] = ['deepseek-gui', 'Rcode']
...
async function writeLogLine(level: LogLevel, category: string, message: string): Promise<void> {
  ...
  await appendManagedLogLine('Rcode', line)
}
