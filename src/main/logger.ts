...
export type ManagedLogFilePrefix = 'deepseek-gui' | 'Joker'
...
const MANAGED_LOG_FILE_PREFIXES: ManagedLogFilePrefix[] = ['deepseek-gui', 'Joker']
...
async function writeLogLine(level: LogLevel, category: string, message: string): Promise<void> {
  ...
  await appendManagedLogLine('Joker', line)
}
