...
import {
  RcodeConfigSchema,
  type RcodeConfig,
  RcodeServeConfigSchema,
  ModelConfigSchema,
  ContextCompactionConfigSchema,
  QualityConfigSchema,
  RuntimeTuningConfigSchema,
  RolesConfigSchema
} from '../../Rcode/src/config/Rcode-config.js'
import { HooksConfigSchema } from '../../Rcode/src/hooks/hook-config.js'
import {
  ...
  WebCapabilityConfig
} from '../../Rcode/src/contracts/capabilities.js'
import {
  resolveClawScheduleMcpCommand,
  resolveRcodeMcpJsonPath,
  type ClawScheduleMcpLaunchConfig
} from './claw-schedule-mcp-config'
import { defaultRcodeDataDir } from './runtime/Rcode-adapter'
import { resolveClaudeBinary } from './agent-sdk-installer'
import { appendManagedLogLine } from './logger'
import {
  RcodeProcessController,
  type RcodeUnexpectedExitInfo
} from './runtime/Rcode-process-controller'
import {
  waitForRcodeStartup
} from './runtime/Rcode-runtime-health-monitor'
import {
...
} from './runtime/Rcode-runtime-model-config'
import {
...
} from './runtime/Rcode-runtime-capability-config'
import {
  buildGuiScheduleRcodeMcpServer,
  GUI_SCHEDULE_MCP_SERVER_NAME,
  readGuiManagedMcpServers,
  readJsonObjectIfExists,
  skillCapabilityConfigForRuntime
} from './runtime/Rcode-runtime-mcp-config'
import { availableBundledExtensionsDirectory } from './bundled-extension-resources'
import { subagentProfilesForRuntime } from './runtime/Rcode-runtime-subagent-config'
import { syncGuiManagedRcodeConfig } from './runtime/Rcode-runtime-config-service'

export { subagentProfilesForRuntime } from './runtime/Rcode-runtime-subagent-config'
export { syncGuiManagedRcodeConfig } from './runtime/Rcode-runtime-config-service'

export type { RcodeUnexpectedExitInfo } from './runtime/Rcode-process-controller'
export { resolveRcodeStartupTimeoutMs } from './runtime/Rcode-runtime-health-monitor'

/**
 * Called when a READY Rcode child exits without the GUI asking for it.
 * Startup failures are excluded: those are already reported to the
 * caller of startRcodeChild via the thrown error.
 */
export function setRcodeUnexpectedExitHandler(
  handler: ((info: RcodeUnexpectedExitInfo) => void) | null
): void {
  processController.setUnexpectedExitHandler(handler)
}

const execFileAsync = promisify(execFile)
const RCODE_STOP_GRACE_MS = 5_000
const RCODE_STOP_FORCE_MS = 1_000
const STDERR_TAIL_MAX_CHARS = 32_768
const MAX_TCP_PORT = 65_535

type RcodeLogStream = 'stdout' | 'stderr' | 'lifecycle'
type RcodeChildLogCapture = {
...
const processController = new RcodeProcessController<RcodeChildLogCapture>()
...
function formatRcodeLogLine(
  stream: RcodeLogStream,
  pid: number | undefined,
  message: string
): string {
  const stamp = new Date().toISOString()
  const pidLabel = typeof pid === 'number' ? `Rcode pid=${pid}` : 'Rcode'
...
function createRcodeChildLogCapture(pid: number | undefined): RcodeChildLogCapture {
  let stdoutRemainder = ''
  let stderrRemainder = ''
  let closed = false
  let pending = Promise.resolve()

  const writeLine = (stream: RcodeLogStream, message: string): void => {
    pending = pending
      .then(() => appendManagedLogLine('Rcode', formatRcodeLogLine(stream, pid, message)))
      .catch(() => undefined)
  }
...
export function resolveRcodeDataDir(runtime: { dataDir: string }): string {
  const trimmed = runtime.dataDir?.trim()
  if (trimmed) return expandHomePath(trimmed)
  return defaultRcodeDataDir()
}
...
export function isRcodeChildRunning(): boolean {
  return processController.isRunning()
}

function isCurrentRcodeChildPid(pid: number): boolean {
  return processController.isCurrentPid(pid)
}

/**
 * Resolve once any in-flight Rcode launch has settled — whether it became
 ...
 *
 * Deadlock-safe by construction: `RcodeStartPromise` is only set once a launch
 ...
 */
export function waitForRcodeStartupSettled(): Promise<void> {
  return processController.waitForStartupSettled()
}

export function startRcodeChild(settings: AppSettingsV1): Promise<void> {
  return processController.start(async () => {
    const runtime = resolveRcodeRuntimeSettings(settings)
    if (isRcodeChildRunning() || !runtime.autoStart) return
    await startRcodeChildOnce(settings, runtime)
  })
}

async function startRcodeChildOnce(
  ...
  const root = appRoot()
  const resolution = resolveRcodeExecutable(root, runtime.binaryPath)
  if (resolution.command === process.execPath && !existsSync(resolution.args[0])) {
    throw new Error(
      `Rcode runtime build is missing at ${resolution.args[0]}. Run \`npm run build:Rcode\` before starting the GUI.`
    )
  }
  const dataDir = resolveRcodeDataDir(runtime)
  await syncGuiManagedRcodeConfig(dataDir, runtime, {
    scheduleMcp: {
      settings,
      launch: {
        appPath: app.getAppPath(),
  ...
  const args = buildRcodeServeArgs({
    ...
    insecure: isRcodeRuntimeInsecure(runtime)
  })
  ...
  // (ELECTRON_RUN_AS_NODE) child to a regular Cocoa app and a second Rcode icon
  // appears in the Dock. In dev, when computer-use is enabled, we instead
  // spawn Rcode as a real Electron instance so it can call `app.dock.hide()`
  // itself (see Rcode/src/cli/serve-entry.ts). Packaged .app executables are not
  // generic Electron script runners: passing serve-entry.js to the main app
  // launches the GUI process instead of Rcode serve, so packaged builds must use
  // the Node helper path even when computer-use is enabled.
  const runAsElectron = shouldRunRcodeServeAsElectronChild({
  ...
  const activeProviderKind = (getModelProviderSettings(settings).providers as ModelProviderProfileV1[]).find(
    (provider) => provider.id?.trim() === getRcodeRuntimeSettings(settings).providerId.trim()
  )?.kind
  ...
  // resolvable from Rcode/node_modules — the SDK auto-resolves it there.
  const claudeBinary = resolveClaudeBinary(app.getPath('userData'), [join(appRoot(), 'Rcode')])
  const childEnv: NodeJS.ProcessEnv = {
    ...process.env,
    RCODE_RUNTIME_TOKEN: runtime.runtimeToken,
    DEEPSEEK_API_KEY: defaultClientApiKey || process.env.DEEPSEEK_API_KEY || '',
    ...(activeProviderKind === 'agent-sdk' ? { RCODE_RUNTIME_PROVIDER_KIND: 'agent-sdk' } : {}),
    ...(claudeBinary ? { RCODE_CLAUDE_BINARY: claudeBinary } : {})
  }
  ...
  if (bundledExtensionsDirectory) {
    childEnv.RCODE_BUNDLED_EXTENSIONS_DIR = bundledExtensionsDirectory
  }
  ...
  const startedLogCapture = createRcodeChildLogCapture(startedChild.pid)
...
export async function stopRcodeChildAndWait(): Promise<void> {
  ...
  const exited = await waitForChildExit(stoppingChild, RCODE_STOP_GRACE_MS)
  if (!exited) {
    try {
    ...
    await waitForChildExit(stoppingChild, RCODE_STOP_FORCE_MS)
  }
...
export async function reclaimRcodePort(
  port: number
): Promise<{ ok: true } | { ok: false; message: string }> {
  if (port <= 0) return { ok: true }
  if (await canBindTcpPort(port, '127.0.0.1')) return { ok: true }
  if (await killStaleRcodeOnPort(port) && await canBindTcpPort(port, '127.0.0.1')) {
    return { ok: true }
  }
  return { ok: false, message: `port ${port} is in use` }
}

export async function resolveAvailableRcodePort(
  ...
  if (preferredPort > 0) {
    ...
    if (isRcodeChildRunning() && processController.childPort === preferredPort) {
      return { port: preferredPort, changed: false }
    }
    if (await canBindTcpPort(preferredPort, '127.0.0.1')) {
      return { port: preferredPort, changed: false }
    }
    // Prefer reclaiming the configured port from a stale Rcode left by a
    // crashed previous app run over silently moving to a new port.
    if (
      await killStaleRcodeOnPort(preferredPort) &&
...
/**
 * Kill a stale Rcode serve process from a previous app run that is still
 ...
 */
async function killStaleRcodeOnPort(port: number): Promise<boolean> {
  const pids = await listListeningPidsOnPort(port)
  let reclaimed = false
  for (const pid of pids) {
    if (isCurrentRcodeChildPid(pid)) continue
    let command = ''
    try {
    ...
    void appendManagedLogLine(
      'Rcode',
      formatRcodeLogLine('lifecycle', pid, `killing stale Rcode process holding port ${port}`)
    )
...
/** Terminate a positively-identified stale Rcode process. */
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen({ port: 0, host, exclusive: true }, () => {
      ...
      server.close((error) => {
        ...
        else reject(new Error('failed to allocate an available Rcode port'))
      })
    })
  })
}
