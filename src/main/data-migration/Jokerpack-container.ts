...
import {
  createJokerpackPassphraseEncryption,
  decryptJokerpackFramesToFile,
  encryptJokerpackFileToHandle,
  type JokerpackFramedEncryptionResult
} from './Jokerpack-crypto'
import {
...
export const JOKERPACK_MAGIC = Buffer.from('JOKERPACK\0', 'ascii')
export const JOKERPACK_PREFIX_BYTES = JOKERPACK_MAGIC.length + 4
export const JOKERPACK_MAX_HEADER_BYTES = 64 * 1024
export const JOKERPACK_MANIFEST_PATH = parsePackageRelativePath('manifest.json')
export const JOKERPACK_CHECKSUMS_PATH = parsePackageRelativePath('checksums.jsonl')

export type JokerpackCatalogInput = {
  path: PackageRelativePath
  value: unknown
  ownerId?: string
}

export type CreateJokerpackPackageInput = {
  outputPath: string
  manifest: DataMigrationManifestV1
  catalogs: readonly JokerpackCatalogInput[]
...
export type CreatedJokerpackPackage = {
  path: string
...
export type ReadJokerpackEnvelopeResult = {
  header: DataMigrationEnvelopeHeaderV1
  headerBytes: Buffer
  payloadOffset: number
}

export function canonicalizeJokerpackJson(value: unknown): unknown {
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return value
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error('Jokerpack canonical JSON does not support non-finite numbers')
    return value
  }
  if (Array.isArray(value)) return value.map((item) => item === undefined ? null : canonicalizeJokerpackJson(item))
  if (typeof value === 'object') {
    const record = value as Record<string, unknown>
    const sorted: Record<string, unknown> = {}
    for (const key of Object.keys(record).sort()) {
      if (record[key] === undefined) continue
      sorted[key] = canonicalizeJokerpackJson(record[key])
    }
    return sorted
  }
  throw new Error(`Jokerpack canonical JSON does not support ${typeof value}`)
}

export function serializeJokerpackJson(value: unknown): Buffer {
  return Buffer.from(`${JSON.stringify(canonicalizeJokerpackJson(value))}\n`, 'utf8')
}

export function serializeJokerpackChecksums(entries: readonly DataMigrationPackageEntry[]): Buffer {
  const lines = [...entries]
    .sort((left, right) => left.path.localeCompare(right.path))
    .map((entry) => JSON.stringify(canonicalizeJokerpackJson(entry)))
  return Buffer.from(lines.length === 0 ? '' : `${lines.join('\n')}\n`, 'utf8')
}

export function parseJokerpackChecksums(contents: Buffer): DataMigrationPackageEntry[] {
  ...
  for (const entry of entries) {
    if (paths.has(entry.path)) throw new Error(`duplicate Jokerpack checksum declaration: ${entry.path}`)
    paths.add(entry.path)
  }
  return entries
}

export async function createJokerpackPackage(input: CreateJokerpackPackageInput): Promise<CreatedJokerpackPackage> {
  const outputDirectory = dirname(input.outputPath)
  await mkdir(outputDirectory, { recursive: true, mode: 0o700 })
  if (await lstat(input.outputPath).then(() => true).catch(() => false)) {
    throw new Error(`Jokerpack destination already exists: ${input.outputPath}`)
  }

  const encryption: DataMigrationEncryption = input.passphrase
    ? input.encryptionSettings ?? createJokerpackPassphraseEncryption()
    : { mode: 'none' }
  if (!input.passphrase && input.encryptionSettings) {
    throw new Error('Jokerpack encryption settings require a passphrase')
  }

  const packageToken = `${process.pid}.${randomUUID()}`
  const zipPath = join(outputDirectory, `.Jokerpack-${packageToken}.zip.tmp`)
  const packagePath = join(outputDirectory, `.Jokerpack-${packageToken}.tmp`)
  const verificationZipPath = join(outputDirectory, `.Jokerpack-${packageToken}.verify.zip.tmp`)
  try {
    const catalogInputs: Zip64ArchiveEntryInput[] = input.catalogs.map((catalog) => {
      PackageRelativePathSchema.parse(catalog.path)
      if (!catalog.path.startsWith('catalog/')) throw new Error(`Jokerpack catalog path must be under catalog/: ${catalog.path}`)
      return {
        path: catalog.path,
        kind: 'catalog',
        ...(catalog.ownerId ? { ownerId: catalog.ownerId } : {}),
        source: { kind: 'buffer', data: serializeJokerpackJson(catalog.value) }
      }
    })
    const payloadEntries = await prepareZip64ArchiveEntries([...catalogInputs, ...input.entries])
    const checksumContents = serializeJokerpackChecksums(payloadEntries.map((entry) => entry.metadata))
    const catalogDigest = digestJokerpackCatalogs(payloadEntries)
    ...
    const structuralEntries = await prepareZip64ArchiveEntries([
      {
        path: JOKERPACK_MANIFEST_PATH,
        kind: 'catalog',
        source: { kind: 'buffer', data: serializeJokerpackJson(manifest) }
      },
      {
        path: JOKERPACK_CHECKSUMS_PATH,
        kind: 'catalog',
    ...
    const header = await writeJokerpackEnvelope({
      zipPath,
...
export async function writeJokerpackEnvelope(input: {
  zipPath: string
  outputPath: string
  ...
  if (!zipStats.isFile()) throw new Error('Jokerpack payload must be a regular file')
  ...
  const headerBytes = serializeJokerpackJson(header)
  if (headerBytes.length > JOKERPACK_MAX_HEADER_BYTES) throw new Error('Jokerpack envelope header is too large')
  const prefix = Buffer.allocUnsafe(JOKERPACK_PREFIX_BYTES)
  JOKERPACK_MAGIC.copy(prefix, 0)
  prefix.writeUInt32BE(headerBytes.length, JOKERPACK_MAGIC.length)
  ...
  try {
    ...
    if (header.encryption.mode === 'passphrase') {
      if (!input.passphrase) throw new Error('Jokerpack passphrase is required')
      await encryptJokerpackFileToHandle({
...
export async function readJokerpackEnvelopeHeader(packagePath: string): Promise<ReadJokerpackEnvelopeResult> {
  const handle = await open(packagePath, 'r')
  try {
    const prefix = Buffer.allocUnsafe(JOKERPACK_PREFIX_BYTES)
    const prefixRead = await handle.read(prefix, 0, prefix.length, 0)
    if (prefixRead.bytesRead !== prefix.length || !prefix.subarray(0, JOKERPACK_MAGIC.length).equals(JOKERPACK_MAGIC)) {
      throw new Error('file is not a Jokerpack package')
    }
    const headerLength = prefix.readUInt32BE(JOKERPACK_MAGIC.length)
    if (headerLength < 2 || headerLength > JOKERPACK_MAX_HEADER_BYTES) {
      throw new Error('Jokerpack envelope header length is invalid')
    }
    ...
    if (headerRead.bytesRead !== headerLength) throw new Error('Jokerpack envelope header is truncated')
...
export async function materializeJokerpackZip(input: {
  packagePath: string
  outputZipPath: string
  passphrase?: string
}): Promise<{ header: DataMigrationEnvelopeHeaderV1; framing?: JokerpackFramedEncryptionResult }> {
  const envelope = await readJokerpackEnvelopeHeader(input.packagePath)
  await mkdir(dirname(input.outputZipPath), { recursive: true, mode: 0o700 })
  let framing: JokerpackFramedEncryptionResult | undefined
  if (envelope.header.encryption.mode === 'passphrase') {
    if (!input.passphrase) throw new Error('Jokerpack passphrase is required')
    framing = await decryptJokerpackFramesToFile({
  ...
  if (details.size !== envelope.header.plainPayloadBytes || digest !== envelope.header.plainPayloadSha256) {
    await rm(input.outputZipPath, { force: true }).catch(() => undefined)
    throw new Error('Jokerpack payload integrity check failed')
  }
  return { header: envelope.header, ...(framing ? { framing } : {}) }
}

export async function verifyJokerpackPackage(input: {
  packagePath: string
  ...
  const { header } = await materializeJokerpackZip({
    packagePath: input.packagePath,
  ...
  try {
    const manifestBytes = await readZip64EntryBuffer(
      ...
      JOKERPACK_MANIFEST_PATH,
      DATA_MIGRATION_MAX_METADATA_BYTES
    )
    const checksumsBytes = await readZip64EntryBuffer(
      input.materializedZipPath,
      JOKERPACK_CHECKSUMS_PATH,
      DATA_MIGRATION_MAX_METADATA_BYTES
    )
    const manifest = DataMigrationManifestV1Schema.parse(JSON.parse(manifestBytes.toString('utf8')))
    if (manifest.formatVersion !== header.formatVersion) throw new Error('Jokerpack envelope and manifest versions differ')
    if (canonicalEncryption(manifest.encryption) !== canonicalEncryption(header.encryption)) {
      throw new Error('Jokerpack envelope and manifest encryption settings differ')
    }
    if (sha256Buffer(checksumsBytes) !== manifest.checksumsSha256) {
      throw new Error('Jokerpack checksum catalog digest mismatch')
    }
    const entries = parseJokerpackChecksums(checksumsBytes)
    if (entries.length !== manifest.counts.entries) throw new Error('Jokerpack entry count differs from manifest')
    const expandedBytes = entries.reduce((total, entry) => total + entry.logicalBytes, 0)
    if (expandedBytes !== manifest.expandedBytes) throw new Error('Jokerpack expanded bytes differ from manifest')
    ...
    if (digestJokerpackCatalogs(catalogPrepared) !== manifest.catalogsSha256) {
      throw new Error('Jokerpack catalog digest mismatch')
    }
    await verifyZip64ArchiveEntries(
      input.materializedZipPath,
      entries,
      new Set([JOKERPACK_MANIFEST_PATH, JOKERPACK_CHECKSUMS_PATH])
    )
...
function digestJokerpackCatalogs(entries: readonly PreparedZip64ArchiveEntry[]): string {
...
function canonicalEncryption(encryption: DataMigrationEncryption): string {
  return serializeJokerpackJson(encryption).toString('utf8')
}
...
export async function writeCanonicalJokerpackJson(path: string, value: unknown): Promise<void> {
  await mkdir(dirname(path), { recursive: true, mode: 0o700 })
  await writeFile(path, serializeJokerpackJson(value), { mode: 0o600, flag: 'wx' })
}

export async function readCanonicalJokerpackJson(path: string): Promise<unknown> {
  const details = await stat(path)
  if (!details.isFile() || details.size > DATA_MIGRATION_MAX_METADATA_BYTES) {
    throw new Error('Jokerpack JSON metadata exceeds allowed size')
  }
