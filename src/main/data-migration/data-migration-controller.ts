...
import type { RuntimeThreadForMigration, JokerMigrationSnapshotClient } from './export-orchestrator'
...
import {
  ...
  type JokerRuntimeMigrationImportClient,
  ...
  registerIpc(): void {
    ...
    handle('data-migration:pick-export', async (raw) => {
      ...
      const result = await dialog.showSaveDialog(this.windowOptions(), {
        ...
        filters: [{ name: 'Joker migration package', extensions: ['Jokerpack'] }]
      })
  ...
  private async recover(operationId: string, action: 'resume' | 'rollback'): Promise<void> {
    ...
    if (journal.phase === 'inspected') {
      for (const mapping of journal.plan.mappings) {
        ...
        const stagingRoot = join(dirname(mapping.destinationRoot), `.Joker-migration-staging-${operationId}-${mapping.workspaceId}`)
...
export async function listRuntimeThreadsForMigration(
  ...
  if (!response.ok) throw new Error(`Joker thread inventory failed (${response.status})`)
...
function runtimeSnapshotClient(runtimeFetch: DataMigrationControllerOptions['runtimeFetch']): JokerMigrationSnapshotClient {
  return {
    ...
    download: async (snapshotId, destinationPath, signal) => {
      ...
      if (!response.ok || !response.body) throw new Error(`Joker snapshot download failed (${response.status})`)
      ...
      const { sha256File } = await import('./Jokerpack-zip')
...
function runtimeImportClient(runtimeFetch: DataMigrationControllerOptions['runtimeFetch']): JokerRuntimeMigrationImportClient {
  ...
  return {
    preflight: async (input) => {
      ...
      }) as { preflight: Awaited<ReturnType<JokerRuntimeMigrationImportClient['preflight']>> }
...
async function fetchJson(
  ...
  if (!response.ok) {
    const message = value && typeof value === 'object' && typeof (value as { message?: unknown }).message === 'string'
      ...
      : `Joker migration request failed (${response.status})`
...
export function publicMigrationError(error: unknown, phase?: DataMigrationProgress['phase']): string {
  ...
  const destinationEffect = phase === 'staging'
    ...
    : phase === 'committing' || phase === 'verifying' || phase === 'rolling-back'
      ? 'changes may have started; Joker will use the operation journal to roll back or recover'
  ...
  const nextAction = code === 'SPACE_INSUFFICIENT'
    ...
    : code === 'IO_PERMISSION_DENIED'
      ...
      : code === 'PACKAGE_PASSWORD_REQUIRED' || code === 'PACKAGE_PASSWORD_INVALID'
        ? 'Enter the package passphrase again; Joker never stores it.'
