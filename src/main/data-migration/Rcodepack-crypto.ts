...
export const RCODEPACK_KEY_BYTES = 32
export const RCODEPACK_NONCE_PREFIX_BYTES = 8
export const RCODEPACK_AUTH_TAG_BYTES = 16
export const RCODEPACK_FRAME_HEADER_BYTES = 8
export const RCODEPACK_SCRYPT_DEFAULTS = Object.freeze({
  cost: 32_768,
  blockSize: 8,
  parallelization: 1,
...
export function createRcodepackPassphraseEncryption(
  ...
  return {
    ...
    noncePrefixBase64: random(RCODEPACK_NONCE_PREFIX_BYTES).toString('base64'),
    frameBytes: DATA_MIGRATION_DEFAULT_FRAME_BYTES,
    cost: RCODEPACK_SCRYPT_DEFAULTS.cost,
    blockSize: RCODEPACK_SCRYPT_DEFAULTS.blockSize,
    parallelization: RCODEPACK_SCRYPT_DEFAULTS.parallelization
  }
}

export function validateRcodepackPassphraseEncryption(settings: PassphraseEncryption): void {
  if (settings.algorithm !== 'aes-256-gcm-framed' || settings.kdf !== 'scrypt') {
    throw new Error('unsupported Rcodepack encryption settings')
  }
  if (!isPowerOfTwo(settings.cost) || settings.cost < 16_384 || settings.cost > 1_048_576) {
    throw new Error('invalid Rcodepack scrypt cost')
  }
  if (!Number.isInteger(settings.blockSize) || settings.blockSize < 1 || settings.blockSize > 32) {
    throw new Error('invalid Rcodepack scrypt block size')
  }
  if (!Number.isInteger(settings.parallelization) || settings.parallelization < 1 || settings.parallelization > 16) {
    throw new Error('invalid Rcodepack scrypt parallelization')
  }
  if (!Number.isInteger(settings.frameBytes) || settings.frameBytes < 64 * 1024 || settings.frameBytes > 64 * 1024 * 1024) {
    throw new Error('invalid Rcodepack encryption frame size')
  }
  if (decodeExactBase64(settings.saltBase64, 'salt').length < 16) {
    throw new Error('invalid Rcodepack encryption salt')
  }
  if (decodeExactBase64(settings.noncePrefixBase64, 'nonce prefix').length !== RCODEPACK_NONCE_PREFIX_BYTES) {
    throw new Error('invalid Rcodepack encryption nonce prefix')
  }
}
...
function decodeExactBase64(value: string, label: string): Buffer {
  ...
  if (!value || decoded.toString('base64') !== value) throw new Error(`invalid Rcodepack ${label}`)
  return decoded
}

export async function deriveRcodepackPassphraseKey(
  passphrase: string,
  settings: PassphraseEncryption
): Promise<Buffer> {
  validateRcodepackPassphraseEncryption(settings)
  if (!passphrase) throw new Error('Rcodepack passphrase is required')
  ...
  return new Promise<Buffer>((resolve, reject) => {
    nodeScrypt(passphrase, salt, RCODEPACK_KEY_BYTES, {
      N: settings.cost,
      r: settings.blockSize,
      p: settings.parallelization,
      maxmem: Math.max(RCODEPACK_SCRYPT_DEFAULTS.maxMemoryBytes, minimumMemory * 2)
    }, (error, key) => {
...
export type RcodepackFramedEncryptionResult = {
  plainBytes: number
  encryptedBytes: number
  frameCount: number
}

export async function encryptRcodepackFileToHandle(input: {
  inputPath: string
...
}): Promise<RcodepackFramedEncryptionResult> {
  validateRcodepackPassphraseEncryption(input.settings)
  const key = await deriveRcodepackPassphraseKey(input.passphrase, input.settings)
  ...
  try {
    while (true) {
      ...
      if (frameIndex > 0xffff_ffff) throw new Error('Rcodepack encryption frame counter exhausted')
...
export async function decryptRcodepackFramesToFile(input: {
  packagePath: string
  ...
  try {
    while (sourcePosition < sourceStats.size) {
      const frameHeader = await readExact(source, RCODEPACK_FRAME_HEADER_BYTES, sourcePosition)
      sourcePosition += frameHeader.length
      const cipherBytes = frameHeader.readUInt32BE(0)
      const frameIndex = frameHeader.readUInt32BE(4)
      if (frameIndex !== expectedFrameIndex) throw new Error('Rcodepack encryption frame sequence is invalid')
      if (cipherBytes < 1 || cipherBytes > input.settings.frameBytes) {
        throw new Error('Rcodepack encrypted frame exceeds declared size')
      }
      const ciphertext = await readExact(source, cipherBytes, sourcePosition)
      sourcePosition += ciphertext.length
      const tag = await readExact(source, RCODEPACK_AUTH_TAG_BYTES, sourcePosition)
      ...
      try {
        ...
        throw new Error('Rcodepack passphrase or authenticated payload is invalid', { cause: error })
      }
...
function frameHeaderFor(length: number, frameIndex: number): Buffer {
  const header = Buffer.allocUnsafe(RCODEPACK_FRAME_HEADER_BYTES)
...
function nonceFor(prefix: Buffer, frameIndex: number): Buffer {
  ...
  prefix.copy(nonce, 0)
  nonce.writeUInt32BE(frameIndex, RCODEPACK_NONCE_PREFIX_BYTES)
  return nonce
}
...
async function readExact(
  ...
  while (offset < length) {
    ...
    if (bytesRead === 0) throw new Error('Rcodepack encrypted payload is truncated')
    offset += bytesRead
  }
  return output
}
...
async function writeAll(
  ...
  while (offset < buffer.length) {
    ...
    if (bytesWritten === 0) throw new Error('failed to write Rcodepack payload')
    offset += bytesWritten
  }
}
