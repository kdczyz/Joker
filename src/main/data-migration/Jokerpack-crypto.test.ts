...
import {
  ...
  deriveJokerpackPassphraseKey,
  encryptJokerpackFileToHandle,
  validateJokerpackPassphraseEncryption
...
const PUBLISHED_VECTOR = Object.freeze({
  ...
  plaintextUtf8: 'Jokerpack fixed vector\n',
...
describe('Jokerpack framed encryption published vector', () => {
  it('derives the expected scrypt key and independently authenticated frame bytes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Jokerpack-crypto-test-'))
    ...
    const key = await deriveJokerpackPassphraseKey(PUBLISHED_VECTOR.passphrase, VECTOR_SETTINGS)
    ...
    await decryptJokerpackFramesToFile({
  ...
  it('rejects invalid or excessive KDF and frame parameters', () => {
    expect(() => validateJokerpackPassphraseEncryption({ ...VECTOR_SETTINGS, cost: 12_345 })).toThrow('scrypt cost')
    expect(() => validateJokerpackPassphraseEncryption({ ...VECTOR_SETTINGS, frameBytes: 1 })).toThrow('frame size')
    expect(() => validateJokerpackPassphraseEncryption({ ...VECTOR_SETTINGS, noncePrefixBase64: 'AA==' })).toThrow('nonce prefix')
  })
})
