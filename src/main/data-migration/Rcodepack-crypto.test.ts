...
import {
  ...
  deriveRcodepackPassphraseKey,
  encryptRcodepackFileToHandle,
  validateRcodepackPassphraseEncryption
...
const PUBLISHED_VECTOR = Object.freeze({
  ...
  plaintextUtf8: 'Rcodepack fixed vector\n',
...
describe('Rcodepack framed encryption published vector', () => {
  it('derives the expected scrypt key and independently authenticated frame bytes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcodepack-crypto-test-'))
    ...
    const key = await deriveRcodepackPassphraseKey(PUBLISHED_VECTOR.passphrase, VECTOR_SETTINGS)
    ...
    await decryptRcodepackFramesToFile({
  ...
  it('rejects invalid or excessive KDF and frame parameters', () => {
    expect(() => validateRcodepackPassphraseEncryption({ ...VECTOR_SETTINGS, cost: 12_345 })).toThrow('scrypt cost')
    expect(() => validateRcodepackPassphraseEncryption({ ...VECTOR_SETTINGS, frameBytes: 1 })).toThrow('frame size')
    expect(() => validateRcodepackPassphraseEncryption({ ...VECTOR_SETTINGS, noncePrefixBase64: 'AA==' })).toThrow('nonce prefix')
  })
})
