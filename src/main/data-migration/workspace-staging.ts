...
import { validateJokerpackLinkMetadata } from './archive-security'
import { stableImportedSiblingPath } from './import-planner'
import { extractZip64ArchiveEntries } from './Jokerpack-zip'
...
export async function stageWorkspaceImport(input: {
  ...
  const stagingRoot = join(dirname(destinationRoot), `.Joker-migration-staging-${input.operationId}-${input.workspaceId}`)
  const backupRoot = join(dirname(destinationRoot), '.Joker-migration-backup', input.operationId, input.workspaceId)
  ...
  validateJokerpackLinkMetadata(files.map((file) => file.entry), { allowLinks: input.supportsSymbolicLinks })
  try {
