...
export async function prepareZip64ArchiveEntries(
  ...
  for (const entry of [...entries].sort((left, right) => left.path.localeCompare(right.path))) {
    ...
    if (paths.has(entry.path)) throw new Error(`duplicate Jokerpack entry: ${entry.path}`)
    ...
    if (entry.source.kind === 'buffer') {
      ...
      if (!details.isFile() || details.isSymbolicLink()) {
        throw new Error(`Jokerpack source must be a regular non-link file: ${entry.path}`)
      }
    ...
    if (entry.logicalBytes !== undefined && entry.logicalBytes !== logicalBytes) {
      throw new Error(`Jokerpack source size changed before packaging: ${entry.path}`)
    }
    if (entry.sha256 !== undefined && entry.sha256 !== sha256) {
      throw new Error(`Jokerpack source digest changed before packaging: ${entry.path}`)
    }
...
export async function readZip64EntryBuffer(
  ...
  try {
    for await (const entry of zipfile.eachEntry()) {
      ...
      if (entry.uncompressedSize > maximumBytes) throw new Error(`Jokerpack entry exceeds read limit: ${requestedPath}`)
      ...
      for await (const chunk of stream) {
        ...
        if (bytes > maximumBytes) {
          stream.destroy()
          throw new Error(`Jokerpack entry exceeds read limit: ${requestedPath}`)
        }
        chunks.push(buffer)
      }
      return Buffer.concat(chunks, bytes)
    }
    throw new Error(`Jokerpack entry is missing: ${requestedPath}`)
  } finally {
    zipfile.close()
  }
}
...
export async function verifyZip64ArchiveEntries(
  ...
  try {
    for await (const entry of zipfile.eachEntry()) {
      if (seen.has(entry.fileName)) throw new Error(`duplicate Jokerpack ZIP entry: ${entry.fileName}`)
      seen.add(entry.fileName)
      if (entry.fileName.endsWith('/')) {
        if (!allowedUnlistedPaths.has(entry.fileName)) throw new Error(`undeclared Jokerpack directory entry: ${entry.fileName}`)
        continue
      }
      ...
      if (!declaration) {
        ...
        throw new Error(`undeclared Jokerpack ZIP entry: ${entry.fileName}`)
      }
      if (entry.uncompressedSize !== declaration.logicalBytes) {
        throw new Error(`Jokerpack entry size mismatch: ${entry.fileName}`)
      }
      const stream = await zipfile.openReadStreamPromise(entry)
      ...
      for await (const chunk of stream) {
        ...
        if (bytes > declaration.logicalBytes) {
          stream.destroy()
          throw new Error(`Jokerpack entry expanded beyond declaration: ${entry.fileName}`)
        }
        digest.update(buffer)
      }
      if (bytes !== declaration.logicalBytes || digest.digest('hex') !== declaration.sha256) {
        throw new Error(`Jokerpack entry integrity mismatch: ${entry.fileName}`)
      }
    }
    for (const path of expected.keys()) {
      if (!seen.has(path)) throw new Error(`declared Jokerpack entry is missing: ${path}`)
    }
  } finally {
    zipfile.close()
  }
  ...
  try {
    for await (const zipEntry of zipfile.eachEntry()) {
      ...
      if (!pathRelativeToRoot || pathRelativeToRoot.startsWith('..') || isAbsolute(pathRelativeToRoot)) {
        throw new Error(`Jokerpack extraction target escapes staging root: ${declaration.path}`)
      }
      ...
      const meter = new Transform({
        transform(chunk: Buffer, _encoding, callback) {
          ...
          if (entryBytes > declaration.logicalBytes) {
            callback(new Error(`Jokerpack entry expanded beyond declaration: ${declaration.path}`))
            return
          }
          digest.update(chunk)
      ...
      if (entryBytes !== declaration.logicalBytes || digest.digest('hex') !== declaration.sha256) {
        await rm(outputPath, { force: true }).catch(() => undefined)
        throw new Error(`Jokerpack extracted entry integrity mismatch: ${declaration.path}`)
      }
    ...
    for (const declaration of input.entries) {
      ...
      if (!extracted.has(declaration.path)) throw new Error(`declared Jokerpack extraction entry is missing: ${declaration.path}`)
    }
