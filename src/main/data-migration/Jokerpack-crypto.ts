...
export const JOKERPACK_KEY_BYTES = 32
export const JOKERPACK_NONCE_PREFIX_BYTES = 8
export const JOKERPACK_AUTH_TAG_BYTES = 16
export const JOKERPACK_FRAME_HEADER_BYTES = 8
export const JOKERPACK_SCRYPT_DEFAULTS = Object.freeze({
  cost: 32_768,
  blockSize: 8,
  parallelization: 1,
...
export function createJokerpackPassphraseEncryption(
  ...
  return {
    ...
    noncePrefixBase64: random(JOKERPACK_NONCE_PREFIX_BYTES).toString('base64'),
    frameBytes: DATA_MIGRATION_DEFAULT_FRAME_BYTES,
    cost: JOKERPACK_SCRYPT_DEFAULTS.cost,
    blockSize: JOKERPACK_SCRYPT_DEFAULTS.blockSize,
    parallelization: JOKERPACK_SCRYPT_DEFAULTS.parallelization
  }
}

export function validateJokerpackPassphraseEncryption(settings: PassphraseEncryption): void {
  if (settings.algorithm !== 'aes-256-gcm-framed' || settings.kdf !== 'scrypt') {
    throw new Error('unsupported Jokerpack encryption settings')
  }
  if (!isPowerOfTwo(settings.cost) || settings.cost < 16_384 || settings.cost > 1_048_576) {
    throw new Error('invalid Jokerpack scrypt cost')
  }
  if (!Number.isInteger(settings.blockSize) || settings.blockSize < 1 || settings.blockSize > 32) {
    throw new Error('invalid Jokerpack scrypt block size')
  }
  if (!Number.isInteger(settings.parallelization) || settings.parallelization < 1 || settings.parallelization > 16) {
    throw new Error('invalid Jokerpack scrypt parallelization')
  }
  if (!Number.isInteger(settings.frameBytes) || settings.frameBytes < 64 * 1024 || settings.frameBytes > 64 * 1024 * 1024) {
    throw new Error('invalid Jokerpack encryption frame size')
  }
  if (decodeExactBase64(settings.saltBase64, 'salt').length < 16) {
    throw new Error('invalid Jokerpack encryption salt')
  }
  if (decodeExactBase64(settings.noncePrefixBase64, 'nonce prefix').length !== JOKERPACK_NONCE_PREFIX_BYTES) {
    throw new Error('invalid Jokerpack encryption nonce prefix')
  }
}
...
function decodeExactBase64(value: string, label: string): Buffer {
  ...
  if (!value || decoded.toString('base64') !== value) throw new Error(`invalid Jokerpack ${label}`)
  return decoded
}

export async function deriveJokerpackPassphraseKey(
  passphrase: string,
  settings: PassphraseEncryption
): Promise<Buffer> {
  validateJokerpackPassphraseEncryption(settings)
  if (!passphrase) throw new Error('Jokerpack passphrase is required')
  ...
  return new Promise<Buffer>((resolve, reject) => {
    nodeScrypt(passphrase, salt, JOKERPACK_KEY_BYTES, {
      N: settings.cost,
      r: settings.blockSize,
      p: settings.parallelization,
      maxmem: Math.max(JOKERPACK_SCRYPT_DEFAULTS.maxMemoryBytes, minimumMemory * 2)
    }, (error, key) => {
...
export type JokerpackFramedEncryptionResult = {
  plainBytes: number
  encryptedBytes: number
  frameCount: number
}

export async function encryptJokerpackFileToHandle(input: {
  inputPath: string
...
}): Promise<JokerpackFramedEncryptionResult> {
  validateJokerpackPassphraseEncryption(input.settings)
  const key = await deriveJokerpackPassphraseKey(input.passphrase, input.settings)
  ...
  try {
    while (true) {
      ...
      if (frameIndex > 0xffff_ffff) throw new Error('Jokerpack encryption frame counter exhausted')
...
export async function decryptJokerpackFramesToFile(input: {
  packagePath: string
  ...
  try {
    while (sourcePosition < sourceStats.size) {
      const frameHeader = await readExact(source, JOKERPACK_FRAME_HEADER_BYTES, sourcePosition)
      sourcePosition += frameHeader.length
      const cipherBytes = frameHeader.readUInt32BE(0)
      const frameIndex = frameHeader.readUInt32BE(4)
      if (frameIndex !== expectedFrameIndex) throw new Error('Jokerpack encryption frame sequence is invalid')
      if (cipherBytes < 1 || cipherBytes > input.settings.frameBytes) {
        throw new Error('Jokerpack encrypted frame exceeds declared size')
      }
      const ciphertext = await readExact(source, cipherBytes, sourcePosition)
      sourcePosition += ciphertext.length
      const tag = await readExact(source, JOKERPACK_AUTH_TAG_BYTES, sourcePosition)
      ...
      try {
        ...
        throw new Error('Jokerpack passphrase or authenticated payload is invalid', { cause: error })
      }
...
function frameHeaderFor(length: number, frameIndex: number): Buffer {
  const header = Buffer.allocUnsafe(JOKERPACK_FRAME_HEADER_BYTES)
...
function nonceFor(prefix: Buffer, frameIndex: number): Buffer {
  ...
  prefix.copy(nonce, 0)
  nonce.writeUInt32BE(frameIndex, JOKERPACK_NONCE_PREFIX_BYTES)
  return nonce
}
...
async function readExact(
  ...
  while (offset < length) {
    ...
    if (bytesRead === 0) throw new Error('Jokerpack encrypted payload is truncated')
    offset += bytesRead
  }
  return output
}
...
async function writeAll(
  ...
  while (offset < buffer.length) {
    ...
    if (bytesWritten === 0) throw new Error('failed to write Jokerpack payload')
    offset += bytesWritten
  }
}
