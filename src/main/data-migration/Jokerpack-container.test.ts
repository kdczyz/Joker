...
import {
  canonicalizeJokerpackJson,
  createJokerpackPackage,
  readJokerpackEnvelopeHeader,
  serializeJokerpackChecksums,
  serializeJokerpackJson,
  verifyJokerpackPackage,
  writeJokerpackEnvelope
} from './Jokerpack-container'
import { createJokerpackPassphraseEncryption } from './Jokerpack-crypto'
...
async function temporaryDirectory(): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), 'Jokerpack-test-'))
...
describe('Jokerpack canonical metadata', () => {
  it('sorts object keys recursively without changing array order', () => {
    expect(canonicalizeJokerpackJson({ z: 1, a: { y: 2, x: 3 }, list: [{ b: 1, a: 2 }] })).toEqual({
      a: { x: 3, y: 2 },
      list: [{ a: 2, b: 1 }],
      z: 1
    })
    expect(serializeJokerpackJson({ z: 1, a: 2 }).toString('utf8')).toBe('{"a":2,"z":1}\n')
  })
  ...
  it('makes checksum catalogs deterministic', () => {
    ...
    expect(serializeJokerpackChecksums([left, right])).toEqual(serializeJokerpackChecksums([right, left]))
  })
})

describe('Jokerpack container', () => {
  it('creates and verifies an unencrypted ZIP64 package without overwriting an existing destination', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'portable.Jokerpack')
    const created = await createJokerpackPackage({
    ...
    await expect(createJokerpackPackage({
      outputPath: packagePath,
      manifest: manifest(),
      ...sampleInputs()
  ...
  it('creates and verifies a passphrase-encrypted framed package', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'encrypted.Jokerpack')
    const settings = createJokerpackPassphraseEncryption((size) => Buffer.alloc(size, size))
    const created = await createJokerpackPackage({
      outputPath: packagePath,
      manifest: manifest(),
      ...sampleInputs(),
  ...
  it('rejects a wrong passphrase, ciphertext tampering, and a truncated frame', async () => {
    ...
    await createJokerpackPackage({
      ...
      encryptionSettings: createJokerpackPassphraseEncryption((size) => Buffer.alloc(size, 7))
    })

    await expect(verifyJokerpackPackage({
      packagePath,
      materializedZipPath: join(root, 'wrong-password.zip'),
      passphrase: 'wrong-password'
    })).rejects.toThrow('passphrase or authenticated payload is invalid')

    const envelope = await readJokerpackEnvelopeHeader(packagePath)
    const tamperedPath = join(root, 'tampered.Jokerpack')
    ...
    const truncatedPath = join(root, 'truncated.Jokerpack')
  ...
  it('detects corruption in an unencrypted package before ZIP parsing', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'plain.Jokerpack')
    await createJokerpackPackage({ outputPath: packagePath, manifest: manifest(), ...sampleInputs() })
  ...
  it('rejects a modified manifest even when the outer payload hash is recomputed', async () => {
    ...
    const packagePath = join(root, 'modified-manifest.Jokerpack')
    ...
    const structuralEntries = await prepareZip64ArchiveEntries([
      {
        ...
        kind: 'catalog',
        source: { kind: 'buffer', data: serializeJokerpackJson(modifiedManifest) }
      },
      {
