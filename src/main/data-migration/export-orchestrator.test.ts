...
import {
  ...
  type JokerMigrationSnapshotClient
...
function runtimeClient(contents = '{"schemaVersion":1,"type":"metadata","value":{}}\n') {
  ...
  const client: JokerMigrationSnapshotClient = {
...
describe('DataMigrationExportOrchestrator', () => {
  it('packages workspace files, runtime history, semantic settings, and a reportable verified archive', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-export-orchestrator-'))
    roots.push(root)
    const workspace = join(root, 'workspace')
    const output = join(root, 'exports', 'portable.Jokerpack')
  ...
  it('requires explicit acknowledgement for plaintext and sensitive workspace files', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-export-policy-'))
    roots.push(root)
    const workspace = join(root, 'workspace')
    await mkdir(workspace, { recursive: true })
    ...
    const base = {
      ...
      outputPath: join(root, 'portable.Jokerpack'),
      settings: settings(workspace),
      runtimeThreads: [],
      selectedWorkspaceIds: [],
      selectedThreadIds: [],
  ...
  it('cleans a partially downloaded runtime snapshot and never publishes on failure', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-export-cleanup-'))
    roots.push(root)
    const workspace = join(root, 'workspace')
    const output = join(root, 'portable.Jokerpack')
    await mkdir(workspace, { recursive: true })
    const runtime = runtimeClient()
  ...
  it('treats an empty thread selection as all runtime histories when history is included', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-export-all-threads-'))
    roots.push(root)
    const workspace = join(root, 'workspace')
    await mkdir(workspace, { recursive: true })
    const runtime = runtimeClient()
    ...
    await orchestrator.export({
      operationId: 'export_all_threads', outputPath: join(root, 'all.Jokerpack'), settings: settings(workspace),
      runtimeThreads: [
  ...
  it('limits automatic history selection to the explicitly selected workspaces', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-export-related-threads-'))
    roots.push(root)
    ...
    const outputPath = join(root, 'related.Jokerpack')
    await orchestrator.export({
    ...
    const verified = await verifyJokerpackPackage({ packagePath: outputPath, materializedZipPath: join(root, 'related.zip') })
