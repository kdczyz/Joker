...
import {
  canonicalizeRcodepackJson,
  createRcodepackPackage,
  readRcodepackEnvelopeHeader,
  serializeRcodepackChecksums,
  serializeRcodepackJson,
  verifyRcodepackPackage,
  writeRcodepackEnvelope
} from './Rcodepack-container'
import { createRcodepackPassphraseEncryption } from './Rcodepack-crypto'
...
async function temporaryDirectory(): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), 'Rcodepack-test-'))
...
describe('Rcodepack canonical metadata', () => {
  it('sorts object keys recursively without changing array order', () => {
    expect(canonicalizeRcodepackJson({ z: 1, a: { y: 2, x: 3 }, list: [{ b: 1, a: 2 }] })).toEqual({
      a: { x: 3, y: 2 },
      list: [{ a: 2, b: 1 }],
      z: 1
    })
    expect(serializeRcodepackJson({ z: 1, a: 2 }).toString('utf8')).toBe('{"a":2,"z":1}\n')
  })
  ...
  it('makes checksum catalogs deterministic', () => {
    ...
    expect(serializeRcodepackChecksums([left, right])).toEqual(serializeRcodepackChecksums([right, left]))
  })
})

describe('Rcodepack container', () => {
  it('creates and verifies an unencrypted ZIP64 package without overwriting an existing destination', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'portable.Rcodepack')
    const created = await createRcodepackPackage({
    ...
    await expect(createRcodepackPackage({
      outputPath: packagePath,
      manifest: manifest(),
      ...sampleInputs()
  ...
  it('creates and verifies a passphrase-encrypted framed package', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'encrypted.Rcodepack')
    const settings = createRcodepackPassphraseEncryption((size) => Buffer.alloc(size, size))
    const created = await createRcodepackPackage({
      outputPath: packagePath,
      manifest: manifest(),
      ...sampleInputs(),
  ...
  it('rejects a wrong passphrase, ciphertext tampering, and a truncated frame', async () => {
    ...
    await createRcodepackPackage({
      ...
      encryptionSettings: createRcodepackPassphraseEncryption((size) => Buffer.alloc(size, 7))
    })

    await expect(verifyRcodepackPackage({
      packagePath,
      materializedZipPath: join(root, 'wrong-password.zip'),
      passphrase: 'wrong-password'
    })).rejects.toThrow('passphrase or authenticated payload is invalid')

    const envelope = await readRcodepackEnvelopeHeader(packagePath)
    const tamperedPath = join(root, 'tampered.Rcodepack')
    ...
    const truncatedPath = join(root, 'truncated.Rcodepack')
  ...
  it('detects corruption in an unencrypted package before ZIP parsing', async () => {
    const root = await temporaryDirectory()
    const packagePath = join(root, 'plain.Rcodepack')
    await createRcodepackPackage({ outputPath: packagePath, manifest: manifest(), ...sampleInputs() })
  ...
  it('rejects a modified manifest even when the outer payload hash is recomputed', async () => {
    ...
    const packagePath = join(root, 'modified-manifest.Rcodepack')
    ...
    const structuralEntries = await prepareZip64ArchiveEntries([
      {
        ...
        kind: 'catalog',
        source: { kind: 'buffer', data: serializeRcodepackJson(modifiedManifest) }
      },
      {
