...
} from './Rcode-runtime-capability-config'
import {
...
} from './Rcode-runtime-model-config'
import { subagentProfilesForRuntime } from './Rcode-runtime-subagent-config'
import {
...
export async function syncGuiManagedRcodeConfig(
  dataDir: string,
  ...
  const existing = sanitizeRcodeConfigSections(await readJsonObjectIfExists(configPath))
  const importedMcpServers = stripGeneratedProjectMcpServers(
    await readGuiManagedMcpServers(
      options?.mcpConfigPath ?? resolveRcodeMcpJsonPath()
    )
  )
    ...
    capabilities: {
      ...
      mcp: {
        ...
        servers: {
          ...
          ...projectMcpServers,
          ...(options?.scheduleMcp ? {
            [GUI_SCHEDULE_MCP_SERVER_NAME]: buildGuiScheduleRcodeMcpServer(
  ...
  const parsed = RcodeConfigSchema.safeParse(next)
  if (!parsed.success) {
    throw new Error(
      `Refusing to write invalid GUI-managed Rcode config at ${configPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
...
function defaultCredentialSourceId(settings: AppSettingsV1): string {
  const storedRuntime = getRcodeRuntimeSettings(settings)
...
type RcodeRuntimeConfigSettings = Pick<RcodeRuntimeSettingsV1,
...
export function classifyManagedRuntimeHotApplyResponse(
  ...
  return {
    result: 'failed',
    message: message || `Rcode hot config apply failed with HTTP ${status}`
  }
}
...
function sanitizeRcodeConfigSections(
  existing: Record<string, unknown> | null
): Record<string, unknown> | null {
  if (!existing) return null
  const hooks = parseRcodeHooksSection(existing.hooks)
  return {
    serve: parseRcodeConfigSection(RcodeServeConfigSchema, existing.serve),
    models: parseRcodeConfigSection(ModelConfigSchema, existing.models),
    contextCompaction: parseRcodeConfigSection(ContextCompactionConfigSchema, existing.contextCompaction),
    runtime: parseRcodeConfigSection(RuntimeTuningConfigSchema, existing.runtime),
    quality: parseRcodeConfigSection(QualityConfigSchema, existing.quality),
    capabilities: sanitizeCapabilities(existing.capabilities),
    ...('roles' in existing ? { roles: parseRcodeConfigSection(RolesConfigSchema, existing.roles) } : {}),
    ...(hooks.length ? { hooks } : {})
  }
}
...
function sanitizeCapabilities(value: unknown): Record<string, unknown> {
  ...
  for (const [key, schema] of Object.entries(schemas)) {
    if (key in raw) next[key] = parseRcodeConfigSection(schema, raw[key])
  }
  return next
}

function parseRcodeConfigSection(schema: SafeParseSchema, value: unknown): Record<string, unknown> {
  const raw = objectValue(value)
...
function parseRcodeHooksSection(value: unknown): unknown[] {
