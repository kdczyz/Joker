...
const RCODE_READY_PREFIX = 'RCODE_READY '
const RCODE_STARTUP_TIMEOUT_FLOOR_MS = 15_000
const RCODE_STARTUP_TIMEOUT_CEILING_MS = 600_000
const STDERR_TAIL_MAX_CHARS = 32_768

export type RcodeStartupHealthOptions = {
  timeoutMs?: number
  healthPollMs?: number
...
export type RcodeRuntimeHealthProbeResult = { healthy: boolean; error: string }

export type RcodeRuntimeHealthMonitorDeps<Settings> = {
...
export class RcodeRuntimeHealthMonitor<Settings> {
  private readonly inFlight = new Map<string, Promise<RcodeRuntimeHealthProbeResult>>()

  constructor(private readonly deps: RcodeRuntimeHealthMonitorDeps<Settings>) {}
  ...
  probeOnce(
    ...
    remainingMs = 1_000
  ): Promise<RcodeRuntimeHealthProbeResult> {
    const existing = this.inFlight.get(base)
    if (existing) return existing
    let task: Promise<RcodeRuntimeHealthProbeResult>
    task = (async () => {
      try {
        ...
        const healthy = response.ok && isRcodeHealthResponseBody(await response.text())
...
export function resolveRcodeStartupTimeoutMs(
  platform: NodeJS.Platform,
  env: NodeJS.ProcessEnv
): number {
  const raw = env.RCODE_STARTUP_TIMEOUT_MS
  if (raw && raw.trim()) {
    const parsed = Number(raw)
    if (Number.isFinite(parsed)) {
      return Math.min(
        RCODE_STARTUP_TIMEOUT_CEILING_MS,
        Math.max(RCODE_STARTUP_TIMEOUT_FLOOR_MS, Math.floor(parsed))
      )
    }
  }
...
export async function waitForRcodeStartup(
  startedChild: ChildProcess,
  port?: number,
  options: RcodeStartupHealthOptions = {}
): Promise<void> {
  const timeoutMs = options.timeoutMs ?? resolveRcodeStartupTimeoutMs(process.platform, process.env)
  const healthPollMs = options.healthPollMs ?? 500
  const probeHealth = options.probeHealth ?? ((targetPort) => probeRcodeHealth(
    targetPort,
    options.healthRequestTimeoutMs ?? 1_000
  ))
  if (startedChild.exitCode !== null) {
    throw new Error(describeRcodeExit(startedChild.exitCode, null))
  }
  await new Promise<void>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      settled = true
      cleanup()
      reject(new Error(describeRcodeStartupTimeout(timeoutMs, stderrTail, readyMarkerSeen && Boolean(port))))
    }, timeoutMs)
    ...
    const tryParseReady = (): boolean => {
      const markerIndex = stdoutBuffer.indexOf(RCODE_READY_PREFIX)
      if (markerIndex < 0) return false
      const afterPrefix = stdoutBuffer.slice(markerIndex + RCODE_READY_PREFIX.length)
      ...
      try {
        ...
        return parsed.service === 'Rcode' && parsed.mode === 'serve' && typeof parsed.port === 'number'
      } catch {
        return false
      }
    }
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      settled = true
      cleanup()
      reject(new Error(describeRcodeExit(code, signal, stderrTail)))
    }
...
export function describeRcodeExit(
  code: number | null,
  signal: NodeJS.Signals | null,
  stderrTail = ''
): string {
  const suffix = stderrTail.trim() ? `\n${stderrTail.trim()}` : ''
  if (signal) return `Rcode exited during startup with signal ${signal}${suffix}`
  if (typeof code === 'number') return `Rcode exited during startup with code ${code}${suffix}`
  return `Rcode exited during startup${suffix}`
}

export function describeRcodeStartupTimeout(
  timeoutMs: number,
  stderrTail: string,
  ...
  const suffix = stderrTail.trim() ? `\n${stderrTail.trim()}` : ''
  if (sawReadyMarker) {
    return `Rcode reported ready but did not pass health checks within ${timeoutMs}ms${suffix}`
  }
  return `Rcode did not report ready within ${timeoutMs}ms${suffix}`
}

export async function probeRcodeHealth(port: number, timeoutMs = 1_000): Promise<boolean> {
  try {
    const response = await fetch(`http://127.0.0.1:${port}/health`, {
      signal: AbortSignal.timeout(timeoutMs)
    })
    if (!response.ok) return false
    return isRcodeHealthResponseBody(await response.text())
  } catch {
    return false
  }
}
