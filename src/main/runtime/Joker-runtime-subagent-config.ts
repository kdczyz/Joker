import type { JokerSubagentsSettingsV1 } from '../../shared/app-settings'
import { SubagentsCapabilityConfig } from '../../../Joker/src/contracts/capabilities.js'
...
export function subagentProfilesForRuntime(
  ...
  for (const profile of subagents.profiles) {
    // Joker installs first-party profiles at composition time, so omitting a
  ...
  void appendManagedLogLine(
    'Joker',
    `[${new Date().toISOString()}] [LIFECYCLE] [Joker] [settings] dropped invalid subagent profiles: ${
