...
} from './Joker-runtime-capability-config'
import {
...
} from './Joker-runtime-model-config'
import { subagentProfilesForRuntime } from './Joker-runtime-subagent-config'
import {
...
export async function syncGuiManagedJokerConfig(
  dataDir: string,
  runtime: JokerRuntimeConfigSettings,
  options?: {
  ...
  const existing = sanitizeJokerConfigSections(await readJsonObjectIfExists(configPath))
  const importedMcpServers = stripGeneratedProjectMcpServers(
    await readGuiManagedMcpServers(
      options?.mcpConfigPath ?? resolveJokerMcpJsonPath()
    )
  )
    ...
    capabilities: {
      ...
      mcp: {
        ...
        servers: {
          ...
          ...projectMcpServers,
          ...(options?.scheduleMcp ? {
            [GUI_SCHEDULE_MCP_SERVER_NAME]: buildGuiScheduleJokerMcpServer(
  ...
  const parsed = JokerConfigSchema.safeParse(next)
  if (!parsed.success) {
    throw new Error(
      `Refusing to write invalid GUI-managed Joker config at ${configPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
...
function defaultCredentialSourceId(settings: AppSettingsV1): string {
  const storedRuntime = getJokerRuntimeSettings(settings)
...
type JokerRuntimeConfigSettings = Pick<JokerRuntimeSettingsV1,
...
export function classifyManagedRuntimeHotApplyResponse(
  ...
  return {
    result: 'failed',
    message: message || `Joker hot config apply failed with HTTP ${status}`
  }
}
...
function sanitizeJokerConfigSections(
  existing: Record<string, unknown> | null
): Record<string, unknown> | null {
  if (!existing) return null
  const hooks = parseJokerHooksSection(existing.hooks)
  return {
    serve: parseJokerConfigSection(JokerServeConfigSchema, existing.serve),
    models: parseJokerConfigSection(ModelConfigSchema, existing.models),
    contextCompaction: parseJokerConfigSection(ContextCompactionConfigSchema, existing.contextCompaction),
    runtime: parseJokerConfigSection(RuntimeTuningConfigSchema, existing.runtime),
    quality: parseJokerConfigSection(QualityConfigSchema, existing.quality),
    capabilities: sanitizeCapabilities(existing.capabilities),
    ...('roles' in existing ? { roles: parseJokerConfigSection(RolesConfigSchema, existing.roles) } : {}),
    ...(hooks.length ? { hooks } : {})
  }
}
...
function sanitizeCapabilities(value: unknown): Record<string, unknown> {
  ...
  for (const [key, schema] of Object.entries(schemas)) {
    if (key in raw) next[key] = parseJokerConfigSection(schema, raw[key])
  }
  return next
}

function parseJokerConfigSection(schema: SafeParseSchema, value: unknown): Record<string, unknown> {
  const raw = objectValue(value)
...
function parseJokerHooksSection(value: unknown): unknown[] {
