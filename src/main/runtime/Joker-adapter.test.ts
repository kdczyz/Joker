...
import { runtimeRequestViaHost } from './Joker-adapter'
...
function settingsForPort(port: number): AppSettingsV1 {
  return {
    ...
    agents: {
      Joker: {
        ...defaultJokerRuntimeSettings(port),
...
describe('runtimeRequestViaHost', () => {
  it('forwards daily usage requests to the Joker runtime with bearer auth', async () => {
    let seenUrl = ''
