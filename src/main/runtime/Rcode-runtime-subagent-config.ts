import type { RcodeSubagentsSettingsV1 } from '../../shared/app-settings'
import { SubagentsCapabilityConfig } from '../../../Rcode/src/contracts/capabilities.js'
...
export function subagentProfilesForRuntime(
  ...
  for (const profile of subagents.profiles) {
    // Rcode installs first-party profiles at composition time, so omitting a
  ...
  void appendManagedLogLine(
    'Rcode',
    `[${new Date().toISOString()}] [LIFECYCLE] [Rcode] [settings] dropped invalid subagent profiles: ${
