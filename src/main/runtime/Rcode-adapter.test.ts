...
import { runtimeRequestViaHost } from './Rcode-adapter'
...
function settingsForPort(port: number): AppSettingsV1 {
  return {
    ...
    agents: {
      Rcode: {
        ...defaultRcodeRuntimeSettings(port),
...
describe('runtimeRequestViaHost', () => {
  it('forwards daily usage requests to the Rcode runtime with bearer auth', async () => {
    let seenUrl = ''
