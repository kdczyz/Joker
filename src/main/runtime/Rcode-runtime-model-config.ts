import {
  defaultRcodeTokenEconomySettings,
...
export function modelConfigForRuntime(
  ...
  const profileDefaults = { ...DEFAULT_RCODE_MODEL_PROFILES, ...guiProfiles }
...
export function providersConfigForRuntime(
  ...
  for (const provider of getModelProviderSettings(settings).providers as ModelProviderProfileV1[]) {
    ...
    out[id] = {
      ...
      // Credential-derived transport headers are reconstructed in Rcode from
...
export function tokenEconomyConfigForRuntime(
  tokenEconomy: Pick<RcodeRuntimeSettingsV1, 'tokenEconomy'>['tokenEconomy'] | undefined,
  existing: Record<string, unknown>
): Record<string, unknown> {
  const defaults = defaultRcodeTokenEconomySettings()
  const normalized = {
    ...defaults,
    ...(tokenEconomy ?? {}),
...
export function toolOutputLimitsConfigForRuntime(
  limits: Pick<RcodeRuntimeSettingsV1, 'toolOutputLimits'>['toolOutputLimits'] | undefined
): Record<string, unknown> {
  return { maxLines: limits?.maxLines, maxBytes: limits?.maxBytes }
}

export function storageConfigForRuntime(
  storage: Pick<RcodeRuntimeSettingsV1, 'storage'>['storage']
): Record<string, unknown> {
...
export function contextCompactionConfigForRuntime(
  value: Pick<RcodeRuntimeSettingsV1, 'contextCompaction'>['contextCompaction'],
  existing: Record<string, unknown>
): Record<string, unknown> {
  return {
    ...existing,
