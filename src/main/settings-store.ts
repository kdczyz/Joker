...
import { atomicWriteFile } from '../../Joker/src/adapters/file/atomic-write.js'
import {
  ...
  mergeClawSettings,
  defaultRemoteAgentSettings,
  mergeRemoteAgentSettings,
...
// 数据默认根目录从 ~/.deepseekgui 升级为 ~/.Joker。老安装的既有目录由
// legacy-data-migration.ts 在启动期搬迁并留兼容链接;settings 里存的旧
// 绝对路径也在那里按迁移结果重写,这里只负责“新值”。
const DEFAULT_WORKSPACE_ROOT = join(homedir(), '.Joker', 'default_workspace')
...
const DEFAULT_CONVERSATION_WORKSPACE_ROOT_ABSOLUTE =
  process.platform === 'linux'
    ? join(homedir(), '.local', 'share', 'Joker', 'conversations')
    : join(homedir(), 'Documents', 'Joker')
const DEFAULT_CLAW_CHANNELS_ROOT = join(homedir(), '.Joker', 'claw')
const DEFAULT_WRITE_WORKSPACE_ROOT_ABSOLUTE = expandHomePath(DEFAULT_WRITE_WORKSPACE_ROOT)
const SETTINGS_FILE_NAME = 'Joker-settings.json'
...
const defaultSettings = (): AppSettingsV1 => ({
  ...
  remoteAgent: defaultRemoteAgentSettings(),
...
function buildMergedSettings(parsed: Partial<AppSettingsV1>): AppSettingsV1 {
  ...
  return {
    ...
    agents: JokerSettingsEnvelope(
      mergeJokerRuntimeSettings(getJokerRuntimeSettings(defaults), migrated.agents?.Joker)
    ),
    ...
    remoteAgent: mergeRemoteAgentSettings(defaults.remoteAgent, migrated.remoteAgent),
...
function hasLegacyProviderPlaintext(settings: AppSettingsV1): boolean {
  ...
  return getJokerRuntimeSettings(settings).apiKey.trim().length > 0
}
...
async function replaceInvalidSettingsWithDefaults(
  ...
  if (backupPath) {
    console.warn(
      `[Joker-gui] Invalid settings were replaced with defaults (${reason}). Backup: ${backupPath}`
    )
  } else {
    console.warn(
      `[Joker-gui] Invalid settings were replaced with defaults (${reason}). Backup could not be written for ${sourcePath}.`
    )
  }
  return defaults
}
...
export class JsonSettingsStore {
  ...
  async load(): Promise<AppSettingsV1> {
    ...
    if (this.options.credentialMigration && hasLegacyProviderPlaintext(prepared)) {
      ...
      if (!backupPath) {
        console.warn('[Joker-gui] Legacy credential migration deferred because the settings backup could not be written.')
        this.cache = prepared
        return this.cache
      }
    }
    ...
    if (shouldPersist) {
      try {
        ...
        console.warn('[Joker-gui] Legacy credential migration settings commit failed; plaintext settings remain authoritative.', {
    ...
    await migration.commit().catch((error) => {
      console.warn('[Joker-gui] Legacy credential migration commit marker is pending recovery.', {
        message: error instanceof Error ? error.message : String(error)
      })
    })
  ...
  async patch(partial: AppSettingsPatch): Promise<AppSettingsV1> {
    ...
    const next = normalizeStoredSettings({
      ...applyJokerRuntimePatch(cur, agentsPatch?.Joker),
      ...restPatch,
  ...
  private async prepareCredentialMigration(
    ...
    try {
      ...
      console.warn('[Joker-gui] Legacy credential migration is unavailable; retaining compatibility settings.', {
        message: error instanceof Error ? error.message : String(error)
      })
      return null
    }
