...
function createSettings(binaryPath: string): AppSettingsV1 {
  return {
    ...
    agents: {
      Joker: {
        ...defaultJokerRuntimeSettings(testJokerPort),
        binaryPath,
        autoStart: true
      }
    },
...
async function readJokerLog(): Promise<string> {
  if (!tempRoot) throw new Error('temp root not initialized')
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const logFile = readdirSync(tempRoot).find((entry) => entry.startsWith('Joker-') && entry.endsWith('.log'))
    if (logFile) return readFileSync(join(tempRoot, logFile), 'utf8')
    await new Promise((resolve) => setTimeout(resolve, 25))
  }
  throw new Error('Expected a Joker log file to be created')
}
...
beforeEach(async () => {
  tempRoot = mkdtempSync(join(tmpdir(), 'Joker-process-'))
  testJokerPort = await allocateTestPort()
  configureLogger({ dir: tempRoot, enabled: true, retentionDays: 7 })
})

afterEach(async () => {
  const module = await import('./Joker-process')
  await module.stopJokerChildAndWait()
...
describe('startJokerChild', () => {
  it('waits for the explicit Joker ready marker before resolving', async () => {
    const script = writeScript(
      'ready-child.js',
      [
        "const http = require('node:http')",
        `const port = ${testJokerPort}`,
        "const server = http.createServer((req, res) => {",
        "  res.setHeader('content-type', 'application/json')",
        "  res.end(JSON.stringify({ service: 'Joker', mode: 'serve', status: 'ok' }))",
        "})",
        "server.listen(port, '127.0.0.1', () => {",
        "  setTimeout(() => {",
        "    process.stdout.write('JOKER_READY ' + JSON.stringify({ service: 'Joker', mode: 'serve', port }) + '\\n')",
        "  }, 50)",
        "})",
        "setInterval(() => {}, 1_000)"
      ].join('\n')
    ...
    const module = await import('./Joker-process')
    await expect(module.startJokerChild(createSettings(script))).resolves.toBeUndefined()
    expect(module.isJokerChildRunning()).toBe(true)
    await module.stopJokerChildAndWait()
    const logText = await readJokerLog()
    expect(logText).toContain('JOKER_READY')
    expect(logText).toContain(`ready marker received on port ${testJokerPort}`)
  })
  ...
  it('does not settle on the ready marker until the /health endpoint responds', async () => {
    ...
    const module = await import('./Joker-process')
    let resolved = false
    const start = module.startJokerChild(createSettings(script)).then(() => {
      resolved = true
    })
  ...
  it('shares the startup promise while Joker is spawned but not ready', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    ...
    const first = module.startJokerChild(settings)

    for (let attempt = 0; attempt < 100 && !module.isJokerChildRunning(); attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10))
    }
    expect(module.isJokerChildRunning()).toBe(true)

    let secondResolved = false
    const second = module.startJokerChild(settings).then(() => {
      secondResolved = true
    })
  ...
  it('rejects when the child exits before reporting ready', async () => {
    ...
    const module = await import('./Joker-process')
    await expect(module.startJokerChild(createSettings(script))).rejects.toThrow(
      /Joker exited during startup with code 23[\s\S]*bind failed on port 18899/
    )
    expect(module.isJokerChildRunning()).toBe(false)
    await module.stopJokerChildAndWait()
    const logText = await readJokerLog()
...
describe('resolveJokerStartupTimeoutMs', () => {
  it('gives Windows the larger default and other platforms a smaller one', async () => {
    const { resolveJokerStartupTimeoutMs } = await import('./Joker-process')
    expect(resolveJokerStartupTimeoutMs('win32', {})).toBe(90_000)
    expect(resolveJokerStartupTimeoutMs('darwin', {})).toBe(60_000)
    expect(resolveJokerStartupTimeoutMs('linux', {})).toBe(60_000)
  })

  it('honors a valid JOKER_STARTUP_TIMEOUT_MS override on every platform', async () => {
    const { resolveJokerStartupTimeoutMs } = await import('./Joker-process')
    expect(resolveJokerStartupTimeoutMs('win32', { JOKER_STARTUP_TIMEOUT_MS: '120000' })).toBe(120_000)
    expect(resolveJokerStartupTimeoutMs('linux', { JOKER_STARTUP_TIMEOUT_MS: ' 30000 ' })).toBe(30_000)
  })
  ...
  it('clamps an out-of-range override to the 15s–10min bounds', async () => {
    ...
    expect(resolveJokerStartupTimeoutMs('linux', { JOKER_STARTUP_TIMEOUT_MS: '99999999' })).toBe(600_000)
  })

  it('falls back to the platform default when the override is not a finite number', async () => {
    const { resolveJokerStartupTimeoutMs } = await import('./Joker-process')
    expect(resolveJokerStartupTimeoutMs('win32', { JOKER_STARTUP_TIMEOUT_MS: 'soon' })).toBe(90_000)
    expect(resolveJokerStartupTimeoutMs('darwin', { JOKER_STARTUP_TIMEOUT_MS: '' })).toBe(60_000)
    expect(resolveJokerStartupTimeoutMs('darwin', { JOKER_STARTUP_TIMEOUT_MS: '   ' })).toBe(60_000)
  })
})

describe('waitForJokerStartupSettled', () => {
  it('resolves immediately when no launch is in flight', async () => {
    const module = await import('./Joker-process')
    let resolved = false
    await Promise.race([
      module.waitForJokerStartupSettled().then(() => {
        resolved = true
      }),
  ...
  it('does not resolve until an in-flight launch settles', async () => {
    ...
    const module = await import('./Joker-process')
    const settings = createSettings(script)
    const start = module.startJokerChild(settings)
    ...
    let settled = false
    const settledPromise = module.waitForJokerStartupSettled().then(() => {
      settled = true
    })
    await new Promise((resolve) => setTimeout(resolve, 50))
...
describe('reclaimJokerPort', () => {
  it('reports a port as unavailable when another listener owns it', async () => {
    ...
    try {
      ...
      await expect(module.reclaimJokerPort(address.port)).resolves.toEqual({
        ok: false,
  ...
  it('allows non-positive ports so Joker can request an ephemeral port', async () => {
    const module = await import('./Joker-process')

    await expect(module.reclaimJokerPort(0)).resolves.toEqual({ ok: true })
  })
  ...
  it('resolves the next available fallback port when the preferred port is unavailable', async () => {
    ...
    try {
      ...
      const resolved = await module.resolveAvailableJokerPort(preferredPort)
      ...
      await expect(module.reclaimJokerPort(resolved.port)).resolves.toEqual({ ok: true })
    } finally {
      await new Promise<void>((resolve) => server.close(() => resolve()))
    }
  })

  it('keeps the configured endpoint when the currently managed Joker child owns it', async () => {
    ...
    const module = await import('./Joker-process')
    const settings = createSettings(script)
    settings.agents.Joker.port = preferredPort

    await module.startJokerChild(settings)
    const resolved = await module.resolveAvailableJokerPort(preferredPort)

    expect(resolved).toEqual({ port: preferredPort, changed: false })
    expect(module.isJokerChildRunning()).toBe(true)
    expect(await readJokerLog()).not.toContain(`killing stale Joker process holding port ${preferredPort}`)
  })
})

describe('resolveJokerDataDir', () => {
  it('expands Windows-style home-relative data directories', async () => {
    const module = await import('./Joker-process')

    expect(module.resolveJokerDataDir({ dataDir: '~\\deepseek\\Joker' })).toBe(join(homedir(), 'deepseek', 'Joker'))
  })

  it('does not expand non-home tilde prefixes', async () => {
    const module = await import('./Joker-process')

    expect(module.resolveJokerDataDir({ dataDir: '~other\\Joker' })).toBe('~other\\Joker')
  })
})
...
describe('parseListeningPidsFromNetstat', () => {
  it('extracts the listening TCP PIDs for the port across IPv4/IPv6, ignoring everything else', async () => {
    const { parseListeningPidsFromNetstat } = await import('./Joker-process')
...
describe('syncGuiManagedJokerConfig', () => {
  it('creates GUI-managed config with attachments enabled for image paste/upload', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Joker-process')

    await module.syncGuiManagedJokerConfig(tempRoot, defaultJokerRuntimeSettings())
  ...
  it('exports per-model max output tokens into Joker model profiles', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Joker-process')

    await module.syncGuiManagedJokerConfig(tempRoot, {
      ...defaultJokerRuntimeSettings(),
      modelProfiles: {
        writer: {
    ...
    const parsed = JSON.parse(readFileSync(configPath, 'utf8')) as any
    expect(JokerConfigSchema.safeParse(parsed).success).toBe(true)
  ...
  it('writes the selected provider endpoint into the default model client config', async () => {
    ...
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Joker-process')
    const settings = createSettings('/tmp/fake-Joker-child.js')
    ...
    await module.syncGuiManagedJokerConfig(tempRoot, resolveJokerRuntimeSettings(settings), {
      scheduleMcp: {
        settings,
        launch: {
  ...
  it('writes the image generation capability and omits cleared fields', async () => {
    ...
    await module.syncGuiManagedJokerConfig(tempRoot, runtime)
  ...
  it('keeps the config stable across repeated syncs with imageGen configured', async () => {
    ...
    // every sync rewrites the file and restarts Joker in a loop.
    await module.syncGuiManagedJokerConfig(tempRoot, runtime)
  ...
  it('adds the built-in schedule MCP server to Joker runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Joker-process')
    const settings = createSettings('/tmp/fake-Joker-child.js')
  ...
  it('adds GUI project and configured global skill roots to Joker runtime capabilities', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
    const module = await import('./Joker-process')
    const settings = createSettings('/tmp/fake-Joker-child.js')
  ...
  it('drops stale Codex plugin cache roots but keeps hand-added manual roots', async () => {
    ...
    // added by hand to the Joker config file.
  ...
  it('writes GUI-managed MCP search settings without removing existing servers', async () => {
    ...
    await module.syncGuiManagedJokerConfig(
      tempRoot,
      {
        ...defaultJokerRuntimeSettings(),
        storage: {
          backend: 'hybrid',
          sqlitePath: '/tmp/Joker-index.sqlite3'
        },
        contextCompaction: {
  ...
  it('does not auto-import repo-local .Joker/mcp.json servers into the runtime', async () => {
    ...
    mkdirSync(join(repo, '.Joker'), { recursive: true })
    writeFileSync(join(repo, '.Joker', 'mcp.json'), JSON.stringify({
      servers: {
  ...
  it('replaces unparsable historical Joker config with a valid GUI-managed config', async () => {
    if (!tempRoot) throw new Error('temp root not initialized')
    const configPath = join(tempRoot, 'config.json')
  ...
  it('does not enable MCP when the capability is explicitly disabled', async () => {
    ...
    await module.syncGuiManagedJokerConfig(tempRoot, defaultJokerRuntimeSettings(), {
      scheduleMcp: {
        settings: createSettings('/tmp/fake-Joker-child.js'),
        launch: {
          appPath: '/tmp/deepseek-gui-test-app',
          execPath: '/tmp/electron',
          isPackaged: false
