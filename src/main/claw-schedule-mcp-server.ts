...
export async function runClawScheduleMcpServerFromArgv(argv: string[]): Promise<boolean> {
  ...
  const server = new McpServer(
    { name: 'Joker-schedule', version: '0.1.0' },
  ...
  const registerListTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. List scheduled tasks managed by the currently running Joker app.'
        : 'List scheduled tasks managed by the currently running Joker app.'
    }, async () => {
      try {
  ...
  const registerCreateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Create a scheduled task in Joker. Supports one-time (`at`), daily, or interval schedules.'
        : 'Create a scheduled task in Joker. Supports one-time (`at`), daily, or interval schedules. When creating from an existing conversation, pass that conversation\'s provider_id, model, and reasoning_effort so the scheduled task keeps the same execution settings.',
      inputSchema: {
        ...
        provider_id: z.string().optional().describe('Optional model provider id configured in Joker settings'),
  ...
  const registerUpdateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Update an existing Joker scheduled task.'
        : 'Update an existing Joker scheduled task.',
      inputSchema: {
  ...
  const registerDeleteTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Delete a scheduled task from Joker.'
        : 'Delete a scheduled task from Joker.',
      inputSchema: {
        task_id: z.string().min(1).describe('Task id returned by gui_schedule_list or gui_schedule_create')
      }
    }, async ({ task_id }) => {
  ...
  // native Joker `create_plan` tool. See RETIRED_CLAW_GUI_PLAN_TOOL_NAMES
  ...
  if (options.workflowBaseUrl) {
    ...
    // without needing a Joker restart: re-fetch on an interval and update the tool
