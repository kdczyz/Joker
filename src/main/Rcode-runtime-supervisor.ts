/**
 * Crash-loop budget and status contract for the GUI-managed Rcode
...
export type RcodeRuntimeStatus = RcodeRuntimeStatusPayload
...
export type RcodeRuntimeSupervisorDeps<Settings> = {
  ...
  isStopped: () => boolean
  publish: (status: RcodeRuntimeStatus) => void
...
export class RcodeRuntimeSupervisor<Settings> {
  ...
  private readonly deps: RcodeRuntimeSupervisorDeps<Settings>
  private watchdogTimer: ReturnType<typeof setInterval> | null = null
  private watchdogFailures = 0
  private watchdogTickInFlight = false
  private crashRecoveryInFlight = false
  private currentStatus: RcodeRuntimeStatus | null = null

  constructor(options: {
    deps: RcodeRuntimeSupervisorDeps<Settings>
  ...
  get lastStatus(): RcodeRuntimeStatus | null {
  ...
  publish(status: Omit<RcodeRuntimeStatus, 'at'>): void {
    const full: RcodeRuntimeStatus = { ...status, at: new Date().toISOString() }
  ...
  handleUnexpectedExit(info: { code: number | null; signal: NodeJS.Signals | null; stderrTail: string }): void {
    void this.recoverFromCrash(info).catch((error: unknown) => {
      this.deps.error('Rcode-supervisor', 'supervised restart crashed', {
  ...
  startWatchdog(): void {
    ...
    const timer = schedule(() => {
      void this.watchdogTick().catch((error: unknown) => {
        this.deps.warn('Rcode-watchdog', 'watchdog tick failed', {
          message: error instanceof Error ? error.message : String(error)
        })
      })
  ...
  async watchdogTick(): Promise<void> {
    ...
    try {
      ...
      this.publish({
        state: 'restarting',
        source: 'watchdog',
        message: 'Rcode stopped responding to health checks; restarting it.'
      })
      try {
        ...
        this.publish({
          state: 'failed',
          source: 'watchdog',
          message: `Rcode is unresponsive and the automatic restart failed: ${
  ...
  private async recoverFromCrash(info: {
    ...
    this.publish({
      state: 'crashed',
      source: 'supervisor',
      message: `Rcode exited unexpectedly (${exitLabel}).`,
    ...
    try {
      ...
      if (!this.deps.canAutoRestart(settings)) {
        this.publish({
          state: 'stopped',
          source: 'supervisor',
          message: 'Rcode exited and automatic restart is unavailable (missing API key or auto-start disabled).'
        })
        return
      }
      let lastError = ''
      for (;;) {
        ...
        if (!verdict.allowed) {
          this.publish({
            ...
            source: 'supervisor',
            message: lastError
              ? `Rcode keeps crashing; automatic restarts are paused. Last error: ${lastError}`
              : 'Rcode keeps crashing; automatic restarts are paused. Check the runtime logs, then retry.',
            stderrTail: info.stderrTail
          })
          return
        }
        this.publish({
          ...
          maxAttempts: 3,
          message: `Restarting Rcode automatically (attempt ${verdict.attempt}/3).`
        })
