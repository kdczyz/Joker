...
import type { ImageGenClient, ImageGenEditRequest, ImageGenRequest } from '../../../Rcode/src/adapters/tool/image-gen-tool-provider.js'
...
describe('write infographic service', () => {
  ...
  it('writes into a nested imageDir and keeps the relative link clean', async () => {
    ...
    const result = await requestWriteInfographic(settingsWithImageGen(), {
      text: '需求：支持扫码登录。',
      filePath: join(workspace, '.Rcodesdd', 'draft', 'dc040c2d', 'requirement.md'),
      workspaceRoot: workspace,
      imageDir: '.Rcodesdd/img',
      kind: 'design'
    }, { client })
    ...
    expect(result.absolutePath).toBe(join(workspace, '.Rcodesdd', 'img', result.fileName))
    expect(existsSync(result.absolutePath)).toBe(true)
  })
  ...
  it('uses selected reference images for design drafts', async () => {
    const client = fakeClient()
    const referencePath = join(workspace, '.Rcodesdd', 'requirements', 'draft-1', 'img', 'source.png')
    mkdirSync(dirname(referencePath), { recursive: true })
    writeFileSync(referencePath, PNG_BYTES)

    const result = await requestWriteInfographic(settingsWithImageGen(), {
      text: '根据参考图重绘一个更精致的旅行社区首页。',
      filePath: join(workspace, '.Rcodesdd', 'requirements', 'draft-1', 'requirement.md'),
      workspaceRoot: workspace,
      imageDir: '.Rcodesdd/requirements/draft-1/img',
      kind: 'design',
