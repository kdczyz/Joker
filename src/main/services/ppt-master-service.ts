 ...
 * in Joker/src/adapters/tool/ppt-master-tool.ts: the installer must fail closed.
 */
...
const INSTALL_METADATA_FILE = '.Joker-ppt-master.json'
const UPSTREAM_ENTRY_FILE = 'PPT_MASTER_UPSTREAM.md'
const MANAGED_BY = 'Joker-gui'
...
export function pptMasterSkillDir(JokerHomeDir: string): string {
  return join(JokerHomeDir, 'skills', 'ppt-master')
}
...
async function ensurePptMasterOnce(options: {
  ...
  const skillDir = pptMasterSkillDir(options.JokerHomeDir)
  ...
  try {
    ...
    await ensureJokerSkillEntry(skillDir)
...
async function ensureJokerSkillEntry(skillDir: string): Promise<void> {
  ...
  await writeFile(join(skillDir, 'SKILL.md'), JOKER_PPT_MASTER_SKILL, 'utf8')
...
// The upstream entry is ~77KB while Joker injects at most 24KB per active skill.
...
const JOKER_PPT_MASTER_SKILL = `---
name: ppt-master
description: Create native, editable PPTX presentations from Markdown in Write mode.
---

# PPT Master for Joker Write
...
- Create the project below \`<workspace>/.Joker-presentations/\`, and import the
  source with \`project_manager.py import-sources ... --copy\`. This overrides
  upstream's \`--move\` default for the Joker Write integration.
- Put the final canonical PPTX in \`<workspace>/presentations/\`. Keep SVG,
  project files, images, and backups under \`.Joker-presentations/\`.
