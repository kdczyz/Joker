...
beforeEach(async () => {
  userDataDir = await mkdtemp(join(tmpdir(), 'Joker-ui-plugin-data-'))
  sourceDir = await mkdtemp(join(tmpdir(), 'Joker-ui-plugin-src-'))
})
...
describe('seedUiPlugin (programmatic plugin seeding)', () => {
  it('seeds a plugin from in-memory bytes and it lists/loads like any other', async () => {
    ...
    const plugins = await listUiPlugins(userDataDir)
    expect(plugins.map((p) => p.manifest.id)).toContain('community-pack')

    const loaded = await loadUiPluginFigures(userDataDir, 'community-pack')
  ...
  it('rejects seeding when figure bytes are missing', async () => {
    const result = await seedUiPlugin(
      userDataDir,
      { id: 'community-pack', name: 'x', version: '1.0.0', figures: { swim: 'img/a.png' } },
      {}
    )
    expect(result.ok).toBe(false)
  })
...
describe('bundled starlight example', () => {
  it('installs and loads end to end', async () => {
    ...
    expect(loaded.ok).toBe(true)
    if (!loaded.ok) return
    expect(loaded.manifest.name).toBe('星夜 Joker')
    expect(loaded.figures.swim?.startsWith('data:image/png;base64,')).toBe(true)
    expect(
