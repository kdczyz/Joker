 ...
 * so the user only has to flip the toggle rather than add Rcode by hand.
 */
type MacPermissions = {
...
/**
 * Proactively enroll Rcode in the relevant macOS permission list and open
  ...
  try {
    if (kind === 'accessibility') {
      if (native?.askForAccessibilityAccess) {
        // Registers Rcode in the Accessibility list + opens the pane.
      ...
      if (native?.askForScreenCaptureAccess) {
        // CGRequestScreenCaptureAccess: registers Rcode in the Screen
