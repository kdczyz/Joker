 ...
 * so the user only has to flip the toggle rather than add Joker by hand.
 */
type MacPermissions = {
...
/**
 * Proactively enroll Joker in the relevant macOS permission list and open
  ...
  try {
    if (kind === 'accessibility') {
      if (native?.askForAccessibilityAccess) {
        // Registers Joker in the Accessibility list + opens the pane.
      ...
      if (native?.askForScreenCaptureAccess) {
        // CGRequestScreenCaptureAccess: registers Joker in the Screen
