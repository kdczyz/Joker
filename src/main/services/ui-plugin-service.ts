...
/**
 * UI 插件落盘服务。插件目录: ~/.Joker/ui-plugins/<id>/
 * 安装走“白名单复制”:只复制 manifest.json 与 figures/backgrounds 引用的图片,
 * 源目录里的其它任何文件(脚本、可执行文件等)一概不进入 Joker 数据目录。
 */
...
export function uiPluginsRootDir(JokerHomeDir: string): string {
  return join(JokerHomeDir, 'ui-plugins')
}
