...
/**
 * Enabled, on-disk skill roots passed to the Joker runtime. Builds the common
 * directory conventions (.agents/.claude/.codex/.Joker/skills + global equivalents)
...
export async function guiSkillRootsForRuntime(
  ...
  const candidates = buildSkillRootCandidates(settings, workspaceRootOverride).filter((candidate) => {
    ...
    // them deliberately and Joker tolerates missing roots); common conventions
