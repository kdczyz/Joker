...
describe('app-ipc-schemas', () => {
  ...
  it('accepts the Joker runtime info endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the Joker runtime tool diagnostics endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
  ...
  it('accepts Joker supply-chain audit endpoints', () => {
  ...
  it('accepts Joker MCP OAuth status and token reset endpoints', () => {
    expect(runtimeRequestPayloadSchema.parse({
      path: '/v1/mcp/oauth',
      method: 'GET'
  ...
  it('accepts the Joker skills endpoint', () => {
    const payload = runtimeRequestPayloadSchema.parse({
      path: '/v1/skills',
      method: 'GET'
    })
  ...
  it('accepts Joker attachment and memory endpoints', () => {
    expect(runtimeRequestPayloadSchema.parse({
      path: '/v1/attachments',
      method: 'POST',
      body: '{}'
  ...
  it('accepts Joker thread goal endpoints', () => {
    expect(runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the Joker thread review endpoint', () => {
    expect(runtimeRequestPayloadSchema.parse({
  ...
  it('accepts the read-only Joker turn status endpoint', () => {
    expect(runtimeRequestPayloadSchema.parse({
  ...
  it('rejects runtime request paths outside the modeled Joker API surface', () => {
    expect(() =>
  ...
  it('accepts a valid settings patch for Joker and write settings', () => {
    ...
    expect(payload.agents?.Joker?.port).toBe(19000)
    expect(payload.agents?.Joker?.modelProfiles?.['custom-vision-model']?.inputModalities).toEqual(['text', 'image'])
    expect(payload.agents?.Joker?.modelProfiles?.['custom-vision-model']?.maxOutputTokens).toBe(32000)
    expect(payload.agents?.Joker?.tokenEconomy?.enabled).toBe(true)
    expect(payload.agents?.Joker?.tokenEconomy?.historyHygiene?.maxToolResultTokens).toBe(4000)
    expect(payload.agents?.Joker?.toolOutputLimits?.maxLines).toBe(30000)
    expect(payload.agents?.Joker?.toolOutputLimits?.maxBytes).toBe(1048576)
    expect(payload.agents?.Joker?.subagents).toEqual({ maxParallel: 5 })
  ...
  it('accepts media generation settings and provider capability patches', () => {
    ...
    expect(payload.agents?.Joker?.textToSpeech?.enabled).toBe(true)
    expect(payload.agents?.Joker?.musicGeneration?.model).toBe('music-2.6')
    expect(payload.agents?.Joker?.videoGeneration?.defaultResolution).toBe('1080P')
  })
  ...
  it('accepts provider and resolved runtime retry settings', () => {
    ...
    expect(payload.agents?.Joker?.retry?.httpStatusCodes).toEqual([429, 503])
  })
  ...
  it('accepts long provider model ids imported from upstream catalogs', () => {
    ...
    expect(payload.agents?.Joker?.model).toBe(longModelId)
    expect(payload.agents?.Joker?.imageGeneration?.defaultResolution).toBe('2K')
    expect(payload.agents?.Joker?.imageGeneration?.quality).toBe('high')
    expect(payload.schedule?.model).toBe(longModelId)
    expect(payload.workflow?.model).toBe(longModelId)
    expect(() => settingsPatchSchema.parse({
      agents: { Joker: { imageGeneration: { defaultResolution: '4K' } } }
    })).toThrow()
  })
  ...
  it('strips legacy settings keys while preserving current skill settings', () => {
    ...
    expect(payload.agents?.Joker?.port).toBe(19001)
    expect(payload.agents?.Joker?.imageRecognition).toEqual({ enabled: true })
  ...
  it('accepts a configurable stream idle timeout in runtime tuning patches', () => {
    ...
    expect(payload.agents?.Joker?.runtimeTuning?.streamIdleTimeoutMs).toBe(300000)
  })
  ...
  it('accepts a configurable maximum turn duration in runtime tuning patches', () => {
    ...
    expect(payload.agents?.Joker?.runtimeTuning?.maxWallTimeMs).toBe(7_200_000)
  })

  it('rejects an out-of-range maximum turn duration', () => {
    expect(() =>
      settingsPatchSchema.parse({
        agents: { Joker: { runtimeTuning: { maxWallTimeMs: 86_400_001 } } }
      })
    ).toThrow()
  })
  ...
  it('rejects an out-of-range stream idle timeout', () => {
    expect(() =>
      settingsPatchSchema.parse({
        agents: { Joker: { runtimeTuning: { streamIdleTimeoutMs: -1 } } }
      })
    ).toThrow()
  })
  ...
  it('rejects out-of-range tool output limits', () => {
    expect(() =>
      settingsPatchSchema.parse({
        agents: { Joker: { toolOutputLimits: { maxBytes: 128 * 1024 * 1024 } } }
      })
    ).toThrow()
  })
  ...
  it('accepts content-only export payloads', () => {
    ...
    expect(payload.title).toBe('Joker answer')
  ...
  it('accepts workspace image pick payloads and rejects extra fields', () => {
    const payload = workspaceImagePickPayloadSchema.parse({
      workspaceRoot: '/tmp/workspace',
      currentFilePath: '/tmp/workspace/.Joker-design/abc/v1.html',
      imageDirectory: 'img'
    })
    expect(payload.workspaceRoot).toBe('/tmp/workspace')
    expect(payload.currentFilePath).toBe('/tmp/workspace/.Joker-design/abc/v1.html')
