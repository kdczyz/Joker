...
import {
  ...
  registerJokerMediaSchemeAsPrivileged
...
async function fixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'Joker-media-protocol-'))
  roots.push(root)
...
describe('Joker-media protocol', () => {
  it('registers a secure streaming scheme without bypassing CSP', () => {
    const registerSchemesAsPrivileged = vi.fn()
    registerJokerMediaSchemeAsPrivileged({ registerSchemesAsPrivileged } as never)
  ...
  it('registers both Extension schemes in the one permitted pre-ready call', () => {
    const registerSchemesAsPrivileged = vi.fn()
    registerJokerExtensionPlatformSchemesAsPrivileged({ registerSchemesAsPrivileged } as never)
    expect(registerSchemesAsPrivileged).toHaveBeenCalledTimes(1)
    expect(registerSchemesAsPrivileged).toHaveBeenCalledWith([
      expect.objectContaining({ scheme: 'Joker-extension' }),
      expect.objectContaining({ scheme: 'Joker-media' })
    ])
  })
  ...
  it('mints a path-free lease only for the exact active View binding', async () => {
    ...
    expect(state.lease.url).toMatch(/^Joker-media:\/\/lease\/[A-Za-z0-9_-]+$/)
  ...
  it('rejects copied URLs in another isolated View and stale sessions', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-media-protocol-isolation-'))
    roots.push(root)
    const path = join(root, 'source.mp4')
