 ...
 * imports no Node built-ins and never forwards the full `JokerGui` bridge.
 */
export function registerExtensionContentScriptPreload(
  ...
  const installPlan = (value: unknown): void => {
    ...
    for (const binding of plan.bindings) {
      ...
      try {
        dependencies.contextBridge.exposeInIsolatedWorld(binding.worldId, 'JokerHost', api)
      } catch (error) {
        void report(
          binding,
...
export function contentScriptIsolationPrelude(): string {
  return `(() => {
    'use strict';
    const hidden = ['JokerGui', 'require', 'module', 'exports', 'process', '__dirname', '__filename'];
    for (const key of hidden) {
      try { Object.defineProperty(globalThis, key, { value: undefined, writable: false, configurable: false }); } catch {}
    }
    const denied = () => { throw new DOMException('Direct network and popup access is disabled for Joker host content scripts.', 'SecurityError'); };
    const deniedFetch = () => Promise.reject(new DOMException('Direct network access is disabled for Joker host content scripts.', 'SecurityError'));
  ...
  return [
    {
      ...
      url: `Joker-extension://${binding.context.extensionId}/__Joker_isolation__.js`
    },
...
function styleInstallationCode(marker: string, css: string): string {
  return `(() => {
    ...
    const install = () => {
      ...
      document.querySelectorAll('style[data-Joker-extension-style]').forEach((node) => {
        if (node.getAttribute('data-Joker-extension-style') === marker) node.remove();
      });
      const style = document.createElement('style');
      style.setAttribute('data-Joker-extension-style', marker);
...
function isBootstrapBinding(value: unknown): value is ExtensionHostContentScriptBootstrapBinding {
  ...
  const prefix = `Joker-extension://${context.extensionId}/`
