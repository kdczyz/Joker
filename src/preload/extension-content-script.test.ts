...
function binding(runAt: 'documentStart' | 'documentEnd'): ExtensionHostContentScriptBootstrapBinding {
  return {
    ...
    scripts: [{
      ...
      url: 'Joker-extension://acme.dom/dist/content.js'
    }],
    styles: [{
      css: '.badge { color: red }',
      url: 'Joker-extension://acme.dom/dist/content.css'
    }]
  }
}
...
describe('Direct DOM workbench preload', () => {
  it('executes documentStart immediately in its assigned isolated world', () => {
    ...
    expect(state.exposed[0]).toMatchObject({ worldId: 18_001, apiKey: 'JokerHost' })
    ...
    expect(sources[0]!.code).toContain("['JokerGui', 'require', 'module'")
    expect(sources[0]!.code).toContain("['fetch', deniedFetch]")
    expect(sources.at(-1)!.url).toBe('Joker-extension://acme.dom/dist/content.js')
  })
  ...
  it('uses a static deactivation source backed by the isolated-world context', () => {
    ...
    expect(source).toContain('globalThis.JokerHost')
