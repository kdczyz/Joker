...
// --- Cloud AI config (account-scoped provider/model sync via lxqandlzy.me) ---
...
export interface CloudAiSavePayload {
  providerId: string;
  displayName?: string;
  baseUrl: string;
  chatCompletionsPath?: string;
...
export async function getCloudAiConfig(): Promise<CloudAiConfig> {
  return webRequest<CloudAiConfig>("/v1/work/ai-config");
}

export async function saveCloudAiConfig(payload: CloudAiSavePayload): Promise<CloudAiConfig> {
  return webRequest<CloudAiConfig>("/v1/work/ai-config", { method: "PUT", body: JSON.stringify(payload) });
}

export async function deleteCloudAiConfig(providerId?: string): Promise<CloudAiConfig> {
  const query = providerId ? `?providerId=${encodeURIComponent(providerId)}` : "";
  return webRequest<CloudAiConfig>(`/v1/work/ai-config${query}`, { method: "DELETE" });
}

export async function discoverCloudAiModels(baseUrl: string, apiKey: string): Promise<{
  providerId: string;
  displayName: string;
  baseUrl: string;
  model: string;
  ...
  return webRequest("/v1/work/ai-discover", { method: "POST", body: JSON.stringify({ baseUrl, apiKey }) });
}