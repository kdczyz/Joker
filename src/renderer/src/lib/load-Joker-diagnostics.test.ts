...
import { loadJokerDiagnostics } from './load-Joker-diagnostics'

describe('loadJokerDiagnostics', () => {
  it('loads runtime info, tool diagnostics, and memory records together', async () => {
    ...
    const loaded = await loadJokerDiagnostics(provider)
  ...
  it('can scope memory loading to the current workspace when explicitly requested', async () => {
    ...
    const loaded = await loadJokerDiagnostics(provider, { listAllMemories: false })
