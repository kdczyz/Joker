...
import { loadRcodeDiagnostics } from './load-Rcode-diagnostics'

describe('loadRcodeDiagnostics', () => {
  it('loads runtime info, tool diagnostics, and memory records together', async () => {
    ...
    const loaded = await loadRcodeDiagnostics(provider)
  ...
  it('can scope memory loading to the current workspace when explicitly requested', async () => {
    ...
    const loaded = await loadRcodeDiagnostics(provider, { listAllMemories: false })
