...
const DESIGN_DOCUMENTS_INDEX_PATH = '.Joker-design/documents.json'
...
async function loadDesignDocumentDirectoryReferences(workspaceRoot: string): Promise<ComposerFileReference[]> {
  if (typeof window.JokerGui?.readWorkspaceFile !== 'function') return []
  const result = await window.JokerGui
...
async function buildWorkspaceFileIndex(root: string): Promise<WorkspaceFileIndex> {
  ...
  while (
    ...
    const result = await window.JokerGui.listWorkspaceDirectory({ workspaceRoot: root, path: current.path })
...
export async function loadWorkspaceDirectoryContextFiles(
  ...
  if (!root || maxFiles <= 0 || typeof window.JokerGui?.listWorkspaceDirectory !== 'function') return []
...
export async function loadWorkspaceMentionPathSuggestions(
  ...
  const task = (async () => {
    const result = await window.JokerGui.listWorkspaceDirectory({ workspaceRoot: root, path: dir })
    if (!result.ok) return []
