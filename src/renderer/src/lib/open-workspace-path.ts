...
async function invokeOpenEditorPath(options: OpenEditorPathOptions): Promise<EditorOpenResult> {
  if (typeof window === 'undefined' || typeof window.JokerGui?.openEditorPath !== 'function') {
    return { ok: false, message: 'Editor bridge is unavailable.' }
  }

  try {
    return await window.JokerGui.openEditorPath(options)
  } catch (error) {
...
async function resolveExactWorkspaceFile(
  ...
  try {
    return await window.JokerGui.resolveWorkspaceFile({ path: targetPath, workspaceRoot: root })
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : String(error) }
  }
}
