...
describe('chat-store-side-actions', () => {
  ...
  it('uses the Joker default model when side creation has no parent or composer model to inherit', async () => {
    ...
    expect(state.sideConversations[id!].model).toBe(DEFAULT_JOKER_MODEL)
  })
  ...
  it('promoteSideConversation clears the relation by PATCH /v1/threads/{id} and refreshes the thread list', async () => {
    const { actions, state } = buildHarness()
    const id = (await actions.spawnSideConversation())!
    const runtimeRequest = globalThis.window.JokerGui.runtimeRequest as ReturnType<typeof vi.fn>
