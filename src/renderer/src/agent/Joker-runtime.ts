...
import {
  JOKER_ATTACHMENT_DIAGNOSTICS_PATH,
  JOKER_ATTACHMENTS_PATH,
  JOKER_MEMORY_DIAGNOSTICS_PATH,
  JOKER_MEMORY_PATH,
  JOKER_MCP_OAUTH_PATH,
  JOKER_RUNTIME_INFO_PATH,
  JOKER_RUNTIME_TOOLS_PATH,
  JOKER_SKILLS_PATH,
  JokerThreadCompactPath,
  JokerThreadEventsPath,
  JokerThreadForkPath,
  JokerThreadGoalPath,
  JokerThreadReviewPath,
  JokerThreadRewindPath,
  JokerThreadTodosPath,
  JokerThreadInterruptPath,
  JokerThreadPath,
  JokerThreadSteerPath,
  JokerThreadTurnsPath,
  JokerAttachmentContentPath,
  JokerUserInputPath,
  JokerMemoryRecordPath,
  JokerMcpOAuthServerPath,
  JokerSessionResumePath,
  normalizeThreadMode,
  type JokerThreadMode
...
/**
 * GUI-side adapter for the Joker HTTP/SSE contract.
 *
 ...
 * mapping live in `Joker-contract.ts` and `Joker-mapper.ts`.
 */
export class JokerRuntimeProvider implements AgentProvider {
  readonly id = 'Joker' as const
  readonly displayName = 'Joker'
  ...
  async getThreadDetail(threadId: string): Promise<{
    ...
    const response = await rendererRuntimeClient.runtimeRequest(JokerThreadPath(threadId), 'GET')
    if (!response.ok) {
  ...
  async rewindThread(threadId: string, turnId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerThreadRewindPath(threadId),
      'POST',
  ...
  async reviewThread(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerThreadReviewPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async steerUserMessage(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerThreadSteerPath(threadId, turnId),
      'POST',
  ...
  async interruptTurn(threadId: string, turnId: string, options?: { discard?: boolean }): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerThreadInterruptPath(threadId, turnId),
      'POST',
  ...
  async archiveThread(threadId: string, archived: boolean): Promise<void> {
    const response = await window.JokerGui.runtimeRequest(
      JokerThreadPath(threadId),
      'PATCH',
  ...
  async deleteThread(threadId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(JokerThreadPath(threadId), 'DELETE')
    if (!response.ok) {
  ...
  async compactThread(threadId: string, reason?: string): Promise<{ replacedTokens: number }> {
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerThreadCompactPath(threadId),
      'POST',
  ...
  async submitApprovalDecision(
    ...
    const protectedResult = await window.JokerGui.resolveJokerApproval({
      approvalId,
      decision,
  ...
  async getRuntimeInfo(): Promise<CoreRuntimeInfoJson> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_RUNTIME_INFO_PATH, 'GET')
    if (!response.ok) {
  ...
  async getToolDiagnostics(): Promise<CoreRuntimeToolDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_RUNTIME_TOOLS_PATH, 'GET')
    if (!response.ok) {
  ...
  async getMcpOAuthDiagnostics(): Promise<CoreMcpOAuthDiagnosticJson[]> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_MCP_OAUTH_PATH, 'GET')
    if (!response.ok) {
  ...
  async clearMcpOAuthCredentials(serverId?: string): Promise<string[]> {
    const response = await rendererRuntimeClient.runtimeRequest(
      serverId ? JokerMcpOAuthServerPath(serverId) : JOKER_MCP_OAUTH_PATH,
      'DELETE'
    )
    if (!response.ok) {
  ...
  async authorizeMcpOAuthCredentials(serverId: string): Promise<CoreMcpOAuthAuthorizeResponseJson> {
    const response = await rendererRuntimeClient.runtimeRequest(JokerMcpOAuthServerPath(serverId), 'POST')
    if (!response.ok) {
  ...
  async listSkills(): Promise<CoreRuntimeSkillJson[]> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_SKILLS_PATH, 'GET')
    if (!response.ok) {
  ...
  async uploadAttachment(input: {
    ...
    if (
      ...
      const result = await window.JokerGui.uploadRuntimeImageAttachment({
  ...
  async getAttachmentDiagnostics(): Promise<CoreAttachmentDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_ATTACHMENT_DIAGNOSTICS_PATH, 'GET')
    if (!response.ok) {
  ...
  async getAttachmentContent(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${JokerAttachmentContentPath(attachmentId)}${query}`,
      'GET'
    )
    if (!response.ok) {
  ...
  async listMemories(options: { workspace?: string; includeDeleted?: boolean; all?: boolean } = {}): Promise<CoreMemoryRecordJson[]> {
    ...
    const response = await rendererRuntimeClient.runtimeRequest(`${JOKER_MEMORY_PATH}${query}`, 'GET')
    if (!response.ok) {
  ...
  async updateMemory(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${JokerMemoryRecordPath(memoryId)}${query}`,
      'PATCH',
      JSON.stringify(patch)
    )
    if (!response.ok) {
  ...
  async deleteMemory(memoryId: string, options: { workspace?: string } = {}): Promise<CoreMemoryRecordJson> {
    const query = buildQuery({ workspace: options.workspace })
    const response = await rendererRuntimeClient.runtimeRequest(`${JokerMemoryRecordPath(memoryId)}${query}`, 'DELETE')
    if (!response.ok) {
  ...
  async getMemoryDiagnostics(): Promise<CoreMemoryDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(JOKER_MEMORY_DIAGNOSTICS_PATH, 'GET')
    if (!response.ok) {
  ...
  async forkThread(
    ...
    const url = JokerThreadForkPath(threadId)
    const response =
  ...
  async resumeSession(
    sessionId: string,
    options?: { model?: string; mode?: JokerThreadMode }
  ): Promise<{ threadId: string; sessionId: string }> {
    const settings = await rendererRuntimeClient.getSettings()
    const runtime = getJokerRuntimeSettings(settings)
    const response = await rendererRuntimeClient.runtimeRequest(
      JokerSessionResumePath(sessionId),
      'POST',
      JSON.stringify({
    ...
    await new Promise<void>(async (resolve) => {
      ...
      const offData = rendererRuntimeClient.onSseEvent((payload) => {
        ...
        const task = dispatchTail.then(async () => {
          ...
          await dispatchJokerRuntimeEvents(batch, sink, (runtimeEvent, eventSink) =>
  ...
  private async handleApprovalRequest(event: CoreRuntimeEventJson, sink: ThreadEventSink): Promise<void> {
    ...
    try {
      ...
      const policy = eventPolicy ?? getJokerRuntimeSettings(await rendererRuntimeClient.getSettings()).approvalPolicy
      switch (policy) {
        case 'auto':
