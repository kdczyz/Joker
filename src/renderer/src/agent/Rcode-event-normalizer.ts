import type { CoreChildRuntimeMetadataJson, CoreRuntimeEventJson, CoreTurnItemJson } from './Rcode-contract'
import type {
...
export type RcodeEventNormalizerDeps = {
...
export function normalizeRcodeTurnItem(
  item: CoreTurnItemJson,
  child: CoreChildRuntimeMetadataJson | undefined,
  deps: RcodeEventNormalizerDeps
...
/** Pure Rcode wire-event to normalized projection-action conversion. */
export function normalizeRcodeRuntimeEvent(
  event: CoreRuntimeEventJson,
  deps: RcodeEventNormalizerDeps
): RuntimeProjectionAction[] {
  switch (event.kind) {
    ...
    case 'tool_call_finished': {
      const action = event.item ? normalizeRcodeTurnItem(event.item, event.child, deps) : null
    ...
    case 'turn_failed': {
      ...
      const payload = deps.runtimeError(event, 'Rcode turn failed')
      return [
