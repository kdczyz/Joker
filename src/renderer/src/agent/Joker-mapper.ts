...
import { normalizeJokerRuntimeEvent, type JokerEventNormalizerDeps } from './Joker-event-normalizer'
...
function extractComponentPrototype(item: CoreTurnItemJson): ComponentPrototypeMetadata | undefined {
  ...
  if (
    !/^\.Joker-design\/component-prototypes\/[^/]+\/prototype\.html$/i.test(relativePath) ||
...
const JokerEventNormalizerDeps: JokerEventNormalizerDeps = {
...
export function runtimeProjectionActionsFromEvent(
  ...
  return normalizeJokerRuntimeEvent(event, JokerEventNormalizerDeps)
}
 ...
 */
export async function dispatchJokerRuntimeEvents(
  ...
  for (const event of events) {
    ...
    await flushDeltas()
    await dispatchJokerRuntimeEvent(event, sink, handleApprovalRequest)
  }
  await flushDeltas()
}

export async function dispatchJokerRuntimeEvent(
  event: CoreRuntimeEventJson,
  sink: ThreadEventSink,
  handleApprovalRequest: (event: CoreRuntimeEventJson, sink: ThreadEventSink) => Promise<void>
): Promise<void> {
