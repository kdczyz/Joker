import type { CoreChildRuntimeMetadataJson, CoreRuntimeEventJson, CoreTurnItemJson } from './Joker-contract'
import type {
...
export type JokerEventNormalizerDeps = {
...
export function normalizeJokerTurnItem(
  item: CoreTurnItemJson,
  child: CoreChildRuntimeMetadataJson | undefined,
  deps: JokerEventNormalizerDeps
...
/** Pure Joker wire-event to normalized projection-action conversion. */
export function normalizeJokerRuntimeEvent(
  event: CoreRuntimeEventJson,
  deps: JokerEventNormalizerDeps
): RuntimeProjectionAction[] {
  switch (event.kind) {
    ...
    case 'tool_call_finished': {
      const action = event.item ? normalizeJokerTurnItem(event.item, event.child, deps) : null
    ...
    case 'turn_failed': {
      ...
      const payload = deps.runtimeError(event, 'Joker turn failed')
      return [
