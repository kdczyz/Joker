...
import {
  chatBlockFromItem,
  dispatchJokerRuntimeEvent,
  mergeChatBlocks,
  runtimeProjectionActionsFromEvent
} from './Joker-mapper'
import type { CoreRuntimeEventJson, CoreTurnItemJson } from './Joker-contract'
...
describe('assistant stream mapping', () => {
  it('does not append completed assistant snapshots after streaming deltas', async () => {
    ...
    await dispatchJokerRuntimeEvent({
...
describe('create_plan tool mapping', () => {
  it('surfaces turn failure messages from Joker lifecycle events', async () => {
  ...
  it('surfaces create_plan tool events through the event sink', () => {
    ...
    void dispatchJokerRuntimeEvent(event, sink, async () => undefined)
  ...
  const item = (status: 'preparing' | 'running' | 'completed' | 'failed'): CoreTurnItemJson => ({
    ...
    output: {
      ...
      componentPrototype: {
        ...
        relativePath: '.Joker-design/component-prototypes/date-range/prototype.html',
...
describe('Joker extension metadata mapping', () => {
...
describe('tool presentation inference', () => {
  it('prefers explicit toolKind from Joker over local heuristics', () => {
    const block = chatBlockFromItem({
  ...
  it('surfaces final output and destination path aliases for generated artifacts', () => {
    ...
    const htmlCopy = chatBlockFromItem({
      ...
      output: {
        content: {
          sourcePath: 'brief.Joker-ppt.html',
          destinationPath: 'brief-copy.Joker-ppt.html',
    ...
    expect(htmlCopy).toMatchObject({
      filePath: 'brief-copy.Joker-ppt.html',
      meta: {
