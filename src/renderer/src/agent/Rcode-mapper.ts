...
import { normalizeRcodeRuntimeEvent, type RcodeEventNormalizerDeps } from './Rcode-event-normalizer'
...
const RcodeEventNormalizerDeps: RcodeEventNormalizerDeps = {
...
export function runtimeProjectionActionsFromEvent(
  ...
  return normalizeRcodeRuntimeEvent(event, RcodeEventNormalizerDeps)
}
 ...
 */
export async function dispatchRcodeRuntimeEvents(
  ...
  for (const event of events) {
    ...
    await flushDeltas()
    await dispatchRcodeRuntimeEvent(event, sink, handleApprovalRequest)
  }
  await flushDeltas()
}

export async function dispatchRcodeRuntimeEvent(
  event: CoreRuntimeEventJson,
  sink: ThreadEventSink,
  handleApprovalRequest: (event: CoreRuntimeEventJson, sink: ThreadEventSink) => Promise<void>
): Promise<void> {
