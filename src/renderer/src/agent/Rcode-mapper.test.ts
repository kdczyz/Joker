...
import {
  chatBlockFromItem,
  dispatchRcodeRuntimeEvent,
  mergeChatBlocks,
  runtimeProjectionActionsFromEvent
} from './Rcode-mapper'
import type { CoreRuntimeEventJson, CoreTurnItemJson } from './Rcode-contract'
...
describe('assistant stream mapping', () => {
  it('does not append completed assistant snapshots after streaming deltas', async () => {
    ...
    await dispatchRcodeRuntimeEvent({
...
describe('create_plan tool mapping', () => {
  it('surfaces turn failure messages from Rcode lifecycle events', async () => {
  ...
  it('surfaces create_plan tool events through the event sink', () => {
    ...
    void dispatchRcodeRuntimeEvent(event, sink, async () => undefined)
  ...
  const item = (status: 'preparing' | 'running' | 'completed' | 'failed'): CoreTurnItemJson => ({
    ...
    output: {
      ...
      componentPrototype: {
        ...
        relativePath: '.Rcode-design/component-prototypes/date-range/prototype.html',
...
describe('Rcode extension metadata mapping', () => {
...
describe('tool presentation inference', () => {
  it('prefers explicit toolKind from Rcode over local heuristics', () => {
    const block = chatBlockFromItem({
  ...
  it('surfaces final output and destination path aliases for generated artifacts', () => {
    ...
    const htmlCopy = chatBlockFromItem({
      ...
      output: {
        content: {
          sourcePath: 'brief.Rcode-ppt.html',
          destinationPath: 'brief-copy.Rcode-ppt.html',
    ...
    expect(htmlCopy).toMatchObject({
      filePath: 'brief-copy.Rcode-ppt.html',
      meta: {
