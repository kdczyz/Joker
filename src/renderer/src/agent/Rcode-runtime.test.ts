...
function installDsGui(overrides: Partial<Window['RcodeGui']>): void {
  vi.stubGlobal('window', {
    RcodeGui: {
      ...
      resolveRcodeApproval: vi.fn(async () => ({
        confirmed: true,
...
describe('RcodeRuntimeProvider', () => {
  it('reports the Rcode id and Rcode display name', () => {
    const provider = new RcodeRuntimeProvider()
    expect(provider.id).toBe('Rcode')
    expect(provider.displayName).toBe('Rcode')
  })
  ...
  it('starts MCP OAuth authorization through the authenticated runtime bridge', async () => {
    ...
    const result = await new RcodeRuntimeProvider().authorizeMcpOAuthCredentials('google_drive')
  ...
  it('maps Rcode thread items into chat blocks', async () => {
    installDsGui({
      runtimeRequest: vi.fn(async () => ({
        ok: true,
        status: 200,
  ...
  it('flags user_input blocks live only when the runtime gate still awaits them (#606)', async () => {
    ...
    const liveDetail = await new RcodeRuntimeProvider().getThreadDetail('thr_1')
    ...
    const staleDetail = await new RcodeRuntimeProvider().getThreadDetail('thr_1')
  ...
  it('posts Rcode turn requests and returns the deterministic user item id', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts per-turn provider ids with Rcode turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('uses the configured Plan-mode model and provider when the turn has no explicit override', async () => {
    ...
    await new RcodeRuntimeProvider().sendUserMessage('thr_1', 'draft a plan', { mode: 'plan' })
  ...
  it('posts workspace checkpoint ids with Rcode turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('posts bounded extension composer context with the next Rcode turn', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('posts attachment ids with Rcode turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts file references with Rcode turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts explicit reasoning effort with Rcode turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts GUI plan context with Rcode plan turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
    ...
    await provider.sendUserMessage('thr_1', 'refine the plan', {
      ...
      guiPlan: {
        ...
        relativePath: '.Rcodesdd/plan/auth.md',
        planId: '/workspace/deepseek-gui:.Rcodesdd/plan/auth.md',
  ...
  it('loads runtime diagnostics and uploads image attachments through Rcode endpoints', async () => {
  ...
  it('lists, toggles, and deletes memory records through Rcode endpoints', async () => {
  ...
  it('calls Rcode fork and user-input compatibility endpoints', async () => {
    const runtimeRequest = vi.fn(async (path: string) => ({
      ok: true,
      status: 200,
  ...
  it('resumes a session through the Rcode HTTP runtime', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 201,
    ...
    expect(runtimeRequest).toHaveBeenCalledWith(
      ...
      JSON.stringify({
        workspace: '/tmp/workspace',
        model: defaultRcodeRuntimeSettings().model,
        mode: 'plan'
      })
    )
  })

  it('maps Rcode SSE deltas into the thread event sink', async () => {
  ...
  it('auto-approves approval requests when policy is auto', async () => {
    let onData: ((payload: { streamId: string; events: unknown[] }) => void) | null = null
    const runtimeRequest = vi.fn(async () => ({ ok: true, status: 200, body: '{}' }))
    const resolveRcodeApproval = vi.fn(async () => ({
    ...
    const autoSettings: AppSettingsV1 = {
      ...settings(),
      agents: { Rcode: { ...defaultRcodeRuntimeSettings(), approvalPolicy: 'auto' } }
    }
    installDsGui({
      getSettings: vi.fn(async () => autoSettings),
      runtimeRequest,
      resolveRcodeApproval,
      onSseEvent: vi.fn((handler) => {
        onData = handler
        return () => undefined
      }),
    ...
    const provider = new RcodeRuntimeProvider()
    await provider.subscribeThreadEvents('thr_1', 0, sink, ac.signal)
    expect(resolveRcodeApproval).toHaveBeenCalledWith({
  ...
  it('uses the approval policy from runtime events before falling back to settings', async () => {
    ...
    const getSettings = vi.fn(async (): Promise<AppSettingsV1> => ({
      ...settings(),
      agents: { Rcode: { ...defaultRcodeRuntimeSettings(), approvalPolicy: 'on-request' } }
    }))
    const ac = new AbortController()
    const sink: ThreadEventSink = {
      onSeq: vi.fn(),
  ...
  it('renders approval cards for suggest and untrusted policies', async () => {
    for (const policy of ['suggest', 'untrusted'] as const) {
      ...
      const policySettings: AppSettingsV1 = {
        ...settings(),
        agents: { Rcode: { ...defaultRcodeRuntimeSettings(), approvalPolicy: policy } }
      }
      installDsGui({
      ...
      expect(resolveRcodeApproval).not.toHaveBeenCalled()
    }
  })
})

describe('registry', () => {
  it('returns a cached provider for the Rcode id', () => {
    resetProviderCacheForTests()
