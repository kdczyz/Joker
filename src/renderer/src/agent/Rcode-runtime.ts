...
import {
  RCODE_ATTACHMENT_DIAGNOSTICS_PATH,
  RCODE_ATTACHMENTS_PATH,
  RCODE_MEMORY_DIAGNOSTICS_PATH,
  RCODE_MEMORY_PATH,
  RCODE_MCP_OAUTH_PATH,
  RCODE_RUNTIME_INFO_PATH,
  RCODE_RUNTIME_TOOLS_PATH,
  RCODE_SKILLS_PATH,
  RcodeThreadCompactPath,
  RcodeThreadEventsPath,
  RcodeThreadForkPath,
  RcodeThreadGoalPath,
  RcodeThreadReviewPath,
  RcodeThreadRewindPath,
  RcodeThreadTodosPath,
  RcodeThreadInterruptPath,
  RcodeThreadPath,
  RcodeThreadSteerPath,
  RcodeThreadTurnsPath,
  RcodeAttachmentContentPath,
  RcodeUserInputPath,
  RcodeMemoryRecordPath,
  RcodeMcpOAuthServerPath,
  RcodeSessionResumePath,
  normalizeThreadMode,
  type RcodeThreadMode
...
/**
 * GUI-side adapter for the Rcode HTTP/SSE contract.
 *
...
export class RcodeRuntimeProvider implements AgentProvider {
  ...
  async getThreadDetail(threadId: string): Promise<{
    ...
    const response = await rendererRuntimeClient.runtimeRequest(RcodeThreadPath(threadId), 'GET')
    if (!response.ok) {
  ...
  async rewindThread(threadId: string, turnId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeThreadRewindPath(threadId),
      'POST',
  ...
  async reviewThread(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeThreadReviewPath(threadId),
      'POST',
      JSON.stringify(body)
    )
    if (!response.ok) {
  ...
  async steerUserMessage(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeThreadSteerPath(threadId, turnId),
      'POST',
  ...
  async interruptTurn(threadId: string, turnId: string, options?: { discard?: boolean }): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeThreadInterruptPath(threadId, turnId),
      'POST',
  ...
  async deleteThread(threadId: string): Promise<void> {
    const response = await rendererRuntimeClient.runtimeRequest(RcodeThreadPath(threadId), 'DELETE')
    if (!response.ok) {
  ...
  async compactThread(threadId: string, reason?: string): Promise<{ replacedTokens: number }> {
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeThreadCompactPath(threadId),
      'POST',
  ...
  async submitApprovalDecision(
    ...
    const protectedResult = await window.RcodeGui.resolveRcodeApproval({
      approvalId,
      decision,
  ...
  async getRuntimeInfo(): Promise<CoreRuntimeInfoJson> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_RUNTIME_INFO_PATH, 'GET')
    if (!response.ok) {
  ...
  async getToolDiagnostics(): Promise<CoreRuntimeToolDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_RUNTIME_TOOLS_PATH, 'GET')
    if (!response.ok) {
  ...
  async getMcpOAuthDiagnostics(): Promise<CoreMcpOAuthDiagnosticJson[]> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_MCP_OAUTH_PATH, 'GET')
    if (!response.ok) {
  ...
  async clearMcpOAuthCredentials(serverId?: string): Promise<string[]> {
    const response = await rendererRuntimeClient.runtimeRequest(
      serverId ? RcodeMcpOAuthServerPath(serverId) : RCODE_MCP_OAUTH_PATH,
      'DELETE'
    )
    if (!response.ok) {
  ...
  async authorizeMcpOAuthCredentials(serverId: string): Promise<CoreMcpOAuthAuthorizeResponseJson> {
    const response = await rendererRuntimeClient.runtimeRequest(RcodeMcpOAuthServerPath(serverId), 'POST')
    if (!response.ok) {
  ...
  async listSkills(): Promise<CoreRuntimeSkillJson[]> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_SKILLS_PATH, 'GET')
    if (!response.ok) {
  ...
  async uploadAttachment(input: {
    ...
    if (
      ...
      const result = await window.RcodeGui.uploadRuntimeImageAttachment({
  ...
  async getAttachmentDiagnostics(): Promise<CoreAttachmentDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_ATTACHMENT_DIAGNOSTICS_PATH, 'GET')
    if (!response.ok) {
  ...
  async getAttachmentContent(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${RcodeAttachmentContentPath(attachmentId)}${query}`,
      'GET'
    )
    if (!response.ok) {
  ...
  async listMemories(options: { workspace?: string; includeDeleted?: boolean; all?: boolean } = {}): Promise<CoreMemoryRecordJson[]> {
    ...
    const response = await rendererRuntimeClient.runtimeRequest(`${RCODE_MEMORY_PATH}${query}`, 'GET')
    if (!response.ok) {
  ...
  async updateMemory(
    ...
    const response = await rendererRuntimeClient.runtimeRequest(
      `${RcodeMemoryRecordPath(memoryId)}${query}`,
      'PATCH',
      JSON.stringify(patch)
    )
    if (!response.ok) {
  ...
  async deleteMemory(memoryId: string, options: { workspace?: string } = {}): Promise<CoreMemoryRecordJson> {
    const query = buildQuery({ workspace: options.workspace })
    const response = await rendererRuntimeClient.runtimeRequest(`${RcodeMemoryRecordPath(memoryId)}${query}`, 'DELETE')
    if (!response.ok) {
  ...
  async getMemoryDiagnostics(): Promise<CoreMemoryDiagnosticsJson> {
    const response = await rendererRuntimeClient.runtimeRequest(RCODE_MEMORY_DIAGNOSTICS_PATH, 'GET')
    if (!response.ok) {
  ...
  async forkThread(
    ...
    const url = RcodeThreadForkPath(threadId)
    const response =
  ...
  async resumeSession(
    sessionId: string,
    options?: { model?: string; mode?: RcodeThreadMode }
  ): Promise<{ threadId: string; sessionId: string }> {
    const settings = await rendererRuntimeClient.getSettings()
    const runtime = getRcodeRuntimeSettings(settings)
    const response = await rendererRuntimeClient.runtimeRequest(
      RcodeSessionResumePath(sessionId),
      'POST',
      JSON.stringify({
    ...
    await new Promise<void>(async (resolve) => {
      ...
      const offData = rendererRuntimeClient.onSseEvent((payload) => {
        ...
        const task = dispatchTail.then(async () => {
          ...
          await dispatchRcodeRuntimeEvents(batch, sink, (runtimeEvent, eventSink) =>
  ...
  private async handleApprovalRequest(event: CoreRuntimeEventJson, sink: ThreadEventSink): Promise<void> {
    ...
    try {
      ...
      const policy = eventPolicy ?? getRcodeRuntimeSettings(await rendererRuntimeClient.getSettings()).approvalPolicy
      switch (policy) {
        case 'auto':
