...
function installDsGui(overrides: Partial<Window['JokerGui']>): void {
  vi.stubGlobal('window', {
    JokerGui: {
      getSettings: vi.fn(async () => settings()),
      runtimeRequest: vi.fn(async () => ({ ok: true, status: 200, body: '{}' })),
      resolveJokerApproval: vi.fn(async () => ({
        confirmed: true,
...
describe('JokerRuntimeProvider', () => {
  it('reports the Joker id and Joker display name', () => {
    const provider = new JokerRuntimeProvider()
    expect(provider.id).toBe('Joker')
    expect(provider.displayName).toBe('Joker')
  })
  ...
  it('starts MCP OAuth authorization through the authenticated runtime bridge', async () => {
    ...
    const result = await new JokerRuntimeProvider().authorizeMcpOAuthCredentials('google_drive')
  ...
  it('maps Joker thread items into chat blocks', async () => {
    installDsGui({
      runtimeRequest: vi.fn(async () => ({
        ok: true,
        status: 200,
  ...
  it('flags user_input blocks live only when the runtime gate still awaits them (#606)', async () => {
    ...
    const liveDetail = await new JokerRuntimeProvider().getThreadDetail('thr_1')
    ...
    const staleDetail = await new JokerRuntimeProvider().getThreadDetail('thr_1')
  ...
  it('posts Joker turn requests and returns the deterministic user item id', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts per-turn provider ids with Joker turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('uses the configured Plan-mode model and provider when the turn has no explicit override', async () => {
    ...
    await new JokerRuntimeProvider().sendUserMessage('thr_1', 'draft a plan', { mode: 'plan' })
  ...
  it('posts workspace checkpoint ids with Joker turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('posts bounded extension composer context with the next Joker turn', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
      body: JSON.stringify({ threadId: 'thr_1', turnId: 'turn_abc', userMessageItemId: 'item_user_real' })
  ...
  it('posts attachment ids with Joker turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts file references with Joker turn requests when provided', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts explicit reasoning effort with Joker turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
  ...
  it('posts GUI plan context with Joker plan turn requests', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 202,
    ...
    await provider.sendUserMessage('thr_1', 'refine the plan', {
      ...
      guiPlan: {
        ...
        relativePath: '.Jokersdd/plan/auth.md',
        planId: '/workspace/deepseek-gui:.Jokersdd/plan/auth.md',
  ...
  it('loads runtime diagnostics and uploads image attachments through Joker endpoints', async () => {
  ...
  it('lists, toggles, and deletes memory records through Joker endpoints', async () => {
  ...
  it('calls Joker fork and user-input compatibility endpoints', async () => {
    const runtimeRequest = vi.fn(async (path: string) => ({
      ok: true,
      status: 200,
  ...
  it('resumes a session through the Joker HTTP runtime', async () => {
    const runtimeRequest = vi.fn(async () => ({
      ok: true,
      status: 201,
    ...
    expect(runtimeRequest).toHaveBeenCalledWith(
      ...
      JSON.stringify({
        workspace: '/tmp/workspace',
        model: defaultJokerRuntimeSettings().model,
        mode: 'plan'
      })
    )
  })

  it('maps Joker SSE deltas into the thread event sink', async () => {
  ...
  it('auto-approves approval requests when policy is auto', async () => {
    let onData: ((payload: { streamId: string; events: unknown[] }) => void) | null = null
    const runtimeRequest = vi.fn(async () => ({ ok: true, status: 200, body: '{}' }))
    const resolveJokerApproval = vi.fn(async () => ({
    ...
    const autoSettings: AppSettingsV1 = {
      ...settings(),
      agents: { Joker: { ...defaultJokerRuntimeSettings(), approvalPolicy: 'auto' } }
    }
    installDsGui({
      getSettings: vi.fn(async () => autoSettings),
      runtimeRequest,
      resolveJokerApproval,
      onSseEvent: vi.fn((handler) => {
        onData = handler
        return () => undefined
      }),
    ...
    const provider = new JokerRuntimeProvider()
    await provider.subscribeThreadEvents('thr_1', 0, sink, ac.signal)
    expect(resolveJokerApproval).toHaveBeenCalledWith({
  ...
  it('uses the approval policy from runtime events before falling back to settings', async () => {
    ...
    const getSettings = vi.fn(async (): Promise<AppSettingsV1> => ({
      ...settings(),
      agents: { Joker: { ...defaultJokerRuntimeSettings(), approvalPolicy: 'on-request' } }
    }))
    const ac = new AbortController()
    const sink: ThreadEventSink = {
      onSeq: vi.fn(),
  ...
  it('renders approval cards for suggest and untrusted policies', async () => {
    for (const policy of ['suggest', 'untrusted'] as const) {
      ...
      const policySettings: AppSettingsV1 = {
        ...settings(),
        agents: { Joker: { ...defaultJokerRuntimeSettings(), approvalPolicy: policy } }
      }
      installDsGui({
      ...
      expect(resolveJokerApproval).not.toHaveBeenCalled()
    }
  })
})

describe('registry', () => {
  it('returns a cached provider for the Joker id', () => {
    resetProviderCacheForTests()
