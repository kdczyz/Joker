...
describe('runDesignPages parallel fanout', () => {
  ...
  it('pre-creates all pages and sends one fanout turn for page generation', async () => {
    ...
    expect(fanoutPrompt).toContain('User goal for this page: Decide whether to join the IJOKER community')
    ...
    expect(artifacts.every((artifact) => artifact.direction?.name === 'IJOKER community')).toBe(true)
    ...
    const projectDesignMdWrite = writeWorkspaceFile.mock.calls.find((call) => {
      ...
      return payload?.path === '.Joker-design/HANDOFF.md'
    })?.[0] as { content?: string } | undefined
    expect(projectDesignMdWrite?.content).toContain('# DESIGN.md: Doc')
    expect(projectDesignMdWrite?.content).toContain('IJOKER community')
