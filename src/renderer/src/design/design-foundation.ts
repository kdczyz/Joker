...
export function designSpecPath(docId: string): string {
  return `.Joker-design/${docId}/design.md`
}
...
export function buildDesignSpecPrompt(options: {
  ...
  const lines = [
    'Joker is asking you to LAY THE FOUNDATION for a multi-page design before any screen is built.',
...
export function buildDesignLogoPrompt(options: {
  ...
  const lines = [
    'Joker is asking you to design the BRAND LOGO for this product.',
    `Workspace: ${options.workspaceRoot}`,
