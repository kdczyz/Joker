...
export async function runDesignPages(deps: RunDesignPagesDeps): Promise<void> {
  ...
  try {
    ...
    if (withFoundation) {
      ...
      const structuredSystem = await window.JokerGui?.readWorkspaceFile?.({
    ...
    const pageDrafts = plan.map((entry, i) => {
      ...
      return {
        entry,
        id,
        relativePath: `.Joker-design/${docId}/${id}/v1.html`,
        designMdPath: `.Joker-design/${docId}/${id}/DESIGN.md`,
    ...
    // run in a parallel batch in Joker's AgentLoop.
