...
describe('canvas-persistence round-trip', () => {
  it('builds a stable document key from workspace and canvas path', () => {
    expect(canvasDocumentKey('/workspace', 'code-thread-1', '.Joker-canvas')).toBe(
      `/workspace\0${canvasDocPath('code-thread-1', '.Joker-canvas')}`
    )
  })
...
describe('persistCanvasDocument debounce', () => {
  ...
  it('does not let one canvas save cancel another canvas save', () => {
    ...
    persistCanvasDocument('/workspace', 'code-thread-1', codeDoc, '.Joker-canvas')
    ...
    expect(writeWorkspaceFile).toHaveBeenCalledWith({
      path: canvasDocPath('code-thread-1', '.Joker-canvas'),
      workspaceRoot: '/workspace',
  ...
  it('keeps debouncing repeated saves for the same canvas', () => {
    ...
    persistCanvasDocument('/workspace', 'code-thread-1', firstDoc, '.Joker-canvas')
    persistCanvasDocument('/workspace', 'code-thread-1', latestDoc, '.Joker-canvas')
    vi.advanceTimersByTime(600)
