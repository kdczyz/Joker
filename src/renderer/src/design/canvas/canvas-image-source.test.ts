...
describe('canvas image source loading', () => {
  ...
  it('does not permanently cache a failed workspace image read', async () => {
    ...
    vi.stubGlobal('window', { JokerGui: { readWorkspaceImage } })
  ...
  it('reads absolute local image paths without applying the canvas workspace boundary', async () => {
    const absolutePath = '/Users/zxy/.Joker/default_workspace/.deepseekgui-images/generated.png'
    ...
    await expect(loadWorkspaceImageDataUrl('/Users/zxy/.Joker/design-workspace', absolutePath))
    ...
    expect(workspaceImageDataUrlCacheStats('/Users/zxy/.Joker/design-workspace').entries).toBe(1)
  })
})
