...
async function readWorkspaceText(workspaceRoot: string, path: string): Promise<string> {
  if (typeof window === 'undefined' || typeof window.JokerGui?.readWorkspaceFile !== 'function') return ''
  const result = await window.JokerGui.readWorkspaceFile({ path, workspaceRoot }).catch(() => null)
