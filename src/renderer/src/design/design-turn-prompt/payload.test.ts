...
function artifact(id: string, title: string): DesignArtifact {
  return {
    ...
    createdAt: now,
    updatedAt: now,
    versions: [{ id: `${id}-v1`, relativePath: `.Joker-design/doc/${id}/v1.html`, createdAt: now, summary: `${title} summary` }]
  }
}
...
describe('design turn prompt payload', () => {
  ...
  it('reads static quality findings through workspace files', async () => {
    ...
    vi.stubGlobal('window', { JokerGui: { readWorkspaceFile } })
  ...
  it('passes selected canvas images as context locations', async () => {
    ...
    expect(payload.prompt).toContain('.Joker-design/assets/logo.png')
  })
})
