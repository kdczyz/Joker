...
export function buildScreenTurnPrompt(options: ScreenTurnOptions): string {
  ...
  const lines = [
    options.basePath
      ? `Joker is asking you to ITERATE on an existing screen design: "${options.screenName}".`
      : `Joker is asking you to design a new screen: "${options.screenName}".`,
...
export function buildCanvasTurnPrompt(options: DesignTurnOptions): string {
  ...
  const lines = [
    codeCanvasMode
      ? 'Joker is asking you to operate the Code sidebar whiteboard with the dedicated canvas tools.'
      : 'Joker is asking you to operate the design canvas with the dedicated design tools.',
    `Workspace: ${options.workspaceRoot}`,
    ...formatProjectDesignSystemLines(options),
    ...
    ...(codeCanvasMode
      ...
      : [
          ...
          '- DESIGN-SYSTEM CLAIMS MUST BE FACTUAL — say that screens share a unified design system only when a valid root `DESIGN.md` was already listed or a `design_system` call succeeded in this turn. Per-screen `.Joker-design/.../DESIGN.md` notes and visually similar page CSS do not count as the project design system.'
        ]),
    '',
    ...
    '- `design_query` / `design.snapshot`: { "ids"?: [...], "limit"?: N }. Read the CURRENT canvas state on demand — shape ids, names, types, absolute bounds, fills, strokes, corner radius, token bindings, and the active selection. Call this whenever you need fresh facts about the board (e.g. after your own edits, or to confirm a shape exists) instead of trusting the snapshot above, which may be stale.',
    ...(codeCanvasMode ? [] : [
