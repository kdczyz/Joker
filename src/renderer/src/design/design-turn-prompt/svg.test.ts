...
describe('SVG design turn prompt', () => {
  it('routes the agent through the structured SVG inspection, editing, animation, and validation tools', () => {
    const prompt = buildDesignTurnPrompt({
      ...
      artifactRelativePath: '.Joker-design/doc/motion/v1.svg',
      designNotesPath: '.Joker-design/doc/motion/DESIGN.md',
    ...
    expect(prompt).toContain('.Joker-design/doc/motion/v1.svg')
  ...
  it('names the previous SVG version when iterating', () => {
    const prompt = buildDesignTurnPrompt({
      ...
      artifactRelativePath: '.Joker-design/doc/motion/v2.svg',
      basePath: '.Joker-design/doc/motion/v1.svg',
      workspaceRoot: '/workspace'
    })
    expect(prompt).toContain('ITERATE on an existing standalone SVG motion artifact')
    expect(prompt).toContain('Previous version to preserve and improve: .Joker-design/doc/motion/v1.svg')
  })
})
