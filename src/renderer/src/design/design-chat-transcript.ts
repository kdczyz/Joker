 ...
 * `.Joker-design/<docId>/chat/<threadId>.md` plus `chat/meta.json`.
 *
...
export function designChatDir(docId: string): string | null {
  ...
  return safeDocId ? `.Joker-design/${safeDocId}/chat` : null
}
...
export async function hydrateDesignChatMetaForDoc(input: {
  ...
  if (!metaPath || typeof window.JokerGui?.readWorkspaceFile !== 'function') return false
  try {
    const read = await window.JokerGui.readWorkspaceFile({
...
export async function persistDesignChatMetaForDoc(input: {
  ...
  try {
    const existing = await window.JokerGui.readWorkspaceFile({
      workspaceRoot: input.workspaceRoot,
      path: metaPath
    })
...
export async function writeDesignChatTranscriptForThread(input: {
  ...
  if (typeof window.JokerGui?.writeWorkspaceFile !== 'function') return false
