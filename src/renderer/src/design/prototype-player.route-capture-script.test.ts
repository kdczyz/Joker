...
describe("prototype-player route capture script", () => {
    it('does not resolve captured target titles when multiple links share the same title', () => {
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=Settings',
          'file:///workspace/.Rcode-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    })
    it('resolves captured target titles through a unique fuzzy match', () => {
      ...
      const currentFileUrl = 'file:///workspace/.Rcode-design/doc/home/v1.html'
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=Stats',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('stats')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=Account',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
    ...
    it('resolves route-style prototype href slugs to unique sibling pages', () => {
      ...
      const settings = artifact('settings', 'Account Settings', {
        relativePath: '.Rcode-design/doc/account-settings/v1.html'
      })
      const stats = artifact('weekly-stats', 'Weekly Stats', {
        relativePath: '.Rcode-design/doc/weekly-stats/v1.html'
      })
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=%2Fsettings',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=..%2Faccount-settings%2F',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#/settings',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=%23%2Fweekly-stats',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('weekly-stats')
    ...
    it('does not resolve ambiguous bare filenames to sibling prototype pages', () => {
      const home = artifact('home', 'Home', { relativePath: '.Rcode-design/doc/home/v1.html' })
      const signup = artifact('signup', 'Signup', { relativePath: '.Rcode-design/doc/signup/v1.html' })
      const settings = artifact('settings', 'Settings', { relativePath: '.Rcode-design/doc/settings/v1.html' })
      const links = resolvePrototypeLinks(home, [home, signup, settings])
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=v1.html',
          'file:///workspace/.Rcode-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    ...
    it('does not resolve ambiguous short relative paths to the first matching page', () => {
      ...
      const projectSettings = artifact('project-settings', 'Project Settings', {
        relativePath: '.Rcode-design/doc/project/settings/v1.html'
      })
      const links = resolvePrototypeLinks(home, [home, accountSettings, projectSettings])
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Rcode-design/doc/home/v1.html#Rcode-proto-nav=settings%2Fv1.html',
          'file:///workspace/.Rcode-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    ...
    it('extracts captured prototype hrefs and leaves ordinary hashes alone', () => {
      expect(extractPrototypeNavigationHref('#Rcode-proto-nav=..%2Fsignup%2Fv1.html')).toBe('../signup/v1.html')
      expect(extractPrototypeNavigationHref('file:///x.html#section')).toBeNull()
    })
    it('detects captured prototype back navigation signals', () => {
      expect(isPrototypeBackNavigation('#Rcode-proto-back=123')).toBe(true)
      expect(isPrototypeBackNavigation('file:///workspace/home.html#Rcode-proto-back=123')).toBe(true)
      expect(isPrototypeBackNavigation('#Rcode-proto-nav=..%2Fsignup%2Fv1.html')).toBe(false)
      expect(isPrototypeBackNavigation('#/settings')).toBe(false)
      expect(prototypeBackNavigationSteps('#Rcode-proto-back=123')).toBe(1)
      expect(prototypeBackNavigationSteps('#Rcode-proto-back=steps%3D2%26t%3D99')).toBe(2)
      expect(prototypeBackNavigationSteps('file:///workspace/home.html#Rcode-proto-back=steps%3D3%26t%3D99')).toBe(3)
      expect(prototypeBackNavigationSteps('#Rcode-proto-nav=..%2Fsignup%2Fv1.html')).toBeNull()
    })
    it('extracts hash-route prototype hrefs without treating plain anchors as routes', () => {
      ...
      expect(extractPrototypeHashRouteHref('#Rcode-proto-nav=..%2Fsettings%2Fv1.html')).toBeNull()
    })
    it('captures unknown local prototype hrefs but lets anchors and external links behave normally', () => {
      const base = 'file:///workspace/.Rcode-design/doc/home/v1.html'
    ...
    it('builds a capture script scoped to known flow hrefs', () => {
      ...
      expect(script).toContain('Rcode-proto-nav=')
      ...
      expect(script).toContain('__RcodePrototypeNavCaptureTitles')
      ...
      expect(script).toContain('__RcodePrototypeOriginalOpen')
      expect(script).toContain('__RcodePrototypeWindowOpenPatched')
      ...
      expect(script).toContain('__RcodePrototypeOriginalPushState')
      expect(script).toContain('__RcodePrototypePushStatePatched')
      expect(script).toContain("patchHistoryMethod('pushState'")
      expect(script).toContain("patchHistoryMethod('replaceState'")
      expect(script).toContain('Rcode-proto-back=')
      expect(script).toContain('__RcodePrototypeBackPatched')
    ...
    it('captures history.pushState prototype navigation from scripted routers', () => {
      ...
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow }) => {
          ...
          expect(fakeWindow.location.hash).toBe('Rcode-proto-nav=..%2Fcheckout%2Fv1.html')
          expect(
            resolvePrototypeNavigationTarget(
    ...
    it('captures history.back and history.go(-1) prototype navigation from scripted routers', () => {
      withInjectedPrototypeCapture(
        buildPrototypeNavigationCaptureScript([]),
        'file:///workspace/.Rcode-design/doc/settings/v1.html',
        ({ fakeWindow }) => {
          fakeWindow.history.back()
          expect(fakeWindow.location.hash).toMatch(/^Rcode-proto-back=/)
  
          fakeWindow.location.hash = ''
          fakeWindow.history.go(-1)
          expect(fakeWindow.location.hash).toMatch(/^Rcode-proto-back=steps%3D1%26t%3D/)
  
          fakeWindow.location.hash = ''
          fakeWindow.history.go(-2)
          expect(fakeWindow.location.hash).toMatch(/^Rcode-proto-back=steps%3D2%26t%3D/)
    ...
    it('captures button hash routes in the injected navigation script', () => {
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow, listeners }) => {
          ...
          expect(fakeWindow.location.hash).toBe('Rcode-proto-nav=%2Fsignup')
        }
      )
    })
    it('captures plain hash links when they uniquely match a prototype screen', () => {
      ...
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow, listeners }) => {
          ...
          expect(event.preventDefault).toHaveBeenCalled()
          expect(event.stopPropagation).toHaveBeenCalled()
          expect(fakeWindow.location.hash).toBe('Rcode-proto-nav=Checkout')
          expect(
            resolvePrototypeNavigationTarget(
              `${currentFileUrl}#${fakeWindow.location.hash}`,
              currentFileUrl,
