...
describe("prototype-player route capture script", () => {
    it('does not resolve captured target titles when multiple links share the same title', () => {
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=Settings',
          'file:///workspace/.Joker-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    })
    it('resolves captured target titles through a unique fuzzy match', () => {
      ...
      const currentFileUrl = 'file:///workspace/.Joker-design/doc/home/v1.html'
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=Stats',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('stats')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=Account',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
    ...
    it('resolves route-style prototype href slugs to unique sibling pages', () => {
      ...
      const settings = artifact('settings', 'Account Settings', {
        relativePath: '.Joker-design/doc/account-settings/v1.html'
      })
      const stats = artifact('weekly-stats', 'Weekly Stats', {
        relativePath: '.Joker-design/doc/weekly-stats/v1.html'
      })
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=%2Fsettings',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=..%2Faccount-settings%2F',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      ...
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#/settings',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('settings')
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=%23%2Fweekly-stats',
          currentFileUrl,
          links
        )?.targetArtifactId
      ).toBe('weekly-stats')
    ...
    it('does not resolve ambiguous bare filenames to sibling prototype pages', () => {
      const home = artifact('home', 'Home', { relativePath: '.Joker-design/doc/home/v1.html' })
      const signup = artifact('signup', 'Signup', { relativePath: '.Joker-design/doc/signup/v1.html' })
      const settings = artifact('settings', 'Settings', { relativePath: '.Joker-design/doc/settings/v1.html' })
      const links = resolvePrototypeLinks(home, [home, signup, settings])
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=v1.html',
          'file:///workspace/.Joker-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    ...
    it('does not resolve ambiguous short relative paths to the first matching page', () => {
      ...
      const projectSettings = artifact('project-settings', 'Project Settings', {
        relativePath: '.Joker-design/doc/project/settings/v1.html'
      })
      const links = resolvePrototypeLinks(home, [home, accountSettings, projectSettings])
  
      expect(
        resolvePrototypeNavigationTarget(
          'file:///workspace/.Joker-design/doc/home/v1.html#Joker-proto-nav=settings%2Fv1.html',
          'file:///workspace/.Joker-design/doc/home/v1.html',
          links
        )
      ).toBeNull()
    ...
    it('extracts captured prototype hrefs and leaves ordinary hashes alone', () => {
      expect(extractPrototypeNavigationHref('#Joker-proto-nav=..%2Fsignup%2Fv1.html')).toBe('../signup/v1.html')
      expect(extractPrototypeNavigationHref('file:///x.html#section')).toBeNull()
    })
    it('detects captured prototype back navigation signals', () => {
      expect(isPrototypeBackNavigation('#Joker-proto-back=123')).toBe(true)
      expect(isPrototypeBackNavigation('file:///workspace/home.html#Joker-proto-back=123')).toBe(true)
      expect(isPrototypeBackNavigation('#Joker-proto-nav=..%2Fsignup%2Fv1.html')).toBe(false)
      expect(isPrototypeBackNavigation('#/settings')).toBe(false)
      expect(prototypeBackNavigationSteps('#Joker-proto-back=123')).toBe(1)
      expect(prototypeBackNavigationSteps('#Joker-proto-back=steps%3D2%26t%3D99')).toBe(2)
      expect(prototypeBackNavigationSteps('file:///workspace/home.html#Joker-proto-back=steps%3D3%26t%3D99')).toBe(3)
      expect(prototypeBackNavigationSteps('#Joker-proto-nav=..%2Fsignup%2Fv1.html')).toBeNull()
    })
    it('extracts hash-route prototype hrefs without treating plain anchors as routes', () => {
      ...
      expect(extractPrototypeHashRouteHref('#Joker-proto-nav=..%2Fsettings%2Fv1.html')).toBeNull()
    })
    it('captures unknown local prototype hrefs but lets anchors and external links behave normally', () => {
      const base = 'file:///workspace/.Joker-design/doc/home/v1.html'
    ...
    it('builds a capture script scoped to known flow hrefs', () => {
      ...
      expect(script).toContain('Joker-proto-nav=')
      ...
      expect(script).toContain('__JokerPrototypeNavCaptureTitles')
      ...
      expect(script).toContain('__JokerPrototypeOriginalOpen')
      expect(script).toContain('__JokerPrototypeWindowOpenPatched')
      ...
      expect(script).toContain('__JokerPrototypeOriginalPushState')
      expect(script).toContain('__JokerPrototypePushStatePatched')
      expect(script).toContain("patchHistoryMethod('pushState'")
      expect(script).toContain("patchHistoryMethod('replaceState'")
      expect(script).toContain('Joker-proto-back=')
      expect(script).toContain('__JokerPrototypeBackPatched')
    ...
    it('captures history.pushState prototype navigation from scripted routers', () => {
      ...
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow }) => {
          ...
          expect(fakeWindow.location.hash).toBe('Joker-proto-nav=..%2Fcheckout%2Fv1.html')
          expect(
            resolvePrototypeNavigationTarget(
    ...
    it('captures history.back and history.go(-1) prototype navigation from scripted routers', () => {
      withInjectedPrototypeCapture(
        buildPrototypeNavigationCaptureScript([]),
        'file:///workspace/.Joker-design/doc/settings/v1.html',
        ({ fakeWindow }) => {
          fakeWindow.history.back()
          expect(fakeWindow.location.hash).toMatch(/^Joker-proto-back=/)
  
          fakeWindow.location.hash = ''
          fakeWindow.history.go(-1)
          expect(fakeWindow.location.hash).toMatch(/^Joker-proto-back=steps%3D1%26t%3D/)
  
          fakeWindow.location.hash = ''
          fakeWindow.history.go(-2)
          expect(fakeWindow.location.hash).toMatch(/^Joker-proto-back=steps%3D2%26t%3D/)
    ...
    it('captures button hash routes in the injected navigation script', () => {
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow, listeners }) => {
          ...
          expect(fakeWindow.location.hash).toBe('Joker-proto-nav=%2Fsignup')
        }
      )
    })
    it('captures plain hash links when they uniquely match a prototype screen', () => {
      ...
      withInjectedPrototypeCapture(
        ...
        ({ fakeWindow, listeners }) => {
          ...
          expect(event.preventDefault).toHaveBeenCalled()
          expect(event.stopPropagation).toHaveBeenCalled()
          expect(fakeWindow.location.hash).toBe('Joker-proto-nav=Checkout')
          expect(
            resolvePrototypeNavigationTarget(
              `${currentFileUrl}#${fakeWindow.location.hash}`,
              currentFileUrl,
