...
function artifact(kind: DesignArtifact['kind'] = 'html'): DesignArtifact {
  return {
    ...
    relativePath: `.Joker-design/doc/${kind}_1/${kind === 'html' ? 'v1.html' : 'canvas.json'}`,
    designMdPath: kind === 'html' ? '.Joker-design/doc/html_1/DESIGN.md' : undefined,
    createdAt: now,
    updatedAt: now,
    versions: []
  }
...
describe('design code roundtrip', () => {
  it('prepares a design-to-code implementation turn from the structured design system', async () => {
    ...
    expect(result.prompt).toContain('Design source (a standalone HTML mockup): .Joker-design/doc/html_1/v1.html')
    expect(result.prompt).toContain('Project design system: DESIGN.md')
    expect(result.prompt).toContain('Target stack: React + Tailwind')
    expect(result.prompt).toContain('Read the design notes `.Joker-design/doc/html_1/DESIGN.md`')
  })
