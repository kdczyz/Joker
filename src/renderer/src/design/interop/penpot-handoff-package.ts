...
export const PENPOT_HANDOFF_PACKAGE_PATH = '.Joker-design/penpot-package.json'
...
function buildFrames(options: BuildPenpotHandoffPackageOptions): PenpotHandoffFrame[] {
  ...
  const graph = buildDesignGraphFromCanvasDocument(options.canvasDocument, {
    projectId: options.document?.id ?? options.canvasDocument.graph?.projectId ?? 'Joker-design',
    artifacts: [...artifacts],
...
export function buildPenpotHandoffPackage(options: BuildPenpotHandoffPackageOptions): PenpotHandoffPackage {
  ...
  return {
    ...
    document: {
      id: options.document?.id ?? 'Joker-design',
      title: options.document?.title ?? 'Joker design project',
