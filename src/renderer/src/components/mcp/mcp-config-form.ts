...
// settings. The on-disk file (`~/.Joker/mcp.json`) is the single source of
...
// (src/main/Joker-process.ts normalizeGuiManagedMcpServer) so what the form
...
function parseServerEntry(name: string, raw: unknown): McpFormServer {
  ...
  // Accept both `transport` (Joker) and `type` (Claude Desktop) field names.
...
export function parseMcpConfigText(text: string): McpParseResult {
  ...
  // Servers can live at the top level under `servers` (Joker) or `mcpServers`
