...
const PREVIEW_URL_STORAGE_KEY = 'Rcode.devPreview.url'
const PREVIEW_AUTO_FOLLOW_STORAGE_KEY = 'Rcode.devPreview.autoFollow'
...
export function DevBrowserPanel({
  ...
  const useElectronWebview = canUseElectronWebviewEnvironment({
    openExternalAvailable: typeof window.RcodeGui?.openExternal === 'function',
  ...
  const openExternal = (): void => {
    ...
    if (typeof window.RcodeGui?.openExternal === 'function') {
      void window.RcodeGui.openExternal(normalized).catch(() => undefined)
      return
    }
      ...
      <div className="relative min-h-0 flex-1 bg-white dark:bg-ds-canvas">
        {!activeUrl ? (
          ...
          <webview
            ...
            ref={webviewRef}
            src={activeUrl}
            partition="persist:Rcode-dev-browser"
