...
const PREVIEW_URL_STORAGE_KEY = 'Joker.devPreview.url'
const PREVIEW_AUTO_FOLLOW_STORAGE_KEY = 'Joker.devPreview.autoFollow'
...
export function DevBrowserPanel({
  ...
  const useElectronWebview = canUseElectronWebviewEnvironment({
    openExternalAvailable: typeof window.JokerGui?.openExternal === 'function',
  ...
  const openExternal = (): void => {
    ...
    if (typeof window.JokerGui?.openExternal === 'function') {
      void window.JokerGui.openExternal(normalized).catch(() => undefined)
      return
    }
      ...
      <div className="relative min-h-0 flex-1 bg-white dark:bg-ds-canvas">
        {!activeUrl ? (
          ...
          <webview
            ...
            ref={webviewRef}
            src={activeUrl}
            partition="persist:Joker-dev-browser"
