...
export function useWorkbenchKeyboardShortcuts({
  ...
  const shortcutPlatform = typeof window === 'undefined' ? undefined : window.JokerGui?.platform
  ...
  useEffect(() => {
    const runDesktopShortcut = (command: DesktopCommand): void => {
      if (typeof window.JokerGui?.runDesktopCommand !== 'function') return
      void window.JokerGui.runDesktopCommand(command)
    }
