...
import {
  JOKER_BACKGROUND_SHELLS_PATH,
  JokerBackgroundShellStopPath
...
async function fetchBackgroundShells(threadId?: string): Promise<BackgroundShellListResponse> {
  ...
  const result = await rendererRuntimeClient.runtimeRequest(`${JOKER_BACKGROUND_SHELLS_PATH}${query}`)
...
async function stopBackgroundShell(sessionId: string): Promise<void> {
  await rendererRuntimeClient.runtimeRequest(JokerBackgroundShellStopPath(sessionId), 'POST')
}
