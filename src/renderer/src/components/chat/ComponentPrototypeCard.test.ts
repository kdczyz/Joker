...
vi.mock('react-i18next', () => {
  const labels: Record<string, string> = {
    ...
    componentPrototypeMainAgent: 'Rcode',
    componentPrototypeSubagent: 'Design Sub-agent',
    componentPrototypeGeneratedByMainAgent: 'Published directly by Rcode',
...
describe('ComponentPrototypeCard metadata helpers', () => {
  it('accepts the versioned component artifact but rejects unsafe paths and viewports', () => {
    ...
    expect(componentPrototypeFromBlock(block({
      ...metadata,
      relativePath: '.Rcode-design/component-prototypes/../secret/prototype.html'
    }))).toBeNull()
    expect(componentPrototypeFromBlock(block({
      ...metadata,
...
describe('ComponentPrototypeCard interactions', () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubGlobal('window', {
      RcodeGui: { readWorkspaceFile: actionMocks.readWorkspaceFile },
      setTimeout
    })
  ...
  it('renders a centered compact surface and keeps secondary controls in one overflow menu', async () => {
    ...
    expect(host.props.partition).toBe('Rcode-component-prototype-tool_component')
  ...
  it('labels direct prototypes as published by Rcode instead of a design sub-agent', async () => {
    const direct = { ...metadata, producer: 'main-agent' as const, profile: undefined, childId: undefined }
    ...
    await openMenu()
    expect(JSON.stringify(renderer.toJSON())).toContain('Rcode')
    expect(renderer.root.findByProps({ title: 'Published directly by Rcode' })).toBeTruthy()
  })
