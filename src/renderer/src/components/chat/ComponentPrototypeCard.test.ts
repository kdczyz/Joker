...
vi.mock('react-i18next', () => {
  const labels: Record<string, string> = {
    ...
    componentPrototypeMainAgent: 'Joker',
    componentPrototypeSubagent: 'Design Sub-agent',
    componentPrototypeGeneratedByMainAgent: 'Published directly by Joker',
...
describe('ComponentPrototypeCard metadata helpers', () => {
  it('accepts the versioned component artifact but rejects unsafe paths and viewports', () => {
    ...
    expect(componentPrototypeFromBlock(block({
      ...metadata,
      relativePath: '.Joker-design/component-prototypes/../secret/prototype.html'
    }))).toBeNull()
    expect(componentPrototypeFromBlock(block({
      ...metadata,
...
describe('ComponentPrototypeCard interactions', () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubGlobal('window', {
      JokerGui: { readWorkspaceFile: actionMocks.readWorkspaceFile },
      setTimeout
    })
  ...
  it('renders a centered compact surface and keeps secondary controls in one overflow menu', async () => {
    ...
    expect(host.props.partition).toBe('Joker-component-prototype-tool_component')
  ...
  it('labels direct prototypes as published by Joker instead of a design sub-agent', async () => {
    const direct = { ...metadata, producer: 'main-agent' as const, profile: undefined, childId: undefined }
    ...
    expect(renderer.root.findByProps({ title: 'Published directly by Joker' })).toBeTruthy()
  })
