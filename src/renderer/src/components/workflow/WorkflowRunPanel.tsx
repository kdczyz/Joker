...
export function WorkflowRunPanel({ enabled }: Props): ReactElement | null {
  ...
  useEffect(() => {
    ...
    const poll = async (): Promise<void> => {
      ...
      try {
        const next = await window.RcodeGui.getWorkflowStatus()
          ...
          {pendingApprovals.map((approval) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => void window.RcodeGui.resolveWorkflowApproval(approval.token, 'approved')}
                ...
                <button
                  type="button"
                  onClick={() => void window.RcodeGui.resolveWorkflowApproval(approval.token, 'rejected')}
