...
export function WorkflowRunPanel({ enabled }: Props): ReactElement | null {
  ...
  useEffect(() => {
    ...
    const poll = async (): Promise<void> => {
      ...
      try {
        const next = await window.JokerGui.getWorkflowStatus()
          ...
          {pendingApprovals.map((approval) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => void window.JokerGui.resolveWorkflowApproval(approval.token, 'approved')}
                ...
                <button
                  type="button"
                  onClick={() => void window.JokerGui.resolveWorkflowApproval(approval.token, 'rejected')}
