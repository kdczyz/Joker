...
import type { SkillListItem, SkillRootListItem } from '@shared/Joker-gui-api'
import type {
  CoreRuntimeInfoJson,
...
const INSTALLED_STORAGE_KEY = 'Joker.installedPlugins'
...
export function PluginMarketplaceView({ leftSidebarCollapsed }: Props): ReactElement {
  ...
  const readMcpConfig = useCallback(async (): Promise<string> => {
    if (typeof window.JokerGui?.getJokerConfigFile !== 'function') return mcpConfigText
    const file = await window.JokerGui.getJokerConfigFile()
  ...
  const refreshMcpRuntimeOverlay = useCallback(async (): Promise<void> => {
    if (typeof window.JokerGui?.runtimeRequest !== 'function') {
      setRuntimeInfo(null)
  ...
  const refreshSkillList = useCallback(async (): Promise<void> => {
    if (typeof window.JokerGui?.listSkills !== 'function') {
      setDiscoveredSkills([])
    ...
    setSkillListError('')
    try {
      const result = await window.JokerGui.listSkills(workspaceRoot || undefined)
      if (!result.ok) {
        setDiscoveredSkills([])
  ...
  const refreshSkillRoots = useCallback(async (): Promise<void> => {
    if (typeof window.JokerGui?.listSkillRoots !== 'function') {
      setSkillRoots([])
      return
    }
    try {
      const result = await window.JokerGui.listSkillRoots(workspaceRoot || undefined)
  ...
  const appendMcpConfig = async (id: string, config: JsonRecord): Promise<void> => {
    ...
    const result = await window.JokerGui.setJokerConfigFile(merged.text)
  ...
  const addItem = async (item: MarketplaceItem): Promise<void> => {
    ...
    try {
      ...
      const result = await window.JokerGui.saveSkillFile(selectedSkillRoot.path, item.id, content)
      if (!result.ok) {
  ...
  const addCustom = async (): Promise<void> => {
    ...
    try {
      if (activeKind === 'mcp') {
        ...
        const result = await window.JokerGui.saveSkillFile(selectedSkillRoot.path, id, content)
        if (!result.ok) {
          setNotice({ tone: 'error', message: result.message })
          return
        }
  ...
  const openManageTarget = async (): Promise<void> => {
    try {
      ...
      const result = await window.JokerGui.openSkillRoot(selectedSkillRoot.path)
      if (!result.ok) setNotice({ tone: 'error', message: result.message ?? t('pluginActionFailed') })
    } catch (e) {
      setNotice({ tone: 'error', message: e instanceof Error ? e.message : String(e) })
    }
  ...
  const addFromGitHub = async (): Promise<void> => {
    ...
    setGithubImportSummary(null)
    try {
      const result = await window.JokerGui.importSkillsFromGitHub(selectedSkillRoot.path, trimmedUrl)
      if (!result.ok) {
...
function runtimeOverlayErrorMessage(error: unknown, fallback: string): string {
  const message = error instanceof Error ? error.message : String(error)
  return /runtimeRequest|JokerGui|Cannot read properties/i.test(message) ? fallback : message
}
...
function OAuthConnectorPreviewDialog({
  ...
  const title = itemTitle(item, t)
  const openDocs = (): void => {
    if (!oauth || typeof window.JokerGui?.openExternal !== 'function') return
    ...
    void window.JokerGui.openExternal(oauth.docsUrl).catch(() => undefined)
  }
