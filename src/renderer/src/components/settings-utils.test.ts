...
import type { AppSettingsV1, JokerRuntimeSettingsV1 } from '@shared/app-settings'
import { coerceRendererSettings, diffSettingsPatch, mergeSettings } from './settings-utils'

function settings(JokerPatch: Partial<JokerRuntimeSettingsV1> = {}): AppSettingsV1 {
...
describe('diffSettingsPatch', () => {
  ...
  it('round-trips a profiles-only subagent diff without losing sibling settings', () => {
    ...
    const next = mergeSettings(base, {
      agents: { Joker: { subagents: { profiles: [replacement] } } }
    })
    ...
    expect(mergeSettings(base, patch).agents.Joker.subagents).toEqual({
      enabled: true,
      maxParallel: 3,
      maxChildRuns: 12,
      defaultToolPolicy: 'inherit',
  ...
  it('emits explicit clear sentinels for normalized-away model and reasoning overrides', () => {
    const base = coerceRendererSettings(settings({
      ...
      contextCompaction: {
        ...settings().agents.Joker.contextCompaction,
    ...
    const roundTripped = mergeSettings(base, patch).agents.Joker
