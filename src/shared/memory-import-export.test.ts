...
`)
  ...
  it('builds stable imported memory content', () => {
    expect(buildMemoryImportContent({
      date: 'unknown',
      category: '项目',
      content: 'Joker: 桌面 AI 应用。',
      tags: []
    })).toBe('[unknown] 项目: Joker: 桌面 AI 应用。')
  })
})
...
describe('buildMemoryMarkdownExport', () => {
  it('exports grouped memory records as markdown', () => {
    ...
    expect(markdown).toContain('# Joker 记忆导出')
    ...
    expect(markdown).toContain('[2026-07-02] - 偏好: 喜欢 TypeScript。 [disabled] (workspace: /tmp/Joker)')
    expect(markdown).not.toContain('不要导出')
  })

  it('uses a dated default export filename', () => {
    expect(defaultMemoryExportFileName(new Date('2026-07-03T12:00:00.000Z'))).toBe('Joker-memory-export-2026-07-03.md')
  })
})
