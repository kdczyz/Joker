...
/**
 * Main -> Joker only. Raw paths and the protected operation token MUST NOT be
...
/** Main -> Joker request to resolve a handle for one authenticated View. */
...
/**
 * Joker -> Main only. `absolutePath` is consumed by the Main lease resolver and
...
/** Joker -> Main only. The absolute path is consumed by the desktop action. */
