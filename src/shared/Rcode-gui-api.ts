...
export type RcodeRuntimeStatusPayload = {
...
export type RcodeProjectConfigServerSummary = {
  id: string
  transport: 'stdio' | 'streamable-http' | 'sse'
  target: string
  enabled: boolean
}
export type RcodeProjectConfigFileResult = {
  workspaceRoot: string
  path: string
  content: string
  exists: boolean
  ...
  message?: string
  digest?: string
  serverSummaries: RcodeProjectConfigServerSummary[]
...
export type LegacySessionSourceKind = 'Rcode' | 'coreagent' | 'custom'
...
export type LegacySessionDetectResult = {
  /** Destination threads directory (current Rcode data dir + /threads). */
  destDir: string
...
export type RcodeGuiApi = ExtensionIpcApi & {
  platform: string
  homeDir: string
  dataMigration: {
  ...
  resolveRcodeApproval: (request: RcodeProtectedApprovalRequest) => Promise<RcodeProtectedApprovalResult>
  ...
  getRcodeConfigFile: () => Promise<DeepseekConfigFileResult>
  setRcodeConfigFile: (content: string) => Promise<DeepseekConfigSaveResult>
  openRcodeConfigDir: () => Promise<PathOpenResult>
  getRcodeProjectConfigFile: (workspaceRoot: string) => Promise<RcodeProjectConfigFileResult>
  setRcodeProjectConfigFile: (workspaceRoot: string, content: string) => Promise<RcodeProjectConfigFileResult>
  setRcodeProjectConfigTrust: (
    workspaceRoot: string,
    trusted: boolean,
    expectedDigest?: string
  ) => Promise<RcodeProjectConfigFileResult>
  openRcodeProjectConfigDir: (workspaceRoot: string) => Promise<PathOpenResult>
  ...
  onRuntimeStatus: (handler: (payload: RcodeRuntimeStatusPayload) => void) => () => void
...
export type RcodeProtectedApprovalRequest = {
  approvalId: string
  decision: 'allow' | 'deny'
  /** Policy decisions are generated by Rcode; user decisions require a protected native confirmation. */
  source: 'policy' | 'user'
}
