...
describe('migration inclusion policy', () => {
  it('hard-excludes Joker-owned credentials under every preset', () => {
  ...
  it('never drops Design and SDD artifacts as generic build output', () => {
    expect(classifyDataMigrationPath({ path: '.Joker-design/screen/dist/index.html', scope: 'workspace', preset: 'smaller' })).toEqual({
      action: 'include',
      ruleId: 'portable-artifact'
    })
    expect(classifyDataMigrationPath({ path: '.Jokersdd/draft/build/spec.md', scope: 'workspace', preset: 'smaller' }).action).toBe('include')
  })
...
describe('cross-platform source paths', () => {
  ...
  it('parses POSIX and home paths', () => {
    ...
    expect(parseMigrationSourcePath('~/.Joker/write_workspace', 'linux')).toMatchObject({
      kind: 'home',
      root: '~',
      segments: ['.Joker', 'write_workspace']
    })
  })
