...
/**
 * Read the Joker thread id from a legacy `agentThreadIds` record.
...
export function readLegacyAgentThreadId(input: unknown): string {
  ...
  const candidates = [
    typeof raw.Joker === 'string' ? raw.Joker.trim() : '',
...
export function buildClawRuntimePrompt(
  ...
  const imageGeneration = settings.provider && settings.agents
    ? resolveJokerImageGenerationSettings(settings as AppSettingsV1)
  ...
  const textToSpeech = settings.provider && settings.agents
    ? resolveJokerTextToSpeechSettings(settings as AppSettingsV1)
  ...
  const musicGeneration = settings.provider && settings.agents
    ? resolveJokerMusicGenerationSettings(settings as AppSettingsV1)
  ...
  const videoGeneration = settings.provider && settings.agents
    ? resolveJokerVideoGenerationSettings(settings as AppSettingsV1)
