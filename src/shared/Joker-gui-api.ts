...
export type JokerRuntimeStatusPayload = {
...
export type JokerProjectConfigServerSummary = {
  id: string
  transport: 'stdio' | 'streamable-http' | 'sse'
  target: string
  enabled: boolean
}
export type JokerProjectConfigFileResult = {
  workspaceRoot: string
  path: string
  content: string
  exists: boolean
  ...
  message?: string
  digest?: string
  serverSummaries: JokerProjectConfigServerSummary[]
...
export type LegacySessionSourceKind = 'Joker' | 'coreagent' | 'custom'
...
export type LegacySessionDetectResult = {
  /** Destination threads directory (current Joker data dir + /threads). */
  destDir: string
...
export type JokerGuiApi = ExtensionIpcApi & {
  ...
  resolveJokerApproval: (request: JokerProtectedApprovalRequest) => Promise<JokerProtectedApprovalResult>
  ...
  getJokerConfigFile: () => Promise<DeepseekConfigFileResult>
  setJokerConfigFile: (content: string) => Promise<DeepseekConfigSaveResult>
  openJokerConfigDir: () => Promise<PathOpenResult>
  getJokerProjectConfigFile: (workspaceRoot: string) => Promise<JokerProjectConfigFileResult>
  setJokerProjectConfigFile: (workspaceRoot: string, content: string) => Promise<JokerProjectConfigFileResult>
  setJokerProjectConfigTrust: (
    workspaceRoot: string,
    trusted: boolean,
    expectedDigest?: string
  ) => Promise<JokerProjectConfigFileResult>
  ...
  pushGitChanges: (workspaceRoot: string) => Promise<GitPushResult>
  ...
  onRuntimeStatus: (handler: (payload: JokerRuntimeStatusPayload) => void) => () => void
  ...
  /* ---- Remote Agent (被控端) ---- */
  remoteAgent: {
    start: (token: string) => Promise<{ ok: boolean; message?: string }>
    stop: () => Promise<{ ok: boolean }>
    getStatus: () => Promise<{ state: 'offline' | 'connecting' | 'online' | 'waiting' }>
    onStatus: (handler: (payload: { state: string; message?: string }) => void) => () => void
    onCommand: (handler: (payload: unknown) => void) => () => void
    onEvent: (handler: (payload: { commandId: string; event: Record<string, unknown> }) => void) => () => void
...
export type JokerProtectedApprovalRequest = {
  approvalId: string
  decision: 'allow' | 'deny'
  /** Policy decisions are generated by Joker; user decisions require a protected native confirmation. */
  source: 'policy' | 'user'
}

export type JokerProtectedApprovalResult =
  | { confirmed: false }
