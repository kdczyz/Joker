...
export class JokerAudioAnalysisUnavailableError extends Error {
  ...
  constructor(
    ...
    super(remediation)
    this.name = 'JokerAudioAnalysisUnavailableError'
  }
}

export class JokerAudioAnalysisJobError extends Error {
  constructor(
    readonly code: string,
    message: string,
    readonly retryable: boolean
    ...
    super(message)
    this.name = 'JokerAudioAnalysisJobError'
  }
}

export class JokerVisualAnalysisUnavailableError extends Error {
  readonly networkUsed = false
  ...
  constructor(
    ...
    super(remediation)
    this.name = 'JokerVisualAnalysisUnavailableError'
  }
}
 ...
 * Joker owns native process execution, durable progress, cancellation, and
 * source-identity calculation.
 */
export class JokerLocalAudioAnalysisBroker implements LocalMediaIntelligenceBroker {
  readonly id = 'Joker.host.audio-analysis'
  ...
  denoiseMetadataCapability: NonNullable<LocalMediaIntelligenceBroker['denoiseMetadataCapability']> = async () => ({
    ...
    remediation: 'This Joker Extension API exposes no verified local noise-profile analyzer yet. No media was uploaded or modified, and no denoise levels were fabricated.',
    retryable: false,
    local: true,
    networkUsed: false
  })
  ...
  visualModelStatus: NonNullable<LocalMediaIntelligenceBroker['visualModelStatus']> = async () => {
    ...
    if (!media.ffmpeg.available || !media.ffprobe.available) {
      return {
        ...
        state: 'failed',
        receipt: undefined,
        remediation: 'The signed local visual package is present, but verified frame decoding requires FFmpeg and FFprobe in a reviewed Joker Host location.'
      }
    }
    return projected
  }
  ...
  indexVisual: NonNullable<LocalMediaIntelligenceBroker['indexVisual']> = async ({
    ...
    for (let offset = 0; offset < samples.length; offset += 16) {
      ...
      if (result.outcome === 'unavailable') {
        throw new JokerVisualAnalysisUnavailableError(
          result.code,
          result.remediation,
          result.retryable
        )
  ...
  extractSyncFeatures: NonNullable<LocalMediaIntelligenceBroker['extractSyncFeatures']> = async ({
    ...
    if (
      ...
      throw new JokerAudioAnalysisJobError(
  ...
  private async startAndWait(
    ...
    if (started.outcome === 'unavailable') {
      throw new JokerAudioAnalysisUnavailableError(
        started.code,
        started.remediation,
        started.retryable
      )
...
function mismatchedResult(expected: string): JokerAudioAnalysisJobError {
  return new JokerAudioAnalysisJobError(
    'AUDIO_ANALYSIS_RESULT_MISMATCH',
