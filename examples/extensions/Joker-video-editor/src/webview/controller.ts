...
export function useEditorController(client: ExtensionHostClient): EditorController {
  ...
  useEffect(() => {
    ...
    const messageSubscription = client.ui.onDidReceiveMessage((message) => {
      if (message.channel === 'Joker.extension.view.overflow') {
        void refreshAll()
        return
      }
      if (message.channel === 'Joker-video-editor.project-changed') {
      ...
      if (message.channel === 'Joker-video-editor.selection-changed') {
        const updated = selectionUpdateFrom(message.payload)
        if (updated) dispatch({ type: 'selection-synced', ...updated })
        return
      }
      if (message.channel === 'Joker-video-editor.derived-changed') {
      ...
      if (message.channel === 'Joker-video-editor.media-intelligence-progress') {
      ...
      if (message.channel === 'Joker-video-editor.generation-progress') {
        const payload = isRecord(message.payload) ? message.payload : undefined
        const record = generationRecordFrom(payload?.record)
        if (record) dispatch({ type: 'generation-record', value: record })
        return
      }
      if (message.channel === 'Joker-video-editor.active-project-changed') {
        const change = projectChange(message.payload, copy('projectChanged'))
        if (change) {
      ...
      if (message.channel === 'Joker.resultPreview.open') {
      ...
      if (message.channel === 'Joker-video-editor.command-progress') {
...
function speakerAdaptersFrom(value: unknown): SpeakerAdapterProjection[] {
  ...
  const adapters = value.flatMap((candidate) => {
    ...
    if (
      ...
      (descriptor.format !== undefined && descriptor.format !== 'Joker-speaker-json-v1') ||
      (descriptor.modelId !== undefined && typeof descriptor.modelId !== 'string') ||
      (descriptor.modelVersion !== undefined && typeof descriptor.modelVersion !== 'string')
    ) return []
    return [{
      descriptor: {
        ...
        ...(descriptor.format === 'Joker-speaker-json-v1' ? { format: 'Joker-speaker-json-v1' as const } : {}),
...
function interchangeLossManifestFrom(
  ...
  if (!isRecord(value) || value.adapterId !== 'Joker.otio-json' || value.adapterVersion !== '1.0.0' ||
    typeof value.portableLossless !== 'boolean' || typeof value.JokerRoundTripLossless !== 'boolean' ||
  ...
  const entries = value.entries.map((entry): InterchangeLossManifestProjection['entries'][number] => {
    if (!isRecord(entry) ||
      ...
      (entry.preservation !== 'otio-standard' && entry.preservation !== 'Joker-metadata') ||
  ...
  return {
    ...
    JokerRoundTripLossless: value.JokerRoundTripLossless,
    entries,
...
function otioExportJobProjectionFrom(
  ...
  if (
    ...
    value.adapterId !== 'Joker.otio-json' || value.adapterVersion !== '1.0.0' ||
...
function otioImportPreviewFrom(value: unknown, invalidMessage: string): OtioImportPreview {
  if (!isRecord(value) || value.outcome !== 'interchange-import-preview' ||
    ...
    (value.fidelity !== 'Joker-metadata' && value.fidelity !== 'portable-otio') ||
