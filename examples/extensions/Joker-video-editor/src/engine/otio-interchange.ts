...
export const OTIO_ADAPTER_ID = 'Joker.otio-json' as const
...
export type InterchangeLossEntry = {
  ...
  feature: string
  nodeId: string
  preservation: 'otio-standard' | 'Joker-metadata'
  message: string
}
...
export type InterchangeLossManifest = {
  ...
  JokerRoundTripLossless: boolean
...
export type OtioInterchangeImport = {
  ...
  fidelity: 'Joker-metadata' | 'portable-otio'
...
export function exportProjectToOtio(project: VideoProject): OtioInterchangeExport {
  ...
  if (validated.linkGroups.length > 0) {
    addLoss(loss, metadataLoss(
      ...
      'A/V and sync link groups are preserved in Joker metadata because OTIO has no portable link-group contract.'
    ))
  }
  if (validated.transcripts.length > 0) {
    addLoss(loss, metadataLoss(
      'transcripts-custom-metadata', 'transcripts', validated.id,
      'Timed transcripts are preserved in Joker metadata and are not portable OTIO timeline objects.'
    ))
  }
  if (validated.derivedReferences.length > 0) {
    addLoss(loss, metadataLoss(
      'derived-media-custom-metadata', 'derived-media', validated.id,
      'Derived-media provenance is preserved in Joker metadata; cache payloads are not exported through OTIO.'
    ))
  }
...
export function importProjectFromOtio(value: unknown): OtioInterchangeImport {
  ...
  assertDocumentBounds(document)
  const documentDigest = canonicalDigest(document)
  validateJokerMediaReferences(document)
  const metadata = optionalRecord(document.metadata)
  const Joker = optionalRecord(metadata?.Joker)
  if (!Joker || Joker.adapterId !== OTIO_ADAPTER_ID || Joker.adapterVersion !== OTIO_ADAPTER_VERSION) {
    return importPortableOtio(document, documentDigest)
  }
  const projectDigest = stringValue(Joker.projectDigest, 'metadata.Joker.projectDigest', 64)
  const rawProject = structuredClone(record(Joker.project, 'metadata.Joker.project'))
  ...
  if (project.id !== Joker.projectId || project.currentRevision !== Joker.projectRevision) {
    invalid('OTIO project identity or revision metadata is inconsistent')
  }
  const loss = parseLossManifest(Joker.lossManifest)
...
function importPortableOtio(
  ...
  addLoss(loss, metadataLoss(
    'portable-import-no-Joker-snapshot', 'project-metadata', 'project',
    'This OTIO document has no Joker round-trip snapshot; only the bounded portable timeline subset is imported.'
  ))
...
function portableSequence(
  ...
  const timelineMetadata = optionalRecord(optionalRecord(timeline.metadata)?.Joker)
  ...
  for (const [trackIndex, rawTrack] of arrayValue(stack.children, 'OTIO stack children').entries()) {
    ...
    const metadata = optionalRecord(optionalRecord(trackNode.metadata)?.Joker)
    ...
    for (const [childIndex, rawChild] of arrayValue(trackNode.children, `OTIO track ${trackIndex} children`).entries()) {
      ...
      const childMetadata = optionalRecord(optionalRecord(child.metadata)?.Joker)
...
function portableClip(
  ...
  const assetId = decodeURIComponent(target.slice('Joker-media://'.length))
...
function portableEffects(value: unknown, itemId: string, loss: LossCollector): EffectInstance[] {
  ...
  for (const [index, raw] of arrayValue(value, `OTIO clip ${itemId} effects`).entries()) {
    ...
    const Joker = optionalRecord(optionalRecord(effect.metadata)?.Joker)
    const full = Joker && typeof Joker.type === 'string' && optionalRecord(Joker.parameters)
      ? Joker as unknown as EffectInstance
      : undefined
    effects.push(full ? structuredClone(full) : {
      id: optionalOtioId(Joker?.id) ?? `${itemId}.effect-${index + 1}`,
...
function otioTimeline(
  ...
  return {
    ...
    tracks: {
      ...
      metadata: { Joker: { id: `${sequence.id}.tracks` } },
      children: tracks,
      effects: [],
      markers: []
    }
  ...
  for (const entry of timelineEntries) {
    ...
    if (entry.startFrame < cursor) {
      addLoss(loss, metadataLoss(
        ...
        `Overlapping timing on track ${track.id} is exact in Joker metadata but sequential OTIO Track semantics may flatten it.`
      ))
    }
...
function otioClip(
  ...
  if (item.nestedSequenceId) {
    addLoss(loss, metadataLoss(
      ...
      `Nested sequence ${item.nestedSequenceId} is preserved by stable ID in Joker metadata; OTIO consumers may flatten it.`
    ))
  }
  if (!identityVisual(item)) {
    addLoss(loss, metadataLoss(
      'visual-transform-custom-metadata', 'visual-transform', item.id,
      'Transform, crop, opacity, and fades are preserved in Joker metadata rather than portable OTIO fields.'
    ))
  }
  if (item.keyframes?.length) {
    addLoss(loss, metadataLoss(
      'keyframes-custom-metadata', 'keyframes', item.id,
      'Keyframe interpolation and property paths are preserved in Joker metadata.'
    ))
  }
  if (item.effects?.length) {
    addLoss(loss, metadataLoss(
      'effect-parameters-custom-metadata', 'effects', item.id,
      'Effect identities are emitted as OTIO Effect objects; parameters remain Joker metadata.'
    ))
  }
  ...
  return {
    ...
    media_reference: item.nestedSequenceId
      ? {
          ...
          metadata: { Joker: { nestedSequenceId: item.nestedSequenceId } },
          available_range: null
        }
      : {
          OTIO_SCHEMA: 'ExternalReference.1',
          name: asset!.name,
          target_url: `Joker-media://${encodeURIComponent(asset!.id)}`,
          available_range: timeRange(
            0,
            Math.max(1, microsecondsToFrames(asset!.durationUs, project.fps, 'nearest')),
            project.fps
          ),
          metadata: { Joker: { assetId: asset!.id } }
        },
    effects,
    markers: [],
    metadata: {
...
function otioCaption(
  ...
  addLoss(loss, metadataLoss(
    ...
    'Editable caption text, word timing, style, and animation are preserved in Joker metadata.'
  ))
...
function otioEffects(
  ...
  const result: Record<string, unknown>[] = effects.map((effect) => ({
    ...
    metadata: { Joker: structuredClone(effect) }
  }))
  if (item.speed.numerator !== item.speed.denominator) {
    result.unshift({
      ...
      metadata: { Joker: { speed: structuredClone(item.speed) } }
    })
  }
  if (keyframes.length > 0 && effects.length === 0) {
    addLoss(loss, metadataLoss(
      'keyframes-without-effect-custom-metadata', 'keyframes', item.id,
      'Property keyframes without an OTIO Effect are retained only in clip-level Joker metadata.'
    ))
  }
  return result
}
...
function otioGap(durationFrames: number, fps: Rational, trackId: string, startFrame: number): Record<string, unknown> {
  return {
    ...
    effects: [],
    markers: [],
    metadata: { Joker: { id: `${trackId}.gap.${startFrame}`, startFrame, durationFrames } }
  }
}
...
function rationalTime(frame: number, fps: Rational): Record<string, unknown> {
  return {
    ...
    metadata: { Joker: { frame, frameRate: structuredClone(fps) } }
  }
}
...
function collectTimecodeMappings(document: Record<string, unknown>, fps: Rational): OtioTimecodeMapping[] {
  ...
  visit(document, (node) => {
    ...
    const Joker = optionalRecord(metadata?.Joker)
    if (
      typeof Joker?.id !== 'string' ||
      typeof Joker.sequenceId !== 'string' ||
      !Number.isSafeInteger(Joker.timelineStartFrame) ||
      !Number.isSafeInteger(Joker.durationFrames)
    ) return
    addMapping(
      mappings,
      fps,
      Joker.sequenceId,
      Joker.id,
      Number(Joker.timelineStartFrame),
      Number(Joker.timelineStartFrame) + Number(Joker.durationFrames)
    )
  })
...
function validateJokerMediaReferences(document: Record<string, unknown>): void {
  visit(document, (node) => {
    if (node.OTIO_SCHEMA !== 'ExternalReference.1') return
    if (typeof node.target_url !== 'string' || !/^Joker-media:\/\/[A-Za-z0-9._~%-]+$/u.test(node.target_url)) {
      invalid('OTIO external references must use bounded Joker-media URLs')
    }
  })
}
...
function parseLossManifest(value: unknown): InterchangeLossManifest {
  ...
  if (typeof manifest.portableLossless !== 'boolean' || typeof manifest.JokerRoundTripLossless !== 'boolean') {
...
function metadataLoss(
  ...
  return { code, severity: 'warning', feature, nodeId, preservation: 'Joker-metadata', message }
}

function lossManifest(
  collector: LossCollector,
  JokerRoundTripLossless = true
