import type { ExtensionHostClient, HostMessage, JobEvent, JobSnapshot, JsonValue, Locale, Theme } from '@joker-code/extension-api'
...
describe('video editor artifact controller integration', () => {
  it('loads bounded generation state and keeps retry authority free of persisted prompts and handles', async () => {
    ...
    await act(async () => {
      emitMessage({
        channel: 'Joker-video-editor.generation-progress',
  ...
  it('opens a derived waveform with an opaque Host lease and reuses the unexpired lease', async () => {
    ...
    const openViewResource = vi.fn(async () => ({
      ...
      handleId,
      url: 'Joker-media://lease/waveform-ready',
      mimeType: 'image/png',
    ...
    expect(firstUrl).toBe('Joker-media://lease/waveform-ready')
    expect(secondUrl).toBe('Joker-media://lease/waveform-ready')
  ...
  it('keeps player media on leases and routes subtitle open/reveal through the trusted Host action', async () => {
    const openViewResource = vi.fn(async ({ handleId }: { handleId: string }) => ({
      ...
      handleId,
      url: `Joker-media://lease/${handleId}`,
      mimeType: 'image/png',
      expiresAt: '2099-01-01T00:00:00.000Z'
    }))
  ...
  it('keeps Joker theme and locale when initialization fails and retries the full controller bootstrap', async () => {
  ...
  it('applies live Joker theme and language changes', async () => {
  ...
  it('does not let delayed initial values overwrite newer Joker events', async () => {
  ...
  it('loads revision-fenced audio capabilities and folds bounded Host progress into the mounted sidebar state', async () => {
      ...
      if (action === 'analysis.capabilities') return { content: {
        ...
        capabilities: {
          ...
          analyses: [
            ...
            { analysis: 'sync-features', available: true, algorithm: 'Joker.pcm-energy-envelope', algorithmVersion: '1.0.0', local: true, networkUsed: false }
          ]
        },
        denoiseMetadata: {
          outcome: 'ready', local: true, networkUsed: false,
          descriptor: {
            adapterId: 'Joker.fixture.denoise', adapterVersion: '1.0.0',
    ...
    await act(async () => {
      emitMessage({
        channel: 'Joker-video-editor.media-intelligence-progress',
        payload: {
          schemaVersion: 1,
  ...
  it('imports reviewed speaker evidence and keeps preview/apply as separate revision-bound actions', async () => {
    ...
    const executeCommand = vi.fn(async (_id: string, args?: JsonValue) => {
      ...
      if (action === 'analysis.capabilities') return { content: {
        ...
        speakerAdapters: [{
          descriptor: { id: 'Joker.imported-speaker-labels', version: '1.0.0', execution: 'import', format: 'Joker-speaker-json-v1' },
          outcome: 'ready', local: true, networkUsed: false
        }, {
          descriptor: { id: 'Joker.host.local-speaker', version: '1.0.0', execution: 'local-model', modelId: 'speaker-diarization', modelVersion: 'unavailable' },
    ...
    const reviewedDocument = JSON.stringify({
      schemaVersion: 1, adapterId: 'Joker.imported-speaker-labels', identities: [identity],
    ...
    expect(executeCommand).toHaveBeenCalledWith('editor-request', {
      ...
      payload: expect.objectContaining({
        ...
        document: expect.objectContaining({ adapterId: 'Joker.imported-speaker-labels' })
      })
    })
  ...
  it('tracks durable OTIO export and enforces picker-preview-confirm import without persisting grants', async () => {
    ...
    const lossManifest = {
      adapterId: 'Joker.otio-json', adapterVersion: '1.0.0',
      portableLossless: false, JokerRoundTripLossless: true,
      entries: [{
        code: 'effects-custom-metadata', severity: 'warning', feature: 'effects',
        nodeId: 'item-interview', preservation: 'Joker-metadata',
        message: 'Effect parameters use Joker metadata.'
      }],
      truncated: 0
    }
    const projection = {
    ...
    const executeCommand = vi.fn(async (_id: string, args?: JsonValue) => {
      ...
      if (action === 'interchange.import-preview') return { content: {
        ...
        lossManifest: { ...lossManifest, JokerRoundTripLossless: false },
    ...
    expect(controller?.state.otioImportPreview).toMatchObject({
      ...
      suggestedProjectId: importedProject.id,
      fidelity: 'portable-otio',
      lossManifest: { JokerRoundTripLossless: false }
    })
  ...
  it('loads a revision-bound Host media page beyond asset 100 and opens its opaque resource', async () => {
    ...
    const openViewResource = vi.fn(async ({ handleId }: { handleId: string }) => ({
      ...
      url: 'Joker-media://lease/media-page-0100', mimeType: 'video/mp4',
      expiresAt: '2099-01-01T00:00:00.000Z'
    }))
    const { client } = fakeClient({ executeCommand, openViewResource })
    let controller: EditorController | undefined
  ...
  it('opens result-preview media from the Host message without loading the full project editor', async () => {
    ...
    expect(controller?.state.activeMediaUrl).toContain('Joker-media://lease/')
  })
  ...
  it('routes sequence, media-folder, preview, and Agent-context commands through bounded revision-safe payloads', async () => {
    ...
    const attachComposerContext = vi.fn(async (request) => ({
      ...
      provenance: {
        ...
        viewContributionId: 'extension:Joker-examples.Joker-video-editor/video-editor',
    ...
    expect(previewResource).toMatchObject({
      ...
      title: 'Source B',
      mediaKind: 'video',
      url: `Joker-media://lease/${project.assets[0]!.mediaHandleId}`
    })
...
function makeArchiveJob(id: string, state: JobSnapshot['state']): JobSnapshot {
  return {
    ...
    kind: 'media.archive',
    kindSchemaVersion: 1,
    ownerExtensionId: 'Joker-examples.Joker-video-editor',
...
function projectChangedMessage(projectId: string): HostMessage {
  return {
    channel: 'Joker-video-editor.active-project-changed',
    payload: {
      schemaVersion: 1,
      projectId,
      revision: 0,
