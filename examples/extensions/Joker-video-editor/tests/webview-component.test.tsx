...
import type { JobSnapshot } from '@joker-code/extension-api'
...
describe('video editor docked workbench', () => {
  ...
  it('renders the persisted Output workspace and a completed atomic project package without sensitive references', () => {
      ...
      result: {
        ...
        data: {
          ...
          generatedMedia: {
            ...
            mode: 'export',
            kind: 'data',
            displayName: '/Users/zxy/private/interview.Joker-video.zip',
            mimeType: 'application/zip',
            byteSize: 768,
            completionIdentity: 'secret-completion-identity',
            workspaceRelativeDisplayLocation: 'private/output/interview.Joker-video.zip',
            revoked: false
          }
        }
      }
    ...
    expect(html).toContain('interview.Joker-video.zip')
  ...
  it('renders localized OTIO export and explicit two-step import without exposing the picker grant', () => {
    const project = makeViewProject()
    const manifest = {
      adapterId: 'Joker.otio-json' as const,
      ...
      entries: [{
        ...
        nodeId: 'item-interview', preservation: 'Joker-metadata' as const,
  ...
  it('renders two protected resources for real side-by-side and wipe preview comparison', () => {
    ...
    const left = {
      ...
      title: 'Source take',
      url: 'Joker-media://lease/preview-left',
      mediaKind: 'video' as const
    }
    const right = {
      entryId: 'preview-right',
      title: 'Generated take',
      url: 'Joker-media://lease/preview-right',
  ...
  it('renders every editing region with accessible landmarks and supported boundaries', () => {
    ...
    expect(html).toContain('Edit with the main Joker Agent')
  ...
  it('renders explicit empty, interaction-required, reconnect and legacy-run states', () => {
    ...
    expect(emptyHtml).toContain('A protected Joker desktop interaction is required.')
  ...
  it('renders the workbench in Simplified Chinese and follows the Joker theme', () => {
    const project = makeViewProject()
    project.revisions[0] = {
      ...project.revisions[0]!,
    ...
    for (const label of ['Joker 视频剪辑', '媒体库', '播放器', '逐字稿', '时间线', '检查器', '字幕', '版本', '预览与校样', 'Agent 协作', '导出任务']) {
      expect(html).toContain(label)
    }
