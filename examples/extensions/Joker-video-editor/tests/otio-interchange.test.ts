...
describe('OTIO JSON professional interchange', () => {
  it('exports stable IDs, exact frame/timecode metadata, and an explicit bounded loss manifest', () => {
    ...
    expect(collectTargetUrls(exported.document)).toEqual(['Joker-media://asset-1', 'Joker-media://asset-1', 'Joker-media://asset-1'])
    ...
    expect(exported.lossManifest).toMatchObject({ portableLossless: false, JokerRoundTripLossless: true, truncated: 0 })
  ...
  it('serializes deterministically and round-trips every sequence plus nested/text/keyframe/effect metadata', () => {
    ...
    expect(imported.fidelity).toBe('Joker-metadata')
  ...
  it('bounds the loss report and counts omitted entries instead of silently dropping them', () => {
    ...
    expect(manifest.JokerRoundTripLossless).toBe(true)
  })
  ...
  it('rejects tampered round-trip metadata and path-bearing external references', () => {
    ...
    const metadata = (tampered.metadata as Record<string, unknown>).Joker as Record<string, unknown>
    ...
    expect(() => importProjectFromOtio(pathBearing)).toThrowError(/Joker-media/u)
  })

  it('imports a bounded portable OTIO timeline without requiring Joker snapshot metadata', () => {
            ...
            children: [
              ...
              },
              {
                ...
                media_reference: {
                  ...
                  name: 'External interview',
                  target_url: 'Joker-media://external-asset',
    ...
    expect(imported).toMatchObject({
      ...
      lossManifest: { portableLossless: false, JokerRoundTripLossless: false }
    })
