...
async function activatedHarness(): Promise<ExtensionTestHarness> {
  const root = await mkdtemp(join(tmpdir(), 'Joker-video-editing-host-'))
  roots.push(root)
...
function projectPath(harness: ExtensionTestHarness, projectId: string): string {
  return join(harness.context.workspaceContext!.root, '.Joker-video', 'projects', projectId, 'project.json')
}
