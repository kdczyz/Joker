...
import {
  JokerLocalAudioAnalysisBroker
} from '../src/host/Joker-audio-analysis-broker.js'
...
describe('Joker local audio-analysis Extension API adapter', () => {
  it('maps the public verified visual broker to bounded real-evidence adapter calls', async () => {
    ...
    harness.media.setVisualModelStatus({
      ...
      descriptor,
      receipt: {
        broker: 'Joker-model-broker', packageSource: 'bundled',
    ...
    const broker = new JokerLocalAudioAnalysisBroker(harness.context)
    ...
    await expect(broker.embedVisualQuery!({
      ...
      name: 'JokerVisualAnalysisUnavailableError',
  ...
  it('surfaces beat/downbeat unavailability without producing marker evidence', async () => {
    ...
    await expect(broker.analyzeBeats!({
      ...
      name: 'JokerAudioAnalysisUnavailableError',
