...
describe('schema-v2 recovery and relink', () => {
  it('opens a corrupt project from its newest snapshot, blocks writes, and preserves evidence on confirm', async () => {
    ...
    const projectPath = join(root, '.Joker-video/projects/recover-project/project.json')
    ...
    expect(await readFile(
      join(root, '.Joker-video/projects/recover-project/backups/unreadable-project-manifest.json'),
      'utf8'
    )).toBe(corrupt)
  ...
  it('keeps unreadable auxiliary manifests, marks media offline, and never replaces them during autosave', async () => {
    ...
    const mediaManifest = join(root, '.Joker-video/projects/manifest-project/media-manifest.json')
    ...
    expect(await readFile(
      join(root, '.Joker-video/projects/manifest-project/backups/unreadable-media-manifest.json'),
      'utf8'
    )).toBe(corrupt)
...
async function setup(): Promise<{ root: string; service: ProjectService }> {
  const root = await mkdtemp(join(tmpdir(), 'Joker-video-recovery-v2-'))
  roots.push(root)
  return {
    root,
