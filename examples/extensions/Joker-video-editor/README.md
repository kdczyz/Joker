...
Joker Video Editor is a local-first, transcript-oriented editor for talking-head,
interview, podcast, and short-form projects. It is the reference Joker Extension
...
optional capabilities. Joker desktop ships the exact same deterministic `.Jokerx`
...
Joker renderer internals, and the Extension Host, so the same project and tool
...
- a 280-760 px docked sidebar with Script, Clips, Timeline, Properties, and
  Output workspaces, live Joker locale/theme updates, keyboard navigation, and
...
- For repository commands, a built Joker Extension API and Joker CLI from this
  checkout. For a standalone project, the CLI must come from the matching Joker
  installation and the required `@Joker` packages must exist in its npm registry.
...
- Visual indexing is opt-in. Joker accepts only the reviewed, signed local adapter
...
- Denoise analysis is metadata-only: a negotiated local adapter may publish a
  ...
  auto-applies a filter. The current public Joker media Broker has no verified
...
FFmpeg is not embedded in the `.Jokerx` package. Install it using the normal
...
does not provide the caption filter required by this example. Joker also searches
...
Fresh Joker desktop profiles install and globally enable the product-bundled
archive through the standard `.Jokerx` validator, immutable package store,
...
Each stable and daily Joker GitHub Release publishes the platform-independent
`Joker-video-editor-0.4.4.Jokerx` asset beside the desktop installers and the three
native evidence JSON files. Download the `.Jokerx` from the same release as the
Joker build you are running; do not copy an archive from an untrusted mirror.

Validate and install the downloaded package with the Joker Extension CLI:

```bash
Joker extension validate ./Joker-video-editor-0.4.4.Jokerx
Joker extension install ./Joker-video-editor-0.4.4.Jokerx
```

Review and accept the declared permissions, enable the extension in a trusted
workspace, then choose **Joker Video Editor** from Code mode's vertical right rail. Joker opens it in its own right-workspace tab. Installation validates the archive's
...
npm run verify:Joker-video-editor-package -- --input dist
```

The pack command builds and validates the extension twice and refuses to publish
unless both `.Jokerx` archives have the same byte length and SHA-256 digest.
...
npm run build:Joker
npm --prefix examples/extensions/Joker-video-editor run typecheck
npm --prefix examples/extensions/Joker-video-editor run test
npm --prefix examples/extensions/Joker-video-editor run build
npm --prefix examples/extensions/Joker-video-editor run validate
npm --prefix examples/extensions/Joker-video-editor run pack
```

The repository-wide example gate runs typecheck, tests, build, manifest
validation, Joker validation, and packing for every example into a temporary
directory:
...
`Joker/dist/cli/serve-entry.js` through a helper anchored to its own file
...
unrelated unscoped npm package named `Joker` as the Joker Agent CLI.

Generate the deterministic local audio/transcript fixture into a disposable
directory when exercising a manual flow:

```bash
npm --prefix examples/extensions/Joker-video-editor run fixture:generate -- \
  --output /tmp/Joker-video-editor-fixture
```
...
1. Use the default installed extension, or build and install the `.Jokerx` with the
   Joker Extension CLI. Grant it in a trusted workspace, then choose its direct
...
3. Use the protected import action. Joker owns the native picker and returns an
...
6. Edit in the spatial timeline, or ask the main Joker Agent to resolve the active
...
The project engine and the declared Agent tools can run under `Joker serve`
...
Workspace data is stored below `.Joker-video/`:
...
- Protected pickers return opaque, owner/workspace-bound handles. View playback
  uses short-lived, sender-bound `Joker-media://` leases rather than file paths.
...
| FFmpeg/ffprobe unavailable or missing `drawtext`/`libx264` | Install a build with the required capabilities (on macOS, `ffmpeg-full`), verify it locally, and restart Joker. Project editing remains available. |
...
| Damaged derived cache | Close the View, delete only the affected `.Joker-video/cache/<project-id>/` subtree, reopen, and regenerate thumbnails/waveforms. Never delete project state or source media as cache cleanup. |
...
Third-party extension authors should copy the boundaries, not private Joker
internals:
...
1. Register a `views.rightSidebar` contribution and keep the View useful at
   ...
   language in the main Joker conversation.
