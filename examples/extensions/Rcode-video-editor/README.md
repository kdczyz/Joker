...
Rcode Video Editor is a local-first, transcript-oriented editor for talking-head,
interview, podcast, and short-form projects. It is the reference Rcode Extension
...
optional capabilities. Rcode desktop ships the exact same deterministic `.Rcodex`
...
Rcode renderer internals, and the Extension Host, so the same project and tool
...
- a 280-760 px docked sidebar with Script, Clips, Timeline, Properties, and
  Output workspaces, live Rcode locale/theme updates, keyboard navigation, and
...
- For repository commands, a built Rcode Extension API and Rcode CLI from this
  checkout. For a standalone project, the CLI must come from the matching Rcode
  installation and the required `@Rcode` packages must exist in its npm registry.
...
- Visual indexing is opt-in. Rcode accepts only the reviewed, signed local adapter
...
- Denoise analysis is metadata-only: a negotiated local adapter may publish a
  ...
  auto-applies a filter. The current public Rcode media Broker has no verified
...
FFmpeg is not embedded in the `.Rcodex` package. Install it using the normal
...
does not provide the caption filter required by this example. Rcode also searches
...
Fresh Rcode desktop profiles install and globally enable the product-bundled
archive through the standard `.Rcodex` validator, immutable package store,
...
Each stable and daily Rcode GitHub Release publishes the platform-independent
`Rcode-video-editor-0.4.4.Rcodex` asset beside the desktop installers and the three
native evidence JSON files. Download the `.Rcodex` from the same release as the
Rcode build you are running; do not copy an archive from an untrusted mirror.

Validate and install the downloaded package with the Rcode Extension CLI:

```bash
Rcode extension validate ./Rcode-video-editor-0.4.4.Rcodex
Rcode extension install ./Rcode-video-editor-0.4.4.Rcodex
```

Review and accept the declared permissions, enable the extension in a trusted
workspace, then choose **Rcode Video Editor** from Code mode's vertical right rail. Rcode opens it in its own right-workspace tab. Installation validates the archive's
...
npm run verify:Rcode-video-editor-package -- --input dist
```

The pack command builds and validates the extension twice and refuses to publish
unless both `.Rcodex` archives have the same byte length and SHA-256 digest.
...
npm run build:Rcode
npm --prefix examples/extensions/Rcode-video-editor run typecheck
npm --prefix examples/extensions/Rcode-video-editor run test
npm --prefix examples/extensions/Rcode-video-editor run build
npm --prefix examples/extensions/Rcode-video-editor run validate
npm --prefix examples/extensions/Rcode-video-editor run pack
```

The repository-wide example gate runs typecheck, tests, build, manifest
validation, Rcode validation, and packing for every example into a temporary
directory:
...
`Rcode/dist/cli/serve-entry.js` through a helper anchored to its own file
...
unrelated unscoped npm package named `Rcode` as the Rcode Agent CLI.

Generate the deterministic local audio/transcript fixture into a disposable
directory when exercising a manual flow:

```bash
npm --prefix examples/extensions/Rcode-video-editor run fixture:generate -- \
  --output /tmp/Rcode-video-editor-fixture
```
...
1. Use the default installed extension, or build and install the `.Rcodex` with the
   Rcode Extension CLI. Grant it in a trusted workspace, then choose its direct
...
3. Use the protected import action. Rcode owns the native picker and returns an
...
6. Edit in the spatial timeline, or ask the main Rcode Agent to resolve the active
...
The project engine and the declared Agent tools can run under `Rcode serve`
...
Workspace data is stored below `.Rcode-video/`:
...
- Protected pickers return opaque, owner/workspace-bound handles. View playback
  uses short-lived, sender-bound `Rcode-media://` leases rather than file paths.
...
| FFmpeg/ffprobe unavailable or missing `drawtext`/`libx264` | Install a build with the required capabilities (on macOS, `ffmpeg-full`), verify it locally, and restart Rcode. Project editing remains available. |
...
| Damaged derived cache | Close the View, delete only the affected `.Rcode-video/cache/<project-id>/` subtree, reopen, and regenerate thumbnails/waveforms. Never delete project state or source media as cache cleanup. |
...
Third-party extension authors should copy the boundaries, not private Rcode
internals:
...
1. Register a `views.rightSidebar` contribution and keep the View useful at
   ...
   language in the main Rcode conversation.
