...
import { parseExtensionManifest, resolveExtensionManifestLocale } from '@Rcode/extension-api'
import { describe, expect, it } from 'vitest'

function readManifest() {
  return parseExtensionManifest(JSON.parse(
    readFileSync(new URL('../Rcode-extension.json', import.meta.url), 'utf8')
  ))
}
...
describe('video editor manifest localization', () => {
  ...
  it('localizes every Host-rendered video surface with English fallback', () => {
    ...
    expect(unsupported.displayName).toBe('Rcode Video Editor')
    expect(unsupported.contributes['views.rightSidebar'][0]?.title).toBe('Rcode Video Editor')
