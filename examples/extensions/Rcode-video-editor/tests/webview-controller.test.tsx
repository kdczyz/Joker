import type { ExtensionHostClient, HostMessage, JobEvent, JobSnapshot, JsonValue, Locale, Theme } from '@Rcode/extension-api'
...
describe('video editor artifact controller integration', () => {
  it('loads bounded generation state and keeps retry authority free of persisted prompts and handles', async () => {
    ...
    await act(async () => {
      emitMessage({
        channel: 'Rcode-video-editor.generation-progress',
  ...
  it('opens a derived waveform with an opaque Host lease and reuses the unexpired lease', async () => {
    ...
    const openViewResource = vi.fn(async () => ({
      ...
      handleId,
      url: 'Rcode-media://lease/waveform-ready',
      mimeType: 'image/png',
    ...
    expect(firstUrl).toBe('Rcode-media://lease/waveform-ready')
    expect(secondUrl).toBe('Rcode-media://lease/waveform-ready')
  ...
  it('keeps player media on leases and routes subtitle open/reveal through the trusted Host action', async () => {
    const openViewResource = vi.fn(async ({ handleId }: { handleId: string }) => ({
      ...
      handleId,
      url: `Rcode-media://lease/${handleId}`,
      mimeType: 'image/png',
      expiresAt: '2099-01-01T00:00:00.000Z'
    }))
  ...
  it('keeps Rcode theme and locale when initialization fails and retries the full controller bootstrap', async () => {
  ...
  it('applies live Rcode theme and language changes', async () => {
  ...
  it('does not let delayed initial values overwrite newer Rcode events', async () => {
  ...
  it('loads revision-fenced audio capabilities and folds bounded Host progress into the mounted sidebar state', async () => {
      ...
      if (action === 'analysis.capabilities') return { content: {
        ...
        capabilities: {
          ...
          analyses: [
            ...
            { analysis: 'sync-features', available: true, algorithm: 'Rcode.pcm-energy-envelope', algorithmVersion: '1.0.0', local: true, networkUsed: false }
          ]
        },
        denoiseMetadata: {
          outcome: 'ready', local: true, networkUsed: false,
          descriptor: {
            adapterId: 'Rcode.fixture.denoise', adapterVersion: '1.0.0',
    ...
    await act(async () => {
      emitMessage({
        channel: 'Rcode-video-editor.media-intelligence-progress',
        payload: {
          schemaVersion: 1,
  ...
  it('imports reviewed speaker evidence and keeps preview/apply as separate revision-bound actions', async () => {
    ...
    const executeCommand = vi.fn(async (_id: string, args?: JsonValue) => {
      ...
      if (action === 'analysis.capabilities') return { content: {
        ...
        speakerAdapters: [{
          descriptor: { id: 'Rcode.imported-speaker-labels', version: '1.0.0', execution: 'import', format: 'Rcode-speaker-json-v1' },
          outcome: 'ready', local: true, networkUsed: false
        }, {
          descriptor: { id: 'Rcode.host.local-speaker', version: '1.0.0', execution: 'local-model', modelId: 'speaker-diarization', modelVersion: 'unavailable' },
    ...
    const reviewedDocument = JSON.stringify({
      schemaVersion: 1, adapterId: 'Rcode.imported-speaker-labels', identities: [identity],
    ...
    expect(executeCommand).toHaveBeenCalledWith('editor-request', {
      ...
      payload: expect.objectContaining({
        ...
        document: expect.objectContaining({ adapterId: 'Rcode.imported-speaker-labels' })
      })
    })
  ...
  it('tracks durable OTIO export and enforces picker-preview-confirm import without persisting grants', async () => {
    ...
    const lossManifest = {
      adapterId: 'Rcode.otio-json', adapterVersion: '1.0.0',
      portableLossless: false, RcodeRoundTripLossless: true,
      entries: [{
        code: 'effects-custom-metadata', severity: 'warning', feature: 'effects',
        nodeId: 'item-interview', preservation: 'Rcode-metadata',
        message: 'Effect parameters use Rcode metadata.'
      }],
      truncated: 0
    }
    const projection = {
    ...
    const executeCommand = vi.fn(async (_id: string, args?: JsonValue) => {
      ...
      if (action === 'interchange.import-preview') return { content: {
        ...
        lossManifest: { ...lossManifest, RcodeRoundTripLossless: false },
    ...
    expect(controller?.state.otioImportPreview).toMatchObject({
      ...
      suggestedProjectId: importedProject.id,
      fidelity: 'portable-otio',
      lossManifest: { RcodeRoundTripLossless: false }
    })
  ...
  it('loads a revision-bound Host media page beyond asset 100 and opens its opaque resource', async () => {
    ...
    const openViewResource = vi.fn(async ({ handleId }: { handleId: string }) => ({
      ...
      url: 'Rcode-media://lease/media-page-0100', mimeType: 'video/mp4',
      expiresAt: '2099-01-01T00:00:00.000Z'
    }))
    const { client } = fakeClient({ executeCommand, openViewResource })
    let controller: EditorController | undefined
  ...
  it('opens result-preview media from the Host message without loading the full project editor', async () => {
    ...
    expect(controller?.state.activeMediaUrl).toContain('Rcode-media://lease/')
  })
  ...
  it('routes sequence, media-folder, preview, and Agent-context commands through bounded revision-safe payloads', async () => {
    ...
    const attachComposerContext = vi.fn(async (request) => ({
      ...
      provenance: {
        ...
        viewContributionId: 'extension:Rcode-examples.Rcode-video-editor/video-editor',
    ...
    expect(previewResource).toMatchObject({
      ...
      title: 'Source B',
      mediaKind: 'video',
      url: `Rcode-media://lease/${project.assets[0]!.mediaHandleId}`
    })
...
function makeArchiveJob(id: string, state: JobSnapshot['state']): JobSnapshot {
  return {
    ...
    kind: 'media.archive',
    kindSchemaVersion: 1,
    ownerExtensionId: 'Rcode-examples.Rcode-video-editor',
...
function projectChangedMessage(projectId: string): HostMessage {
  return {
    channel: 'Rcode-video-editor.active-project-changed',
    payload: {
      schemaVersion: 1,
      projectId,
      revision: 0,
