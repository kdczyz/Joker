...
async function activatedHarness(): Promise<ExtensionTestHarness> {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-video-editing-host-'))
  roots.push(root)
...
function projectPath(harness: ExtensionTestHarness, projectId: string): string {
  return join(harness.context.workspaceContext!.root, '.Rcode-video', 'projects', projectId, 'project.json')
}
