...
describe('OTIO JSON professional interchange', () => {
  it('exports stable IDs, exact frame/timecode metadata, and an explicit bounded loss manifest', () => {
    ...
    expect(collectTargetUrls(exported.document)).toEqual(['Rcode-media://asset-1', 'Rcode-media://asset-1', 'Rcode-media://asset-1'])
    ...
    expect(exported.lossManifest).toMatchObject({ portableLossless: false, RcodeRoundTripLossless: true, truncated: 0 })
  ...
  it('serializes deterministically and round-trips every sequence plus nested/text/keyframe/effect metadata', () => {
    ...
    expect(imported.fidelity).toBe('Rcode-metadata')
  ...
  it('bounds the loss report and counts omitted entries instead of silently dropping them', () => {
    ...
    expect(manifest.RcodeRoundTripLossless).toBe(true)
  })
  ...
  it('rejects tampered round-trip metadata and path-bearing external references', () => {
    ...
    const metadata = (tampered.metadata as Record<string, unknown>).Rcode as Record<string, unknown>
    ...
    expect(() => importProjectFromOtio(pathBearing)).toThrowError(/Rcode-media/u)
  })

  it('imports a bounded portable OTIO timeline without requiring Rcode snapshot metadata', () => {
            ...
            children: [
              ...
              },
              {
                ...
                media_reference: {
                  ...
                  name: 'External interview',
                  target_url: 'Rcode-media://external-asset',
    ...
    expect(imported).toMatchObject({
      ...
      lossManifest: { portableLossless: false, RcodeRoundTripLossless: false }
    })
