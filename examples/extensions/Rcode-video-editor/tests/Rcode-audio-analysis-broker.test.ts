...
import {
  RcodeLocalAudioAnalysisBroker
} from '../src/host/Rcode-audio-analysis-broker.js'
...
describe('Rcode local audio-analysis Extension API adapter', () => {
  it('maps the public verified visual broker to bounded real-evidence adapter calls', async () => {
    ...
    harness.media.setVisualModelStatus({
      ...
      descriptor,
      receipt: {
        broker: 'Rcode-model-broker', packageSource: 'bundled',
    ...
    const broker = new RcodeLocalAudioAnalysisBroker(harness.context)
    ...
    await expect(broker.embedVisualQuery!({
      ...
      name: 'RcodeVisualAnalysisUnavailableError',
  ...
  it('surfaces beat/downbeat unavailability without producing marker evidence', async () => {
    ...
    await expect(broker.analyzeBeats!({
      ...
      name: 'RcodeAudioAnalysisUnavailableError',
