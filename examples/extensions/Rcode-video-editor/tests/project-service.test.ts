...
async function workspace(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-video-project-'))
  roots.push(root)
  return root
}
...
describe('ProjectService', () => {
  it('creates, lists, atomically saves, and loads immutable revisions', async () => {
    ...
    const timeline = await readFile(join(root, '.Rcode-video/projects/demo-project/timeline.md'), 'utf8')
  ...
  it('isolates a damaged project while listing the remaining valid projects', async () => {
    ...
    await mkdir(join(root, '.Rcode-video/projects/damaged'), { recursive: true })
    await writeFile(join(root, '.Rcode-video/projects/damaged/project.json'), '{broken json', 'utf8')
  ...
  it('recovers timeline and snapshot when failure follows the project commit point', async () => {
    ...
    const timeline = await readFile(
      join(root, '.Rcode-video/projects/recoverable/timeline.md'),
      'utf8'
    )
    expect(timeline).toContain('Revision: `1`')
    await expect(readFile(
      join(root, '.Rcode-video/projects/recoverable/.pending-commit.json'),
      'utf8'
  ...
  it('rejects traversal and symbolic-link project roots', async () => {
    ...
    await symlink(outside, join(linkedRoot, '.Rcode-video'))
  ...
  it('refuses a project state file replaced by a symbolic link', async () => {
    ...
    const projectPath = join(root, '.Rcode-video/projects/linked-state/project.json')
