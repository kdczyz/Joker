import type { AgentRunEvent, JobEvent, MediaResourceLease } from '@Rcode/extension-api'
...
describe('video editor bounded View state', () => {
    ...
    const ticket: OtioExportTicket = {
      ...
      lossManifest: {
        ...
        entries: [{
          ...
          nodeId: 'item-1', preservation: 'Rcode-metadata', message: 'Parameters use Rcode metadata.'
        }],
        truncated: 0
      },
      createdAt: '2026-07-14T00:00:00.000Z'
  ...
  it('revokes stale media leases without retaining reusable URLs', () => {
    const lease: MediaResourceLease = {
      ...
      url: 'Rcode-media://session/token1234567890',
      mimeType: 'video/mp4',
  ...
  it('clears media, jobs, selections, script and Agent state when switching projects', () => {
    ...
    state = {
      ...
      activeMediaUrl: 'Rcode-media://lease/first',
      script: { revision: 0, digest: 'digest', markdown: '# first', dirty: false },
      jobs: [makeJob('running')],
      agentRun: {
        id: 'run-1', threadId: 'thread-1', ownerExtensionId: 'Rcode-examples.Rcode-video-editor',
