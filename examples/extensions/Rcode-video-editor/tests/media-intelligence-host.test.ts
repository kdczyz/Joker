import { createExtensionTestHarness, type ExtensionTestHarness } from '@Rcode/extension-test'
...
const descriptor: VisualModelDescriptor = {
  adapterId: 'Rcode.local.visual-fixture',
  adapterVersion: '1.0.0',
  modelId: 'visual-fixture',
  modelVersion: '1.0.0',
  packageId: 'Rcode-model.visual-fixture',
...
describe('Host media-intelligence orchestration', () => {
  ...
  it('requires workspace opt-in before an approved Broker can install and attest a local visual model', async () => {
    ...
    const broker: LocalMediaIntelligenceBroker = {
      id: 'Rcode.fixture.visual-installer',
      version: '1.0.0',
  ...
  it('verifies, indexes, reports monotonic progress, deduplicates, searches moments, and caches VAD', async () => {
    let visualRuns = 0
    const broker: LocalMediaIntelligenceBroker = {
      id: 'Rcode.fixture.media-intelligence',
      version: '1.0.0',
      visualModelStatus: async () => ({
        schemaVersion: 1,
        state: 'installed',
    ...
    const progress = harness.webview.messages.flatMap((message) => {
      if (!isRecord(message) || message.channel !== 'Rcode-video-editor.media-intelligence-progress') return []
  ...
  it('fences externally cancelled visual indexing even when a Broker returns after cancellation', async () => {
    ...
    let brokerWaiting = false
    const broker: LocalMediaIntelligenceBroker = {
      id: 'Rcode.fixture.visual-cancellable',
      version: '1.0.0',
      visualModelStatus: async () => ({
        schemaVersion: 1,
        state: 'installed',
  ...
  it('rejects visual search after a source grant changes and after the verified model identity changes', async () => {
    ...
    let visualRuns = 0
    const broker: LocalMediaIntelligenceBroker = {
      id: 'Rcode.fixture.visual-binding',
      version: '1.0.0',
      visualModelStatus: async () => ({
        schemaVersion: 1,
        state: 'installed',
    ...
    modelDescriptor = {
      ...descriptor,
      modelVersion: '2.0.0',
      packageId: 'Rcode-model.visual-fixture-v2',
