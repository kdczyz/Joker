...
export const OTIO_ADAPTER_ID = 'Rcode.otio-json' as const
...
export type InterchangeLossEntry = {
  ...
  feature: string
  nodeId: string
  preservation: 'otio-standard' | 'Rcode-metadata'
  message: string
}
...
export type InterchangeLossManifest = {
  ...
  RcodeRoundTripLossless: boolean
...
export type OtioInterchangeImport = {
  ...
  fidelity: 'Rcode-metadata' | 'portable-otio'
...
export function exportProjectToOtio(project: VideoProject): OtioInterchangeExport {
  ...
  if (validated.linkGroups.length > 0) {
    addLoss(loss, metadataLoss(
      ...
      'A/V and sync link groups are preserved in Rcode metadata because OTIO has no portable link-group contract.'
    ))
  }
  if (validated.transcripts.length > 0) {
    addLoss(loss, metadataLoss(
      'transcripts-custom-metadata', 'transcripts', validated.id,
      'Timed transcripts are preserved in Rcode metadata and are not portable OTIO timeline objects.'
    ))
  }
  if (validated.derivedReferences.length > 0) {
    addLoss(loss, metadataLoss(
      'derived-media-custom-metadata', 'derived-media', validated.id,
      'Derived-media provenance is preserved in Rcode metadata; cache payloads are not exported through OTIO.'
    ))
  }
...
export function importProjectFromOtio(value: unknown): OtioInterchangeImport {
  ...
  assertDocumentBounds(document)
  const documentDigest = canonicalDigest(document)
  validateRcodeMediaReferences(document)
  const metadata = optionalRecord(document.metadata)
  const Rcode = optionalRecord(metadata?.Rcode)
  if (!Rcode || Rcode.adapterId !== OTIO_ADAPTER_ID || Rcode.adapterVersion !== OTIO_ADAPTER_VERSION) {
    return importPortableOtio(document, documentDigest)
  }
  const projectDigest = stringValue(Rcode.projectDigest, 'metadata.Rcode.projectDigest', 64)
  const rawProject = structuredClone(record(Rcode.project, 'metadata.Rcode.project'))
  ...
  if (project.id !== Rcode.projectId || project.currentRevision !== Rcode.projectRevision) {
    invalid('OTIO project identity or revision metadata is inconsistent')
  }
  const loss = parseLossManifest(Rcode.lossManifest)
...
function importPortableOtio(
  ...
  addLoss(loss, metadataLoss(
    'portable-import-no-Rcode-snapshot', 'project-metadata', 'project',
    'This OTIO document has no Rcode round-trip snapshot; only the bounded portable timeline subset is imported.'
  ))
...
function portableSequence(
  ...
  const timelineMetadata = optionalRecord(optionalRecord(timeline.metadata)?.Rcode)
  ...
  for (const [trackIndex, rawTrack] of arrayValue(stack.children, 'OTIO stack children').entries()) {
    ...
    const metadata = optionalRecord(optionalRecord(trackNode.metadata)?.Rcode)
    ...
    for (const [childIndex, rawChild] of arrayValue(trackNode.children, `OTIO track ${trackIndex} children`).entries()) {
      ...
      const childMetadata = optionalRecord(optionalRecord(child.metadata)?.Rcode)
...
function portableClip(
  ...
  const assetId = decodeURIComponent(target.slice('Rcode-media://'.length))
...
function portableEffects(value: unknown, itemId: string, loss: LossCollector): EffectInstance[] {
  ...
  for (const [index, raw] of arrayValue(value, `OTIO clip ${itemId} effects`).entries()) {
    ...
    const Rcode = optionalRecord(optionalRecord(effect.metadata)?.Rcode)
    const full = Rcode && typeof Rcode.type === 'string' && optionalRecord(Rcode.parameters)
      ? Rcode as unknown as EffectInstance
      : undefined
    effects.push(full ? structuredClone(full) : {
      id: optionalOtioId(Rcode?.id) ?? `${itemId}.effect-${index + 1}`,
...
function otioTimeline(
  ...
  return {
    ...
    tracks: {
      ...
      metadata: { Rcode: { id: `${sequence.id}.tracks` } },
      children: tracks,
      effects: [],
      markers: []
    }
  ...
  for (const entry of timelineEntries) {
    ...
    if (entry.startFrame < cursor) {
      addLoss(loss, metadataLoss(
        ...
        `Overlapping timing on track ${track.id} is exact in Rcode metadata but sequential OTIO Track semantics may flatten it.`
      ))
    }
...
function otioClip(
  ...
  if (item.nestedSequenceId) {
    addLoss(loss, metadataLoss(
      ...
      `Nested sequence ${item.nestedSequenceId} is preserved by stable ID in Rcode metadata; OTIO consumers may flatten it.`
    ))
  }
  if (!identityVisual(item)) {
    addLoss(loss, metadataLoss(
      'visual-transform-custom-metadata', 'visual-transform', item.id,
      'Transform, crop, opacity, and fades are preserved in Rcode metadata rather than portable OTIO fields.'
    ))
  }
  if (item.keyframes?.length) {
    addLoss(loss, metadataLoss(
      'keyframes-custom-metadata', 'keyframes', item.id,
      'Keyframe interpolation and property paths are preserved in Rcode metadata.'
    ))
  }
  if (item.effects?.length) {
    addLoss(loss, metadataLoss(
      'effect-parameters-custom-metadata', 'effects', item.id,
      'Effect identities are emitted as OTIO Effect objects; parameters remain Rcode metadata.'
    ))
  }
  ...
  return {
    ...
    media_reference: item.nestedSequenceId
      ? {
          ...
          metadata: { Rcode: { nestedSequenceId: item.nestedSequenceId } },
          available_range: null
        }
      : {
          OTIO_SCHEMA: 'ExternalReference.1',
          name: asset!.name,
          target_url: `Rcode-media://${encodeURIComponent(asset!.id)}`,
          available_range: timeRange(
            0,
            Math.max(1, microsecondsToFrames(asset!.durationUs, project.fps, 'nearest')),
            project.fps
          ),
          metadata: { Rcode: { assetId: asset!.id } }
        },
    effects,
    markers: [],
    metadata: {
...
function otioCaption(
  ...
  addLoss(loss, metadataLoss(
    ...
    'Editable caption text, word timing, style, and animation are preserved in Rcode metadata.'
  ))
...
function otioEffects(
  ...
  const result: Record<string, unknown>[] = effects.map((effect) => ({
    ...
    metadata: { Rcode: structuredClone(effect) }
  }))
  if (item.speed.numerator !== item.speed.denominator) {
    result.unshift({
      ...
      metadata: { Rcode: { speed: structuredClone(item.speed) } }
    })
  }
  if (keyframes.length > 0 && effects.length === 0) {
    addLoss(loss, metadataLoss(
      'keyframes-without-effect-custom-metadata', 'keyframes', item.id,
      'Property keyframes without an OTIO Effect are retained only in clip-level Rcode metadata.'
    ))
  }
  return result
}
...
function otioGap(durationFrames: number, fps: Rational, trackId: string, startFrame: number): Record<string, unknown> {
  return {
    ...
    effects: [],
    markers: [],
    metadata: { Rcode: { id: `${trackId}.gap.${startFrame}`, startFrame, durationFrames } }
  }
}
...
function rationalTime(frame: number, fps: Rational): Record<string, unknown> {
  return {
    ...
    metadata: { Rcode: { frame, frameRate: structuredClone(fps) } }
  }
}
...
function collectTimecodeMappings(document: Record<string, unknown>, fps: Rational): OtioTimecodeMapping[] {
  ...
  visit(document, (node) => {
    ...
    const Rcode = optionalRecord(metadata?.Rcode)
    if (
      typeof Rcode?.id !== 'string' ||
      typeof Rcode.sequenceId !== 'string' ||
      !Number.isSafeInteger(Rcode.timelineStartFrame) ||
      !Number.isSafeInteger(Rcode.durationFrames)
    ) return
    addMapping(
      mappings,
      fps,
      Rcode.sequenceId,
      Rcode.id,
      Number(Rcode.timelineStartFrame),
      Number(Rcode.timelineStartFrame) + Number(Rcode.durationFrames)
    )
  })
...
function validateRcodeMediaReferences(document: Record<string, unknown>): void {
  visit(document, (node) => {
    if (node.OTIO_SCHEMA !== 'ExternalReference.1') return
    if (typeof node.target_url !== 'string' || !/^Rcode-media:\/\/[A-Za-z0-9._~%-]+$/u.test(node.target_url)) {
      invalid('OTIO external references must use bounded Rcode-media URLs')
    }
  })
}
...
function parseLossManifest(value: unknown): InterchangeLossManifest {
  ...
  if (typeof manifest.portableLossless !== 'boolean' || typeof manifest.RcodeRoundTripLossless !== 'boolean') {
...
function metadataLoss(
  ...
  return { code, severity: 'warning', feature, nodeId, preservation: 'Rcode-metadata', message }
}

function lossManifest(
  collector: LossCollector,
  RcodeRoundTripLossless = true
