...
import { MAX_MEDIA_OTIO_TEXT_BYTES } from '@Rcode/extension-api'
import type {
  ExtensionContext,
  ExtensionErrorData,
  GeneratedArtifact,
...
import { RcodeLocalAudioAnalysisBroker } from './Rcode-audio-analysis-broker.js'
import {
...
export class VideoEditorTools {
    ...
    if (action === 'speaker') {
      ...
      if (!local || local.outcome !== 'ready') {
        return result({
          ...
          remediation: local?.remediation ?? 'This Rcode build has no approved local speaker inference broker.',
    ...
    if (action === 'export-capabilities') {
      try {
        ...
        return result({
          ...
          retryable: true,
          message: 'Rcode could not inspect the local FFmpeg capability inventory; no codec or GPU capability was assumed.'
    ...
    if (action === 'otio-export-preview') {
      ...
      return result({
        ...
        message: documentInline
          ? 'This bounded OTIO JSON is an inline preview. Rcode has not written a durable export artifact.'
          : 'The OTIO document exceeds the bounded inline preview limit. Rcode needs an atomic JSON output broker before it can write a durable artifact.'
  ...
  private async videoProjectPackage(
    ...
    if (input.outputHandleId === undefined) {
      const selected = await this.context.media.pickSaveTarget({
        suggestedName: `${safeProjectPackageName(project.name)}.Rcode-video.zip`,
        filters: [{
          name: 'Rcode Video Project Package',
          extensions: ['zip'],
  ...
  private async videoSelectionAttachment(input: ToolInput): Promise<ToolResult> {
    ...
    await this.context.ui.postMessage({
      channel: 'Rcode-video-editor.selection-attached',
  ...
  private async publishPreviewHistory(history: PreviewHistory): Promise<void> {
    await this.context.ui.postMessage({
      channel: 'Rcode-video-editor.preview-history-changed',
      payload: {
        schemaVersion: 1,
  ...
  private async selectActiveProject(
    ...
    await this.context.ui.postMessage({
      channel: 'Rcode-video-editor.project-changed',
      payload: {
        schemaVersion: 1,
        projectId: project.id,
  ...
  private async videoProbeBatch(
    ...
    try {
      ...
      return result({
        ...
        changedIds: [],
        retryable: true,
        message: 'Rcode could not inspect local ffprobe availability. No selected media was bound to the project.'
    ...
    if (!capabilities.ffprobe.available) {
      return result({
        ...
        changedIds: [],
        retryable: true,
        message: 'Rcode cannot import the selected media because ffprobe is unavailable. No selected media was bound to the project.'
  ...
  private async videoProbe(
    ...
    try {
      ...
      return result({
        ...
        changedIds: [],
        retryable: true,
        message: 'Rcode could not inspect local ffprobe availability. Install or configure the local media tools and retry; no project data was changed.'
    ...
    if (!capabilities.ffprobe.available) {
      return result({
        ...
        changedIds: [],
        retryable: true,
        message: 'Rcode cannot import this media because ffprobe is unavailable. Install or configure ffprobe and retry; no project data was changed.'
    ...
    if (
      ...
      return result({
        ...
        changedIds: [],
        retryable: true,
        message: 'Rcode cannot generate the requested thumbnail or waveform because FFmpeg is unavailable. Retry without derived outputs, or install or configure FFmpeg; no project data was changed.'
    ...
    if (input.mediaHandleId === undefined) {
      ...
      try {
        ...
        const interaction = interactionRequired(error, 'Select media in the Rcode desktop editor, then retry with the granted mediaHandleId.')
  ...
  private async videoRender(input: ToolInput, invocation: ToolInvocationContext): Promise<ToolResult> {
    ...
    if (suppliedOutputHandleId === undefined) {
      ...
      try {
        ...
        const interaction = interactionRequired(error, 'Choose an export target in the Rcode desktop editor, then retry with its outputHandleId.')
  ...
  private async renderCapabilityAssessment(
    ...
    try {
      ...
      return { failure: result({
        ...
        message: `Rcode could not inspect local FFmpeg and ffprobe capabilities for the ${kind} render. ` +
    ...
    if (!capabilities.ffprobe.available) {
      missing.push({
        ...
        guidance: 'Install or configure ffprobe so Rcode can validate generated media.'
      })
    }
  ...
  private intelligenceService(): MediaIntelligenceService {
    this.mediaIntelligenceService ??= new MediaIntelligenceService(
      this.context,
      new RcodeLocalAudioAnalysisBroker(this.context)
    )
  ...
  private commandInvocation(action: string): ToolInvocationContext {
    ...
    return {
      ...
      reportProgress: async (progress) => {
        await this.context.ui.postMessage({
          channel: 'Rcode-video-editor.command-progress',
          payload: {
            schemaVersion: 1,
            action,
            invocationId,
  ...
  private async publishSelectionChange(updated: {
    ...
    await this.context.ui.postMessage({
      channel: 'Rcode-video-editor.selection-changed',
      payload: {
        schemaVersion: 1,
...
function safeProjectPackageName(value: string): string {
  const leaf = value.split(/[\\/]/u).filter(Boolean).at(-1) ?? 'Rcode-video-project'
  const safe = leaf
    .replace(/[^A-Za-z0-9._~-]+/gu, '-')
    .replace(/^-+|-+$/gu, '')
    .slice(0, 120)
  return safe || 'Rcode-video-project'
}
...
function storedOtioExportRecord(
  ...
  try {
    ...
    if (record.schemaVersion !== 1 || record.adapterId !== 'Rcode.otio-json' ||
...
function storedInterchangeLossManifest(value: unknown): InterchangeLossManifest | undefined {
  ...
  if (
    manifest.adapterId !== 'Rcode.otio-json' || manifest.adapterVersion !== '1.0.0' ||
    typeof manifest.portableLossless !== 'boolean' ||
    typeof manifest.RcodeRoundTripLossless !== 'boolean' ||
  ...
  const entries = manifest.entries.flatMap((entry) => {
    ...
    if (
      ...
      (candidate.preservation !== 'otio-standard' && candidate.preservation !== 'Rcode-metadata') ||
    ...
    return [{
      ...
      preservation: candidate.preservation as 'otio-standard' | 'Rcode-metadata',
  ...
  return {
    ...
    RcodeRoundTripLossless: manifest.RcodeRoundTripLossless,
    entries,
...
function validOtioArtifacts(
  ...
  if (
    ...
    metadata?.RcodeRoundTripLossless !== record.lossManifest.RcodeRoundTripLossless
  ) return []
  return [artifact]
}
...
function interactionRequired(error: unknown, continuation: string): JsonObject | undefined {
  ...
  return {
    ...
    message: 'This operation requires a protected Rcode desktop picker.',
    continuation
  }
}
...
function agentActorId(invocation?: ToolInvocationContext): string {
  return invocation?.invocation.runId ?? invocation?.invocation.threadId ?? 'Rcode-agent'
}
...
function strictSpeakerImportDocument(value: unknown): {
  ...
  if (document.adapterId !== 'Rcode.imported-speaker-labels') {
    throw new ToolInputError('Speaker import document must use the registered Rcode.imported-speaker-labels adapter.')
  }
