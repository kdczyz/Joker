...
export class MediaIntelligenceService {
  ...
  async audioCapabilities(): Promise<MediaAudioAnalysisCapabilities> {
    ...
    const remediation = 'Install or enable Rcode\'s verified local audio-analysis runtime; no media was uploaded and no evidence was fabricated.'
    return {
      schemaVersion: 1,
  ...
  async denoiseMetadataCapability(): Promise<DenoiseMetadataCapability> {
    const fallback = (): Extract<DenoiseMetadataCapability, { outcome: 'unavailable' }> => ({
      ...
      remediation: 'This Rcode Host does not expose verified local noise-profile analysis. No media was uploaded or modified, and no denoise values were fabricated.',
      retryable: false,
      local: true,
      networkUsed: false
    })
  ...
  speakerAdapters(): SpeakerDiarizationAdapterStatus[] {
    ...
    return defaultSpeakerDiarizationAdapterRegistry({
      localDescriptor: {
        adapterId: 'Rcode.host.local-speaker',
  ...
  async importSpeakerEvidence(input: {
    ...
    const adapter = defaultSpeakerDiarizationAdapterRegistry()
      .requireReady('Rcode.imported-speaker-labels')
    ...
    if (!this.broker?.visualModelStatus) {
      return {
        projection: visualProvisioningProjection({
          ...
          verification: emptyVerification,
          remediation: 'This Rcode build has no approved model download/install Broker. Update Rcode or install a Host build that exposes verified local-model provisioning; filename and transcript search remain available.',
          checkedAt
        })
      }
    }
...
function verifyVisualReceiptProjection(
  ...
  return {
    brokerAttested: receipt.broker === 'Rcode-model-broker',
