...
export class RcodeAudioAnalysisUnavailableError extends Error {
  ...
  constructor(
    ...
    super(remediation)
    this.name = 'RcodeAudioAnalysisUnavailableError'
  }
}

export class RcodeAudioAnalysisJobError extends Error {
  constructor(
    readonly code: string,
    message: string,
    readonly retryable: boolean
    ...
    super(message)
    this.name = 'RcodeAudioAnalysisJobError'
  }
}

export class RcodeVisualAnalysisUnavailableError extends Error {
  readonly networkUsed = false
  ...
  constructor(
    ...
    super(remediation)
    this.name = 'RcodeVisualAnalysisUnavailableError'
  }
}
 ...
 * Rcode owns native process execution, durable progress, cancellation, and
 * source-identity calculation.
 */
export class RcodeLocalAudioAnalysisBroker implements LocalMediaIntelligenceBroker {
  readonly id = 'Rcode.host.audio-analysis'
  ...
  denoiseMetadataCapability: NonNullable<LocalMediaIntelligenceBroker['denoiseMetadataCapability']> = async () => ({
    ...
    remediation: 'This Rcode Extension API exposes no verified local noise-profile analyzer yet. No media was uploaded or modified, and no denoise levels were fabricated.',
    retryable: false,
    local: true,
    networkUsed: false
  })
  ...
  visualModelStatus: NonNullable<LocalMediaIntelligenceBroker['visualModelStatus']> = async () => {
    ...
    if (!media.ffmpeg.available || !media.ffprobe.available) {
      return {
        ...
        state: 'failed',
        receipt: undefined,
        remediation: 'The signed local visual package is present, but verified frame decoding requires FFmpeg and FFprobe in a reviewed Rcode Host location.'
      }
    }
    return projected
  }
  ...
  indexVisual: NonNullable<LocalMediaIntelligenceBroker['indexVisual']> = async ({
    ...
    for (let offset = 0; offset < samples.length; offset += 16) {
      ...
      if (result.outcome === 'unavailable') {
        throw new RcodeVisualAnalysisUnavailableError(
          result.code,
          result.remediation,
          result.retryable
        )
  ...
  extractSyncFeatures: NonNullable<LocalMediaIntelligenceBroker['extractSyncFeatures']> = async ({
    ...
    if (
      ...
      throw new RcodeAudioAnalysisJobError(
  ...
  private async startAndWait(
    ...
    if (started.outcome === 'unavailable') {
      throw new RcodeAudioAnalysisUnavailableError(
        started.code,
        started.remediation,
        started.retryable
      )
...
function mismatchedResult(expected: string): RcodeAudioAnalysisJobError {
  return new RcodeAudioAnalysisJobError(
    'AUDIO_ANALYSIS_RESULT_MISMATCH',
