...
- `echo-api-key` uses a Joker-managed API-key account;
- `echo-oauth` uses a Joker-managed OAuth 2.0 PKCE account.
...
`authentication.authenticatedFetch` so Joker injects credentials without revealing
