...
- `echo-api-key` uses a Rcode-managed API-key account;
- `echo-oauth` uses a Rcode-managed OAuth 2.0 PKCE account.
...
`authentication.authenticatedFetch` so Rcode injects credentials without revealing
