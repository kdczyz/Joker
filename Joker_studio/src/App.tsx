...
const PREVIEW_USER: User = { id: "preview", email: "preview@joker.local", username: "preview", displayName: "林秋" };
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: (session: AuthResult) => void }) {
  ...
  return <main className="authScreen">
    <section className="authIntro"><div className="authWordmark"><span>RC</span><strong>Joker</strong></div><p className="eyebrow">你的电脑，就在手边</p><h1>项目、会话、<br />工作都在继续。</h1><p className="authLead">像聊天一样管理电脑上的 Agent 工作，随时切换项目与模型。</p><div className="trustLine"><ShieldCheck size={17} /><span>账号隔离 · 敏感操作仍需审批</span></div></section>
    <form className="authForm" onSubmit={submit}>
      <div className="formHeading"><div><h2>{mode === "login" ? "登录 Joker" : "创建账号"}</h2><p>{mode === "login" ? "连接你的电脑工作区" : "在手机与电脑之间建立统一身份"}</p></div><LockKeyhole size={21} /></div>
      {mode === "login" ? <label><span>邮箱或用户名</span><input name="identifier" autoCapitalize="none" autoComplete="username" required placeholder="name@example.com" /></label> : <div className="registerFields"><label><span>显示名称</span><input name="displayName" required placeholder="你的名字" /></label><label><span>用户名</span><input name="username" autoCapitalize="none" required placeholder="joker_user" /></label><label><span>邮箱</span><input name="email" type="email" autoCapitalize="none" required placeholder="name@example.com" /></label></div>}
...
function SettingsScreen({ user, apiBase, devices, preferences, workConfig, onPreferencesChange, onWorkConfigChange, onLogout }: { user: User; apiBase: string; devices: RemoteDevice[]; preferences: MobilePreferences; workConfig: WorkAiConfig; onPreferencesChange: (preferences: MobilePreferences) => void; onWorkConfigChange: (config: WorkAiConfig) => void; onLogout: () => void }) {
  ...
  return <section className="rootPage settingsPage">
    <div className="profileHeader"><div className="profileAvatar">{user.displayName.slice(0, 1).toUpperCase()}</div><div><p className="overline">JOKER ACCOUNT</p><h1>{user.displayName}</h1><span>@{user.username}</span></div></div>
    ...
    <section className="settingsSection"><div className="settingsHeading"><span>连接与账号</span><small>{online}/{devices.length} 台电脑在线</small></div><div className="settingsList"><div><CircleUserRound size={19} /><span><strong>{user.email}</strong><small>账号与会话身份</small></span></div><div><Smartphone size={19} /><span><strong>Android 客户端</strong><small>Joker Mobile 0.9.0</small></span></div><div><LockKeyhole size={19} /><span><strong>{serviceHost(apiBase)}</strong><small>端到端远程服务</small></span></div></div></section>
