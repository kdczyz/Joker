...
function imageGenConfig(overrides: Record<string, unknown> = {}) {
  return JokerCapabilitiesConfig.parse({
    imageGen: {
      enabled: true,
...
function attachmentStore(rootDir: string, overrides: Record<string, unknown> = {}) {
  return new FileAttachmentStore({
    rootDir,
    config: JokerCapabilitiesConfig.parse({ attachments: { enabled: true, ...overrides } }).attachments,
...
describe('Image gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Joker-imagegen-'))
  })
  ...
  it('rejects generated-image writes through an escaping workspace symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'Joker-imagegen-outside-'))
    try {
  ...
  it('does not read a reference image through an escaping workspace symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'Joker-imagegen-reference-outside-'))
    try {
