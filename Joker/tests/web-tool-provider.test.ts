  ...
  return new DeterministicWebProvider({
    ...
    searchResults: {
      'Joker web': [
        {
          url: 'https://docs.example.test/page',
          title: 'Joker Web Docs',
          snippet: 'How Joker web access works.'
        }
      ]
    }
  })
...
describe('Web tool provider', () => {
  ...
  it('runs keyless search through the built-in engine chain over the shared transport', async () => {
    ...
    const result = await host.execute({
      callId: 'call_keyless',
      toolName: 'web_search',
      arguments: { query: 'Joker web' }
    }, buildContext())

    expect(requestedUrls).toEqual([BING_ENGINE.requestUrl('Joker web')])
    expect(result.item).toMatchObject({ kind: 'tool_result', isError: false })
    if (result.item.kind === 'tool_result') {
      expect(result.item.output).toMatchObject({
        provider: 'keyless',
  ...
  it('searches through a configured provider with citations and telemetry', async () => {
    ...
    const result = await host.execute({
      callId: 'call_1',
      toolName: 'web_search',
      arguments: { query: 'Joker web', limit: 3 }
    }, buildContext())
