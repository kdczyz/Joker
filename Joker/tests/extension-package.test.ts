...
import {
  ...
  ExtensionStateStore,
  extractJokerxArchive,
  inspectJokerxArchive,
  packJokerx,
...
describe('extension package management', () => {
  it('packs deterministically, installs immutable versions, persists enablement, and rolls back', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-package-'))
    try {
      const source = join(root, 'source')
      const v1Archive = join(root, 'acme.demo-1.0.0.Jokerx')
      const v1ArchiveAgain = join(root, 'acme.demo-1.0.0-again.Jokerx')
      await writeExtensionSource(source, '1.0.0')
      const first = await packJokerx(source, v1Archive, { compatibility })
      const second = await packJokerx(source, v1ArchiveAgain, { compatibility })
      ...
      const v2Archive = join(root, 'acme.demo-2.0.0.Jokerx')
      await packJokerx(source, v2Archive, { compatibility })
  ...
  it('serializes activation admission behind an in-flight disable transaction', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-disable-admission-'))
    try {
      const source = join(root, 'source')
      const archive = join(root, 'acme.demo-1.0.0.Jokerx')
      await writeExtensionSource(source, '1.0.0')
      await packJokerx(source, archive, { compatibility })
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'extension-data')
      })
  ...
  it('serializes activation admission behind workspace permission revocation', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-permission-admission-'))
    try {
      const source = join(root, 'source')
      const archive = join(root, 'acme.demo-1.0.0.Jokerx')
  ...
  it('atomically rejects a stale selected version before changing workspace permissions', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-permission-version-'))
    try {
      const source = join(root, 'source')
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
      ...
      await writeExtensionSource(source, '1.0.0', 0, ['commands.register'])
      const v1Archive = join(root, 'acme.demo-1.0.0.Jokerx')
      await packJokerx(source, v1Archive, { compatibility })
  ...
  it('requires a fresh workspace review when a selected version adds permission authority', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-permission-update-'))
    try {
      const source = join(root, 'source')
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
  ...
  it('leaves the prior selected version usable when version admission fails', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-install-rollback-'))
    try {
      const source = join(root, 'source')
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
      ...
      await writeExtensionSource(source, '1.0.0')
      const v1 = join(root, 'v1.Jokerx')
      await packJokerx(source, v1, { compatibility })
      await initialManager.installArchive(v1, { grantedPermissions: [] })

      await writeExtensionSource(source, '2.0.0')
      const v2 = join(root, 'v2.Jokerx')
      await packJokerx(source, v2, { compatibility })
  ...
  it('cleans interrupted staging and unregistered canonical versions during recovery', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-install-recovery-'))
    try {
      const source = join(root, 'source')
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
      ...
      const manager = new ExtensionPackageManager(paths, registry, { compatibility })
      await writeExtensionSource(source, '1.0.0')
      const archive = join(root, 'v1.Jokerx')
      await packJokerx(source, archive, { compatibility })
  ...
  it('commits an installed update and migrated state through one version-switch transaction', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-transactional-update-'))
    try {
      const source = join(root, 'source')
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
  ...
  it('packs a manifest allowlist and applies explicit safe include and ignore paths', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-pack-selection-'))
    try {
      const source = join(root, 'source')
      await writeExtensionSource(source, '1.0.0')
      ...
      const defaultPack = await packJokerx(source, join(root, 'default.Jokerx'), { compatibility })
      expect(Object.keys(defaultPack.integrity.files).sort()).toEqual([
        'LICENSE',
        'README.md',
        'dist/main.mjs',
        'Joker-extension.json'
      ])

      const selectedPack = await packJokerx(source, join(root, 'selected.Jokerx'), {
        compatibility,
        include: ['dist/chunks'],
      ...
      await expect(
        packJokerx(source, join(root, 'escape.Jokerx'), { compatibility, include: ['../outside'] })
      ).rejects.toMatchObject({ code: 'EXTENSION_PACKAGE_RULE_INVALID' })
      await expect(
        packJokerx(source, join(root, 'secret.Jokerx'), { compatibility, include: ['.env'] })
      ).rejects.toMatchObject({ code: 'EXTENSION_PACKAGE_FORBIDDEN_PATH' })

      if (process.platform !== 'win32') {
        await symlink(root, join(source, 'dist/chunks/escape'))
        await expect(
          packJokerx(source, join(root, 'link.Jokerx'), {
            compatibility,
            include: ['dist/chunks'],
            ignore: ['dist/chunks/debug.map']
          })
        ...
        await symlink(join(source, 'Joker-extension.json'), join(linkedSource, 'Joker-extension.json'))
        await expect(
          packJokerx(linkedSource, join(root, 'manifest-link.Jokerx'), { compatibility })
        ).rejects.toMatchObject({ code: 'EXTENSION_PACKAGE_LINK_FORBIDDEN' })

        const linkedRoot = join(root, 'linked-root')
        await symlink(source, linkedRoot)
        await expect(
          packJokerx(linkedRoot, join(root, 'source-link.Jokerx'), { compatibility })
  ...
  it('rejects traversal, case collisions, symlinks, integrity mismatches, and package limits', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-attacks-'))
    try {
      const collision = join(root, 'collision.Jokerx')
      await writeZip(collision, [
        ['A.txt', Buffer.from('a'), 0o100644],
        ['a.txt', Buffer.from('b'), 0o100644]
      ])
      await expect(
        extractJokerxArchive(collision, join(root, 'collision-out'))
      ).rejects.toMatchObject({ code: 'EXTENSION_ARCHIVE_PATH_COLLISION' })

      const symlink = join(root, 'symlink.Jokerx')
      await writeZip(symlink, [['link', Buffer.from('../outside'), 0o120777]])
      await expect(
        extractJokerxArchive(symlink, join(root, 'symlink-out'))
      ).rejects.toMatchObject({ code: 'EXTENSION_ARCHIVE_LINK_FORBIDDEN' })

      const traversalBase = join(root, 'traversal-base.Jokerx')
      ...
      await expect(
        inspectJokerxArchive(invalidIntegrity, { compatibility })
      ...
      await expect(
        packJokerx(source, join(root, 'large.Jokerx'), {
          include: ['large.bin'],
  ...
  it('registers mutable development sources but reloads only explicitly', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-dev-'))
    try {
      ...
      await writeFile(
        join(source, 'Joker-extension.json'),
        `${JSON.stringify({
          ...manifestFor('1.0.0'),
          contributes: {
            'views.rightSidebar': [{
  ...
  it('loads an HTTPS Index only on explicit request and installs the exact digest', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-index-'))
    try {
      const source = join(root, 'source')
      const archive = join(root, 'package.Jokerx')
      await writeExtensionSource(source, '1.2.3')
      const packed = await packJokerx(source, archive, { compatibility })
      ...
      const index = {
        ...
        extensions: [{
          ...
          versions: [{
            version: '1.2.3',
            url: 'https://plugins.example/acme.demo-1.2.3.Jokerx',
      ...
      expect(requests).toEqual([
        'https://plugins.example/index.json',
        'https://plugins.example/acme.demo-1.2.3.Jokerx'
      ])
  ...
  it('rejects every HTTPS redirect hop that downgrades an index or package request', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-index-redirect-'))
    try {
      const index = {
        schemaVersion: 1,
        extensions: [{
      ...
      const packageClient = new ExtensionIndexClient({
        fetch: (async (input: string | URL | Request, init?: RequestInit) => {
          ...
          return String(input).endsWith('index.json')
            ...
            : new Response(null, {
                status: 307,
                headers: { location: 'http://attacker.example/package.Jokerx' }
              })
        }) as typeof fetch
      })
      const paths = new ExtensionPaths({
...
async function writeExtensionSource(
  ...
  await writeFile(
    join(root, 'Joker-extension.json'),
...
function requiredFiles(manifest: ReturnType<typeof manifestFor>): Record<string, Buffer> {
  return {
    'Joker-extension.json': Buffer.from(JSON.stringify(manifest)),
