...
import { DEFAULT_JOKER_MODEL } from '../src/config/Joker-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Joker-todos-'))
    try {
      const relativePath = '.Jokersdd/plan/demo.md'
  ...
  it('refuses to patch a plan path that escapes through a symlink', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Joker-todos-workspace-'))
    const outside = await mkdtemp(join(tmpdir(), 'Joker-todos-outside-'))
    try {
      const relativePath = '.Jokersdd/plan/demo.md'
      const planDir = join(workspace, '.Jokersdd', 'plan')
...
describe('ThreadService.resumeSession', () => {
  it('uses the Joker default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_JOKER_MODEL)
  })
})
