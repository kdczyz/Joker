    ...
    const todos = ThreadTodoListSchema.parse({
      ...
      items: [{
        ...
        source: {
          kind: 'plan',
          planId: 'plan_1',
          relativePath: '.Jokersdd/plan/plan.md',
          ordinal: 0,
          contentHash: 'abc'
        },
        createdAt: '2026-06-03T00:00:00.000Z',
...
describe('cli', () => {
  ...
  it('accepts the product-owned bundled extension directory from CLI or environment', () => {
    expect(parseServeOptions([
      '--data-dir=/srv/ca',
      '--bundled-extensions-dir=/opt/Joker/bundled-extensions'
    ]).bundledExtensionsDir).toBe('/opt/Joker/bundled-extensions')
    expect(parseServeOptions(['--data-dir=/srv/ca'], {
      JOKER_BUNDLED_EXTENSIONS_DIR: '/Applications/Joker/resources/bundled-extensions'
    }).bundledExtensionsDir).toBe('/Applications/Joker/resources/bundled-extensions')
  })
  ...
  it('applies CLI and standard OTLP environment precedence over config', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'Joker-observability-config-'))
    try {
      const configPath = join(dir, 'Joker.config.json')
  ...
  it('rejects a defaultProfile that is not defined in profiles', () => {
    expect(() => JokerCapabilitiesConfig.parse({
  ...
  it('surfaces subagent profiles and policy in the runtime capability manifest', () => {
    const manifest = buildRuntimeCapabilityManifest({
      ...
      config: JokerCapabilitiesConfig.parse({
        subagents: {
          enabled: true,
          maxParallel: 2,
          maxChildRuns: 6,
  ...
  it('loads config.json from the data dir when present', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-data-'))
    try {
  ...
  it('exposes a usage string', () => {
    expect(SERVE_USAGE).toContain('Joker serve')
  })
