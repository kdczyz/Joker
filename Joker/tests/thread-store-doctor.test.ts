...
describe('scanThreadStore', () => {
    ...
    try {
      ...
      db.transaction(() => {
        for (let index = 0; index < 10_000; index += 1) {
          const id = `thr_Joker_doctor_probe_${index}`
  ...
  it.each([
    ...
    },
    {
      ...
      title: 'Joker doctor schema probe'
    }
...
async function makeRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'Joker-thread-store-doctor-'))
  roots.push(root)
  return root
}
...
function createCanonicalSqliteSchema(
  ...
  const titleCheck = options.titleCheck
    ? "TEXT NOT NULL CHECK (title <> 'Joker doctor schema probe')"
    : 'TEXT NOT NULL'
