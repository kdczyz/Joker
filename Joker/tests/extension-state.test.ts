...
describe('extension state store', () => {
  it('isolates global/workspace state and migrates every namespace transactionally', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-'))
    try {
  ...
  it('retains prior committed state when migration throws or times out', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-failure-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
  ...
  it('refuses inferred downgrades and reports unavailable versions', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-diagnostic-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
  ...
  it('coordinates host migrations before selection and restores state if selection fails', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-coordinator-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
  ...
  it('keeps host-owned state outside extension migration code and preserves it across upgrades', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-host-owned-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
      ...
      await store.setGlobal('acme.demo', 'value', 'old')
      await store.setGlobal('acme.demo', '__Joker_configuration_document_v1', configuration)
      await store.setWorkspace('acme.demo', workspaceKey, 'visible', true)
      await store.setWorkspace('acme.demo', workspaceKey, '__Joker_view_state_v1', { selected: 'result' })
      const migrateState = vi.fn(async (
        ...
        expect(state).not.toHaveProperty('__Joker_configuration_document_v1')
        expect(state).not.toHaveProperty('__Joker_view_state_v1')
      ...
      expect(await store.read('acme.demo')).toMatchObject({
        ...
        global: {
          ...
          __Joker_configuration_document_v1: configuration
        },
        workspaces: {
          [workspaceKey]: {
            visible: true,
            migratedScope: 'workspace',
            __Joker_view_state_v1: { selected: 'result' }
          }
        }
      })
    } finally {
  ...
  it('rejects attempts by extension migration code to create host-owned state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-host-owned-injection-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
      ...
      const manager = {
        ...
        migrateState: vi.fn(async () => ({
          value: 'new',
          __Joker_configuration_document_v1: { revision: 999 }
        }))
      } as unknown as ExtensionManager
      const coordinator = new ExtensionStateMigrationCoordinator(
        store,
  ...
  it('does not commit package selection when a coordinated migration times out', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-timeout-switch-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
  ...
  it('starts a fresh install at its declared schema and removes provisional state on commit failure', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-fresh-install-'))
    try {
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
        dataRoot: join(root, 'data')
  ...
  it.each([
    ...
    async ({ registryCommitted, selectedVersion, schemaVersion, value, packageRetained }) => {
      const root = await mkdtemp(join(tmpdir(), 'Joker-extension-state-crash-recovery-'))
      try {
        const paths = new ExtensionPaths({
          packageRoot: join(root, 'extensions'),
          dataRoot: join(root, 'data')
...
function installedRecord(
  ...
  return {
    ...
    source: { type: 'local', locator: join(paths.packageRoot, `${version}.Jokerx`) },
