...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'Joker-skills-'))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'Joker-skill-real-'))
    try {
  ...
  it('rejects a skill entry symlinked outside its package', async (ctx) => {
    const outside = await mkdtemp(join(tmpdir(), 'Joker-skill-secret-'))
    try {
  ...
  it('rejects workspace-discovered skill packages symlinked outside the workspace', async (ctx) => {
    const workspace = await mkdtemp(join(tmpdir(), 'Joker-skill-workspace-boundary-'))
    const external = await mkdtemp(join(tmpdir(), 'Joker-skill-external-'))
    try {
      await writeSkillAt(external, 'outside', {
        id: 'outside', name: 'Outside', triggers: { commands: ['/outside'] }
      }, 'PRIVATE KEY MATERIAL')
      await mkdir(join(workspace, '.Joker', 'skills'), { recursive: true })
      try {
        await symlink(join(external, 'outside'), join(workspace, '.Joker', 'skills', 'outside'), 'dir')
      } catch {
        ctx.skip()
        return
      }
      const config = JokerCapabilitiesConfig.parse({
  ...
  it('scopes the dynamic skill catalog to the current workspace', async () => {
    const workspaceA = await mkdtemp(join(tmpdir(), 'Joker-skill-workspace-a-'))
    const workspaceB = await mkdtemp(join(tmpdir(), 'Joker-skill-workspace-b-'))
    try {
      ...
      const disabledConfig = JokerCapabilitiesConfig.parse({
        skills: {
          enabled: true,
          roots: [rootA],
          workspaceRoots: [workspaceA, workspaceB],
  ...
  it('discovers workspace-local Joker skills from .Joker/skills', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Joker-skill-workspace-Joker-'))
    try {
      const skillRoot = join(workspace, '.Joker', 'skills')
