...
describe('extension host processes', () => {
  ...
  it('rejects mismatched API or RPC handshakes before requesting extension entrypoint load', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-host-handshake-'))
    try {
  ...
  it('runs the built-in runner with the canonical SDK context and command/notification round trips', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-real-runner-'))
...
`)
      ...
      await expect(
        host.invoke('commands.invoke:command-1', { name: 'Joker' })
      ).resolves.toEqual({
        greeting: 'hello Joker',
  ...
  it('preserves public API failures through a real Node Host broker round trip', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-api-error-'))
    try {
      const packagePath = join(root, 'extension')
      await mkdir(packagePath, { recursive: true })
      await writeFile(join(packagePath, 'main.mjs'), `
...
`)
  ...
  it('isolates processes, binds identity, minimizes environment, cancels calls, and shuts down', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-host-'))
    const previousSecret = process.env.JOKER_EXTENSION_TEST_SECRET
    process.env.JOKER_EXTENSION_TEST_SECRET = 'must-not-leak'
    try {
      ...
      expect(identity.envKeys).not.toContain('JOKER_EXTENSION_TEST_SECRET')
      ...
      if (previousSecret === undefined) delete process.env.JOKER_EXTENSION_TEST_SECRET
      else process.env.JOKER_EXTENSION_TEST_SECRET = previousSecret
      await rm(root, { recursive: true, force: true })
    }
  })

  it('bounds activation and memory independently of the Joker process', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-host-limits-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
      const paths = new ExtensionPaths({
        packageRoot: join(root, 'extensions'),
  ...
  it('keeps headless hosts isolated across a crash, restarts only the failed host, and shuts down active calls', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-isolation-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
  ...
  it('does not spawn a Node host for a browser-only extension', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-browser-only-'))
    try {
  ...
  it('admits every workspaceRoots entry instead of trusting only workspaceRoot', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-workspace-trust-'))
    try {
  ...
  it('does not share a pending activation promise across workspace trust scopes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-concurrent-workspace-trust-'))
    try {
  ...
  it('re-admits a trusted scope after a concurrent scope activation fails', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-concurrent-scope-retry-'))
    try {
  ...
  it('invalidates a pending activation when workspace permission lifecycle changes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-pending-invalidation-'))
    try {
  ...
  it('deactivates only the Host admitted for the revoked workspace', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-workspace-stop-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
  ...
  it('invalidates only the pending activation for the revoked workspace', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-workspace-pending-stop-'))
    try {
  ...
  it('activates lazily once, applies restart backoff, and opens a per-extension crash circuit', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
  ...
  it('deactivates a pure View Host only after all concurrent View references close', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-view-idle-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
  ...
  it('cancels idle timers on reopen and waits for old Host cleanup before reactivation', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-view-reopen-'))
  ...
  it('keeps tool, Provider, and startup/background Hosts alive after their last View closes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-manager-background-view-'))
    try {
      const runnerPath = await writeFixtureRunner(root)
  ...
  it('rotates extension-scoped logs within the configured retention bound', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-extension-logs-'))
    try {
