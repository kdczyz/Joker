...
describe('extension runtime compatibility admission', () => {
  it('negotiates current and earlier minors and rejects future minor, future major, and engine mismatches', () => {
    ...
    expect(() => assertManifestCompatible(
      manifestFor({ apiVersion: '1.2.0', enginesJoker: '>=0.2.0' }),
      minorPolicy
  ...
  it('re-admits installed versions before activation, selection, and rollback', async () => {
    const root = await temporaryRoot('Joker-extension-compat-installed-')
    ...
    const v1Archive = join(root, 'v1.Jokerx')
    await packJokerx(source, v1Archive, { compatibility: installPolicy })
    ...
    const v2Archive = join(root, 'v2.Jokerx')
    await packJokerx(source, v2Archive, { compatibility: installPolicy })
  ...
  it('re-admits mutable development manifests against current API and engine policy', async () => {
    const root = await temporaryRoot('Joker-extension-compat-development-')
    const paths = new ExtensionPaths({
      packageRoot: join(root, 'extensions'),
      dataRoot: join(root, 'data')
    })
    ...
    const newerJoker = new ExtensionPackageManager(paths, registry, {
      compatibility: policy({ apiVersion: '1.0.0', JokerVersion: '0.2.0' })
    })
    await newerJoker.registerDevelopment(engineSource, { grantedPermissions: [] })
...
function policy(options: { apiVersion: string; JokerVersion?: string }): ExtensionCompatibility {
  return {
    JokerVersion: options.JokerVersion ?? '0.1.0',
    supportedManifestVersions: [1],
...
function manifestFor(options: {
  ...
  return parseExtensionManifest({
    ...
    engines: { Joker: options.enginesJoker ?? '*' },
    main: 'dist/main.mjs',
