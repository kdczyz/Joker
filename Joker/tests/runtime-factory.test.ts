...
import { createJokerServeRuntime, seedUsageCarryover } from '../src/server/runtime-factory.js'
import type { UsageSnapshot } from '../src/contracts/usage.js'
import type { SessionStore } from '../src/ports/session-store.js'
import { JokerCapabilitiesConfig } from '../src/contracts/capabilities.js'
...
describe('runtime factory usage carryover', () => {
  ...
  it('hot-applies model and tool capabilities into runtime info and diagnostics', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-apply-'))
    tempDirs.push(dataDir)
    const runtime = await createJokerServeRuntime({
      host: '127.0.0.1',
      port: 0,
      dataDir,
      runtimeToken: 'tok',
      ...
      capabilities: JokerCapabilitiesConfig.parse({})
    })
  ...
  it('requires restart instead of acknowledging an unapplied observability change', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-observability-apply-'))
    tempDirs.push(dataDir)
    const runtime = await createJokerServeRuntime({
      host: '127.0.0.1',
      port: 0,
  ...
  it('clears per-thread runtime memory when a thread is deleted', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-delete-'))
    tempDirs.push(dataDir)
    const runtime = await createJokerServeRuntime({
      host: '127.0.0.1',
      port: 0,
  ...
  it('invalidates lazy extension preparation after install, reload, and host crash', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-preparation-'))
    const sourceDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-source-'))
    ...
    try {
      ...
      if (!toolHost) {
        throw new Error('Expected the Joker runtime tool host to be available')
      }
  ...
  it('passes an explicitly trusted workspace context to headless extension tools', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-workspace-'))
    const sourceDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-workspace-source-'))
  ...
  it('routes workspace configuration changes to only the owning Host and View while global changes fan out', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-config-scope-'))
    const sourceDir = await mkdtemp(join(tmpdir(), 'Joker-runtime-extension-config-source-'))
