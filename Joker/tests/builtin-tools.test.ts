...
describe('Joker built-in tools', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Joker-tools-'))
    backgroundShellDataDir = await mkdtemp(join(tmpdir(), 'Joker-bg-shell-data-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello Joker' },
    ...
    expect(editedDisk).toContain('hello Joker')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello Joker')
