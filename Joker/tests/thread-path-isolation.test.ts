...
describe('thread path isolation', () => {
  ...
  it('accepts Windows descendants without accepting sibling or drive escapes', () => {
    const root = 'C:\\Users\\runner\\Joker\\threads'

    expect(isPathBelowDirectory(root, `${root}\\thr_valid`, win32)).toBe(true)
    expect(isPathBelowDirectory(root, 'c:\\users\\RUNNER\\Joker\\threads\\thr_case', win32))
      .toBe(true)
    expect(isPathBelowDirectory(root, root, win32)).toBe(false)
    expect(isPathBelowDirectory(root, 'C:\\Users\\runner\\Joker\\threads-old\\thr_sibling', win32))
      .toBe(false)
    expect(isPathBelowDirectory(root, 'C:\\Users\\runner\\Joker\\outside', win32)).toBe(false)
    expect(isPathBelowDirectory(root, 'D:\\Joker\\threads\\thr_other_drive', win32)).toBe(false)
  })

  it('does not let the file store delete outside its threads directory', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-file-thread-store-'))
    cleanup.push(dataDir)
    const victim = join(dirname(dataDir), `Joker-victim-${Date.now()}`)
    cleanup.push(victim)
    await mkdir(victim)
  ...
  it('does not let the hybrid store delete outside its threads directory', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-hybrid-thread-store-'))
    cleanup.push(dataDir)
    const victim = join(dirname(dataDir), `Joker-victim-${Date.now()}`)
    cleanup.push(victim)
    await mkdir(victim)
