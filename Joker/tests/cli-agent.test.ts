...
describe('Joker agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'Joker-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitJokerCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitJokerCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitJokerCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from Joker exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through Joker exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from Joker exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'Joker.config.json')
