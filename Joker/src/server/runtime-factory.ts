...
import { JOKER_SYSTEM_PROMPT } from '../prompt/Joker-system-prompt.js'
...
export type JokerServeRuntimeOptions = {
  host: string
  port: number
  configPath?: string
  dataDir: string
  /** Product-owned catalog of default local .Jokerx packages. */
  ...
  capabilities?: JokerCapabilitiesConfig
...
export type JokerServeHandle = NodeHttpServerHandle & {
 ...
 */
export async function createJokerServeRuntime(
  options: JokerServeRuntimeOptions
): Promise<ServerRuntime> {
  await mkdir(options.dataDir, { recursive: true, mode: 0o700 })
  let activeOptions: JokerServeRuntimeOptions = { ...options }
  ...
  let prefix = createImmutablePrefix({
    systemPrompt: JOKER_SYSTEM_PROMPT,
    pinnedConstraints: [
      'system: preserve user intent across compaction',
      'system: keep the HTTP/SSE contract stable for the GUI',
      'system: keep the stable Joker prefix byte-stable for prompt-cache reuse',
  ...
  const migrateLegacyProviderCredentials = async (): Promise<void> => {
    const sources = [
      ...(activeOptions.apiKey.trim() && !activeOptions.credentialSourceId ? [{
        ...
        providerId: 'default',
        providerName: 'Joker default provider',
  ...
  const withBackgroundShellTools = (
    tools: LocalTool[],
    optionsForTools: JokerServeRuntimeOptions = activeOptions
  ): LocalTool[] => {
  ...
  // subscription set as the main model). Joker-process signals that via env so we
  // route default-provider turns to the SDK too, not just per-provider ones.
	  const defaultIsAgentSdk = process.env.JOKER_RUNTIME_PROVIDER_KIND === 'agent-sdk'
	  ...
	  if (agentSdkProviderIds.size > 0 || defaultIsAgentSdk) {
	    sdkRuntimeDeps = {
	      ...
	      ...(attachmentStore ? { attachmentStore } : {}),
	      ...(memoryStore ? { memoryStore } : {}),
	      ...(process.env.JOKER_CLAUDE_BINARY
	        ? { pathToClaudeCodeExecutable: process.env.JOKER_CLAUDE_BINARY }
	        : {})
	    }
	  }
  ...
  stopThreadAuxiliaryWork = async (threadId) => {
    ...
    runtimeDataDir: activeOptions.dataDir,
	  ...
	  const extensionMediaProcesses = new ExtensionMediaProcessService({
	    ...
	    ...(process.env.JOKER_FFPROBE_PATH ? { ffprobePath: process.env.JOKER_FFPROBE_PATH } : {}),
	    ...(process.env.JOKER_FFMPEG_PATH ? { ffmpegPath: process.env.JOKER_FFMPEG_PATH } : {})
	  })
...
): Promise<JokerServeRuntimeOptions> {
...
function agentSdkProviderIdsForOptions(options: JokerServeRuntimeOptions): Set<string> {
...
function agentSdkProviderSignature(options: JokerServeRuntimeOptions): string {
  return [...agentSdkProviderIdsForOptions(options)].sort().join('\n')
}

function mergeRuntimeConfigApplyOptions(
  current: JokerServeRuntimeOptions,
  request: RuntimeConfigApplyRequest
): JokerServeRuntimeOptions {
...
function tokenEconomyConfigForOptions(
  options: Pick<JokerServeRuntimeOptions, 'tokenEconomyMode' | 'tokenEconomy'>
): TokenEconomyConfig {
  return {
...
function toolOutputLimitsForOptions(
  options: Pick<JokerServeRuntimeOptions, 'toolOutputLimits'>
...
function builtinToolOptionsForOptions(options: JokerServeRuntimeOptions) {
...
export async function startJokerServe(
  options: JokerServeRuntimeOptions
): Promise<JokerServeHandle> {
  ...
  // progress" forever with nothing running (kdczyz/Joker#370).
  void runtime.turnService
    .reconcileOrphanedTurns()
    .then(async (threadIds) => {
      if (threadIds.length > 0) {
        console.warn(`[Joker] marked orphaned turn(s) on ${threadIds.length} thread(s) as failed after restart`)
      }
      if (threadIds.length > 0 && runtime.resumeInterruptedGoals) {
        const resumed = await runtime.resumeInterruptedGoals(threadIds)
        if (resumed > 0) {
          console.warn(`[Joker] auto-resumed ${resumed} interrupted goal(s) after restart`)
        }
      }
    })
    .catch((error) => {
      console.warn('[Joker] orphaned turn reconciliation failed:', error)
    })
  ...
  void runtime.delegationRuntime
    ...
    .then((count) => {
      if (count > 0) {
        console.warn(`[Joker] marked ${count} orphaned subagent run(s) as failed after restart`)
      }
    })
    .catch((error) => {
      console.warn('[Joker] orphaned child-run reconciliation failed:', error)
    })
  return {
    ...server,
    runtime,
