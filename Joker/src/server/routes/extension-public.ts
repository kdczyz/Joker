...
export const EXTENSION_SESSION_ID_HEADER = 'x-Joker-extension-session-id'
export const EXTENSION_SESSION_NONCE_HEADER = 'x-Joker-extension-session-nonce'
  ...
  const extensions = Object.values(registry.extensions)
    ...
    .flatMap((entry) => {
      ...
      const compatible = compatibility.api.compatible &&
        compatibility.JokerEngine.compatible &&
...
class ProtectedMediaOperationTokenRegistry {
  ...
  consume(token: string): void {
    ...
    if (this.consumed.size >= 65_536) {
      throw new ExtensionBrokerError(
        'conflict',
        'Protected media operation capacity was reached; restart Joker before retrying'
      )
    }
...
async function resolveViewTarget(
  ...
  try {
    ...
    throw new ExtensionViewSessionError('not_found', 'Extension view is incompatible with this Joker version')
  }
