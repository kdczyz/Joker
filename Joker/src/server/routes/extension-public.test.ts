...
import { parseExtensionManifest } from '@joker-code/extension-api'
import {
  ExtensionPaths,
  ExtensionRegistry,
...
describe('extension public routes', () => {
  ...
  it('derives a core-bound view identity and never accepts a forged or runtime guest credential', async () => {
    ...
    expect(created.body.src).toBe('Joker-extension://acme.dashboard/webview/index.html')
...
async function createFixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'Joker-extension-public-routes-'))
