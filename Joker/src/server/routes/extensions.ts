 ...
 *
 * This is deliberately separate from `buildRouter` so the Joker composition root
...
async function inspectExtension(
  ...
  const inspection = await inspectJokerxArchive(body.data.path, runtime.validation)
  return jsonResponse({
    schemaVersion: 1,
