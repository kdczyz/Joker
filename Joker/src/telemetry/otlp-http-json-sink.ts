...
export class OtlpHttpJsonAgentObservabilitySink implements AgentObservabilitySink {
  ...
  emit(span: AgentObservabilitySpan): void {
    if (this.closed) {
      if (!this.warnedAfterShutdown) {
        ...
        console.warn('[Joker] OTLP trace exporter is closed; dropping late spans')
      }
      return
    }
    if (this.queue.length >= this.maxQueueSize) {
      ...
      if (!this.warnedAboutOverflow) {
        ...
        console.warn('[Joker] OTLP trace queue full; dropping oldest spans')
      }
    }
    ...
    this.inFlight = this.exportBatch(batch)
      ...
      .catch((error: unknown) => {
        if (error instanceof PermanentOtlpExportError) {
          this.retryAttempt = 0
          console.warn(`[Joker] OTLP trace export rejected; dropping batch: ${error.message}`)
          return
        }
        if (this.closed) {
          this.requeue(batch)
        ...
        console.warn(`[Joker] OTLP trace export failed; retrying in ${delayMs}ms: ${message}`)
  ...
  private async drainAndClose(): Promise<void> {
    ...
    while (this.queue.length > 0) {
      ...
      if (remainingMs <= 0) {
        console.warn(`[Joker] OTLP trace shutdown timed out; dropping ${this.queue.length} queued span(s)`)
        this.queue = []
        return
      }
      const batch = this.queue.splice(0, this.batchSize)
      try {
        ...
        const message = error instanceof Error ? error.message : String(error)
        console.warn(`[Joker] OTLP trace export failed during shutdown; dropping batch: ${message}`)
      }
    }
  }
...
function toExportTraceServiceRequest(spans: AgentObservabilitySpan[]): Record<string, unknown> {
  return {
    resourceSpans: [{
      resource: {
        attributes: [attribute('service.name', 'Joker-runtime')]
      },
      scopeSpans: [{
        scope: { name: 'Joker.agent-observability' },
