...
import type { ObservabilityConfig } from '../config/Joker-config.js'
...
export class AgentObservabilityRecorder implements RuntimeEventObserver {
  ...
  async record(event: RuntimeEvent): Promise<void> {
    switch (event.kind) {
      ...
      case 'model_request_retry':
        ...
        return
      case 'pipeline_stage':
        this.addTurnEvent(event.threadId, event.turnId, 'Joker.pipeline_stage', {
          'Joker.pipeline.stage': event.stage,
          ...(event.label ? { 'Joker.pipeline.label': event.label } : {})
        }, event.timestamp)
        return
      case 'usage':
      ...
      case 'error':
        this.addTurnEvent(event.threadId, event.turnId, 'exception', {
          ...
          ...(event.severity ? { 'Joker.error.severity': event.severity } : {})
        }, event.timestamp)
        return
      default:
        return
  ...
  private startTurn(event: RuntimeEvent): void {
    ...
    this.turns.set(key, {
      ...
      attributes: {
        'service.name': 'Joker-runtime',
        'gen_ai.operation.name': 'chat',
        'Joker.thread.id': event.threadId,
        'Joker.turn.id': event.turnId
      },
      events: []
    })
  }
  ...
  private recordTtft(event: RuntimeEvent): void {
    ...
    span.attributes['Joker.ttft_ms'] = Math.max(0, timestampMs(event.timestamp) - span.startMs)
  }
  ...
  private async recordItemEvent(event: RuntimeEvent): Promise<void> {
    ...
    if (item.kind === 'tool_call' && event.kind === 'item_created') {
      ...
      this.tools.set(key, {
        ...
        name: 'Joker.tool ' + item.toolName,
        kind: 'client',
        startMs: timestampMs(event.timestamp),
        startTimeUnixNano: unixNano(event.timestamp),
        attributes: {
          'tool.name': item.toolName,
          'Joker.tool.call_id': item.callId,
          'Joker.tool.kind': item.toolKind,
          'Joker.thread.id': item.threadId,
          'Joker.turn.id': item.turnId
        },
        events: []
      })
      return
    ...
    if (item.kind === 'tool_result' && (event.kind === 'item_created' || event.kind === 'item_completed')) {
      ...
      if (!span) return
      span.attributes['Joker.tool.is_error'] = item.isError
...
function usageAttributes(usage: UsageSnapshot, model: string | undefined): Record<string, AgentObservabilityAttributeValue> {
  return {
    ...
    ...(usage.cacheMissTokens !== undefined ? { 'Joker.cache.miss_tokens': usage.cacheMissTokens } : {}),
    ...(usage.cacheHitRate !== null && usage.cacheHitRate !== undefined ? { 'Joker.cache.hit_rate': usage.cacheHitRate } : {}),
    ...(usage.costUsd !== undefined ? { 'Joker.usage.cost_usd': usage.costUsd } : {}),
    ...(usage.costCny !== undefined ? { 'Joker.usage.cost_cny': usage.costCny } : {})
  }
}
