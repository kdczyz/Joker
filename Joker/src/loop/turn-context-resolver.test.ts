...
describe('TurnContextResolver', () => {
  it('snapshots plan, policy, attachments, memory, skills, instructions, and discovered tools', async () => {
    ...
    const planTurn = turn({
      mode: 'plan',
      guiPlan: {
        operation: 'draft', workspaceRoot: '/workspace', relativePath: '.Jokersdd/plan/plan.md', planId: 'plan_1'
      }
    })
  ...
  it('drops stale plan state and forces SVG turns to agent mode', () => {
    const stalePlan = turn({
      mode: 'plan',
      guiPlan: {
        operation: 'draft', workspaceRoot: '/other-workspace', relativePath: '.Jokersdd/plan/plan.md', planId: 'plan_1'
      }
    })
    ...
    const svg = turn({
      mode: 'plan',
      guiDesignArtifact: { kind: 'svg', artifactId: 'artifact_1', relativePath: '.Joker-design/a/v1.svg' }
    })
