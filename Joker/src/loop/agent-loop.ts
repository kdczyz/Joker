...
import type { RolesConfig } from '../config/Joker-config.js'
...
export type AgentLoopOptions = {
  ...
  /** Joker runtime data root for sandbox-safe background shell output reads. */
  ...
  /**
   * Tuning + test seams for goal auto-resume (kdczyz/Joker#370). Defaults
   ...
   * Agent SDK instead of Joker's own model loop, billing the user's Claude
   * subscription. Joker's tools/persona/permissions are injected into the SDK.
   */
...
export class AgentLoop {
  ...
  */
  ...
  private async runTurnOwned(threadId: string, turnId: string): Promise<TurnExecutionStatus> {
    ...
    // the user's subscription; Joker's brain is injected). All other providers
    // fall through to Joker's native loop below.
    ...
    try {
      ...
      if (delegatedSdkRuntime) {
        // The delegated SDK owns its model stream and cannot consume Joker's
      ...
      // instead of the bare "Joker turn failed" string. See issue #26.
  ...
  static defaultPrefix(): ImmutablePrefix {
    return createImmutablePrefix({
      systemPrompt: 'You are Joker, a careful and helpful assistant.',
