...
import type { GeneratedArtifact, MediaProbeResult } from '@joker-code/extension-api'
...
async function fixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'Joker-media-job-'))
  roots.push(root)
  ...
  const principal: ExtensionPrincipal = {
    extensionId: 'Joker.video-editor',
...
describe('ExtensionMediaJobService', () => {
  ...
  it('runs FFmpeg as a durable job, post-probes output, and publishes artifacts', async () => {
    ...
    expect(test.media.probe).toHaveBeenCalledWith(
      expect.objectContaining({ extensionId: 'Joker.video-editor' }),
      'media_123456789012',
  ...
  it('accepts duration-less SRT and atomically publishes safe render provenance', async () => {
    ...
    const reference = await test.adapter.start(test.principal, {
      ...
      metadata: {
        ...
        producerId: 'Joker-video-editor.preview',
  ...
  it('publishes a schema-validated OTIO document without probing it as media', async () => {
    ...
    const content = JSON.stringify({
      ...
      name: 'Revision 8',
      children: [],
      metadata: { Joker: { projectId: 'project-1', projectRevision: 8 } }
    })
    const interchange = {
      ...test.generated,
    ...
    const request = {
      ...
      metadata: {
        ...
        sequenceId: 'sequence-main',
        pinnedRevision: 8,
        interchangeAdapterId: 'Joker.otio-json',
...
async function transactionalFixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'Joker-media-job-transaction-'))
  roots.push(root)
  const workspace = join(root, 'workspace')
