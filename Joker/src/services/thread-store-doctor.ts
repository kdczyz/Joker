...
function probeHybridThreadStoreWrites(db: BetterSqliteDatabase): boolean {
  ...
  try {
    ...
    db.exec('SAVEPOINT Joker_doctor_write_probe')
    savepointStarted = true
    threadUpsert.run(threadWriteProbeRow(firstId, 'Joker doctor schema probe'))
    threadUpsert.run(threadWriteProbeRow(firstId, 'Joker doctor schema probe updated'))
    threadUpsert.run(threadWriteProbeRow(secondId, 'JOKER DOCTOR SCHEMA PROBE'))
    ...
    db.exec('ROLLBACK TO Joker_doctor_write_probe')
    db.exec('RELEASE Joker_doctor_write_probe')
...
function findWriteProbeThreadIds(db: BetterSqliteDatabase): [string, string] {
  const candidates = Array.from({ length: 4 }, () => {
    ...
    const first = `thr_Joker_doctor_probe_${suffix}`
    const second = `thr_Joker_doctor_probe_${suffix.toUpperCase()}`
...
function usageWriteProbeRow(
  ...
  return {
    ...
    seq,
    timestamp,
    turn_id: 'turn_Joker_doctor_probe',
    model: 'deepseek-chat',
