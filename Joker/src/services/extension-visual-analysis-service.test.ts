...
import type { MediaVisualAdapterBinding } from '@joker-code/extension-api'
...
import {
  ...
  JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR,
...
describe('ExtensionVisualAnalysisService', () => {
  ...
  it.skipIf(!systemFfmpeg || !systemFfprobe)(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'Joker-visual-real-'))
      roots.push(root)
...
async function createFixture(scriptBody: string) {
  const root = await mkdtemp(join(tmpdir(), 'Joker-visual-analysis-'))
  roots.push(root)
  const workspace = join(root, 'workspace')
  const dataDir = join(root, 'data')
...
function adapterBinding(): MediaVisualAdapterBinding {
  return {
    id: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.adapterId,
    version: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.adapterVersion,
    ...
    embeddingDimensions: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.embeddingDimensions,
    execution: 'local'
  }
}
