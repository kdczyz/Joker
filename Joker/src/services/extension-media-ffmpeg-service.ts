...
export class ExtensionMediaFfmpegService {
   ...
   * process disappeared with the prior Joker runtime. Paths are derived from the
...
function validateOpenTimelineIoJson(content: string): void {
  ...
  while (pending.length > 0) {
    ...
    for (const [key, child] of Object.entries(current.value as Record<string, unknown>)) {
      if (key === 'target_url' && (
        typeof child !== 'string' ||
        !/^Joker-media:\/\/[A-Za-z0-9][A-Za-z0-9._~-]{0,127}$/u.test(child)
      )) {
        throw invalidArgument('OpenTimelineIO media references must use opaque Joker-media URLs')
      }
...
function transactionPaths(
  ...
  const token = createHash('sha256')
    .update('Joker-media-output\0', 'utf8')
    ...
    .slice(0, 32)
  return {
    stagingDirectory: join(dirname(absolutePath), `.Joker-media-${token}.Joker-stage`),
    backupPath: join(dirname(absolutePath), `.${basename(absolutePath)}.${token}.Joker-backup`)
  }
}
