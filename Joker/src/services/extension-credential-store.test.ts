...
describe('ExtensionCredentialStore', () => {
  it('uses authenticated encrypted fallback without plaintext-at-rest', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-extension-credentials-'))
  ...
  it('prefers an available primary credential backend', async () => {
    ...
    const store = new ExtensionCredentialStore({
      dataDir: await mkdtemp(join(tmpdir(), 'Joker-extension-primary-')),
      profileId: 'profile-a',
      primary
    })
  ...
  it('uses Joker OS-backed key protection and binds ciphertext to profile and reference', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Joker-extension-os-credentials-'))
    const keyProvider = {
    ...
    await expect(store.protection()).resolves.toMatchObject({
      ...
      backend: 'Joker-os-credential-key'
    })
  ...
  it('reports Joker key-file protection as the degraded encrypted fallback', async () => {
    const store = new ExtensionCredentialStore({
      dataDir: await mkdtemp(join(tmpdir(), 'Joker-extension-degraded-')),
      profileId: 'profile-a',
      keyProvider: {
        encryptor: createAesEncryptor(randomBytes(32)),
        osKeychain: false,
    ...
    await expect(store.protection()).resolves.toMatchObject({
      mode: 'encrypted-fallback', degraded: true, available: true,
      backend: 'Joker-encrypted-secret-store'
    })
