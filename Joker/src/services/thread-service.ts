...
import { DEFAULT_JOKER_MODEL } from '../config/Joker-config.js'
...
export class ThreadService {
  ...
  async resumeSession(
    ...
    const record = createThreadRecord({
      ...
      model: options.model ?? sourceThread?.model ?? DEFAULT_JOKER_MODEL,
...
async function resolveWorkspaceRelativePath(workspace: string, relativePath: string): Promise<string> {
  ...
  // `.Jokersdd/plan` cannot redirect a status update outside the workspace.
