...
const runNative = process.env.JOKER_RUN_MEDIA_SMOKE === '1'
...
suite('native FFmpeg media broker smoke', () => {
  it('probes, renders a proof frame, exports H.264, post-probes, and cancels safely', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'Joker-native-media-'))
    roots.push(root)
...
async function executable(name: 'ffmpeg' | 'ffprobe'): Promise<string> {
  const configured = process.env[`JOKER_${name.toUpperCase()}_PATH`]
  ...
  throw new Error(`${name} is required when JOKER_RUN_MEDIA_SMOKE=1`)
}
