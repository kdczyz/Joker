...
const MODEL_PAYLOAD = '{"schemaVersion":1,"packageId":"Joker-bundled.visual-features-v1","modelId":"Joker-visual-features","modelVersion":"1.0.0","embeddingDimensions":24,"algorithm":"Joker.rgb-edge-features","algorithmVersion":"1.0.0","querySemantics":"bounded-visual-features-v1","features":["mean-red","mean-green","mean-blue","brightness","darkness","saturation","low-saturation","contrast","low-contrast","edge-density","smoothness","warmth","coolness","red-dominance","green-dominance","blue-dominance","near-black","dark","midtone","bright","highlight","horizontal-edge","vertical-edge","neutral-bias"]}\n'
...
const MODEL_MANIFEST = '{"schemaVersion":1,"packageId":"Joker-bundled.visual-features-v1","modelId":"Joker-visual-features","modelVersion":"1.0.0","adapterId":"Joker.local.visual-features","adapterVersion":"1.0.0","embeddingDimensions":24,"querySemantics":"bounded-visual-features-v1","files":[{"name":"visual-features-v1.json","sha256":"969024f5de7e8f8013f7ada5e08d53c209d221a0b313bd8b35862ce57387351a","byteSize":582}]}\n'
...
export const JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR: MediaVisualModelDescriptor = {
  adapterId: 'Joker.local.visual-features',
  adapterVersion: '1.0.0',
  modelId: 'Joker-visual-features',
  modelVersion: '1.0.0',
  packageId: 'Joker-bundled.visual-features-v1',
...
/**
 * Generic Joker-owned local visual adapter. The installed package is a small,
...
export class ExtensionVisualAnalysisService {
  ...
  async status(principal: ExtensionPrincipal): Promise<MediaVisualModelStatus> {
    ...
    if (installed.receipt) {
      return MediaVisualModelStatusSchema.parse({
        schemaVersion: 1,
        state: 'installed',
        descriptor: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR,
    ...
    return MediaVisualModelStatusSchema.parse({
      ...
      remediation: installed.invalid
        ? 'The bundled visual feature package failed digest, signature, or receipt verification. Reinstall it through Joker before indexing.'
        : 'Install Joker\'s signed bundled visual feature package. This copies only verified local bytes and does not download or upload media.',
      local: true,
      networkUsedForInference: false,
      rawPathsExposed: false,
      urlsAccepted: false
  ...
  async analyzeFrames(
    ...
    try {
      ...
      return MediaAnalyzeVisualFramesResultSchema.parse({
        ...
        provenance: {
          algorithm: 'Joker.rgb-edge-features',
  ...
  private async installOnce(principal: ExtensionPrincipal): Promise<MediaVisualModelStatus> {
    ...
    const receipt = MediaVisualModelInstallReceiptSchema.parse({
      broker: 'Joker-model-broker',
      packageSource: 'bundled',
      packageId: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.packageId,
      modelId: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.modelId,
      modelVersion: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.modelVersion,
      manifestSha256: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.manifestSha256,
      files: JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR.files,
  ...
  private async readyFor(
    ...
    if (!sameAdapter(adapter, JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR)) {
      return visualUnavailable(
        'VISUAL_MODEL_MISMATCH',
        'The requested adapter identity does not match Joker\'s verified local visual package. Refresh capability state and rebuild the immutable index.',
        false
      )
    }
  ...
  private async verifyDirectory(path: string): Promise<InstalledState> {
    try {
      ...
      if (!receiptMatchesDescriptor(receipt, JOKER_LOCAL_VISUAL_MODEL_DESCRIPTOR)) return { invalid: true }
...
function receiptMatchesDescriptor(
  ...
  return receipt.broker === 'Joker-model-broker' &&
...
function visualProcessUnavailable(error: unknown): MediaAnalyzeVisualFramesResult {
  ...
  return visualUnavailable(
    ...
    error.code === 'executable_unavailable'
      ? 'Install FFmpeg and FFprobe in a reviewed Joker Host location, then retry local visual indexing.'
      : 'Joker could not decode the requested authorized visual frame with its fixed local profile. Reauthorize a supported video/image source or use filename/transcript search.',
    error.retryable
  )
}
