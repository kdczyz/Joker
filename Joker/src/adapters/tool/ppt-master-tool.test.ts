...
async function workspaceWithMarkdown(): Promise<string> {
  const workspace = await mkdtemp(join(tmpdir(), 'Joker-ppt-master-'))
  workspaces.push(workspace)
  await writeFile(join(workspace, 'brief.md'), '# Brief\n\nA source document.\n', 'utf8')
  await mkdir(join(workspace, '.Joker-presentations'), { recursive: true })
  return workspace
}
...
describe('PPT Master local tool', () => {
  it('uses fixed scripts, copies the Markdown source, and confines the output layout', async () => {
    ...
    const projectPath = join(workspace, '.Joker-presentations', 'brief_ppt169_20260710')
    ...
    expect(calls[0]).toEqual([
      ...
      'ppt169',
      '--dir',
      join(workspace, '.Joker-presentations')
    ])

    const imported = await tool.execute({
      action: 'import_markdown',
      project_path: '.Joker-presentations/brief_ppt169_20260710',
  ...
  it('rejects project and output paths outside the managed Write locations', async () => {
    ...
    expect(wrongRoot.output).toMatchObject({ error: 'projects_root must be .Joker-presentations' })
    ...
    expect(wrongProject.output).toMatchObject({ error: 'project_path must be inside .Joker-presentations/' })

    const wrongOutput = await tool.execute({
      action: 'export',
      project_path: '.Joker-presentations/brief',
