...
import { ExtensionApiError, type ExtensionErrorCode } from '@joker-code/extension-api'
 ...
 * LocalTool entries so every invocation keeps Joker's approval, sandbox,
...
export class ExtensionToolRegistry {
  ...
  /** Rebind live extension registrations after Joker rebuilds its base registry. */
...
function validateDeclaration(input: ExtensionToolDeclaration): ExtensionToolDeclaration {
  ...
  if (RESERVED_TOOL_NAMES.has(name) || name.startsWith('Joker.')) throw new Error(`reserved extension tool name: ${name}`)
