...
async function svgFileContext(context: ToolHostContext, write: boolean) {
  ...
  // thread otherwise runs with danger-full-access. A planted .Joker-design
  ...
  if (!resolved.relativePath.startsWith('.Joker-design/') || !/\/v\d+\.svg$/i.test(resolved.relativePath)) {
    throw new Error('SVG artifact path must be a versioned file under .Joker-design')
  }
...
async function atomicWrite(path: string, content: string, signal?: AbortSignal): Promise<void> {
  const temp = `${path}.Joker-${process.pid}-${randomBytes(4).toString('hex')}.tmp`
  try {
