...
export function createDesignCanvasTool(): LocalTool {
  return LocalToolHost.defineTool({
    ...
    description: [
      'Create or update the active GUI canvas: Design mode canvas or Code-mode sidebar whiteboard. Use this only when Joker is in a canvas turn.',
...
export function createDesignExportCanvasTool(): LocalTool {
  return LocalToolHost.defineTool({
    ...
    execute: async (args, context) => {
      ...
      const stem = safeCanvasExportStem(requestedName || 'Joker-whiteboard')
...
function safeCanvasExportStem(value: string): string {
  const normalized = value
    ...
    .replace(/^[._-]+|[._-]+$/g, '')
    .slice(0, 64)
  return normalized || 'Joker-whiteboard'
}
