...
const MANAGED_SKILL_DIR = join(homedir(), '.Joker', 'skills', 'ppt-master')
...
/**
 * Opens Joker's native structured-input card and issues a short-lived token only
  ...
  return LocalToolHost.defineTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        projects_root: {
          type: 'string',
          description: 'For init_project: workspace-relative project parent. Defaults to .Joker-presentations.'
        },
        project_path: {
          type: 'string',
...
async function commandForAction(
  ...
  if (action === 'init_project') {
    ...
    const rootInput = stringArg(args.projects_root) || '.Joker-presentations'
    ...
    if (!isManagedProjectRoot(root.relativePath)) {
      return { error: 'projects_root must be .Joker-presentations' }
    }
  ...
  if (!isManagedProjectPath(project.relativePath)) {
    return { error: 'project_path must be inside .Joker-presentations/' }
  }
...
function isManagedProjectRoot(relativePath: string): boolean {
  return normalizedRelativePath(relativePath) === '.Joker-presentations'
}

function isManagedProjectPath(relativePath: string): boolean {
  return normalizedRelativePath(relativePath).startsWith('.Joker-presentations/')
}
