...
const TOOL_DESCRIPTION = [
  ...
  '.Jokersdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .Jokersdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.Jokersdd/plan/foo.md`)
 ...
 *    `.Jokersdd/plan/<feature>.md` under the active workspace.
...
export async function executeCreatePlanTool(
  ...
  try {
    ...
    // dangling symlink before writing so a planted .Jokersdd link cannot escape
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .Jokersdd/plan' }
  }
 ...
 * allocates a fresh, non-colliding `.Jokersdd/plan/<feature>.md`.
 */
