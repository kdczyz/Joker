...
export function componentPrototypeRelativePath(title: string, artifactId: string): string {
  ...
  return `.Joker-design/component-prototypes/${slug}-${suffix}/prototype.html`
}
...
export function buildComponentDesignerPrompt(input: ComponentDesignArgs & {
  ...
  lines.push(
    ...
    '- Include `<meta name="Joker-component-prototype" content="1">` in `<head>`.',
    '- Put the single visible component demo inside one root with `data-Joker-component-root`.',
...
export function hardenComponentPrototypeHtml(content: string): string {
  ...
  if (!/<meta\b[^>]*name\s*=\s*(["'])Joker-component-prototype\1[^>]*>/i.test(trimmed)) {
    throw new Error('component prototype is missing the Joker-component-prototype marker')
  }
  const roots = trimmed.match(/<[a-z][^>]*\sdata-Joker-component-root(?:\s*=\s*(?:["'][^"']*["']|[^\s>]+))?[^>]*>/gi) ?? []
  if (roots.length !== 1) throw new Error('component prototype must contain exactly one data-Joker-component-root')
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    [data-Joker-component-root] { display: grid; justify-items: center; gap: 14px; text-align: center; }
...
<body>
  <main data-Joker-component-root aria-live="polite">
    <span class="pulse" aria-hidden="true"></span>
    <strong>Designing ${safeTitle}</strong>
    <p>Joker is preparing a live component prototype.</p>
  </main>
</body>
</html>`)
}
