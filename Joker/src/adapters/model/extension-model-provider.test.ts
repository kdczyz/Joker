...
async function harness(
  ...
  const store = new ExtensionProviderAccountStore({
    dataDir: await mkdtemp(join(tmpdir(), 'Joker-extension-model-')),
...
function request(providerId: string, accountId: string, signal = new AbortController().signal): ModelRequest {
  return {
    ...
    providerId,
    accountId,
    systemPrompt: 'Joker stable system prompt',
...
describe('ExtensionModelProviderRegistry', () => {
  it('normalizes full Joker requests and validates ordered provider streams', async () => {
    ...
    expect(captured).toMatchObject({
      ...
      instructions: ['Joker stable system prompt', 'Extension profile overlay'],
      tools: [{ name: 'read' }],
