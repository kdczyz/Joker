...
export class DelegationRuntime {
  ...
  async runChild(input: {
    ...
    // Workspace overlay: `.Joker/agents/*.md` in the call's workspace wins
    ...
    if (input.detach) {
      ...
      const logIgnoredParentAbort = (): void => {
        console.warn(`[Joker] detached subagent ignored parent abort child=${record.id} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
      }
      if (input.signal.aborted) logIgnoredParentAbort()
      else input.signal.addEventListener('abort', logIgnoredParentAbort, { once: true })
      console.warn(`[Joker] detached subagent started with independent abort signal child=${record.id} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
      ...
      void this.executeChild({
        ...
        .finally(() => {
          ...
          console.warn(`[Joker] detached subagent finished background tracking child=${record.id}`)
        })
      return record
    }
  ...
  abortChild(childId: string): boolean {
    ...
    if (!controller) {
      console.warn(`[Joker] detached subagent abort requested but no running child found child=${childId}`)
      return false
    }
    console.warn(`[Joker] detached subagent abort requested child=${childId}`)
    controller.abort()
    console.warn(`[Joker] detached subagent abort signal fired child=${childId}`)
    return true
  }
   ...
   * in-flight indefinitely, and the parent thread stays wedged (kdczyz/Joker#621).
