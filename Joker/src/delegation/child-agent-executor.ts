...
import type { RuntimeTuningConfig } from '../config/Joker-config.js'
...
export function createChildAgentExecutor(options: ChildAgentExecutorOptions): ChildRunExecutor {
  return async (input) => {
    ...
    // A custom system prompt augments the base prefix (Joker tool/safety
    ...
    const abortChild = (): void => {
      console.warn(`[Joker] foreground subagent parent abort received child=${thread.id} turn=${started.turnId} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
    ...
    if (input.signal.aborted) {
      console.warn(`[Joker] foreground subagent started with aborted parent signal child=${thread.id} turn=${started.turnId}`)
      abortChild()
    } else {
      console.warn(`[Joker] foreground subagent abort bridge armed child=${thread.id} turn=${started.turnId} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
    ...
    console.warn(`[Joker] foreground subagent turn settled child=${thread.id} turn=${started.turnId} status=${status}`)
