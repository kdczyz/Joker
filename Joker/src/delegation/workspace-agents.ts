 ...
 *
 * Loads `<workspace>/.Joker/agents/*.md` and produces a profile map that
 ...
 *     color: "#3b82f6"
 *     ---
 *     Body becomes the systemPrompt verbatim (Joker's base prompt is
...
export async function loadWorkspaceAgentProfiles(workspace: string): Promise<WorkspaceAgentProfile[]> {
  ...
  const dir = join(workspaceRoot, '.Joker', 'agents')
