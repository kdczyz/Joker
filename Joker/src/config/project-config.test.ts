...
import {
  JOKER_PROJECT_CONFIG_RELATIVE_PATH,
  JokerProjectConfigSchema,
  MAX_JOKER_PROJECT_CONFIG_BYTES,
  MAX_JOKER_PROJECT_MCP_SERVERS,
  MAX_JOKER_PROJECT_SKILL_ROOTS,
  loadJokerProjectConfig,
  validateJokerProjectConfigText,
  writeJokerProjectConfig
} from './project-config.js'

describe('Joker project config', () => {
  let workspace = ''

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Joker-project-config-'))
  })
  ...
  it('reports missing without inheriting a parent project file', async () => {
    ...
    await mkdir(join(workspace, '.Joker'), { recursive: true })
    await mkdir(child)
    await writeFile(join(workspace, JOKER_PROJECT_CONFIG_RELATIVE_PATH), JSON.stringify({ version: 1 }))
    const realChild = await realpath(child)

    await expect(loadJokerProjectConfig(child)).resolves.toMatchObject({
      status: 'missing',
  ...
  it('normalizes valid config and gives formatting-equivalent JSON the same digest', async () => {
    ...
    const first = await validateJokerProjectConfigText(workspace, compact)
    const second = await validateJokerProjectConfigText(workspace, reordered)
  ...
  it.each([
    ...
    await expect(validateJokerProjectConfigText(workspace, content)).rejects.toThrow(expected as RegExp)
  })

  it('rejects oversized content', async () => {
    await expect(validateJokerProjectConfigText(
      workspace,
      `${JSON.stringify({ version: 1, $schema: '' }).slice(0, -2)}${'x'.repeat(MAX_JOKER_PROJECT_CONFIG_BYTES)}}`
    )).rejects.toThrow(/exceeds/)
  })

  it('rejects oversized MCP and Skill collections', async () => {
    const servers = Object.fromEntries(Array.from(
      { length: MAX_JOKER_PROJECT_MCP_SERVERS + 1 },
      (_, index) => [`server-${index}`, { transport: 'stdio', command: 'node' }]
    ))
    await expect(validateJokerProjectConfigText(workspace, JSON.stringify({
      version: 1,
      mcp: { servers }
    }))).rejects.toThrow(/at most/)

    const roots = Array.from(
      { length: MAX_JOKER_PROJECT_SKILL_ROOTS + 1 },
  ...
  it('rejects a project path symlinked outside the workspace', async (ctx) => {
    const outside = await mkdtemp(join(tmpdir(), 'Joker-project-outside-'))
    try {
      try {
  ...
  it('writes atomically without following a project.json symlink', async (ctx) => {
    ...
    await mkdir(join(workspace, '.Joker'))
    await writeFile(outside, 'outside')
    try {
      await symlink(outside, join(workspace, JOKER_PROJECT_CONFIG_RELATIVE_PATH))
    } catch {
      ctx.skip()
      return
    }

    await expect(writeJokerProjectConfig(workspace, JSON.stringify({ version: 1 })))
      .rejects.toThrow(/symbolic link/)
    await expect(readFile(outside, 'utf8')).resolves.toBe('outside')
  })

  it('round-trips a safely written config', async () => {
    const written = await writeJokerProjectConfig(workspace, JSON.stringify({ version: 1 }, null, 2))
    expect(written.path).toBe(join(await realpath(workspace), JOKER_PROJECT_CONFIG_RELATIVE_PATH))
    await expect(loadJokerProjectConfig(workspace)).resolves.toMatchObject({
      status: 'valid',
  ...
  it('leaves the previous file unchanged when an edit is invalid', async () => {
    ...
    const written = await writeJokerProjectConfig(workspace, original)

    await expect(writeJokerProjectConfig(workspace, '{')).rejects.toThrow(/must be JSON/)
  ...
  it('keeps the documented project config example schema-valid', async () => {
    const example = JSON.parse(await readFile(
      resolve(process.cwd(), '..', 'docs', 'examples', 'Joker-project.json'),
      'utf8'
    )) as unknown

    expect(JokerProjectConfigSchema.safeParse(example).success).toBe(true)
  })
})
