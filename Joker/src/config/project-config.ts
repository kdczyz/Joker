...
export const JOKER_PROJECT_CONFIG_RELATIVE_PATH = '.Joker/project.json'
export const JOKER_PROJECT_CONFIG_VERSION = 1
export const MAX_JOKER_PROJECT_CONFIG_BYTES = 256 * 1024
export const MAX_JOKER_PROJECT_MCP_SERVERS = 32
export const MAX_JOKER_PROJECT_SKILL_ROOTS = 32
...
export const JokerProjectMcpServerConfig = z
  .object({
...
export type JokerProjectMcpServerConfig = z.infer<typeof JokerProjectMcpServerConfig>

export const JokerProjectConfigSchema = z
  .object({
    $schema: z.string().trim().min(1).max(4_096).optional(),
    version: z.literal(JOKER_PROJECT_CONFIG_VERSION),
    mcp: z
      .object({
        servers: z
          .record(z.string().trim().min(1).max(128), JokerProjectMcpServerConfig)
          .refine((servers) => Object.keys(servers).length <= MAX_JOKER_PROJECT_MCP_SERVERS, {
            message: `project MCP config supports at most ${MAX_JOKER_PROJECT_MCP_SERVERS} servers`
          })
          .default({})
      })
      .strict()
    ...
    skills: z
      .object({
        ...
        roots: z.array(RelativeProjectPath).max(MAX_JOKER_PROJECT_SKILL_ROOTS).default([]),
  ...
  .strict()
export type JokerProjectConfig = z.infer<typeof JokerProjectConfigSchema>

export type ResolvedJokerProjectConfig = {
  workspaceRoot: string
  path: string
  digest: string
  config: JokerProjectConfig
  mcp: {
    servers: Record<string, JokerProjectMcpServerConfig & { cwd?: string }>
  }
  skills: {
    enabled: boolean
...
export type JokerProjectConfigLoadResult =
  | {
      status: 'missing'
      workspaceRoot: string
      path: string
  ...
  | ({ status: 'valid' } & ResolvedJokerProjectConfig)

export type JokerProjectConfigSource = {
  workspaceRoot: string
  path: string
  exists: boolean
  content: string
...
export function JokerProjectConfigPath(workspaceRoot: string): string {
  return join(resolve(workspaceRoot), '.Joker', 'project.json')
}

export function JokerProjectConfigDigest(config: JokerProjectConfig): string {
  return createHash('sha256')
    .update(stableJson(config))
    .digest('hex')
}

export async function parseAndResolveJokerProjectConfig(
  workspaceRoot: string,
  value: unknown,
  path = JokerProjectConfigPath(workspaceRoot)
): Promise<ResolvedJokerProjectConfig> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const parsed = JokerProjectConfigSchema.safeParse(value)
  if (!parsed.success) {
  ...
  const skillRoots = config.skills.enabled
    ...
    : []
  const servers: ResolvedJokerProjectConfig['mcp']['servers'] = {}
  ...
  return {
    ...
    path,
    digest: JokerProjectConfigDigest(config),
    config,
    mcp: { servers },
    skills: {
...
export async function loadJokerProjectConfig(
  workspaceRoot: string
): Promise<JokerProjectConfigLoadResult> {
  ...
  const path = JokerProjectConfigPath(resolvedWorkspaceRoot)
  ...
  try {
    const text = await readBoundedRegularFile(resolvedPath, MAX_JOKER_PROJECT_CONFIG_BYTES)
    const value = JSON.parse(text) as unknown
    const resolved = await parseAndResolveJokerProjectConfig(resolvedWorkspaceRoot, value, path)
...
export async function readJokerProjectConfigSource(
  workspaceRoot: string
): Promise<JokerProjectConfigSource> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const path = JokerProjectConfigPath(resolvedWorkspaceRoot)
  let resolvedPath: string
  try {
  ...
  return {
    ...
    path,
    exists: true,
    content: await readBoundedRegularFile(resolvedPath, MAX_JOKER_PROJECT_CONFIG_BYTES)
  }
}

export async function validateJokerProjectConfigText(
  workspaceRoot: string,
  content: string
): Promise<ResolvedJokerProjectConfig> {
  if (Buffer.byteLength(content, 'utf8') > MAX_JOKER_PROJECT_CONFIG_BYTES) {
    throw new Error(`project config exceeds ${MAX_JOKER_PROJECT_CONFIG_BYTES} byte limit`)
  }
  let value: unknown
  try {
  ...
  return parseAndResolveJokerProjectConfig(workspaceRoot, value)
}

export async function writeJokerProjectConfig(
  workspaceRoot: string,
  content: string
): Promise<ResolvedJokerProjectConfig> {
  const validated = await validateJokerProjectConfigText(workspaceRoot, content)
  const configDir = join(validated.workspaceRoot, '.Joker')
...
export async function ensureJokerProjectConfigDirectory(
  workspaceRoot: string
): Promise<string> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const configDir = join(resolvedWorkspaceRoot, '.Joker')
...
function projectConfigValidationMessage(
  ...
  if (isObject(value) && value.version !== undefined && value.version !== JOKER_PROJECT_CONFIG_VERSION) {
