...
import {
  DEFAULT_JOKER_CAPABILITIES_CONFIG,
  JokerCapabilitiesConfig,
  ModelInputModality,
...
export const JOKER_CONFIG_FILENAME = 'config.json'
export const DEFAULT_JOKER_MODEL = 'deepseek-v4-pro'
...
export const JokerServeConfigSchema = z
  .object({
...
export const JokerConfigSchema = z
  .object({
    serve: JokerServeConfigSchema.optional(),
    models: ModelConfigSchema.optional(),
    contextCompaction: ContextCompactionConfigSchema.optional(),
    runtime: RuntimeTuningConfigSchema.optional(),
    roles: RolesConfigSchema.optional(),
    capabilities: JokerCapabilitiesConfig.default(DEFAULT_JOKER_CAPABILITIES_CONFIG),
...
export type JokerServeConfig = z.infer<typeof JokerServeConfigSchema>
...
export type LoadedJokerConfig = {
  path: string
  config: JokerConfig
}

export function readJokerConfigFile(path: string): LoadedJokerConfig {
  ...
  try {
    ...
    throw new Error(`Failed to parse Joker config JSON at ${resolvedPath}: ${message}`)
  }
  const parsed = JokerConfigSchema.safeParse(json)
  if (!parsed.success) {
    throw new Error(
      `Invalid Joker config at ${resolvedPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
  return { path: resolvedPath, config: parsed.data }
}

export function readOptionalJokerConfigFile(path: string | undefined): LoadedJokerConfig | null {
  if (!path) return null
  const resolvedPath = expandHomePath(path)
  if (!existsSync(resolvedPath)) return null
  return readJokerConfigFile(resolvedPath)
}

export function JokerConfigPathForDataDir(dataDir: string | undefined): string | undefined {
  const trimmed = dataDir?.trim()
  if (!trimmed) return undefined
  return join(expandHomePath(trimmed), JOKER_CONFIG_FILENAME)
}
