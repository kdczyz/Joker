...
import { createJokerServeRuntime } from '../server/runtime-factory.js'
...
export const JOKER_CLI_USAGE = `Joker <command> [options]
...
Common options:
  ...
  --data-dir <path>          Root directory for Joker data
  ...
  --jsonl                    Stream one machine-readable event per line for Joker run
...
export type JokerCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitJokerCliCommand(argv: readonly string[]): {
  command: JokerCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<JokerCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'Joker run')
  const jsonl = hasFlag(argv, 'jsonl')
  if (parsed.json && jsonl) {
    io.stderr.write('Joker run: --json and --jsonl are mutually exclusive\n')
    return ServeExitCode.usage
  }
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('Joker run: missing prompt\n')
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    io.stderr.write(`Joker run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'Joker run')
  }
}
...
async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'Joker chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`Joker chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'Joker chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'Joker exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`Joker exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('Joker exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`Joker exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'Joker exec')
  }
}
...
function parseSharedOptions(argv: readonly string[], io: CliIo): SharedOptionsResult {
  ...
  return {
    ...
    workspace: stringFlag(argv, ['workspace']) ?? io.env?.JOKER_WORKSPACE ?? io.cwd?.() ?? process.cwd(),
...
function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createJokerServeRuntime(options)
}
