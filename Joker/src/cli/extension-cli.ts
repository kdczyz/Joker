...
const JOKER_EXTENSION_CLI_SCHEMA_VERSION = 1
const DEFAULT_JOKER_VERSION = '0.1.0'
const DEFAULT_LOG_BYTES = 256 * 1024
const MAX_LOG_BYTES = 1024 * 1024

export const JOKER_EXTENSION_CLI_USAGE = `Joker extension <command> [options]

Commands:
  create <directory>          Scaffold a node, webview, or React extension
  validate <path>            Validate a source directory or .Jokerx archive
  pack <directory>           Build a deterministic .Jokerx archive
  install <path>             Install a .Jokerx, development directory, or exact Index version
...
Pack/source-validation options:
  --output <path>            .Jokerx file or output directory
...
export function createExtensionCliServices(options: {
  ...
  const compatibility: ExtensionCompatibility = {
    JokerVersion: options.JokerVersion ?? DEFAULT_JOKER_VERSION,
...
/**
 * Runs argv following the `Joker extension` prefix. The top-level CLI dispatcher
...
export async function runExtensionCommand(
  ...
  try {
    ...
    io.stderr.write(`Joker extension: ${errorMessage(error)}\n`)
    io.stderr.write(JOKER_EXTENSION_CLI_USAGE)
    return ServeExitCode.usage
  }
  if (parsed.command === 'help' || parsed.flags.has('help')) {
    io.stdout.write(JOKER_EXTENSION_CLI_USAGE)
    return ServeExitCode.ok
  }
  if (!COMMANDS.has(parsed.command)) {
    io.stderr.write(`Joker extension: unknown command: ${parsed.command}\n`)
    io.stderr.write(JOKER_EXTENSION_CLI_USAGE)
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    const services = suppliedServices ?? createExtensionCliServices({
      dataDir: parsed.values.get('data-dir') ?? io.env?.JOKER_DATA_DIR,
    ...
    await ownedServices?.manager.shutdown().catch((error: unknown) => {
      io.stderr.write(`Joker extension: shutdown failed: ${errorMessage(error)}\n`)
    })
  }
}
...
async function runValidate(
  ...
  if (details.isFile()) {
    const inspection = await inspectJokerxArchive(path, { compatibility: services.compatibility })
    writeResult(parsed, io, 'Extension archive is valid', projectInspection(inspection))
    return ServeExitCode.ok
  }
  if (!details.isDirectory()) throw new ExtensionError('EXTENSION_PACKAGE_SOURCE_INVALID', 'Validation path must be a directory or .Jokerx file')
  ...
  const temporary = await mkdtemp(join(tmpdir(), 'Joker-extension-validate-'))
  try {
    const inspection = await packJokerx(path, join(temporary, 'validation.Jokerx'), {
...
async function runPack(
  ...
  const defaultName = `${extensionId(development.manifest)}-${development.manifest.version}.Jokerx`
  ...
  const inspection = await packJokerx(source, output, {
    compatibility: services.compatibility,
...
async function runInstall(
  ...
  const [input] = parsed.positionals
  if (input === undefined) throw usageError('install requires a .Jokerx path or --development/--index')
  const archivePath = resolveFromCwd(input, io)
  const inspection = await inspectJokerxArchive(archivePath, { compatibility: services.compatibility })
  const id = extensionId(inspection.manifest)
  requirePermissionAcceptance(parsed, io, id, inspection.manifest.permissions)
...
async function runList(
  ...
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: JOKER_EXTENSION_CLI_SCHEMA_VERSION, revision: registry.revision, extensions })
  } else if (extensions.length === 0) {
    io.stdout.write('No Joker extensions installed.\n')
  } else {
...
async function runDoctor(
  ...
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: JOKER_EXTENSION_CLI_SCHEMA_VERSION, healthy, diagnostics })
...
async function runLogs(
  ...
  if (parsed.json) {
    writeJson(io.stdout, {
      schemaVersion: JOKER_EXTENSION_CLI_SCHEMA_VERSION,
      extensionId: id,
      logPath,
...
function projectInspection(inspection: Awaited<ReturnType<typeof inspectJokerxArchive>>) {
  return {
    ...
    enginesJoker: inspection.manifest.engines.Joker,
...
function resolvePackOutput(output: string, defaultName: string): string {
  return extname(output).toLowerCase() === '.Jokerx' ? output : join(output, defaultName)
}

async function loadScaffolder(): Promise<NonNullable<ExtensionCliIo['scaffold']>> {
  const packageName = 'create-Joker-extension'
  try {
    ...
    throw new ExtensionError(
      ...
      'create-Joker-extension is not installed with this Joker distribution',
      {},
      { cause: error }
    )
  }
...
function writeResult(parsed: ParsedArguments, io: ExtensionCliIo, message: string, result: unknown): void {
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: JOKER_EXTENSION_CLI_SCHEMA_VERSION, result: redactSecrets(result) })
  } else {
...
function writeCommandError(parsed: ParsedArguments, io: ExtensionCliIo, error: unknown): void {
  const normalized = normalizeCliError(error)
  if (parsed.json) {
    io.stderr.write(`${JSON.stringify({ schemaVersion: JOKER_EXTENSION_CLI_SCHEMA_VERSION, error: normalized })}\n`)
  } else {
    io.stderr.write(`Joker extension ${parsed.command}: ${normalized.code}: ${normalized.message}\n`)
