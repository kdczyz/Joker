...
export const JOKER_READY_PREFIX = 'JOKER_READY '

/**
 * Serve-mode command. Kept separate from the dispatcher so GUI startup
 * still has the exact same JOKER_READY handshake behavior.
 */
async function serveMain(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.ok) {
    process.stderr.write(`Joker serve: ${parsed.message}\n`)
    if (parsed.issues) {
      process.stderr.write(`${JSON.stringify(parsed.issues, null, 2)}\n`)
    }
    return parsed.exitCode
  }
  let handle: JokerServeHandle | null = null
  installServeCrashHandlers(() => handle)
  const server = await startJokerServe(parsed.options)
  handle = server
  ...
  const startupInfo = {
    ...
    pid: info.pid,
    message: `Joker runtime listening on http://${server.host}:${server.port}`
  }
  process.stdout.write(`${JOKER_READY_PREFIX}${JSON.stringify(startupInfo)}\n`)
...
/**
 * When the GUI launches Joker without `ELECTRON_RUN_AS_NODE` (the host
...
async function selfVerifyHealth(host: string, port: number): Promise<void> {
  ...
  process.stderr.write(
    `[Joker] warning: self-health-probe on http://${host}:${port}/health did not pass within ${SELF_VERIFY_TIMEOUT_MS}ms — proceeding anyway\n`
  )
}
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  const command = splitJokerCliCommand(argv)
  if (command.command === 'help') {
    if (command.error) {
      process.stderr.write(`Joker: ${command.error}\n`)
      process.stderr.write(JOKER_CLI_USAGE)
      return ServeExitCode.usage
    }
    process.stdout.write(JOKER_CLI_USAGE)
    return ServeExitCode.ok
  }
...
main(process.argv.slice(2)).then(
  ...
  (error) => {
    process.stderr.write(`Joker serve: ${String(error)}\n`)
