...
import type { JokerServeHandle } from '../server/runtime-factory.js'
 ...
 */
export function installServeCrashHandlers(getHandle: () => JokerServeHandle | null): void {
  let crashing = false
  const crash = (kind: string, error: unknown): void => {
    if (crashing) return
    crashing = true
    const detail = error instanceof Error ? (error.stack ?? error.message) : String(error)
    process.stderr.write(`Joker serve: ${kind}: ${detail}\n`)
  ...
  process.on('unhandledRejection', (reason) => {
    ...
    process.stderr.write(`Joker serve: unhandledRejection (non-fatal, runtime stays up): ${detail}\n`)
  })
}
