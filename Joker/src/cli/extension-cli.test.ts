...
describe('Joker extension CLI', () => {
  it('scaffolds through the official create-Joker-extension adapter', async () => {
  ...
  it('accepts repeatable safe include and ignore paths for source validation and packing', async () => {
    ...
    const archive = join(root, 'selected.Jokerx')
  ...
  it('validates bounded manifest locale overlays through the public CLI', async () => {
    ...
    const manifestPath = join(source, 'Joker-extension.json')
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'Joker-extension-cli-'))
