...
import {
  JokerConfigPathForDataDir,
  readJokerConfigFile,
  readOptionalJokerConfigFile,
  type LoadedJokerConfig
} from '../config/Joker-config.js'
import { parseOtlpHeaders, resolveOtlpTracesEndpoint } from '../telemetry/otlp-http-json-sink.js'

/**
 * Parse the `Joker serve` command line into validated options.
 *
...
export function parseServeOptions(
  ...
  const observabilityExporterValue =
    ...
    env.JOKER_OBSERVABILITY_EXPORTER ??
  ...
  const bundledExtensionsDir =
    ...
    nonEmptyEnv(env.JOKER_BUNDLED_EXTENSIONS_DIR)
  const merged: ServeOptions = {
    ...
    host:
      typeof raw.host === 'string'
        ? raw.host
        : env.JOKER_HOST ?? configServe.host ?? DEFAULT_SERVE_OPTIONS.host,
    port:
    ...
    endpointFormat:
      typeof raw['endpoint-format'] === 'string'
        ...
        : typeof raw.endpointFormat === 'string'
          ...
          : env.JOKER_ENDPOINT_FORMAT as ServeOptions['endpointFormat'] | undefined ??
    ...
    model:
      typeof raw.model === 'string'
        ? raw.model
        : env.JOKER_MODEL ?? configServe.model ?? DEFAULT_SERVE_OPTIONS.model,
    approvalPolicy:
 ...
 * main process when Joker is started programmatically.
 */
...
export const SERVE_USAGE = `Joker serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedJokerConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.JOKER_CONFIG
  if (explicitConfigPath) {
    return readJokerConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalJokerConfigFile(JokerConfigPathForDataDir(dataDir))
}
