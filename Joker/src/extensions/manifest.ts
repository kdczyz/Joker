...
export function parseExtensionManifest(value: unknown): ExtensionManifest {
  ...
  if (semver.validRange(manifest.engines.Joker) === null) {
    throw extensionError('EXTENSION_ENGINE_RANGE_INVALID', 'engines.Joker must be a valid SemVer range', {
      value: manifest.engines.Joker
    })
  }
...
export function assertManifestCompatible(
  ...
  if (!report.JokerEngine.compatible) {
    throw extensionError('EXTENSION_ENGINE_INCOMPATIBLE', 'Running Joker version is outside engines.Joker', {
      running: compatibility.JokerVersion,
      declared: manifest.engines.Joker,
      compatibility: report
    })
  }
...
export function manifestCompatibilityReport(
  ...
  const engineCompatible = semver.satisfies(compatibility.JokerVersion, manifest.engines.Joker, {
  ...
  if (!engineCompatible) {
    diagnostics.push({
      ...
      dimension: 'engine',
      declared: manifest.engines.Joker,
      supported: compatibility.JokerVersion,
      code: 'EXTENSION_ENGINE_INCOMPATIBLE',
      message: `Running Joker ${compatibility.JokerVersion} is outside ${manifest.engines.Joker}`
    })
  }
  if (!rpcCompatible) {
    diagnostics.push({
