...
const IndexVersionSchema = z.object({
  ...
  engines: z.object({ Joker: z.string() }).strict(),
  apiVersion: z.string(),
...
export class ExtensionIndexClient {
  ...
  async installExact(
    ...
    const downloadPath = join(manager.paths.downloadsRoot, `${randomUUID()}.Jokerx`)
    try {
      ...
      return await manager.installArchive(downloadPath, {
        ...
        expected: {
          ...
          enginesJoker: selected.engines.Joker,
...
function validateIndexSemantics(index: ExtensionIndexDocument): void {
  ...
  for (const extension of index.extensions) {
    ...
    for (const version of extension.versions) {
      ...
      if (!semver.valid(version.apiVersion) || semver.validRange(version.engines.Joker) === null) {
