    ...
    switch (method) {
      ...
      case 'host.load': {
        ...
        extensionModule = await import(
          `${pathToFileURL(entrypoint).href}?JokerHost=${encodeURIComponent(initialized.identity.lifecycleNonce)}`
        ) as ExtensionModule
        return { loaded: true }
      }
...
if (process.env.JOKER_EXTENSION_HOST_RUNNER === '1') {
  void runExtensionHost().catch((error: unknown) => {
    process.stderr.write(
      `[Joker-extension-host] ${error instanceof Error ? error.stack ?? error.message : String(error)}\n`
    )
    process.exit(1)
  })
}
