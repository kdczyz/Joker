...
const ARCHIVE_NAME = /^[0-9A-Za-z][0-9A-Za-z._-]*\.Jokerx$/
...
function expectedPackage(descriptor: BundledExtensionCatalogEntry): ExpectedIndexedPackage {
  return {
    ...
    enginesJoker: descriptor.enginesJoker,
...
function validateCatalogEntry(
  ...
  const enginesJoker = stringField(value, 'enginesJoker')
  const apiVersion = stringField(value, 'apiVersion')
  assertExtensionId(id)
  if (semver.valid(version) === null || semver.valid(apiVersion) === null || semver.validRange(enginesJoker) === null) {
    invalidCatalogEntry()
  }
  if (
    archive !== basename(archive) ||
    extname(archive).toLowerCase() !== '.Jokerx' ||
