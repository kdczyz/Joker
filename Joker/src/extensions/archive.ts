...
export const EXTENSION_MANIFEST_FILE = 'Joker-extension.json'
export const EXTENSION_INTEGRITY_FILE = 'Joker-extension.integrity.json'
...
export async function extractJokerxArchive(
  archivePath: string,
  destination: string,
  options: ArchiveValidationOptions = {}
): Promise<ExtractedJokerx> {
...
export async function inspectJokerxArchive(
  archivePath: string,
  options: ArchiveValidationOptions = {}
): Promise<Omit<ExtractedJokerx, 'destination'>> {
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'Joker-extension-inspect-'))
  const destination = join(temporaryRoot, 'package')
  try {
    const { destination: _destination, ...inspection } = await extractJokerxArchive(
      archivePath,
      destination,
      options
    )
...
export type PackJokerxOptions = ArchiveValidationOptions & {
  overwrite?: boolean
...
export async function packJokerx(
  sourceDirectory: string,
  outputPath: string,
  options: PackJokerxOptions = {}
): Promise<Omit<ExtractedJokerx, 'destination'>> {
  const limits = archiveLimits(options.limits)
  ...
  try {
    ...
    const inspection = await inspectJokerxArchive(temporaryOutput, options)
...
async function collectPackFiles(
  ...
  options: Pick<PackJokerxOptions, 'include' | 'ignore'>
): Promise<PackageFile[]> {
...
function forbiddenPackagePathReason(packagePath: string): string | undefined {
  ...
  if (fileName.endsWith('.Jokerx')) return 'nested-package'
