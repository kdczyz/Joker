...
export class ExtensionPackageManager {
  ...
  async installArchive(
    ...
    let destination = ''
    try {
      const extracted = await extractJokerxArchive(archivePath, extractedPath, this.validation)
...
function validateExpectedPackage(
  ...
  if (extracted.manifest.engines.Joker !== expected.enginesJoker) mismatches.push('engines.Joker')
