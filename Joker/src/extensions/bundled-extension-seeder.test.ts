...
import { packJokerx } from './archive.js'
import {
...
describe('bundled extension seeding', () => {
  ...
  it('classifies a pre-existing package as user-managed without changing it', async () => {
    ...
    const userArchive = join(harness.root, 'user-install.Jokerx')
    await writeSource(harness.source, '1.0.0', [], 'user-owned')
    await packJokerx(harness.source, userArchive, { compatibility })
...
async function createHarness() {
  const root = await mkdtemp(join(tmpdir(), 'Joker-bundled-seeder-'))
  roots.push(root)
...
async function writeBundle(
  ...
  const archiveName = `acme-demo-${version}.Jokerx`
  const archive = join(harness.bundles, archiveName)
  const packed = await packJokerx(harness.source, archive, { compatibility, overwrite: true })
...
async function writeSource(
  ...
  await writeFile(join(root, 'Joker-extension.json'), `${JSON.stringify(manifest, null, 2)}\n`)
