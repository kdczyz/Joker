/**
 * Re-exposes Joker's own tools to the Claude Agent SDK as in-process MCP tools.
 * This is the inbound half of the fusion: the SDK owns the loop, but when the
 * model calls one of Joker's tools the handler runs Joker's real executor in-process
 * — so generate_image, computer_use, memory, web search, delegate_task (and thus
 * Joker's richer subagents), etc. all keep working on a subscription turn.
 *
 ...
 * native ones. We only bridge Joker-EXCLUSIVE tools. `delegate_task` is bridged
 * rather than mapped to the SDK `agents` option because Joker's delegation is
...
/** Structural view of a Joker LocalTool (decoupled from Joker's tool internals). */
...
export interface JokerToolResult {
  output: unknown
  isError?: boolean
}

/** Executes a Joker tool by name for the active turn (closes over ToolHostContext). */
export type JokerToolExecutor = (toolName: string, args: Record<string, unknown>) => Promise<JokerToolResult>
...
/** Joker built-ins that overlap Claude Code built-ins — use the SDK's instead. */
...
/**
 * Joker tools better handled by the SDK's own surfaces or meaningless here.
 * NOTE: user_input/request_user_input are intentionally NOT excluded — they are
 * bridged so the model uses Joker's own GUI input panel (wired via the tool
...
/** Filter a tool catalog down to the Joker-exclusive tools worth bridging. */
...
/** Collapse a Joker tool result into the SDK MCP tool content shape. */
export function mapJokerResultToSdkContent(result: JokerToolResult): SdkToolContent {
  const text =
...
/**
 * Build SDK MCP tool specs whose handlers run Joker's executor. A throwing or
  ...
  return tools.map((tool) => ({
    ...
    handler: async (args: Record<string, unknown>): Promise<SdkToolContent> => {
      try {
        ...
        return mapJokerResultToSdkContent(result)
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
        return {
          content: [{ type: 'text', text: `Joker tool "${tool.name}" failed: ${message}` }],
          isError: true
        }
      }
    }
 ...
 * Joker validates arguments inside its own executor, so this only needs to convey
 ...
 * server named `Joker`. The model sees these as `mcp__Joker__<toolName>`.
...
export function bridgedToolModelNames(specs: readonly BridgedToolSpec[], serverName = 'Joker'): string[] {
