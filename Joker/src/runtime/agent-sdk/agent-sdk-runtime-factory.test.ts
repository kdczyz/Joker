...
import { JokerCapabilitiesConfig } from '../../contracts/capabilities.js'
...
const planTurn = (id: string, workspaceRoot: string): ThreadRecord['turns'][number] =>
  ({
    id,
    prompt: 'plan it',
    guiPlan: { operation: 'draft', workspaceRoot, relativePath: '.Joker/plan.md', planId: 'p1' }
...
describe('resolveTurnPlanContext', () => {
  test('exposes the GUI plan + planMode for a plan turn in the same workspace', () => {
    ...
    expect(resolved.guiPlan?.relativePath).toBe('.Joker/plan.md')
...
describe('createAgentSdkRuntime turn context', () => {
  test('scopes dedicated SVG turns to structured tools and the artifact-specific policy', async () => {
    ...
    await deps.executeJokerTool('th', 'tn', 'design_svg_edit', { ops: [] })

    expect(listedContexts).toEqual([expect.objectContaining({
      guiDesignArtifact: { kind: 'svg', artifactId: 'motion', relativePath: '.Joker-design/doc/motion/v1.svg' },
  ...
  test('bridges skill-gated PPT tools with the same active skill ids at execution time', async () => {
    ...
    const deps = (runtime as unknown as {
      deps: {
        ...
        executeJokerTool(threadId: string, turnId: string, toolName: string, args: Record<string, unknown>): Promise<unknown>
      }
    }).deps
    ...
    await expect(deps.executeJokerTool('th', 'tn', 'ppt_master_run', {})).resolves.toEqual({
      output: { ok: true },
      isError: false
    })
  ...
  test('pre-bridges visible skill tools but requires load_skill activation before SDK execution', async () => {
    ...
    await expect(deps.executeJokerTool('th', 'tn', 'ppt_master_run', {})).resolves.toMatchObject({
      isError: true,
      output: expect.stringContaining('not advertised')
    })
    await expect(deps.executeJokerTool('th', 'tn', 'load_skill', { skill_id: 'ppt-master' })).resolves.toMatchObject({
      isError: false
    })
    await expect(deps.executeJokerTool('th', 'tn', 'ppt_master_run', {})).resolves.toEqual({
      output: { ran: true },
  ...
  test('does not fall back to the process workspace when a thread or turn has disappeared', async () => {
    ...
    await expect(deps.executeJokerTool(
      'deleted-thread',
      'deleted-turn',
      'bash',
      {},
  ...
  test('runs bridged Joker tools through the canonical host policy boundary', async () => {
    const executed: string[] = []
    const tools = [
    ...
    await expect(deps.executeJokerTool('th', 'tn', 'approval_required', {})).resolves.toMatchObject({
      isError: true,
      output: expect.objectContaining({ code: 'approval_denied' })
    })
    await expect(deps.executeJokerTool('th', 'tn', 'disabled_tool', {})).resolves.toMatchObject({
      isError: true,
  ...
  test('gives concurrent bridged calls distinct approval identities', async () => {
    ...
    const first = deps.executeJokerTool('th', 'tn', 'approval_required', {})
    const second = deps.executeJokerTool('th', 'tn', 'approval_required', {})
  ...
  test('does not duplicate an HTTP-recorded user input resolution event', async () => {
    ...
    const deps = (runtime as unknown as {
      deps: { executeJokerTool(threadId: string, turnId: string, toolName: string, args: Record<string, unknown>): Promise<unknown> }
    }).deps

    await deps.executeJokerTool('th', 'tn', 'user_input', {})
  ...
  test('arms SDK user input before publishing user_input_requested', async () => {
    ...
    const deps = (runtime as unknown as {
      deps: {
        executeJokerTool(threadId: string, turnId: string, toolName: string, args: Record<string, unknown>): Promise<{ output: unknown; isError?: boolean }>
      }
    }).deps

    await expect(deps.executeJokerTool('th', 'tn', 'user_input', {})).resolves.toEqual({
  ...
  test('injects native AGENTS.md instructions and records turn metadata', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Joker-sdk-instructions-'))
    try {
      ...
      const runtime = createAgentSdkRuntime({
        ...
        instructionRuntime: new InstructionRuntime(
          JokerCapabilitiesConfig.parse({ instructions: { enabled: true } }).instructions,
          { homeDir: home }
        )
      })
      const deps = (runtime as unknown as {
