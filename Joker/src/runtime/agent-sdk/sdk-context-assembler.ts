/**
 * Pure assembly of the per-turn context that Joker injects into a subscription
 * (Claude Agent SDK) turn. The SDK owns the loop, but — unlike Joker's native
 * loop — it does NOT see Joker's conversation history, skill catalog, memories, or
 ...
 *
 * Design: Joker owns the canonical history, so every SDK turn is stateless from
 * Joker's side — we replay the prior conversation as a transcript preamble each
