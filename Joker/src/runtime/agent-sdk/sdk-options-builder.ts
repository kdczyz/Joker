/**
 * Assembles the `query()` options for a Joker subscription turn — the glue that
 * injects Joker's brain (persona, tools, permissions) into the SDK's loop.
 *
 * The assembly is pure and unit-tested. The two callbacks it carries
 * (`canUseTool`, hook callbacks) are factories that close over Joker's real
 ...
 * deliberately did NOT bridge from Joker). Listed in allowedTools so they are
...
/**
 * Claude Code built-in tools we suppress on the Joker-driven SDK path.
 * AskUserQuestion has no UI in this embedding (the model would ask and get no
 * answer); Joker's own bridged `user_input` panel handles interactive questions.
 */
...
/**
 * Map Joker's ApprovalPolicy onto the SDK permission mode. Joker's fine-grained
...
/**
 * Claude Code (the subscription engine) only accepts Anthropic models. A Joker
...
/** Compose Joker's persona append text for the claude_code system-prompt preset. */
export function buildClaudeSystemPrompt(
  JokerSystemPrompt: string,
  threadPersona?: string
): SdkSystemPromptPreset {
  const base = JokerSystemPrompt.trim()
...
/** Joker's permission decision for a (toolName, input) pair on the active turn. */
...
/**
 * Bridge Joker's approval engine to the SDK `canUseTool` callback. Every tool the
 * SDK is about to run is adjudicated by Joker (which can route to the GUI
  ...
  return async (toolName, input): Promise<SdkPermissionResult> => {
    ...
    try {
      ...
      if (decision.allow) {
        ...
        // Echo the original input through when Joker doesn't rewrite it.
        return { behavior: 'allow', updatedInput: decision.updatedInput ?? safeInput }
      }
      // The deny variant requires a non-empty `message`.
      return {
        behavior: 'deny',
        message: decision.message ?? 'Denied by Joker permission policy',
...
export interface AssembleSdkOptionsParams {
  ...
  sandboxMode?: SandboxMode
  planMode?: boolean
  /** `mcp__Joker__*` names from the tool bridge. */
  ...
  /** Native Joker maxSteps mapped onto the Agent SDK's loop ceiling. */
  maxTurns?: number
}
...
export function assembleSdkOptions(params: AssembleSdkOptionsParams): SdkQueryOptions {
  ...
  const options: SdkQueryOptions = {
    cwd: params.cwd,
    systemPrompt: buildClaudeSystemPrompt(params.JokerSystemPrompt, params.threadPersona),
    ...
    // native Read/Write/Edit/Bash alongside the structured Joker MCP tools.
    ...
    // Only load Joker-provided config; don't auto-absorb the host's ~/.claude.
