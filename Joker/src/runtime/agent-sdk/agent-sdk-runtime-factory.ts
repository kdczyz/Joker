/**
 * Binds the decoupled {@link AgentSdkRuntime} to Joker's real runtime services.
 * This is the only place that touches the SDK package and Joker's concrete stores,
...
import type { BridgeableTool, JokerToolResult } from './sdk-tool-bridge.js'
...
import type { ServeProviderConfig } from '../../config/Joker-config.js'
...
export interface AgentSdkRuntimeFactoryDeps {
  ...
  /**
   * The canonical host boundary for bridged Joker tool execution. Serve always
  ...
  /** Native Joker AGENTS.md instruction engine — injects global/workspace instructions per turn. */
  instructionRuntime?: InstructionRuntime
  /** Long-term memory store — injects relevant memories per turn. */
  memoryStore?: MemoryStore
  /** Interactive-input gate — lets the bridged `user_input` tool surface Joker's GUI panel. */
...
/** Lazily load the real SDK without a static import (so Joker typechecks without it). */
 ...
 * GUI plan — the SDD "下一步"/Plan-mode flow — we must expose it so the Joker
...
export function createAgentSdkRuntime(deps: AgentSdkRuntimeFactoryDeps): AgentSdkRuntime {
  ...
  // resume from it: Joker owns the canonical history and replays it as a transcript
  ...
  /**
   * Bridge Joker's `user_input` tool to its GUI panel: persist the request item +
  ...
  const toolContext = (
    ...
    // (executeJokerTool) on plan turns — both are gated on it.
    ...
    // Wire interactive input to Joker's GUI panel (advertises `user_input`).
  ...
  const runtimeDeps: SdkRuntimeDeps = {
    ...
    async loadTurnContext(threadId, turnId): Promise<SdkTurnContext | null> {
      ...
      // Plan turns expose create_plan (and narrow Joker tools to the plan-allowed
      ...
      // by skills visible in this workspace; executeJokerTool still re-resolves
      ...
      // The SDK doesn't see Joker's history or per-turn context, so assemble both
      // here (parity with the native loop's `contextInstructions`). Joker owns the
    ...
    async executeJokerTool(threadId, turnId, toolName, args, signal): Promise<JokerToolResult> {
      const thread = await deps.threadStore.get(threadId)
      const turn = thread?.turns.find((candidate) => candidate.id === turnId)
      ...
      if (!deps.toolHost) {
        return { output: 'Joker tool host is unavailable; tool execution was denied', isError: true }
      }
      ...
      try {
        ...
        if (result.item.kind !== 'tool_result') {
          return {
            output: `Joker tool ${toolName} returned an invalid result item`,
            isError: true
          }
        }
    ...
    async decideToolApproval(threadId, turnId, toolName, input, signal): Promise<ToolApprovalDecision> {
      // Bridged Joker tools perform their own per-tool policy check through the
      // LocalToolHost context above; asking here too would create two prompts.
      if (toolName.startsWith('mcp__Joker__')) return { allow: true }
      const thread = await deps.threadStore.get(threadId)
      const turn = thread?.turns.find((candidate) => candidate.id === turnId)
    ...
    JokerSystemPrompt: () => deps.prefix.systemPrompt,
