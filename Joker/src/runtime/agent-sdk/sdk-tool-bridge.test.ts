...
describe('selectBridgeableTools', () => {
  test('drops Claude Code overlap tools and excluded tools, keeps Joker-exclusive', () => {
    const tools = [
      tool('read'),
      tool('bash'),
      tool('edit'),
    ...
    // now bridged so Joker's GUI input panel handles interactive questions.
...
describe('mapJokerResultToSdkContent', () => {
  test('passes string output through', () => {
    expect(mapJokerResultToSdkContent({ output: 'hello' })).toEqual({
      content: [{ type: 'text', text: 'hello' }]
    })
  })

  test('JSON-stringifies structured output', () => {
    const out = mapJokerResultToSdkContent({ output: { a: 1 } })
    expect(out.content[0].text).toContain('"a": 1')
  })

  test('flags errors', () => {
    expect(mapJokerResultToSdkContent({ output: 'bad', isError: true })).toMatchObject({ isError: true })
  })
})

describe('buildBridgedToolSpecs', () => {
  test('handler invokes the Joker executor and maps the result', async () => {
  ...
  test('a Joker isError result is propagated', async () => {
...
describe('bridgedToolModelNames', () => {
  test('namespaces under mcp__Joker__', () => {
    const specs = buildBridgedToolSpecs([tool('memory_create'), tool('lsp')], async () => ({ output: '' }))
    expect(bridgedToolModelNames(specs)).toEqual(['mcp__Joker__memory_create', 'mcp__Joker__lsp'])
  })
})
