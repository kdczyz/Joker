/**
 * Translates the Claude Agent SDK message stream into Joker's native runtime
 * events, so a subscription turn renders in the GUI exactly like a turn driven
 * by Joker's own agent loop. This is the load-bearing half of the fusion: the SDK
 * owns the loop, but every assistant token, tool call, and tool result it emits
 * is re-projected onto Joker's event contract.
 *
 ...
 *
 * Streaming model (mirrors Joker's native loop exactly, or the GUI double-renders):
 * a Joker `assistant_text_delta` event carries an INCREMENTAL CHUNK (the GUI
...
export interface SdkTurnFinal {
  ...
  message?: string
  /** Stable Joker error code when the SDK reports a native-equivalent limit. */
...
/** Claude Code built-in tool names that imply a richer Joker tool kind. */
...
/** Collapse an SDK tool_result content payload into a Joker tool output value. */
...
/**
 * Map the Agent SDK's per-request usage onto Joker's UsageSnapshot. Anthropic's
