...
export const GuiPlanContextSchema = z.object({
  ...
  relativePath: z
    ...
    .refine(isGuiPlanRelativePath, {
      message: 'relativePath must be a direct Markdown file under .Jokersdd/plan'
    }),
...
export const GuiDesignArtifactContextSchema = z.object({
  ...
  relativePath: z.string().min(1).refine((value) => {
    ...
    return normalized === value &&
      normalized.startsWith('.Joker-design/') &&
      !normalized.split('/').includes('..') &&
      /\/v\d+\.svg$/i.test(normalized)
  }, { message: 'relativePath must be a versioned SVG file under .Joker-design' })
})
...
export const TurnSchema = z.object({
  ...
  /**
   * True for renderer-owned design canvas turns. Joker advertises the
   ...
   * headless runs). Joker hides `user_input`/`request_user_input` and
   * rejects calls to them instead of blocking on a GUI answer.
   */
  disableUserInput: z.boolean().optional(),
  /**
   * True when this turn originated from an IM bridge. Joker exposes
...
export const StartTurnRequest = z.object({
   ...
   * mode Joker advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  workspaceCheckpointId: z.string().min(1).optional(),
  /**
   * Optional GUI plan context. When set, Joker advertises the
