...
export const SkillsCapabilityConfig = CapabilityToggleConfig.extend({
  ...
  workspaceRoots: z.array(z.string().min(1)).default([]),
  /** Global skill roots (e.g. ~/.Joker/skills). Scanned after project roots. */
  globalRoots: z.array(z.string().min(1)).default([]),
  /** Read workspace-local `.Joker/project.json` Skill policy on demand. */
...
export const JokerCapabilitiesConfig = z
  .object({
  ...
  .strict()
export type JokerCapabilitiesConfig = z.infer<typeof JokerCapabilitiesConfig>

export const DEFAULT_JOKER_CAPABILITIES_CONFIG: JokerCapabilitiesConfig = JokerCapabilitiesConfig.parse({})
...
export function buildRuntimeCapabilityManifest(input: {
  config?: JokerCapabilitiesConfig
  model: ModelCapabilityMetadata
  mcp?: {
  ...
  const config = JokerCapabilitiesConfig.parse(input.config ?? {})
