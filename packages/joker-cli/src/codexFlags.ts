 ...
 *
 * Codex exposes two orthogonal axes (approval policy + sandbox); Joker folds
...
/** Resolves the effective Joker permission mode from codex-style flags. */
export function resolvePermissionMode(flags: {
  ...
  switch (flags.sandbox) {
    ...
    case undefined:
    default:
      // codex defaults to read-only; Joker's "default" mode is the equivalent
      return 'default';
  }
}
