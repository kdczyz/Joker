...
export interface ParsedArgs {
  /** Codex-style invocation: `joker`, `joker exec`, `joker resume`, `joker mcp`, ... */
  command: string;
  rest: string[];
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case '--config':
        ...
        break;
      case '--mode': {
        // escape hatch to the native joker axis
...
export function printHelp(bin = 'joker'): void {
  console.log(`${paint('1', paint('36', 'Joker'))}${paint('1', ' cli')} ${paint('2', '— terminal coding agent over the Joker runtime')}
...
Config:
  ~/.joker/config.json merges under flags and env vars.
`);
}

/** Entry point. Returns the process exit code. */
export async function runCli(argv: string[], bin = 'joker'): Promise<number> {
  ...
  try {
    ...
    switch (parsed.command) {
      ...
      case 'resume': {
        ...
        if (!conversationId) {
          console.error(paint('31', 'no conversation id given and none found; pass one: joker resume <id>'));
          return 1;
        }
        await runSession({
          client,
...
const LAST_SESSION_FILE = '.joker-last-session';

/** Persists the last conversation id per project so `joker resume` works bare. */
