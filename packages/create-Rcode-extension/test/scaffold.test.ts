...
describe('create-Rcode-extension', () => {
  it.each([
    ['node', ['.gitignore', 'Rcode-extension.json', 'src/extension.ts']],
    ['webview', ['.gitignore', 'Rcode-extension.json', 'src/webview/index.html']],
    ['react', ['.gitignore', 'Rcode-extension.json', 'src/host/extension.ts', 'src/webview/main.tsx']]
  ])('creates an atomic schema-valid %s project', async (template, expectedFiles) => {
    const parent = await mkdtemp(join(tmpdir(), 'Rcode-scaffold-test-'))
    ...
    const manifest = JSON.parse(await readFile(join(targetDirectory, 'Rcode-extension.json'), 'utf8'))
  ...
  it('builds a framework-neutral Webview with browser-resolvable bundled assets', async () => {
    const parent = await mkdtemp(join(repositoryRoot, '.Rcode-scaffold-build-'))
    temporaryDirectories.push(parent)
    ...
    expect(html).not.toContain('@Rcode/extension-api')
    ...
    for (const path of javascriptFiles) {
      ...
      expect(source).not.toContain('@Rcode/extension-api')
