...
/**
 * Codex-style MCP management: `rcode mcp <add|list|remove|tools|test> ...`
...
async function mcpList(client: AgentClient): Promise<void> {
  ...
  if (!servers.length) {
    console.log('No MCP servers configured. Add one: rcode mcp add <name> <command-or-url>');
    return;
  }
...
async function mcpAdd(client: AgentClient, args: string[]): Promise<void> {
  ...
  if (!name || !target) {
    throw new Error('usage: rcode mcp add <name> <command-or-url>');
  }
...
async function mcpRemove(client: AgentClient, id: string | undefined): Promise<void> {
  if (!id) throw new Error('usage: rcode mcp remove <id>');
...
async function mcpTools(client: AgentClient, id: string | undefined): Promise<void> {
  if (!id) throw new Error('usage: rcode mcp tools <id>');
...
async function mcpTest(client: AgentClient, id: string | undefined): Promise<void> {
  if (!id) throw new Error('usage: rcode mcp test <id>');
