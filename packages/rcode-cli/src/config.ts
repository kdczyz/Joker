 ...
 *   defaults → ~/.rcode/config.json → env vars → CLI flags
...
const CONFIG_DIR = join(homedir(), '.rcode');
...
export function resolveConfig(overrides: CliOverrides = {}): CliConfig {
  ...
  return {
    baseUrl: overrides.baseUrl ?? user.baseUrl ?? process.env.RCODE_URL ?? 'http://127.0.0.1:8787',
