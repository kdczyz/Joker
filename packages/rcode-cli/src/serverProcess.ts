...
/**
 * Locates the Rcode server runtime. The CLI never embeds a second runtime —
...
export function findServerEntrypoint(): ServerEntrypoint {
  ...
  // still find a sibling checkout of Rcode.
...
export async function ensureServer(options: {
  ...
  if (!options.autoStart) {
    throw new Error(`Rcode server is not reachable at ${options.baseUrl}. Start it or pass --url.`);
  }
  ...
  const logPath = join(findProjectRoot(), '.cache', 'rcode-cli-server.log');
  ...
  throw new Error('Timed out waiting for the Rcode server to start.');
}
 ...
 */
const TOKEN_FILE = join(homedir(), '.rcode', 'server-tokens.json');
