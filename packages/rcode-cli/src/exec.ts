...
export function humanizeError(message: string): string {
  if (/\bfetch failed\b/i.test(message)) {
    return `${message} — 无法连接上游模型接口。请检查网络/代理，或在 Rcode GUI 的 AI 接口设置中确认 baseUrl 与密钥（CLI 与 GUI 共用同一 provider 配置）。`;
  }
  ...
  if (/not configured|missing key/i.test(message)) {
    return `${message} — 先在 Rcode 桌面端完成 provider 配置，或用 --model 覆盖默认模型。`;
  }
  return message;
}
...
/**
 * Headless one-shot execution (`rcode exec "<task>"`), the scripting/CI
