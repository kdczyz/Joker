...
Framework-neutral, stable public contracts and Host client for Rcode extensions.
The package has no React or Electron dependency. `ExtensionManifestSchema` is the
canonical runtime source for `schema/Rcode-extension.schema.json`.
...
npm install @Rcode/extension-api@^1.2.0
```
...
[Manifest JSON Schema](./schema/Rcode-extension.schema.json)
