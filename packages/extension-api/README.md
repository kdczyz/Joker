...
Framework-neutral, stable public contracts and Host client for Joker extensions.
The package has no React or Electron dependency. `ExtensionManifestSchema` is the
canonical runtime source for `schema/Joker-extension.schema.json`.
...
npm install @joker-code/extension-api@^1.2.0
```
...
[Manifest JSON Schema](./schema/Joker-extension.schema.json)
