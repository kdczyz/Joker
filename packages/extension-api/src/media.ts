...
export const MediaResourceLeaseSchema = z.strictObject({
  ...
  handleId: MediaHandleIdSchema,
  url: z.string().min(24).max(2048).regex(new RegExp('^Joker-media://')),
  mimeType: z.string().min(3).max(128),
 ...
 * side effect; only failures explicitly classified as transient by Joker qualify.
 */
    ...
    for (const [key, child] of Object.entries(current.value as Record<string, unknown>)) {
      if (key === 'target_url' && (
        ...
        context.addIssue({
          code: 'custom',
          path: ['content'],
          message: 'OTIO media references must use bounded opaque Joker-media URLs'
        })
        return
      }
...
export const MediaVisualModelInstallReceiptSchema = z.strictObject({
  broker: z.literal('Joker-model-broker'),
...
export const MediaAnalyzeVisualFramesResultSchema = z.discriminatedUnion('outcome', [
  z.strictObject({
    ...
    provenance: z.strictObject({
      algorithm: z.literal('Joker.rgb-edge-features'),
