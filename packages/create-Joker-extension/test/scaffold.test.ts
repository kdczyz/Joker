...
describe('create-Joker-extension', () => {
  it.each([
    ['node', ['.gitignore', 'Joker-extension.json', 'src/extension.ts']],
    ['webview', ['.gitignore', 'Joker-extension.json', 'src/webview/index.html']],
    ['react', ['.gitignore', 'Joker-extension.json', 'src/host/extension.ts', 'src/webview/main.tsx']]
  ])('creates an atomic schema-valid %s project', async (template, expectedFiles) => {
    const parent = await mkdtemp(join(tmpdir(), 'Joker-scaffold-test-'))
    ...
    const manifest = JSON.parse(await readFile(join(targetDirectory, 'Joker-extension.json'), 'utf8'))
  ...
  it('builds a framework-neutral Webview with browser-resolvable bundled assets', async () => {
    const parent = await mkdtemp(join(repositoryRoot, '.Joker-scaffold-build-'))
    temporaryDirectories.push(parent)
    ...
    expect(html).not.toContain('@joker-code/extension-api')
    ...
    for (const path of javascriptFiles) {
      ...
      expect(source).not.toContain('@joker-code/extension-api')
