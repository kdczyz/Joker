...
cyan "Smoking packaged Joker Video Editor native workflow..."
npm run smoke:packaged-video-editor-native \
  || die "Windows packaged Joker Video Editor native workflow smoke failed"
...
collect "Windows exe" "dist/Joker-*-win-*.exe"
collect "Windows blockmap" "dist/Joker-*-win-*.exe.blockmap"
...
if $PUBLISH; then
  ...
  cyan "Release remains draft — publish only after macOS, Windows, Linux, evidence, and .Jokerx assets are ready."
fi
