...
const JOKER_NAMED_RELEASE_ASSET = /^Joker-/

const FINAL_ARTIFACTS = [
  {
    platform: 'darwin',
    role: 'mac-arm64-dmg',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-mac-arm64\\.dmg$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-mac-arm64\\.dmg\\.blockmap$`)
  },
  {
    platform: 'darwin',
    role: 'mac-arm64-zip',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-mac-arm64\\.zip$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-mac-arm64\\.zip\\.blockmap$`)
  },
  {
    platform: 'darwin',
    role: 'mac-x64-dmg',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-mac-x64\\.dmg$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-mac-x64\\.dmg\\.blockmap$`)
  },
  {
    platform: 'darwin',
    role: 'mac-x64-zip',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-mac-x64\\.zip$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-mac-x64\\.zip\\.blockmap$`)
  },
  {
    platform: 'win32',
    role: 'win-x64-exe',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-win-x64\\.exe$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-win-x64\\.exe\\.blockmap$`)
  },
  {
    platform: 'linux',
    role: 'linux-x64-appimage',
    pattern: new RegExp(`^Joker-(${VERSION_PART})-linux-x86_64\\.AppImage$`),
    ancillaryPattern: new RegExp(`^Joker-(${VERSION_PART})-linux-x86_64\\.AppImage\\.blockmap$`)
  }
]
...
export async function verifyNativeEvidenceBundle({
  ...
  for (const file of files) {
    const name = basename(file)
    if (!JOKER_NAMED_RELEASE_ASSET.test(name)) continue
    ...
    throw new Error(`Downloaded release contains unexpected Joker-named asset: ${name}`)
  }
