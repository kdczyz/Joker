...
const extensionRoot = join(root, 'examples', 'extensions', 'rcode-video-editor')
const manifestPath = join(extensionRoot, 'Rcode-extension.json')
...
export function videoEditorArchiveName(manifest) {
  if (manifest?.name !== 'rcode-video-editor') {
    throw new Error(`Expected the rcode-video-editor manifest, got: ${String(manifest?.name)}`)
  }
  if (typeof manifest.version !== 'string' ||
      !/^[0-9A-Za-z][0-9A-Za-z._-]*$/.test(manifest.version)) {
    throw new Error(`Invalid Rcode Video Editor version: ${String(manifest?.version)}`)
  }
...
export function videoEditorBundledCatalog(manifest, archiveName, sha256) {
  ...
  if (id !== 'rcode-examples.rcode-video-editor') {
    throw new Error(`Unexpected Rcode Video Editor extension id: ${id}`)
  }
  if (!Array.isArray(manifest.permissions) || manifest.permissions.some((permission) =>
    typeof permission !== 'string' || permission.length === 0
  )) {
    throw new Error('Rcode Video Editor manifest permissions are invalid')
  }
  if (typeof manifest.engines?.Rcode !== 'string' || typeof manifest.apiVersion !== 'string') {
    throw new Error('Rcode Video Editor compatibility metadata is invalid')
  }
  if (!/^[a-f0-9]{64}$/u.test(sha256)) {
    throw new Error(`Invalid Rcode Video Editor archive digest: ${String(sha256)}`)
  }
  return {
    schemaVersion: 1,
    extensions: [{
...
export async function assertDeterministicArchives(first, second) {
  ...
  if (!firstDetails.isFile() || !secondDetails.isFile() ||
    ...
    throw new Error('Rcode Video Editor pack must produce two non-empty regular archives')
  }
  if (firstDetails.size !== secondDetails.size || firstHash !== secondHash) {
    throw new Error(
      `Rcode Video Editor pack is not deterministic: ` +
...
export async function assertStandaloneVideoEditorHostBundle(hostDirectory) {
  ...
  if (!entryDetails.isFile() || entryDetails.isSymbolicLink()) {
    throw new Error(`Rcode Video Editor Host entry must be a regular file: ${entry}`)
  }
  const modules = []
  ...
  await visit(root)
  if (modules.length === 0) throw new Error('Rcode Video Editor Host bundle contains no JavaScript modules')
  ...
  if (bareImports.length > 0) {
    throw new Error(
      `Rcode Video Editor Host bundle is not standalone; unresolved imports: ${bareImports.join(', ')}`
    )
  }
...
export async function findReleaseArchive(input, expectedName) {
  ...
  if (details.isSymbolicLink()) {
    throw new Error(`Rcode Video Editor release input must not be a symlink: ${candidate}`)
  }
  if (details.isFile()) {
  ...
  if (!details.isDirectory()) {
    throw new Error(`Rcode Video Editor release input must be a file or directory: ${candidate}`)
  }
  ...
  const visit = async (directory) => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entryDetails.isSymbolicLink()) {
        if (/^rcode-video-editor-.+\.Rcodex$/u.test(entry.name)) {
          throw new Error(`Rcode Video Editor release archive must not be a symlink: ${path}`)
        }
        continue
      }
      if (entryDetails.isDirectory()) await visit(path)
      else if (entryDetails.isFile() && /^rcode-video-editor-.+\.Rcodex$/u.test(entry.name)) {
  ...
  if (unexpected.length > 0) {
    throw new Error(
      `Downloaded release contains unexpected Rcode Video Editor archives: ${unexpected.join(', ')}`
    )
  }
  if (matches.length !== 1) {
    throw new Error(
...
export async function verifyVideoEditorArchive({ input, run = runRequired } = {}) {
  ...
  if (!details.isFile() || details.isSymbolicLink() || details.size === 0) {
    throw new Error(`Rcode Video Editor release archive must be a non-empty regular file: ${archive}`)
  }
...
export async function assertVideoEditorMatchesBundledArchive({
  ...
  const matches = Array.isArray(catalog?.extensions)
    ? catalog.extensions.filter((entry) => entry?.id === 'rcode-examples.rcode-video-editor')
    : []
  if (catalog?.schemaVersion !== 1 || matches.length !== 1) {
    throw new Error('Bundled extension catalog must contain exactly one Rcode Video Editor entry')
  }
  const entry = matches[0]
  if (entry.version !== version || entry.sha256 !== sha256) {
    throw new Error(
      `Standalone and bundled Rcode Video Editor identities differ: ` +
  ...
  if (typeof entry.archive !== 'string' || basename(entry.archive) !== entry.archive) {
    throw new Error('Bundled Rcode Video Editor archive name is invalid')
  }
  ...
  if (
    ...
    throw new Error('Standalone and bundled Rcode Video Editor archive bytes differ')
  }
...
export async function packVideoEditor({ output, catalog = false } = {}) {
  ...
  const archive = extname(requestedOutput).toLowerCase() === '.Rcodex'
    ? requestedOutput
    : join(requestedOutput, archiveName)
  if (catalog && extname(requestedOutput).toLowerCase() === '.Rcodex') {
    throw new Error('Bundled catalog output must be a directory')
  }
  if (basename(archive) !== archiveName) {
    throw new Error(`Rcode Video Editor release archive must be named ${archiveName}`)
  }

  await mkdir(dirname(archive), { recursive: true, mode: 0o700 })
  const temporary = await mkdtemp(join(dirname(archive), '.rcode-video-editor-pack-'))
  const first = join(temporary, 'first.Rcodex')
  const second = join(temporary, 'second.Rcodex')
  try {
    runRequiredNpm({
      label: 'Rcode Video Editor build',
...
async function removeStaleVideoEditorArchives(directory, expectedName) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if (entry.name === expectedName || !/^rcode-video-editor-.+\.Rcodex$/u.test(entry.name)) continue
...
async function main() {
  ...
  if (verify) {
    ...
    process.stdout.write(
      `Rcode Video Editor release archive OK: ${relative(root, result.archive).split(sep).join('/')}, ` +
  ...
  if (requireBundledIdentity) await assertVideoEditorMatchesBundledArchive(result)
  process.stdout.write(
    `Rcode Video Editor deterministic pack OK: ` +
