...
const SUCCESS_MARKER = 'Packaged Joker Video Editor desktop E2E OK ('
const DEFAULT_TIMEOUT_MS = 180_000
const DEFAULT_JOB_TIMEOUT_MS = 120_000
const MAX_CLEANUP_TIMEOUT_MS = 15_000
const MODEL_NAME = 'Joker-video-editor-desktop-e2e'
...
async function main() {
  ...
  if (!runtimeExecutable) {
    throw new Error(
      `The packaged Joker application at ${resourcesDir} is not host-native for ${process.arch}`
    )
  }
  ...
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'Joker-video-editor-desktop-e2e-'))
  ...
  try {
    ...
    isolatedEnvironment.JOKER_FFMPEG_PATH = mediaExecutables.ffmpeg
    isolatedEnvironment.JOKER_FFPROBE_PATH = mediaExecutables.ffprobe
    ...
    let snapshot = await waitForGuestSnapshot(
      ...
      (value) => value.ready &&
        ...
        value.text.includes('Joker 视频剪辑'),
      'Chinese/light Joker Video Editor View',
      timeoutMs
    )
    ...
    snapshot = await waitForGuestSnapshot(
      ...
      'English/dark View update from Joker settings',
      timeoutMs
    )
    assertNoGuestErrors(snapshot, 'following English/dark Joker settings')
    ...
    if (process.env.JOKER_KEEP_VIDEO_EDITOR_DESKTOP_E2E === '1') {
  ...
  if (primaryError || cleanupErrors.length > 0) {
    const message = primaryError instanceof Error
      ...
      : primaryError === undefined
        ? 'Joker Video Editor desktop E2E cleanup failed'
        : String(primaryError)
...
async function resolveVideoEditorArchive(resourcesDir, explicit) {
  if (explicit) {
    ...
    assertRegularFile(archive, 'explicit Joker Video Editor .Jokerx')
    if (!archive.endsWith('.Jokerx')) throw new Error(`Video editor archive must end with .Jokerx: ${archive}`)
    return archive
  }
  ...
  assertRegularFile(archive, 'packaged bundled Joker Video Editor .Jokerx')
  return archive
}
...
function createVideoFixture(ffmpegPath, output, cwd, timeoutMs) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      ...
      'Install an FFmpeg build with libx264 and AAC, or set JOKER_FFMPEG_PATH. ' +
...
async function installNativeDialogStubs(electronApplication, {
  ...
  await electronApplication.evaluate(({ dialog }, queues) => {
    ...
    globalThis.__JokerVideoEditorDesktopE2eDialogs = state
...
async function findWorkbenchWindow(electronApplication, timeoutMs) {
  return pollUntil(async () => {
    for (const window of electronApplication.windows()) {
      try {
        if (await window.evaluate(() => typeof globalThis.JokerGui === 'object')) return window
      } catch {
        // Window may still be navigating.
      }
    }
    return undefined
  }, { timeoutMs, description: 'packaged Joker workbench window' })
}
...
async function openUntrustedVideoEditor(
  ...
  if (await hasVideoEditorGuest(electronApplication)) {
    throw new Error('Joker Video Editor opened before the first-launch workspace trust review')
  }
  ...
  const authorize = card.getByRole('button', { name: /^(?:授权后打开 Joker 视频编辑器|Authorize to open Joker Video Editor)$/ })
  ...
  const open = card.getByRole('button', { name: /^(?:打开 Joker 视频编辑器|Open Joker Video Editor)$/ })
...
async function assertVideoEditorHiddenFromRightRail(workbench, workspaceRoot, timeoutMs) {
  const discovery = await pollUntil(async () => workbench.evaluate(
    async ({ extensionId, workspaceRoot: currentWorkspaceRoot }) => {
      const result = await globalThis.JokerGui.extensionGetWorkbench({
  ...
  if (!discovery || await workbench.locator(`button[data-contribution-id="${CONTRIBUTION_ID}"]`).count() !== 0) {
    throw new Error('Joker Video Editor must not be displayed in the Code right rail by default')
  }
}
...
async function hasVideoEditorGuest(electronApplication) {
  return electronApplication.evaluate(({ webContents }, extensionId) =>
    webContents.getAllWebContents().some((contents) =>
      ...
      contents.getURL().startsWith(`Joker-extension://${extensionId}/`)
    ), EXTENSION_ID)
}
...
async function evaluateVideoEditorGuest(electronApplication, expression) {
  return electronApplication.evaluate(async ({ webContents }, input) => {
    const guest = webContents.getAllWebContents().find((contents) =>
      contents.getType() === 'webview' &&
      contents.getURL().startsWith(`Joker-extension://${input.extensionId}/`)
    )
    if (!guest || guest.isDestroyed()) throw new Error('Joker Video Editor guest WebContents is unavailable')
...
async function setGuestFormValue(electronApplication, selector, value) {
  ...
  if (!updated) throw new Error(`Cannot set Joker Video Editor form control: ${selector}`)
}
...
async function submitGuestForm(electronApplication, selector) {
  ...
  if (!submitted) throw new Error(`Cannot submit Joker Video Editor form: ${selector}`)
}
...
async function applyWorkbenchSettings(workbench, patch) {
  const result = await workbench.evaluate(async (settingsPatch) => {
    const saved = await globalThis.JokerGui.setSettings(settingsPatch)
    globalThis.dispatchEvent(new CustomEvent('Joker:settings-changed', { detail: saved }))
    return { locale: saved.locale, theme: saved.theme }
  }, patch)
  if (result.locale !== patch.locale || result.theme !== patch.theme) {
    throw new Error(`Joker did not persist locale/theme E2E patch: ${JSON.stringify(result)}`)
  }
}
...
async function startAgentToolTurn(workbench, workspaceRoot) {
  return workbench.evaluate(async ({ workspace, model }) => {
    const request = async (path, method, body) => {
      const response = await globalThis.JokerGui.runtimeRequest(
        path,
        method,
    ...
    const thread = await request('/v1/threads', 'POST', {
      title: 'Joker Video Editor desktop E2E Agent sync',
      workspace,
      model,
      mode: 'agent',
      approvalPolicy: 'auto',
...
async function waitForAgentTurn(workbench, turn, timeoutMs) {
  return pollUntil(async () => {
    const response = await workbench.evaluate(async ({ threadId, turnId }) => {
      return globalThis.JokerGui.runtimeRequest(
  ...
  const server = createHttpServer(async (request, response) => {
    ...
    if (!state.toolCallIssued && state.targetProjectId && projectTool) {
      ...
      for (const frame of openAiTextFrames(
        sawToolResult
          ? 'Joker Video Editor desktop E2E selected the project through the extension tool.'
          : 'Joker Video Editor desktop E2E model fixture is ready.'
      )) response.write(frame)
    }
    response.end()
  })
...
function openAiToolCallFrames({ toolName, argumentsJson }) {
  const id = 'chatcmpl-Joker-video-editor-desktop-e2e'
  return [
    sseFrame({
      id,
...
function openAiTextFrames(text) {
  const id = 'chatcmpl-Joker-video-editor-desktop-e2e-text'
  return [
    sseFrame({
      id,
      object: 'chat.completion.chunk',
...
function assertNoGuestErrors(snapshot, operation) {
  ...
  if (errors.length > 0) {
    throw new Error(
      `Joker Video Editor reported an error while ${operation}: ${JSON.stringify(errors)}. ` +
      ...
      `JOKER_FFMPEG_PATH/JOKER_FFPROBE_PATH values. Guest state: ${guestDiagnostic(snapshot)}`
    )
  }
}
