...
export const ARTIFACT_RULES = {
  darwin: {
    platformLike: /^Joker-.*-mac-/i,
    pattern: new RegExp(`^Joker-${VERSION_PART}-mac-(arm64|x64)\\.(dmg|zip)$`),
    ancillary: new RegExp(`^Joker-${VERSION_PART}-mac-(arm64|x64)\\.(dmg|zip)\\.blockmap$`),
    required: [
      /-mac-arm64\.dmg$/,
      /-mac-arm64\.zip$/,
      /-mac-x64\.dmg$/,
  ...
  win32: {
    platformLike: /^Joker-.*-win-/i,
    pattern: new RegExp(`^Joker-${VERSION_PART}-win-x64\\.exe$`),
    ancillary: new RegExp(`^Joker-${VERSION_PART}-win-x64\\.exe\\.blockmap$`),
    required: [/-win-x64\.exe$/]
  },
  linux: {
    platformLike: /^Joker-.*-linux-/i,
    pattern: new RegExp(`^Joker-${VERSION_PART}-linux-x86_64\\.AppImage$`),
    ancillary: new RegExp(`^Joker-${VERSION_PART}-linux-x86_64\\.AppImage\\.blockmap$`),
...
export function inspectMediaToolchain({
  ...
  const ffmpeg = optionalString(environment.JOKER_FFMPEG_PATH) ?? 'ffmpeg'
  const ffprobe = optionalString(environment.JOKER_FFPROBE_PATH) ?? 'ffprobe'
...
export function resolveEvidenceCommit({
  ...
  const expected = optionalString(environment.JOKER_EVIDENCE_COMMIT) ??
