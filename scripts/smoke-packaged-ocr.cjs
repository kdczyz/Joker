...
const REEXEC_MARKER = 'JOKER_PACKAGED_OCR_SMOKE_REEXEC'
...
function resolveResourcesDir({ root = process.cwd(), environment = process.env } = {}) {
  const resourcesDir = firstExisting([
    environment.JOKER_PACKAGED_RESOURCES_DIR,
    join(root, 'dist', 'linux-unpacked', 'resources'),
    join(root, 'dist', 'win-unpacked', 'resources'),
    join(root, 'dist', 'mac-arm64', 'Joker.app', 'Contents', 'Resources'),
    join(root, 'dist', 'mac', 'Joker.app', 'Contents', 'Resources')
  ].map((candidate) => candidate && resolve(root, candidate)))
  if (!resourcesDir) {
    fail('Could not find packaged app resources. Set JOKER_PACKAGED_RESOURCES_DIR or build a packaged app first.')
  }
  return resourcesDir
}
  ...
  return {
    ...
    options: {
      ...
      env: {
        ...
        JOKER_PACKAGED_RESOURCES_DIR: resolve(resourcesDir),
        [REEXEC_MARKER]: '1'
      },
      shell: false,
      encoding: 'utf8',
