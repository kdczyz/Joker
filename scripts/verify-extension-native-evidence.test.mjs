...
async function fixture(t) {
  const root = await mkdtemp(join(tmpdir(), 'Joker-native-evidence-bundle-test-'))
...
test('rejects inconsistent platform versions and unproved media toolchain capabilities', async (t) => {
  ...
  const oldLinux = join(inconsistent, 'Joker-1.2.3-linux-x86_64.AppImage')
  const newLinux = join(inconsistent, 'Joker-1.2.4-linux-x86_64.AppImage')
...
test('rejects every extra native-looking release asset outside the six-file allowlist', async (t) => {
  for (const name of [
    ...
    'Joker-1.2.3-win-arm64.exe',
    'Joker-1.2.3-win-x64.EXE',
    'Joker-1.2.3-win-x64.MSI',
    'Joker-1.2.3-mac-universal.dmg',
    'Joker-1.2.3-mac-universal.zip'
  ]) {
    await t.test(name, async (t) => {
      const root = await fixture(t)
      if (name === 'Joker-1.2.3-win-x64.EXE') {
        await rm(join(root, 'Joker-1.2.3-win-x64.exe'))
      }
      ...
      await assert.rejects(verifyNativeEvidenceBundle({
        ...
        expectedCommit: COMMIT,
        expectedVersion: VERSION
      }), /unexpected Joker-named asset: Joker-/i)
    })
  }
})
...
test('allows only canonical same-version blockmaps as unrecorded ancillary assets', async (t) => {
  const root = await fixture(t)
  await writeFile(join(root, 'Joker-1.2.3-win-x64.exe.blockmap'), 'canonical blockmap')
  ...
  await writeFile(join(root, 'Joker-9.9.9-linux-x86_64.AppImage.blockmap'), 'stale blockmap')
  await assert.rejects(verifyNativeEvidenceBundle({
    directory: root,
    expectedCommit: COMMIT,
    expectedVersion: VERSION
