...
const extensionRoot = join(root, 'examples', 'extensions', 'joker-video-editor')
const manifestPath = join(extensionRoot, 'Joker-extension.json')
const cliPath = join(root, 'Joker', 'dist', 'cli', 'serve-entry.js')
...
export function videoEditorArchiveName(manifest) {
  if (manifest?.name !== 'joker-video-editor') {
    throw new Error(`Expected the joker-video-editor manifest, got: ${String(manifest?.name)}`)
  }
  if (typeof manifest.version !== 'string' ||
      !/^[0-9A-Za-z][0-9A-Za-z._-]*$/.test(manifest.version)) {
    throw new Error(`Invalid Joker Video Editor version: ${String(manifest?.version)}`)
  }
  return `${manifest.name}-${manifest.version}.Jokerx`
}
...
export function videoEditorBundledCatalog(manifest, archiveName, sha256) {
  ...
  if (id !== 'joker-examples.joker-video-editor') {
    throw new Error(`Unexpected Joker Video Editor extension id: ${id}`)
  }
  if (!Array.isArray(manifest.permissions) || manifest.permissions.some((permission) =>
    typeof permission !== 'string' || permission.length === 0
  )) {
    throw new Error('Joker Video Editor manifest permissions are invalid')
  }
  if (typeof manifest.engines?.Joker !== 'string' || typeof manifest.apiVersion !== 'string') {
    throw new Error('Joker Video Editor compatibility metadata is invalid')
  }
  if (!/^[a-f0-9]{64}$/u.test(sha256)) {
    throw new Error(`Invalid Joker Video Editor archive digest: ${String(sha256)}`)
  }
  return {
    schemaVersion: 1,
    extensions: [{
      ...
      archive: archiveName,
      sha256,
      enginesJoker: manifest.engines.Joker,
...
export async function assertDeterministicArchives(first, second) {
  ...
  if (!firstDetails.isFile() || !secondDetails.isFile() ||
    ...
    throw new Error('Joker Video Editor pack must produce two non-empty regular archives')
  }
  if (firstDetails.size !== secondDetails.size || firstHash !== secondHash) {
    throw new Error(
      `Joker Video Editor pack is not deterministic: ` +
...
export async function assertStandaloneVideoEditorHostBundle(hostDirectory) {
  ...
  if (!entryDetails.isFile() || entryDetails.isSymbolicLink()) {
    throw new Error(`Joker Video Editor Host entry must be a regular file: ${entry}`)
  }
  const modules = []
  ...
  await visit(root)
  if (modules.length === 0) throw new Error('Joker Video Editor Host bundle contains no JavaScript modules')
  ...
  if (bareImports.length > 0) {
    throw new Error(
      `Joker Video Editor Host bundle is not standalone; unresolved imports: ${bareImports.join(', ')}`
    )
  }
...
export async function findReleaseArchive(input, expectedName) {
  ...
  if (details.isSymbolicLink()) {
    throw new Error(`Joker Video Editor release input must not be a symlink: ${candidate}`)
  }
  if (details.isFile()) {
  ...
  if (!details.isDirectory()) {
    throw new Error(`Joker Video Editor release input must be a file or directory: ${candidate}`)
  }
  ...
  const visit = async (directory) => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entryDetails.isSymbolicLink()) {
        if (/^joker-video-editor-.+\.Jokerx$/u.test(entry.name)) {
          throw new Error(`Joker Video Editor release archive must not be a symlink: ${path}`)
        }
        continue
      }
      if (entryDetails.isDirectory()) await visit(path)
      else if (entryDetails.isFile() && /^joker-video-editor-.+\.Jokerx$/u.test(entry.name)) {
  ...
  if (unexpected.length > 0) {
    throw new Error(
      `Downloaded release contains unexpected Joker Video Editor archives: ${unexpected.join(', ')}`
    )
  }
  if (matches.length !== 1) {
    throw new Error(
...
export async function verifyVideoEditorArchive({ input, run = runRequired } = {}) {
  ...
  if (!details.isFile() || details.isSymbolicLink() || details.size === 0) {
    throw new Error(`Joker Video Editor release archive must be a non-empty regular file: ${archive}`)
  }
...
export async function assertVideoEditorMatchesBundledArchive({
  ...
  const matches = Array.isArray(catalog?.extensions)
    ? catalog.extensions.filter((entry) => entry?.id === 'joker-examples.joker-video-editor')
    : []
  if (catalog?.schemaVersion !== 1 || matches.length !== 1) {
    throw new Error('Bundled extension catalog must contain exactly one Joker Video Editor entry')
  }
  const entry = matches[0]
  if (entry.version !== version || entry.sha256 !== sha256) {
    throw new Error(
      `Standalone and bundled Joker Video Editor identities differ: ` +
  ...
  if (typeof entry.archive !== 'string' || basename(entry.archive) !== entry.archive) {
    throw new Error('Bundled Joker Video Editor archive name is invalid')
  }
  ...
  if (
    ...
    throw new Error('Standalone and bundled Joker Video Editor archive bytes differ')
  }
...
export async function packVideoEditor({ output, catalog = false } = {}) {
  ...
  const archive = extname(requestedOutput).toLowerCase() === '.Jokerx'
    ? requestedOutput
    : join(requestedOutput, archiveName)
  if (catalog && extname(requestedOutput).toLowerCase() === '.Jokerx') {
    throw new Error('Bundled catalog output must be a directory')
  }
  if (basename(archive) !== archiveName) {
    throw new Error(`Joker Video Editor release archive must be named ${archiveName}`)
  }

  await mkdir(dirname(archive), { recursive: true, mode: 0o700 })
  const temporary = await mkdtemp(join(dirname(archive), '.joker-video-editor-pack-'))
  const first = join(temporary, 'first.Jokerx')
  const second = join(temporary, 'second.Jokerx')
  try {
    runRequiredNpm({
      label: 'Joker Video Editor build',
...
async function removeStaleVideoEditorArchives(directory, expectedName) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if (entry.name === expectedName || !/^joker-video-editor-.+\.Jokerx$/u.test(entry.name)) continue
...
async function main() {
  ...
  if (verify) {
    ...
    process.stdout.write(
      `Joker Video Editor release archive OK: ${relative(root, result.archive).split(sep).join('/')}, ` +
  ...
  if (requireBundledIdentity) await assertVideoEditorMatchesBundledArchive(result)
  process.stdout.write(
    `Joker Video Editor deterministic pack OK: ` +
