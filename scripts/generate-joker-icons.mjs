#!/usr/bin/env node
/**
 * 根据一张方形源图生成 Joker 桌面端 + Android 端所需的全部图标资源。
 *
 * 用法：
 *   node scripts/generate-joker-icons.mjs <source.png>
 *
...
const BG = '#0f172a' // slate-900, consistent with current Joker dark theme
...
async function main() {
  ...
  // Desktop app icons
  const jokerPng = await createIcon(1024, { rounded: false })
  const jokerMacPng = await createIcon(1024, { rounded: true })
  const trayPng = await createIcon(128, { rounded: true, iconScale: 0.72 })

  writeFileSync(join(PROJECT_ROOT, 'src/asset/img/Joker.png'), jokerPng)
  writeFileSync(join(PROJECT_ROOT, 'src/asset/img/Joker_mac.png'), jokerMacPng)
  writeFileSync(join(PROJECT_ROOT, 'src/asset/img/Joker_tray.png'), trayPng)
  console.log('Wrote src/asset/img/Joker.png, Joker_mac.png, Joker_tray.png')
  ...
  try {
    execFileSync(
      'npx',
      ['--yes', 'png2icons@2.0.1', join(PROJECT_ROOT, 'src/asset/img/Joker_mac.png'), join(PROJECT_ROOT, 'build/icon'), '-icowe', '-bc'],
      { stdio: 'inherit' }
    )
  ...
  for (const d of androidDensities) {
    const dir = join(PROJECT_ROOT, 'Joker_apk/android/app/src/main/res', `mipmap-${d.name}`)
