...
} from './pack-Rcode-video-editor.mjs'

async function temporaryRoot(t) {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-video-editor-pack-test-'))
...
test('derives the stable release asset name from the extension manifest', () => {
  assert.equal(
    videoEditorArchiveName({ name: 'Rcode-video-editor', version: '0.1.0' }),
    'Rcode-video-editor-0.1.0.Rcodex'
  )
  assert.throws(() => videoEditorArchiveName({ name: 'other', version: '0.1.0' }), /Expected/)
  assert.throws(
    () => videoEditorArchiveName({ name: 'Rcode-video-editor', version: '../bad' }),
    /Invalid/
  )
})
...
test('requires standalone release bytes to match the product-bundled archive', async (t) => {
  ...
  const release = join(root, 'Rcode-video-editor-0.1.0.Rcodex')
  const archiveName = 'Rcode-video-editor-0.1.0.Rcodex'
  await mkdir(bundled)
...
test('derives a bounded bundled catalog from the canonical manifest and archive digest', () => {
  ...
  }, 'Rcode-video-editor-0.1.0.Rcodex', digest), {
    schemaVersion: 1,
    extensions: [{
      id: 'Rcode-examples.Rcode-video-editor',
      version: '0.1.0',
  ...
  assert.throws(() => videoEditorBundledCatalog({
    ...
    engines: { Rcode: '*' },
    permissions: []
  }, 'Rcode-video-editor-0.1.0.Rcodex', digest), /Unexpected/)
})

test('accepts only byte-identical deterministic archives', async (t) => {
  const root = await temporaryRoot(t)
  const first = join(root, 'first.Rcodex')
  const second = join(root, 'second.Rcodex')
...
test('rejects Host bundles that retain repository-only runtime imports', async (t) => {
  ...
  await writeFile(
    join(host, 'extension.js'),
    'import { MediaAudioAnalysisResultSchema } from "@Rcode/extension-api"\n'
  )
  await assert.rejects(
    assertStandaloneVideoEditorHostBundle(host),
    /not standalone[\s\S]*@Rcode\/extension-api/u
  )
})

test('finds exactly one regular release archive and rejects duplicate or linked trees', async (t) => {
  const root = await temporaryRoot(t)
  const name = 'Rcode-video-editor-0.1.0.Rcodex'
  ...
  await writeFile(join(nested, 'Rcode-video-editor-9.9.9.Rcodex'), 'stale')
  await assert.rejects(findReleaseArchive(root, name), /unexpected Rcode Video Editor archives/)

  await rm(join(nested, 'Rcode-video-editor-9.9.9.Rcodex'))
  await symlink(release, join(nested, 'Rcode-video-editor-9.9.9.Rcodex'))
