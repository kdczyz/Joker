...
release_export_update_channel() {
  ...
  export JOKER_UPDATE_CHANNEL="${RELEASE_CHANNEL}"
...
release_load_local_env() {
  local env_file="${JOKER_RELEASE_ENV:-${DEEPSEEK_GUI_RELEASE_ENV:-}}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="Joker ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_export_app_version() {
  ...
  export JOKER_APP_VERSION="${RELEASE_VERSION}"
  export DEEPSEEK_GUI_APP_VERSION="${RELEASE_VERSION}"
  cyan "  App:     ${JOKER_APP_VERSION}"
}
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/Joker-* "${ROOT}"/dist/DeepSeek-GUI-* "${ROOT}"/dist/DeepSeek\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
  rm -f "${ROOT}"/dist/extension-native-evidence-*.json "${ROOT}"/dist/Joker-video-editor-*.Jokerx
}
