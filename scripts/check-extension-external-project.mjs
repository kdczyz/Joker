...
const temporaryRoot = await mkdtemp(join(tmpdir(), 'Joker-extension-external-release-'))
...
const archivePath = join(temporaryRoot, 'Joker-release-fixtures.external-release-1.0.0.Jokerx')
...
try {
  ...
  const JokerArtifact = await packPublishableJokerCli(artifactsDirectory)
  artifacts.set('Joker', JokerArtifact)
  ...
  const cliEntry = join(projectDirectory, 'node_modules/Joker/dist/cli/serve-entry.js')
  const validateSource = runJokerCli(cliEntry, ['validate', '.', '--json'])
  assertJsonValue(validateSource.stdout, ['result', 'id'], 'Joker-release-fixtures.external-release')
  const packed = runJokerCli(cliEntry, ['pack', '.', '--output', archivePath, '--json'])
  assertJsonValue(packed.stdout, ['result', 'id'], 'Joker-release-fixtures.external-release')
  const validateArchive = runJokerCli(cliEntry, ['validate', archivePath, '--json'])
  assertJsonValue(validateArchive.stdout, ['result', 'id'], 'Joker-release-fixtures.external-release')
  const installed = runJokerCli(cliEntry, [
    'install',
    archivePath,
    '--data-dir',
    profileDirectory,
  ...
  assertJsonValue(installed.stdout, ['result', 'id'], 'Joker-release-fixtures.external-release')
  const listed = runJokerCli(cliEntry, ['list', '--data-dir', profileDirectory, '--json'])
  ...
  const doctor = runJokerCli(cliEntry, [
    'doctor',
    'Joker-release-fixtures.external-release',
    '--data-dir',
    profileDirectory,
    '--json'
  ])
  assertJsonValue(doctor.stdout, ['healthy'], true)
  const uninstalled = runJokerCli(cliEntry, [
    'uninstall',
    'Joker-release-fixtures.external-release',
    '--data-dir',
    profileDirectory,
  ...
  const installedApi = JSON.parse(
    await readFile(join(projectDirectory, 'node_modules/@joker-code/extension-api/package.json'), 'utf8')
  )
  ...
  if (process.env.JOKER_KEEP_EXTERNAL_RELEASE_PROJECT === '1') {
...
async function packPublishableJokerCli(destination) {
  const sourceManifest = JSON.parse(await readFile(join(root, 'Joker/package.json'), 'utf8'))
  ...
  const scaffoldManifest = JSON.parse(
    await readFile(join(root, 'packages/create-Joker-extension/package.json'), 'utf8')
  )
  const stage = join(temporaryRoot, 'Joker-cli-package')
  await mkdir(stage, { recursive: true })
  await Promise.all([
    cp(join(root, 'Joker/dist'), join(stage, 'dist'), { recursive: true }),
    cp(join(root, 'Joker/README.md'), join(stage, 'README.md'))
  ])
  const manifest = {
    ...sourceManifest,
    private: false,
    ...
    dependencies: {
      ...
      '@joker-code/extension-api': apiManifest.version,
      'create-Joker-extension': scaffoldManifest.version
    }
  }
  assertPublishableManifest(manifest, 'Joker packaged CLI')
  await writeFile(join(stage, 'package.json'), `${JSON.stringify(manifest, null, 2)}\n`)
  return packPackage(stage, destination, 'Joker packaged CLI')
}

function localTarballSpecifier(project, tarball) {
  const path = relative(project, tarball).split(sep).join('/')
  if (!path.endsWith('.tgz')) throw new Error(`Joker dependency is not a tarball: ${tarball}`)
...
async function assertExternalLockfile(project) {
  ...
  if (lockText.split('\\').join('/').includes(normalizedRoot)) {
    throw new Error('External project lockfile references the Joker source tree')
  }
  const lock = JSON.parse(lockText)
  for (const name of [
    '@joker-code/extension-api',
...
function runJokerCli(cliEntry, args) {
  return runRequiredCommand({
    label: `packaged CLI: Joker extension ${args[0]}`,
    command: process.execPath,
    args: [cliEntry, 'extension', ...args],
    cwd: projectDirectory,
    capture: true,
    env: { JOKER_DATA_DIR: profileDirectory }
  })
}
