...
async function fixture(arch) {
  const root = join(tmpdir(), `Joker-packaged-macos-${process.pid}-${Date.now()}-${Math.random()}`)
  const resources = join(root, 'Joker.app', 'Contents', 'Resources')
  ...
  await mkdir(join(root, 'Joker.app', 'Contents', 'MacOS'), { recursive: true })
  ...
  await writeFile(join(root, 'Joker.app', 'Contents', 'MacOS', 'Joker'), 'main')
