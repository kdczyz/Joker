#!/usr/bin/env bash
# watch-rcode-logs.sh — 实时监控 Rcode (Electron) 运行日志
#
# 日志由 src/main/logger.ts 写入,按天分文件:
#   ~/Library/Application Support/Rcode/logs/Rcode-YYYY-MM-DD.log
# 行格式: [ISO时间戳] [级别] [分类] 消息
#   级别: ERROR / WARN / INFO / STDERR(子进程标准错误)
#
# 用法见 ./watch-rcode-logs.sh --help

set -euo pipefail

LOG_HOME="$HOME/Library/Application Support/Rcode/logs"
TODAY="Rcode-$(date +%F).log"
LEVELS=""
ALL=""
NO_COLOR=""
LINES=30
...
用法:
  ./watch-rcode-logs.sh [选项]

选项:
  -l, --level LEVELS   只显示指定级别,逗号分隔: error,warn,info,stderr
  -a, --all            跟踪所有 Rcode-*.log (不仅是今天的)
  -n, --lines N        启动先显示最近 N 行 (默认 30)
      --no-color       关闭颜色高亮
  -h, --help           显示本帮助

示例:
  ./watch-rcode-logs.sh                  # 跟踪今天的日志,带颜色
  ./watch-rcode-logs.sh -l error,warn    # 只看 error 和 warn
  ./watch-rcode-logs.sh -a -l stderr     # 所有日期,只看子进程 stderr
EOF
}
...
if [[ ! -d "$LOG_HOME" ]]; then
  ...
  echo "(Rcode 可能还没运行过,或日志目录已被移动)" >&2
  exit 1
fi

if [[ -n "$ALL" ]]; then
  shopt -s nullglob
  FILES=("$LOG_HOME"/Rcode-*.log)
  shopt -u nullglob
  if [[ ${#FILES[@]} -eq 0 ]]; then
    echo "目录下没有任何 Rcode-*.log 文件" >&2
    exit 1
  fi
else
