...
import type { RuntimeTuningConfig } from '../config/Rcode-config.js'
...
export function createChildAgentExecutor(options: ChildAgentExecutorOptions): ChildRunExecutor {
  return async (input) => {
    ...
    // A custom system prompt augments the base prefix (Rcode tool/safety
    ...
    const abortChild = (): void => {
      console.warn(`[Rcode] foreground subagent parent abort received child=${thread.id} turn=${started.turnId} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
    ...
    if (input.signal.aborted) {
      console.warn(`[Rcode] foreground subagent started with aborted parent signal child=${thread.id} turn=${started.turnId}`)
      abortChild()
    } else {
      console.warn(`[Rcode] foreground subagent abort bridge armed child=${thread.id} turn=${started.turnId} parentThread=${input.parentThreadId} parentTurn=${input.parentTurnId}`)
    ...
    console.warn(`[Rcode] foreground subagent turn settled child=${thread.id} turn=${started.turnId} status=${status}`)
