...
export const DESIGN_REVIEWER_PROFILE: SubagentProfileConfig = {
  ...
  toolPolicy: 'readOnly',
  promptPreamble: [
    '你是 Rcode 内置的设计审查者，以只读方式审查前端代码与原型的视觉与交互质量。',
...
export const OVER_ENGINEERING_REVIEWER_PROFILE: SubagentProfileConfig = {
  ...
  toolPolicy: 'readOnly',
  promptPreamble: [
    '你是 Rcode 内置的「过度设计审查者」，以只读方式审查代码的过度设计与不必要的复杂度——只找“能删什么、能用标准库/平台能力替换什么”，',
...
export const GENERAL_PROFILE: SubagentProfileConfig = {
  ...
  promptPreamble: [
    '你是 Rcode 内置的「通用代理」(General)。你能研究复杂问题并执行多步骤任务,',
...
export const EXPLORE_PROFILE: SubagentProfileConfig = {
  ...
  promptPreamble: [
    '你是 Rcode 内置的「探索代理」(Explore),一个快速的只读代码库代理。',
...
export const COMPONENT_DESIGNER_PROFILE: SubagentProfileConfig = {
  ...
  promptPreamble: [
    '你是 Rcode 内置的「组件交互设计代理」(Component Designer)。',
    ...
    '产物必须是完整的 standalone HTML，包含 `<meta name="Rcode-component-prototype" content="1">`，',
    '且唯一的可见演示根节点带 `data-Rcode-component-root`；CSS 与 JavaScript 全部内联。',
    '完成后简洁说明关键交互状态和写入路径。'
  ].join('')
}
