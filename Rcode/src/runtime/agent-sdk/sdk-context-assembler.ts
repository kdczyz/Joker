/**
 * Pure assembly of the per-turn context that Rcode injects into a subscription
 * (Claude Agent SDK) turn. The SDK owns the loop, but — unlike Rcode's native
 * loop — it does NOT see Rcode's conversation history, skill catalog, memories, or
 ...
 *
 * Design: Rcode owns the canonical history, so every SDK turn is stateless from
 * Rcode's side — we replay the prior conversation as a transcript preamble each
