/**
 * Translates the Claude Agent SDK message stream into Rcode's native runtime
 * events, so a subscription turn renders in the GUI exactly like a turn driven
 * by Rcode's own agent loop. This is the load-bearing half of the fusion: the SDK
 * owns the loop, but every assistant token, tool call, and tool result it emits
 * is re-projected onto Rcode's event contract.
 *
 ...
 *
 * Streaming model (mirrors Rcode's native loop exactly, or the GUI double-renders):
 * a Rcode `assistant_text_delta` event carries an INCREMENTAL CHUNK (the GUI
...
export interface SdkTurnFinal {
  ...
  message?: string
  /** Stable Rcode error code when the SDK reports a native-equivalent limit. */
...
/** Claude Code built-in tool names that imply a richer Rcode tool kind. */
...
/** Collapse an SDK tool_result content payload into a Rcode tool output value. */
...
/**
 * Map the Agent SDK's per-request usage onto Rcode's UsageSnapshot. Anthropic's
