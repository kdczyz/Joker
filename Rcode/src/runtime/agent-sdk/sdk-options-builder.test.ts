...
describe('buildClaudeSystemPrompt', () => {
  test('appends Rcode persona onto the claude_code preset', () => {
    const sp = buildClaudeSystemPrompt('You are Rcode.', 'Persona: terse.')
    expect(sp).toEqual({ type: 'preset', preset: 'claude_code', append: 'You are Rcode.\n\nPersona: terse.' })
  })
  test('omits persona when absent', () => {
    expect(buildClaudeSystemPrompt('You are Rcode.').append).toBe('You are Rcode.')
  })
})
...
describe('buildCanUseTool', () => {
  test('allow passes through, with optional updatedInput', async () => {
    ...
    // when Rcode doesn't rewrite, it echoes the original input through.
...
describe('assembleSdkOptions', () => {
  const base = {
    cwd: '/ws',
    RcodeSystemPrompt: 'You are Rcode.',
    approvalPolicy: 'on-request' as const,
    bridgedToolModelNames: ['mcp__Rcode__generate_image', 'mcp__Rcode__memory_create'],
    baseEnv: { ANTHROPIC_API_KEY: 'k', PATH: '/bin' },
    oauthToken: 'sk-ant-oat01-z'
  }

  test('unions SDK built-ins with bridged Rcode tools and turns on partial streaming', () => {
    ...
    expect((opts.systemPrompt as { append: string }).append).toBe('You are Rcode.')
