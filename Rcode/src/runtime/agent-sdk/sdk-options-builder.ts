/**
 * Assembles the `query()` options for a Rcode subscription turn — the glue that
 * injects Rcode's brain (persona, tools, permissions) into the SDK's loop.
 *
 * The assembly is pure and unit-tested. The two callbacks it carries
 * (`canUseTool`, hook callbacks) are factories that close over Rcode's real
 ...
 * deliberately did NOT bridge from Rcode). Listed in allowedTools so they are
...
/**
 * Claude Code built-in tools we suppress on the Rcode-driven SDK path.
 * AskUserQuestion has no UI in this embedding (the model would ask and get no
 * answer); Rcode's own bridged `user_input` panel handles interactive questions.
 */
...
/**
 * Map Rcode's ApprovalPolicy onto the SDK permission mode. Rcode's fine-grained
...
/**
 * Claude Code (the subscription engine) only accepts Anthropic models. A Rcode
...
/** Compose Rcode's persona append text for the claude_code system-prompt preset. */
export function buildClaudeSystemPrompt(
  RcodeSystemPrompt: string,
  threadPersona?: string
): SdkSystemPromptPreset {
  const base = RcodeSystemPrompt.trim()
...
/** Rcode's permission decision for a (toolName, input) pair on the active turn. */
...
/**
 * Bridge Rcode's approval engine to the SDK `canUseTool` callback. Every tool the
 * SDK is about to run is adjudicated by Rcode (which can route to the GUI
  ...
  return async (toolName, input): Promise<SdkPermissionResult> => {
    ...
    try {
      ...
      if (decision.allow) {
        ...
        // Echo the original input through when Rcode doesn't rewrite it.
        return { behavior: 'allow', updatedInput: decision.updatedInput ?? safeInput }
      }
      // The deny variant requires a non-empty `message`.
      return {
        behavior: 'deny',
        message: decision.message ?? 'Denied by Rcode permission policy',
...
export interface AssembleSdkOptionsParams {
  ...
  sandboxMode?: SandboxMode
  planMode?: boolean
  /** `mcp__Rcode__*` names from the tool bridge. */
  ...
  /** Native Rcode maxSteps mapped onto the Agent SDK's loop ceiling. */
  maxTurns?: number
}
...
export function assembleSdkOptions(params: AssembleSdkOptionsParams): SdkQueryOptions {
  ...
  const options: SdkQueryOptions = {
    cwd: params.cwd,
    systemPrompt: buildClaudeSystemPrompt(params.RcodeSystemPrompt, params.threadPersona),
    ...
    // native Read/Write/Edit/Bash alongside the structured Rcode MCP tools.
    ...
    // Only load Rcode-provided config; don't auto-absorb the host's ~/.claude.
