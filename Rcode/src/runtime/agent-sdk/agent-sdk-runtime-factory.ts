/**
 * Binds the decoupled {@link AgentSdkRuntime} to Rcode's real runtime services.
 * This is the only place that touches the SDK package and Rcode's concrete stores,
...
import type { BridgeableTool, RcodeToolResult } from './sdk-tool-bridge.js'
...
import type { ServeProviderConfig } from '../../config/Rcode-config.js'
...
export interface AgentSdkRuntimeFactoryDeps {
  ...
  /**
   * The canonical host boundary for bridged Rcode tool execution. Serve always
  ...
  /** Native Rcode AGENTS.md instruction engine — injects global/workspace instructions per turn. */
  instructionRuntime?: InstructionRuntime
  /** Long-term memory store — injects relevant memories per turn. */
  memoryStore?: MemoryStore
  /** Interactive-input gate — lets the bridged `user_input` tool surface Rcode's GUI panel. */
...
/** Lazily load the real SDK without a static import (so Rcode typechecks without it). */
 ...
 * GUI plan — the SDD "下一步"/Plan-mode flow — we must expose it so the Rcode
...
export function createAgentSdkRuntime(deps: AgentSdkRuntimeFactoryDeps): AgentSdkRuntime {
  ...
  // resume from it: Rcode owns the canonical history and replays it as a transcript
  ...
  /**
   * Bridge Rcode's `user_input` tool to its GUI panel: persist the request item +
  ...
  const toolContext = (
    ...
    // (executeRcodeTool) on plan turns — both are gated on it.
    ...
    // Wire interactive input to Rcode's GUI panel (advertises `user_input`).
  ...
  const runtimeDeps: SdkRuntimeDeps = {
    ...
    async loadTurnContext(threadId, turnId): Promise<SdkTurnContext | null> {
      ...
      // Plan turns expose create_plan (and narrow Rcode tools to the plan-allowed
      ...
      // by skills visible in this workspace; executeRcodeTool still re-resolves
      ...
      // The SDK doesn't see Rcode's history or per-turn context, so assemble both
      // here (parity with the native loop's `contextInstructions`). Rcode owns the
    ...
    async executeRcodeTool(threadId, turnId, toolName, args, signal): Promise<RcodeToolResult> {
      const thread = await deps.threadStore.get(threadId)
      const turn = thread?.turns.find((candidate) => candidate.id === turnId)
      ...
      if (!deps.toolHost) {
        return { output: 'Rcode tool host is unavailable; tool execution was denied', isError: true }
      }
      ...
      try {
        ...
        if (result.item.kind !== 'tool_result') {
          return {
            output: `Rcode tool ${toolName} returned an invalid result item`,
            isError: true
          }
        }
    ...
    async decideToolApproval(threadId, turnId, toolName, input, signal): Promise<ToolApprovalDecision> {
      // Bridged Rcode tools perform their own per-tool policy check through the
      // LocalToolHost context above; asking here too would create two prompts.
      if (toolName.startsWith('mcp__Rcode__')) return { allow: true }
      const thread = await deps.threadStore.get(threadId)
      const turn = thread?.turns.find((candidate) => candidate.id === turnId)
    ...
    RcodeSystemPrompt: () => deps.prefix.systemPrompt,
