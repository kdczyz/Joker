...
import type { ObservabilityConfig } from '../config/Rcode-config.js'
...
export class AgentObservabilityRecorder implements RuntimeEventObserver {
    ...
    switch (event.kind) {
      ...
      case 'model_request_retry':
        this.addTurnEvent(event.threadId, event.turnId, 'gen_ai.model.retry', {
          ...
          'Rcode.retry.attempt': event.attempt,
          'Rcode.retry.max_attempts': event.maxAttempts,
          'Rcode.retry.delay_ms': event.delayMs
        }, event.timestamp)
        return
      case 'pipeline_stage':
        this.addTurnEvent(event.threadId, event.turnId, 'Rcode.pipeline_stage', {
          'Rcode.pipeline.stage': event.stage,
          ...(event.label ? { 'Rcode.pipeline.label': event.label } : {})
        }, event.timestamp)
        return
      case 'usage':
      ...
      case 'error':
        this.addTurnEvent(event.threadId, event.turnId, 'exception', {
          ...
          ...(event.severity ? { 'Rcode.error.severity': event.severity } : {})
        }, event.timestamp)
        return
      default:
        return
  ...
  private startTurn(event: RuntimeEvent): void {
    ...
    this.turns.set(key, {
      ...
      attributes: {
        'service.name': 'Rcode-runtime',
        'gen_ai.operation.name': 'chat',
        'Rcode.thread.id': event.threadId,
        'Rcode.turn.id': event.turnId
      },
      events: []
    })
  }
  ...
  private recordTtft(event: RuntimeEvent): void {
    ...
    span.attributes['Rcode.ttft_ms'] = Math.max(0, timestampMs(event.timestamp) - span.startMs)
  }
  ...
  private async recordItemEvent(event: RuntimeEvent): Promise<void> {
    ...
    if (item.kind === 'tool_call' && event.kind === 'item_created') {
      ...
      this.tools.set(key, {
        ...
        name: 'Rcode.tool ' + item.toolName,
        kind: 'client',
        startMs: timestampMs(event.timestamp),
        startTimeUnixNano: unixNano(event.timestamp),
        attributes: {
          'tool.name': item.toolName,
          'Rcode.tool.call_id': item.callId,
          'Rcode.tool.kind': item.toolKind,
          'Rcode.thread.id': item.threadId,
          'Rcode.turn.id': item.turnId
        },
        events: []
      })
      return
    ...
    if (item.kind === 'tool_result' && (event.kind === 'item_created' || event.kind === 'item_completed')) {
      ...
      if (!span) return
      span.attributes['Rcode.tool.is_error'] = item.isError
...
function usageAttributes(usage: UsageSnapshot, model: string | undefined): Record<string, AgentObservabilityAttributeValue> {
  return {
    ...
    ...(usage.cacheMissTokens !== undefined ? { 'Rcode.cache.miss_tokens': usage.cacheMissTokens } : {}),
    ...(usage.cacheHitRate !== null && usage.cacheHitRate !== undefined ? { 'Rcode.cache.hit_rate': usage.cacheHitRate } : {}),
    ...(usage.costUsd !== undefined ? { 'Rcode.usage.cost_usd': usage.costUsd } : {}),
    ...(usage.costCny !== undefined ? { 'Rcode.usage.cost_cny': usage.costCny } : {})
  }
}
