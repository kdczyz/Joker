...
function probeHybridThreadStoreWrites(db: BetterSqliteDatabase): boolean {
  ...
  try {
    ...
    db.exec('SAVEPOINT Rcode_doctor_write_probe')
    savepointStarted = true
    threadUpsert.run(threadWriteProbeRow(firstId, 'Rcode doctor schema probe'))
    threadUpsert.run(threadWriteProbeRow(firstId, 'Rcode doctor schema probe updated'))
    threadUpsert.run(threadWriteProbeRow(secondId, 'RCODE DOCTOR SCHEMA PROBE'))
    ...
    db.exec('ROLLBACK TO Rcode_doctor_write_probe')
    db.exec('RELEASE Rcode_doctor_write_probe')
...
function findWriteProbeThreadIds(db: BetterSqliteDatabase): [string, string] {
  const candidates = Array.from({ length: 4 }, () => {
    ...
    const first = `thr_Rcode_doctor_probe_${suffix}`
    const second = `thr_Rcode_doctor_probe_${suffix.toUpperCase()}`
...
function usageWriteProbeRow(
  ...
  return {
    ...
    seq,
    timestamp,
    turn_id: 'turn_Rcode_doctor_probe',
    model: 'deepseek-chat',
