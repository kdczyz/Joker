...
import { DEFAULT_RCODE_MODEL } from '../config/Rcode-config.js'
...
export class ThreadService {
  ...
  async resumeSession(
    ...
    const record = createThreadRecord({
      ...
      model: options.model ?? sourceThread?.model ?? DEFAULT_RCODE_MODEL,
...
async function resolveWorkspaceRelativePath(workspace: string, relativePath: string): Promise<string> {
  ...
  // `.Rcodesdd/plan` cannot redirect a status update outside the workspace.
