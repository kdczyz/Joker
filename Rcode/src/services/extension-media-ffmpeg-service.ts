...
export class ExtensionMediaFfmpegService {
   ...
   * process disappeared with the prior Rcode runtime. Paths are derived from the
  ...
  while (pending.length > 0) {
    ...
    for (const [key, child] of Object.entries(current.value as Record<string, unknown>)) {
      if (key === 'target_url' && (
        ...
        throw invalidArgument('OpenTimelineIO media references must use opaque Rcode-media URLs')
      }
...
function transactionPaths(
  ...
  const token = createHash('sha256')
    .update('Rcode-media-output\0', 'utf8')
    ...
    .slice(0, 32)
  return {
    stagingDirectory: join(dirname(absolutePath), `.Rcode-media-${token}.Rcode-stage`),
    backupPath: join(dirname(absolutePath), `.${basename(absolutePath)}.${token}.Rcode-backup`)
  }
}
