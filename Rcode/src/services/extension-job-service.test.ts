...
import { JobEventSchema, JobSnapshotSchema } from '@Rcode/extension-api'
...
describe('ExtensionJobService subscriptions', () => {
  ...
  it('returns a safe gap snapshot when the requested replay cursor expired', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-extension-job-service-'))
    roots.push(root)
...
describe('ExtensionJobService conformance', () => {
  ...
  it('redacts credentials, media URLs, and local paths before durable failure storage', async () => {
    ...
    const error = Object.assign(
      new Error('Authorization: Bearer abc123 /Users/alice/private.mov Rcode-media://lease-secret'),
      {
        code: 'oauth failure',
        category: 'internal',
        details: {
