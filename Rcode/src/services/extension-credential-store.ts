...
export type ExtensionCredentialStoreOptions = {
  ...
  /**
   * Rcode's platform key provider. When its key is held by Keychain/DPAPI this
...
export class ExtensionCredentialStore {
    ...
    if (this.keyProviderActive && this.options.keyProvider) {
      return this.options.keyProvider.osKeychain
        ? {
            ...
            backend: 'Rcode-os-credential-key', reason: this.options.keyProvider.reason
          }
        : {
            mode: 'encrypted-fallback', degraded: true, available: true,
            backend: 'Rcode-encrypted-secret-store', reason: this.options.keyProvider.reason
          }
    }
    if (this.fallbackKey) {
  ...
  private scopedReference(reference: string): string {
    return `Rcode:${this.options.profileId}:${reference}`
  }

  private aad(reference: string): Buffer {
    return Buffer.from(`Rcode-extension-credential:v1:${this.options.profileId}:${reference}`, 'utf8')
  }
