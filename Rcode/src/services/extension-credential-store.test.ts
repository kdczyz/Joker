...
describe('ExtensionCredentialStore', () => {
  it('uses authenticated encrypted fallback without plaintext-at-rest', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Rcode-extension-credentials-'))
  ...
  it('prefers an available primary credential backend', async () => {
    ...
    const store = new ExtensionCredentialStore({
      dataDir: await mkdtemp(join(tmpdir(), 'Rcode-extension-primary-')),
      profileId: 'profile-a',
      primary
    })
  ...
  it('uses Rcode OS-backed key protection and binds ciphertext to profile and reference', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Rcode-extension-os-credentials-'))
    const keyProvider = {
    ...
    await expect(store.protection()).resolves.toMatchObject({
      ...
      backend: 'Rcode-os-credential-key'
    })
  ...
  it('reports Rcode key-file protection as the degraded encrypted fallback', async () => {
    const store = new ExtensionCredentialStore({
      dataDir: await mkdtemp(join(tmpdir(), 'Rcode-extension-degraded-')),
      profileId: 'profile-a',
      keyProvider: {
        encryptor: createAesEncryptor(randomBytes(32)),
        osKeychain: false,
    ...
    await expect(store.protection()).resolves.toMatchObject({
      mode: 'encrypted-fallback', degraded: true, available: true,
      backend: 'Rcode-encrypted-secret-store'
    })
