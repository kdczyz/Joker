...
export class ExtensionHostBroker {
  ...
  private async storage(principal: ExtensionPrincipal, method: string, params: JsonValue) {
    ...
    if (method === 'storage.keys') {
      ...
      return Object.keys(values).filter((key) => !key.startsWith('__Rcode_')).sort()
    }
    const keyed = input as z.infer<typeof StorageRequestSchema>
    if (keyed.key.startsWith('__Rcode_')) throw new Error('Reserved extension state key')
  ...
  private async createAccountSession(
    ...
    if (provider.oauthPkce) {
      ...
      session = {
        ...
        message: 'Complete authorization in the protected Rcode account window.'
      }
      ...
      session = {
        ...
        expiresAt: pending.expiresAt,
        message: 'Complete device authorization, then return to Rcode.'
      }
    } else {
      session = {
        id, extensionId: principal.extensionId,
        ...
        message: 'API keys must be entered in the protected Rcode account window.'
      }
    }
...
function publicAccountSession(
  ...
  if (session.status === 'pending') {
    redacted.message = 'Interaction required. Continue in Rcode Settings > Extensions > Provider accounts.'
  }
...
function viewStateKey(principal: ExtensionPrincipal): string {
  return `__Rcode_view_state__:${principal.viewContributionId ?? 'default'}`
}
...
async function readBoundedResponseBody(
  ...
  try {
    ...
    if (truncated) await reader.cancel('Rcode response limit reached').catch(() => undefined)
    reader.releaseLock()
  }
