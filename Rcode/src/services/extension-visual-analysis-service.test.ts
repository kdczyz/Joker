...
import type { MediaVisualAdapterBinding } from '@Rcode/extension-api'
...
import {
  ...
  RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR,
...
describe('ExtensionVisualAnalysisService', () => {
  ...
  it.skipIf(!systemFfmpeg || !systemFfprobe)(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'Rcode-visual-real-'))
      roots.push(root)
...
async function createFixture(scriptBody: string) {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-visual-analysis-'))
  roots.push(root)
  const workspace = join(root, 'workspace')
  const dataDir = join(root, 'data')
...
function adapterBinding(): MediaVisualAdapterBinding {
  return {
    id: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.adapterId,
    version: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.adapterVersion,
    ...
    embeddingDimensions: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.embeddingDimensions,
    execution: 'local'
  }
}
