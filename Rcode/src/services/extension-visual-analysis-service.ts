...
const MODEL_PAYLOAD = '{"schemaVersion":1,"packageId":"Rcode-bundled.visual-features-v1","modelId":"Rcode-visual-features","modelVersion":"1.0.0","embeddingDimensions":24,"algorithm":"Rcode.rgb-edge-features","algorithmVersion":"1.0.0","querySemantics":"bounded-visual-features-v1","features":["mean-red","mean-green","mean-blue","brightness","darkness","saturation","low-saturation","contrast","low-contrast","edge-density","smoothness","warmth","coolness","red-dominance","green-dominance","blue-dominance","near-black","dark","midtone","bright","highlight","horizontal-edge","vertical-edge","neutral-bias"]}\n'
...
const MODEL_MANIFEST = '{"schemaVersion":1,"packageId":"Rcode-bundled.visual-features-v1","modelId":"Rcode-visual-features","modelVersion":"1.0.0","adapterId":"Rcode.local.visual-features","adapterVersion":"1.0.0","embeddingDimensions":24,"querySemantics":"bounded-visual-features-v1","files":[{"name":"visual-features-v1.json","sha256":"969024f5de7e8f8013f7ada5e08d53c209d221a0b313bd8b35862ce57387351a","byteSize":582}]}\n'
...
export const RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR: MediaVisualModelDescriptor = {
...
/**
 * Generic Rcode-owned local visual adapter. The installed package is a small,
...
export class ExtensionVisualAnalysisService {
  ...
  async status(principal: ExtensionPrincipal): Promise<MediaVisualModelStatus> {
    ...
    if (installed.receipt) {
      return MediaVisualModelStatusSchema.parse({
        schemaVersion: 1,
        state: 'installed',
        descriptor: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR,
    ...
    return MediaVisualModelStatusSchema.parse({
      ...
      remediation: installed.invalid
        ? 'The bundled visual feature package failed digest, signature, or receipt verification. Reinstall it through Rcode before indexing.'
        : 'Install Rcode\'s signed bundled visual feature package. This copies only verified local bytes and does not download or upload media.',
      local: true,
      networkUsedForInference: false,
      rawPathsExposed: false,
      urlsAccepted: false
  ...
  async analyzeFrames(
    ...
    try {
      ...
      return MediaAnalyzeVisualFramesResultSchema.parse({
        ...
        provenance: {
          algorithm: 'Rcode.rgb-edge-features',
  ...
  private async installOnce(principal: ExtensionPrincipal): Promise<MediaVisualModelStatus> {
    ...
    const receipt = MediaVisualModelInstallReceiptSchema.parse({
      ...
      packageId: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.packageId,
      modelId: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.modelId,
      modelVersion: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.modelVersion,
      manifestSha256: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.manifestSha256,
      files: RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR.files,
  ...
  private async readyFor(
    ...
    if (!sameAdapter(adapter, RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR)) {
      return visualUnavailable(
        'VISUAL_MODEL_MISMATCH',
        'The requested adapter identity does not match Rcode\'s verified local visual package. Refresh capability state and rebuild the immutable index.',
        false
      )
    }
  ...
  private async verifyDirectory(path: string): Promise<InstalledState> {
    try {
      ...
      if (!receiptMatchesDescriptor(receipt, RCODE_LOCAL_VISUAL_MODEL_DESCRIPTOR)) return { invalid: true }
...
function receiptMatchesDescriptor(
  ...
  return receipt.broker === 'Rcode-model-broker' &&
...
function visualProcessUnavailable(error: unknown): MediaAnalyzeVisualFramesResult {
  ...
  return visualUnavailable(
    ...
    error.code === 'executable_unavailable'
      ? 'Install FFmpeg and FFprobe in a reviewed Rcode Host location, then retry local visual indexing.'
      : 'Rcode could not decode the requested authorized visual frame with its fixed local profile. Reauthorize a supported video/image source or use filename/transcript search.',
    error.retryable
  )
}
