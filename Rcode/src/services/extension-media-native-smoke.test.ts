...
const runNative = process.env.RCODE_RUN_MEDIA_SMOKE === '1'
...
suite('native FFmpeg media broker smoke', () => {
  it('probes, renders a proof frame, exports H.264, post-probes, and cancels safely', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'Rcode-native-media-'))
    roots.push(root)
...
async function executable(name: 'ffmpeg' | 'ffprobe'): Promise<string> {
  const configured = process.env[`RCODE_${name.toUpperCase()}_PATH`]
  ...
  throw new Error(`${name} is required when RCODE_RUN_MEDIA_SMOKE=1`)
}
