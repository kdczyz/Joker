...
import {
  DEFAULT_RCODE_CAPABILITIES_CONFIG,
  RcodeCapabilitiesConfig,
  ModelInputModality,
...
export const RCODE_CONFIG_FILENAME = 'config.json'
...
export const RcodeServeConfigSchema = z
  .object({
...
export const RcodeConfigSchema = z
  .object({
    serve: RcodeServeConfigSchema.optional(),
    models: ModelConfigSchema.optional(),
    contextCompaction: ContextCompactionConfigSchema.optional(),
    runtime: RuntimeTuningConfigSchema.optional(),
    roles: RolesConfigSchema.optional(),
    capabilities: RcodeCapabilitiesConfig.default(DEFAULT_RCODE_CAPABILITIES_CONFIG),
    hooks: HooksConfigSchema.optional(),
    quality: QualityConfigSchema.optional()
  })
  .strict()

export type RcodeConfig = z.infer<typeof RcodeConfigSchema>
export type QualityConfig = z.infer<typeof QualityConfigSchema>
export const DEFAULT_QUALITY_CONFIG: QualityConfig = QualityConfigSchema.parse({})
export type RcodeServeConfig = z.infer<typeof RcodeServeConfigSchema>
...
export type LoadedRcodeConfig = {
  path: string
  config: RcodeConfig
}

export function readRcodeConfigFile(path: string): LoadedRcodeConfig {
  ...
  try {
    ...
    throw new Error(`Failed to parse Rcode config JSON at ${resolvedPath}: ${message}`)
  }
  const parsed = RcodeConfigSchema.safeParse(json)
  if (!parsed.success) {
    throw new Error(
      `Invalid Rcode config at ${resolvedPath}: ${JSON.stringify(parsed.error.issues, null, 2)}`
    )
  }
  return { path: resolvedPath, config: parsed.data }
}

export function readOptionalRcodeConfigFile(path: string | undefined): LoadedRcodeConfig | null {
  if (!path) return null
  const resolvedPath = expandHomePath(path)
  if (!existsSync(resolvedPath)) return null
  return readRcodeConfigFile(resolvedPath)
}

export function RcodeConfigPathForDataDir(dataDir: string | undefined): string | undefined {
  const trimmed = dataDir?.trim()
  if (!trimmed) return undefined
  return join(expandHomePath(trimmed), RCODE_CONFIG_FILENAME)
}
