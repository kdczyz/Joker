...
export const RCODE_PROJECT_CONFIG_RELATIVE_PATH = '.Rcode/project.json'
export const RCODE_PROJECT_CONFIG_VERSION = 1
export const MAX_RCODE_PROJECT_CONFIG_BYTES = 256 * 1024
export const MAX_RCODE_PROJECT_MCP_SERVERS = 32
export const MAX_RCODE_PROJECT_SKILL_ROOTS = 32
...
export const RcodeProjectMcpServerConfig = z
  .object({
...
export type RcodeProjectMcpServerConfig = z.infer<typeof RcodeProjectMcpServerConfig>

export const RcodeProjectConfigSchema = z
  .object({
    $schema: z.string().trim().min(1).max(4_096).optional(),
    version: z.literal(RCODE_PROJECT_CONFIG_VERSION),
    mcp: z
      .object({
        servers: z
          .record(z.string().trim().min(1).max(128), RcodeProjectMcpServerConfig)
          .refine((servers) => Object.keys(servers).length <= MAX_RCODE_PROJECT_MCP_SERVERS, {
            message: `project MCP config supports at most ${MAX_RCODE_PROJECT_MCP_SERVERS} servers`
          })
          .default({})
      })
      .strict()
    ...
    skills: z
      .object({
        ...
        roots: z.array(RelativeProjectPath).max(MAX_RCODE_PROJECT_SKILL_ROOTS).default([]),
  ...
  .strict()
export type RcodeProjectConfig = z.infer<typeof RcodeProjectConfigSchema>

export type ResolvedRcodeProjectConfig = {
  workspaceRoot: string
  path: string
  digest: string
  config: RcodeProjectConfig
  mcp: {
    servers: Record<string, RcodeProjectMcpServerConfig & { cwd?: string }>
  }
  skills: {
    enabled: boolean
...
export type RcodeProjectConfigLoadResult =
  | {
      status: 'missing'
      workspaceRoot: string
      path: string
  ...
  | ({ status: 'valid' } & ResolvedRcodeProjectConfig)

export type RcodeProjectConfigSource = {
  workspaceRoot: string
  path: string
  exists: boolean
  content: string
...
export function RcodeProjectConfigPath(workspaceRoot: string): string {
  return join(resolve(workspaceRoot), '.Rcode', 'project.json')
}

export function RcodeProjectConfigDigest(config: RcodeProjectConfig): string {
  return createHash('sha256')
    .update(stableJson(config))
    .digest('hex')
}

export async function parseAndResolveRcodeProjectConfig(
  workspaceRoot: string,
  value: unknown,
  path = RcodeProjectConfigPath(workspaceRoot)
): Promise<ResolvedRcodeProjectConfig> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const parsed = RcodeProjectConfigSchema.safeParse(value)
  if (!parsed.success) {
  ...
  const skillRoots = config.skills.enabled
    ...
    : []
  const servers: ResolvedRcodeProjectConfig['mcp']['servers'] = {}
  ...
  return {
    ...
    path,
    digest: RcodeProjectConfigDigest(config),
    config,
    mcp: { servers },
    skills: {
...
export async function loadRcodeProjectConfig(
  workspaceRoot: string
): Promise<RcodeProjectConfigLoadResult> {
  ...
  const path = RcodeProjectConfigPath(resolvedWorkspaceRoot)
  ...
  try {
    const text = await readBoundedRegularFile(resolvedPath, MAX_RCODE_PROJECT_CONFIG_BYTES)
    const value = JSON.parse(text) as unknown
    const resolved = await parseAndResolveRcodeProjectConfig(resolvedWorkspaceRoot, value, path)
...
export async function readRcodeProjectConfigSource(
  workspaceRoot: string
): Promise<RcodeProjectConfigSource> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const path = RcodeProjectConfigPath(resolvedWorkspaceRoot)
  let resolvedPath: string
  try {
  ...
  return {
    ...
    path,
    exists: true,
    content: await readBoundedRegularFile(resolvedPath, MAX_RCODE_PROJECT_CONFIG_BYTES)
  }
}

export async function validateRcodeProjectConfigText(
  workspaceRoot: string,
  content: string
): Promise<ResolvedRcodeProjectConfig> {
  if (Buffer.byteLength(content, 'utf8') > MAX_RCODE_PROJECT_CONFIG_BYTES) {
    throw new Error(`project config exceeds ${MAX_RCODE_PROJECT_CONFIG_BYTES} byte limit`)
  }
  let value: unknown
  try {
  ...
  return parseAndResolveRcodeProjectConfig(workspaceRoot, value)
}

export async function writeRcodeProjectConfig(
  workspaceRoot: string,
  content: string
): Promise<ResolvedRcodeProjectConfig> {
  const validated = await validateRcodeProjectConfigText(workspaceRoot, content)
  const configDir = join(validated.workspaceRoot, '.Rcode')
...
export async function ensureRcodeProjectConfigDirectory(
  workspaceRoot: string
): Promise<string> {
  const resolvedWorkspaceRoot = await resolveRealWorkspaceRoot(workspaceRoot)
  const configDir = join(resolvedWorkspaceRoot, '.Rcode')
...
function projectConfigValidationMessage(
  ...
  if (isObject(value) && value.version !== undefined && value.version !== RCODE_PROJECT_CONFIG_VERSION) {
