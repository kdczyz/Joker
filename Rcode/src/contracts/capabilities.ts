...
export const SkillsCapabilityConfig = CapabilityToggleConfig.extend({
  ...
  workspaceRoots: z.array(z.string().min(1)).default([]),
  /** Global skill roots (e.g. ~/.Rcode/skills). Scanned after project roots. */
  globalRoots: z.array(z.string().min(1)).default([]),
  /** Read workspace-local `.Rcode/project.json` Skill policy on demand. */
...
export const RcodeCapabilitiesConfig = z
  .object({
  ...
  .strict()
export type RcodeCapabilitiesConfig = z.infer<typeof RcodeCapabilitiesConfig>

export const DEFAULT_RCODE_CAPABILITIES_CONFIG: RcodeCapabilitiesConfig = RcodeCapabilitiesConfig.parse({})
...
export function buildRuntimeCapabilityManifest(input: {
  config?: RcodeCapabilitiesConfig
  model: ModelCapabilityMetadata
  mcp?: {
  ...
  const config = RcodeCapabilitiesConfig.parse(input.config ?? {})
