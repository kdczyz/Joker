...
export const GuiPlanContextSchema = z.object({
  ...
  relativePath: z
    ...
    .refine(isGuiPlanRelativePath, {
      message: 'relativePath must be a direct Markdown file under .Rcodesdd/plan'
    }),
...
export const GuiDesignArtifactContextSchema = z.object({
  ...
  relativePath: z.string().min(1).refine((value) => {
    ...
    return normalized === value &&
      normalized.startsWith('.Rcode-design/') &&
      !normalized.split('/').includes('..') &&
      /\/v\d+\.svg$/i.test(normalized)
  }, { message: 'relativePath must be a versioned SVG file under .Rcode-design' })
})
...
export const TurnSchema = z.object({
  ...
  /**
   * True for renderer-owned design canvas turns. Rcode advertises the
   ...
   * headless runs). Rcode hides `user_input`/`request_user_input` and
   * rejects calls to them instead of blocking on a GUI answer.
   */
  disableUserInput: z.boolean().optional(),
  /**
   * True when this turn originated from an IM bridge. Rcode exposes
...
export const StartTurnRequest = z.object({
   ...
   * mode Rcode advertises `create_plan` for the whole conversation.
   */
  mode: TurnModeSchema.optional(),
  attachments: z
    .array(
  ...
  workspaceCheckpointId: z.string().min(1).optional(),
  /**
   * Optional GUI plan context. When set, Rcode advertises the
