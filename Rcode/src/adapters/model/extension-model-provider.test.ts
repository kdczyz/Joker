...
async function harness(
  ...
  const store = new ExtensionProviderAccountStore({
    dataDir: await mkdtemp(join(tmpdir(), 'Rcode-extension-model-')),
...
function request(providerId: string, accountId: string, signal = new AbortController().signal): ModelRequest {
  return {
    ...
    providerId,
    accountId,
    systemPrompt: 'Rcode stable system prompt',
...
describe('ExtensionModelProviderRegistry', () => {
  it('normalizes full Rcode requests and validates ordered provider streams', async () => {
    ...
    expect(captured).toMatchObject({
      ...
      instructions: ['Rcode stable system prompt', 'Extension profile overlay'],
      tools: [{ name: 'read' }],
