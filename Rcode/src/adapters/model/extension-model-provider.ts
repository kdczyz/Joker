...
function assertModelRequestCapabilities(request: ModelRequest, model: ProviderModel): void {
  ...
  if (!capabilities.output.includes('text') && !capabilities.tools) {
    throw new Error(`extension provider model has no Rcode-compatible output capability: ${model.id}`)
  }
}
 ...
 * Map only recognized semantic tokens into a fixed Rcode vocabulary, retaining
