...
const TOOL_DESCRIPTION = [
  ...
  '.Rcodesdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .Rcodesdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.Rcodesdd/plan/foo.md`)
 ...
 *    `.Rcodesdd/plan/<feature>.md` under the active workspace.
...
export async function executeCreatePlanTool(
  ...
  try {
    ...
    // dangling symlink before writing so a planted .Rcodesdd link cannot escape
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .Rcodesdd/plan' }
  }
 ...
 * allocates a fresh, non-colliding `.Rcodesdd/plan/<feature>.md`.
 */
