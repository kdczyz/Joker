...
const relativePath = '.Rcode-design/doc/artifact/v1.svg'
...
async function workspaceWithSvg(source = [
  ...
  const workspace = await mkdtemp(join(tmpdir(), 'Rcode-design-svg-'))
  workspaces.push(workspace)
  const absolutePath = join(workspace, relativePath)
  await mkdir(join(workspace, '.Rcode-design/doc/artifact'), { recursive: true })
...
describe('design SVG tools', () => {
  ...
  it('rejects artifact paths that escape the reserved .Rcode-design version layout', async () => {
    const workspace = await workspaceWithSvg()
  ...
  it('enforces the workspace boundary for reserved SVG paths even in full-access mode', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-design-svg-boundary-'))
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-design-svg-outside-'))
    workspaces.push(workspace, outside)
    await mkdir(join(workspace, '.Rcode-design/doc'), { recursive: true })
    await writeFile(join(outside, 'v1.svg'), [
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">',
      '<title id="title">Outside</title><desc id="desc">Must not change.</desc><g id="artwork" />',
      '</svg>'
    ].join(''), 'utf8')
    await symlink(outside, join(workspace, '.Rcode-design/doc/artifact'))
