...
function validHtml(label = 'Date picker'): string {
  return `<!doctype html>
<html><head>
<meta name="Rcode-component-prototype" content="1">
<title>${label}</title>
<style>button { color: #123; }</style>
</head><body><main data-Rcode-component-root><button type="button">${label}</button></main></body></html>`
}
...
describe('component designer profile', () => {
  it('is built in with a narrow file-authoring allow-list', () => {
    ...
    expect(config.profiles[COMPONENT_DESIGN_PROFILE_NAME]?.promptPreamble).toContain('data-Rcode-component-root')
  })
})
...
describe('design_component tool', () => {
  it('reserves an artifact, forwards bounded implementation context, and returns hardened metadata', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-'))
    ...
    expect(String(prototype.relativePath)).toMatch(/^\.Rcode-design\/component-prototypes\/.+\/prototype\.html$/)
    ...
    expect(String(prompts[0]?.workspace)).toMatch(/\.Rcode-design\/component-prototypes\/.+$/)
  ...
  it('fails closed when the child writes a remotely dependent prototype', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-invalid-'))
    workspaces.push(workspace)
    const runtime = {
      enabled: () => true,
      runChild: async (input: Record<string, unknown>) => {
    ...
    expect(result.output).toMatchObject({
      ...
      componentPrototype: {
        ...
        relativePath: expect.stringMatching(/^\.Rcode-design\/component-prototypes\//),
        error: expect.stringContaining('remote resources')
      }
    })
  })
  ...
  it('preserves a replayable failed card when source context cannot be loaded', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-source-failure-'))
    workspaces.push(workspace)
    const runtime = {
      enabled: () => true,
      runChild: async () => {
  ...
  it('fails when the child writes anything outside the single-file artifact contract', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-extra-file-'))
    workspaces.push(workspace)
    const runtime = {
      enabled: () => true,
      runChild: async (input: Record<string, unknown>) => {
  ...
  it('publishes supplied HTML directly without requiring a delegation runtime', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-direct-'))
    workspaces.push(workspace)
  ...
  it('prefers supplied HTML over request delegation when both are present', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-direct-precedence-'))
    workspaces.push(workspace)
  ...
  it('keeps the tool advertised and explains how to proceed when request delegation is disabled', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-disabled-'))
    workspaces.push(workspace)
  ...
  it('fails closed for invalid directly supplied HTML and preserves its inline card', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-component-design-direct-invalid-'))
    workspaces.push(workspace)
    const tool = buildComponentDesignToolProviders(undefined)[0]!.tools[0]!
...
describe('component prototype hardening', () => {
  ...
  it('requires one component marker root and rejects embedded documents', () => {
    expect(() => hardenComponentPrototypeHtml(validHtml().replace(' data-Rcode-component-root', '')))
      .toThrow(/exactly one/)
...
describe('component designer prompt', () => {
  it('keeps the task component-scoped and points at the reserved output', () => {
    const prompt = buildComponentDesignerPrompt({
      ...
      relativePath: '.Rcode-design/component-prototypes/search/prototype.html',
      sourceContext: ''
    })
    expect(prompt).toContain('write exactly one file')
    expect(prompt).toContain('no site header, sidebar, page navigation')
    expect(prompt).toContain('data-Rcode-component-root')
  })
})
