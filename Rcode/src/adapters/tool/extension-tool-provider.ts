...
import { ExtensionApiError, type ExtensionErrorCode } from '@Rcode/extension-api'
 ...
 * LocalTool entries so every invocation keeps Rcode's approval, sandbox,
...
export class ExtensionToolRegistry {
  ...
  /** Rebind live extension registrations after Rcode rebuilds its base registry. */
...
function validateDeclaration(input: ExtensionToolDeclaration): ExtensionToolDeclaration {
  ...
  if (RESERVED_TOOL_NAMES.has(name) || name.startsWith('Rcode.')) throw new Error(`reserved extension tool name: ${name}`)
