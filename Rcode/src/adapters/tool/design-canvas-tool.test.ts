...
describe('dedicated design tools', () => {
  it('queues a deterministic renderer-backed whiteboard image export only in Code canvas turns', async () => {
    ...
    expect(result.output).toMatchObject({
      ...
      exportRequest: {
        format: 'png',
        fileName: expect.stringMatching(/^Rcode-whiteboard-[a-f0-9]{12}\.png$/),
        relativePath: expect.stringMatching(/^\.deepseekgui-images\/Rcode-whiteboard-[a-f0-9]{12}\.png$/)
      },
      generatedFiles: [{
  ...
  it('queues a structured project design-system operation without board placement', async () => {
    ...
    const result = await tool.execute(
      { name: 'IRCODE World', seedColor: '#D4AF37', mode: 'dark', template: 'game' },
      context()
    )
    expect(result.output).toMatchObject({
      ok: true,
