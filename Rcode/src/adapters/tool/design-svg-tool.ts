...
async function svgFileContext(context: ToolHostContext, write: boolean) {
  ...
  // thread otherwise runs with danger-full-access. A planted .Rcode-design
  ...
  if (!resolved.relativePath.startsWith('.Rcode-design/') || !/\/v\d+\.svg$/i.test(resolved.relativePath)) {
    throw new Error('SVG artifact path must be a versioned file under .Rcode-design')
  }
...
async function atomicWrite(path: string, content: string, signal?: AbortSignal): Promise<void> {
  const temp = `${path}.Rcode-${process.pid}-${randomBytes(4).toString('hex')}.tmp`
  try {
