...
export class FileSessionStore implements SessionStore {
  ...
  async loadItems(threadId: string): Promise<TurnItem[]> {
    ...
    // event loop for seconds on large threads, starving /health (kdczyz/Rcode#621).
    ...
    if (elapsedMs >= SLOW_LOAD_ITEMS_LOG_MS) {
      ...
      console.warn(
        `[Rcode] loadItems(${threadId}) took ${Math.round(elapsedMs)}ms ` +
...
function warnUsageCompaction(threadId: string, error: unknown): void {
  ...
  console.warn(`[Rcode] usage event compaction failed for ${threadId}; keeping append-only log: ${message}`)
}
