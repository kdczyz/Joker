...
export const EXTENSION_MANIFEST_FILE = 'Rcode-extension.json'
export const EXTENSION_INTEGRITY_FILE = 'Rcode-extension.integrity.json'
...
export async function extractRcodexArchive(
  archivePath: string,
  destination: string,
  options: ArchiveValidationOptions = {}
): Promise<ExtractedRcodex> {
...
export async function inspectRcodexArchive(
  archivePath: string,
  options: ArchiveValidationOptions = {}
): Promise<Omit<ExtractedRcodex, 'destination'>> {
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'Rcode-extension-inspect-'))
  const destination = join(temporaryRoot, 'package')
  try {
    const { destination: _destination, ...inspection } = await extractRcodexArchive(
      archivePath,
      destination,
      options
    )
...
export type PackRcodexOptions = ArchiveValidationOptions & {
  overwrite?: boolean
...
export async function packRcodex(
  sourceDirectory: string,
  outputPath: string,
  options: PackRcodexOptions = {}
): Promise<Omit<ExtractedRcodex, 'destination'>> {
  const limits = archiveLimits(options.limits)
  ...
  try {
    ...
    const inspection = await inspectRcodexArchive(temporaryOutput, options)
...
async function collectPackFiles(
  ...
  options: Pick<PackRcodexOptions, 'include' | 'ignore'>
): Promise<PackageFile[]> {
...
function forbiddenPackagePathReason(packagePath: string): string | undefined {
  ...
  if (fileName.endsWith('.Rcodex')) return 'nested-package'
