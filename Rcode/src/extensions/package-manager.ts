...
export class ExtensionPackageManager {
  ...
  async installArchive(
    ...
    let destination = ''
    try {
      const extracted = await extractRcodexArchive(archivePath, extractedPath, this.validation)
...
function validateExpectedPackage(
  ...
  if (extracted.manifest.engines.Rcode !== expected.enginesRcode) mismatches.push('engines.Rcode')
