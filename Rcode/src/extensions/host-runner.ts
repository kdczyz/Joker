    ...
    switch (method) {
      ...
      case 'host.load': {
        ...
        extensionModule = await import(
          `${pathToFileURL(entrypoint).href}?RcodeHost=${encodeURIComponent(initialized.identity.lifecycleNonce)}`
        ) as ExtensionModule
        return { loaded: true }
      }
...
if (process.env.RCODE_EXTENSION_HOST_RUNNER === '1') {
  void runExtensionHost().catch((error: unknown) => {
    process.stderr.write(
      `[Rcode-extension-host] ${error instanceof Error ? error.stack ?? error.message : String(error)}\n`
    )
    process.exit(1)
  })
}
