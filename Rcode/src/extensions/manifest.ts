...
export function parseExtensionManifest(value: unknown): ExtensionManifest {
  ...
  if (semver.validRange(manifest.engines.Rcode) === null) {
    throw extensionError('EXTENSION_ENGINE_RANGE_INVALID', 'engines.Rcode must be a valid SemVer range', {
      value: manifest.engines.Rcode
    })
  }
...
export function assertManifestCompatible(
  ...
  if (!report.RcodeEngine.compatible) {
    throw extensionError('EXTENSION_ENGINE_INCOMPATIBLE', 'Running Rcode version is outside engines.Rcode', {
      running: compatibility.RcodeVersion,
      declared: manifest.engines.Rcode,
      compatibility: report
    })
  }
...
export function manifestCompatibilityReport(
  ...
  const engineCompatible = semver.satisfies(compatibility.RcodeVersion, manifest.engines.Rcode, {
  ...
  if (!engineCompatible) {
    diagnostics.push({
      ...
      dimension: 'engine',
      declared: manifest.engines.Rcode,
      supported: compatibility.RcodeVersion,
      code: 'EXTENSION_ENGINE_INCOMPATIBLE',
      message: `Running Rcode ${compatibility.RcodeVersion} is outside ${manifest.engines.Rcode}`
    })
  }
  if (!rpcCompatible) {
    diagnostics.push({
