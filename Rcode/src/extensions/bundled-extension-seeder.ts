...
const ARCHIVE_NAME = /^[0-9A-Za-z][0-9A-Za-z._-]*\.Rcodex$/
...
function expectedPackage(descriptor: BundledExtensionCatalogEntry): ExpectedIndexedPackage {
  return {
    ...
    enginesRcode: descriptor.enginesRcode,
...
function validateCatalogEntry(
  ...
  const enginesRcode = stringField(value, 'enginesRcode')
  const apiVersion = stringField(value, 'apiVersion')
  assertExtensionId(id)
  if (semver.valid(version) === null || semver.valid(apiVersion) === null || semver.validRange(enginesRcode) === null) {
    invalidCatalogEntry()
  }
  if (
    archive !== basename(archive) ||
    extname(archive).toLowerCase() !== '.Rcodex' ||
