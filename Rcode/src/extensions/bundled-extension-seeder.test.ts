...
import { packRcodex } from './archive.js'
import {
...
describe('bundled extension seeding', () => {
  ...
  it('classifies a pre-existing package as user-managed without changing it', async () => {
    ...
    const userArchive = join(harness.root, 'user-install.Rcodex')
    await writeSource(harness.source, '1.0.0', [], 'user-owned')
    await packRcodex(harness.source, userArchive, { compatibility })
...
async function createHarness() {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-bundled-seeder-'))
  roots.push(root)
...
async function writeBundle(
  ...
  const archiveName = `acme-demo-${version}.Rcodex`
  const archive = join(harness.bundles, archiveName)
  const packed = await packRcodex(harness.source, archive, { compatibility, overwrite: true })
...
async function writeSource(
  ...
  await writeFile(join(root, 'Rcode-extension.json'), `${JSON.stringify(manifest, null, 2)}\n`)
