...
const KEYCHAIN_SERVICE = 'Rcode-secret-key'
const KEYCHAIN_ACCOUNT = 'Rcode'
    ...
    if (platform === 'darwin') {
      ...
      if (res.code === 0 && res.stdout.trim()) {
        ...
        return key.length === 32
          ? { status: 'found', key }
          : { status: 'unavailable', reason: 'macOS keychain returned an invalid Rcode key' }
      }
      ...
      if (res.code === 0 && res.stdout.trim()) {
        ...
        return key.length === 32
          ? { status: 'found', key }
          : { status: 'unavailable', reason: 'Linux secret service returned an invalid Rcode key' }
      }
...
async function tryWriteOsKey(platform: NodeJS.Platform, run: CommandRunner, key: Buffer): Promise<boolean> {
  ...
  try {
    ...
    if (platform === 'linux') {
      const res = await run('secret-tool', ['store', '--label=Rcode secret key', 'service', KEYCHAIN_SERVICE, 'account', KEYCHAIN_ACCOUNT], b64)
      return res.code === 0
    }
  } catch {
    return false
...
export const DISABLE_OS_CREDENTIAL_STORE_ENV = 'RCODE_DISABLE_OS_CREDENTIAL_STORE'
...
export async function createSecretEncryptor(options: CreateKeyProviderOptions): Promise<KeyProviderResult> {
  ...
  if (useKeychain && run) {
    ...
    if (existing.status === 'unavailable') {
      throw new Error(`${existing.reason}; refusing to replace the existing Rcode encryption key`)
    }
