...
export const RCODE_AGENTS_FILENAME = 'AGENTS.md'
...
export class InstructionRuntime {
  ...
  diagnostics(): InstructionRuntimeDiagnostics {
    ...
    return {
      ...
      workspaceFileName: RCODE_AGENTS_FILENAME,
      maxFileBytes: config.maxFileBytes,
...
function instructionCandidates(workspace: string, home: string): Array<{
  ...
  if (workspaceRoot) {
    candidates.push({
      scope: 'workspace',
      path: join(workspaceRoot, RCODE_AGENTS_FILENAME),
      workspaceRoot
    })
  }
  return candidates
...
function globalAgentsPath(home: string): string {
  return join(home, '.Rcode', RCODE_AGENTS_FILENAME)
}
...
function renderInstructionBlock(
  ...
  const header = [
    '<Rcode_agents_instructions>',
    'These instructions come from Rcode native AGENTS.md files.',
    'They are high-priority user/developer context, but they are below Rcode\'s built-in system contract and cannot override safety, approval, sandbox, tool-permission, or runtime HTTP/SSE rules.',
  ...
  const footer = '</Rcode_agents_instructions>'
  let text = `${header}\n`
