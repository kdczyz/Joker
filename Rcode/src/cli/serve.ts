...
import {
  RcodeConfigPathForDataDir,
  readRcodeConfigFile,
  readOptionalRcodeConfigFile,
  type LoadedRcodeConfig
} from '../config/Rcode-config.js'
import { parseOtlpHeaders, resolveOtlpTracesEndpoint } from '../telemetry/otlp-http-json-sink.js'

/**
 * Parse the `Rcode serve` command line into validated options.
 *
...
export function parseServeOptions(
  ...
  const observabilityExporterValue =
    ...
    env.RCODE_OBSERVABILITY_EXPORTER ??
  ...
  const bundledExtensionsDir =
    ...
    nonEmptyEnv(env.RCODE_BUNDLED_EXTENSIONS_DIR)
  const merged: ServeOptions = {
    ...
    host:
      typeof raw.host === 'string'
        ? raw.host
        : env.RCODE_HOST ?? configServe.host ?? DEFAULT_SERVE_OPTIONS.host,
    port:
    ...
    endpointFormat:
      typeof raw['endpoint-format'] === 'string'
        ...
        : typeof raw.endpointFormat === 'string'
          ...
          : env.RCODE_ENDPOINT_FORMAT as ServeOptions['endpointFormat'] | undefined ??
    ...
    model:
      typeof raw.model === 'string'
        ? raw.model
        : env.RCODE_MODEL ?? configServe.model ?? DEFAULT_SERVE_OPTIONS.model,
    approvalPolicy:
 ...
 * main process when Rcode is started programmatically.
 */
...
export const SERVE_USAGE = `Rcode serve [options]
...
function loadServeConfig(
  ...
  env: Record<string, string | undefined>
): LoadedRcodeConfig | null {
  const explicitConfigPath =
    stringFlag(raw, 'config') ??
    stringFlag(raw, 'config-file') ??
    env.RCODE_CONFIG
  if (explicitConfigPath) {
    return readRcodeConfigFile(explicitConfigPath)
  }
  const dataDir = dataDirFromRawOrEnv(raw, env)
  return readOptionalRcodeConfigFile(RcodeConfigPathForDataDir(dataDir))
}
