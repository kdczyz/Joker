...
import type { RcodeServeHandle } from '../server/runtime-factory.js'
 ...
 */
export function installServeCrashHandlers(getHandle: () => RcodeServeHandle | null): void {
  let crashing = false
  const crash = (kind: string, error: unknown): void => {
    if (crashing) return
    crashing = true
    const detail = error instanceof Error ? (error.stack ?? error.message) : String(error)
    process.stderr.write(`Rcode serve: ${kind}: ${detail}\n`)
  ...
  process.on('unhandledRejection', (reason) => {
    ...
    process.stderr.write(`Rcode serve: unhandledRejection (non-fatal, runtime stays up): ${detail}\n`)
  })
}
