...
import { createRcodeServeRuntime } from '../server/runtime-factory.js'
...
export const RCODE_CLI_USAGE = `Rcode <command> [options]
...
Common options:
  ...
  --data-dir <path>          Root directory for Rcode data
  ...
  --jsonl                    Stream one machine-readable event per line for Rcode run
...
export type RcodeCliCommand = 'serve' | 'run' | 'chat' | 'exec' | 'help'

export function splitRcodeCliCommand(argv: readonly string[]): {
  command: RcodeCliCommand
  args: string[]
  error?: string
} {
  const first = argv[0]
...
export async function runAgentCommand(
  command: Exclude<RcodeCliCommand, 'serve' | 'help'>,
...
async function runOneShot(argv: readonly string[], io: CliIo): Promise<number> {
  ...
  if (!parsed.ok) return writeParseError(parsed, io, 'Rcode run')
  const jsonl = hasFlag(argv, 'jsonl')
  if (parsed.json && jsonl) {
    io.stderr.write('Rcode run: --json and --jsonl are mutually exclusive\n')
    return ServeExitCode.usage
  }
  const prompt = stringFlag(argv, ['prompt', 'p']) ?? positionals(argv).join(' ').trim()
  if (!prompt) {
    io.stderr.write('Rcode run: missing prompt\n')
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    io.stderr.write(`Rcode run: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'Rcode run')
  }
}
...
async function runChat(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'Rcode chat')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
    const thread = await runtime.threadService.create({
    ...
    io.stderr.write(`Rcode chat: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  } finally {
    await shutdownRuntime(runtime, io, 'Rcode chat')
  }
}
...
async function runExec(argv: readonly string[], io: CliIo): Promise<number> {
  const parsed = parseSharedOptions(argv, io)
  if (!parsed.ok) return writeParseError(parsed, io, 'Rcode exec')
  let runtime: ServerRuntime | undefined
  try {
    runtime = await createRuntime(parsed.options, io)
  } catch (error) {
    io.stderr.write(`Rcode exec: ${errorMessage(error)}\n`)
    return ServeExitCode.runtime
  }
  ...
  try {
    ...
    if (!toolName) {
      io.stderr.write('Rcode exec: missing tool name (use --list-tools to inspect tools)\n')
      return ServeExitCode.usage
    }
    const argsText = stringFlag(argv, ['args']) ?? '{}'
    const args = parseJsonObject(argsText)
    if (!args.ok) {
      io.stderr.write(`Rcode exec: ${args.message}\n`)
    ...
    await shutdownRuntime(runtime, io, 'Rcode exec')
  }
}
...
function parseSharedOptions(argv: readonly string[], io: CliIo): SharedOptionsResult {
  ...
  return {
    ...
    workspace: stringFlag(argv, ['workspace']) ?? io.env?.RCODE_WORKSPACE ?? io.cwd?.() ?? process.cwd(),
...
function createRuntime(options: ServeOptions, io: CliIo): Promise<ServerRuntime> {
  return io.createRuntime ? io.createRuntime(options) : createRcodeServeRuntime(options)
}
