...
describe('Rcode extension CLI', () => {
  it('scaffolds through the official create-Rcode-extension adapter', async () => {
  ...
  it('accepts repeatable safe include and ignore paths for source validation and packing', async () => {
    ...
    const archive = join(root, 'selected.Rcodex')
  ...
  it('validates bounded manifest locale overlays through the public CLI', async () => {
    ...
    const manifestPath = join(source, 'Rcode-extension.json')
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'Rcode-extension-cli-'))
