...
import {
  RCODE_CLI_USAGE,
  runAgentCommand,
  splitRcodeCliCommand
} from './agent-cli.js'
import { startRcodeServe, type RcodeServeHandle } from '../server/runtime-factory.js'
import {
...
export const RCODE_READY_PREFIX = 'RCODE_READY '

/**
 * Serve-mode command. Kept separate from the dispatcher so GUI startup
 * still has the exact same RCODE_READY handshake behavior.
 */
async function serveMain(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.ok) {
    process.stderr.write(`Rcode serve: ${parsed.message}\n`)
    if (parsed.issues) {
      process.stderr.write(`${JSON.stringify(parsed.issues, null, 2)}\n`)
    }
    return parsed.exitCode
  }
  let handle: RcodeServeHandle | null = null
  installServeCrashHandlers(() => handle)
  const server = await startRcodeServe(parsed.options)
  handle = server
  ...
  const startupInfo = {
    ...
    pid: info.pid,
    message: `Rcode runtime listening on http://${server.host}:${server.port}`
  }
  process.stdout.write(`${RCODE_READY_PREFIX}${JSON.stringify(startupInfo)}\n`)
...
/**
 * When the GUI launches Rcode without `ELECTRON_RUN_AS_NODE` (the host
...
async function selfVerifyHealth(host: string, port: number): Promise<void> {
  ...
  process.stderr.write(
    `[Rcode] warning: self-health-probe on http://${host}:${port}/health did not pass within ${SELF_VERIFY_TIMEOUT_MS}ms — proceeding anyway\n`
  )
}
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  const command = splitRcodeCliCommand(argv)
  if (command.command === 'help') {
    if (command.error) {
      process.stderr.write(`Rcode: ${command.error}\n`)
      process.stderr.write(RCODE_CLI_USAGE)
      return ServeExitCode.usage
    }
    process.stdout.write(RCODE_CLI_USAGE)
    return ServeExitCode.ok
  }
...
main(process.argv.slice(2)).then(
  ...
  (error) => {
    process.stderr.write(`Rcode serve: ${String(error)}\n`)
