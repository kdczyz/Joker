...
const RCODE_EXTENSION_CLI_SCHEMA_VERSION = 1
const DEFAULT_RCODE_VERSION = '0.1.0'
const DEFAULT_LOG_BYTES = 256 * 1024
const MAX_LOG_BYTES = 1024 * 1024

export const RCODE_EXTENSION_CLI_USAGE = `Rcode extension <command> [options]

Commands:
  create <directory>          Scaffold a node, webview, or React extension
  validate <path>            Validate a source directory or .Rcodex archive
  pack <directory>           Build a deterministic .Rcodex archive
  install <path>             Install a .Rcodex, development directory, or exact Index version
...
Pack/source-validation options:
  --output <path>            .Rcodex file or output directory
...
export function createExtensionCliServices(options: {
  ...
  const compatibility: ExtensionCompatibility = {
    RcodeVersion: options.RcodeVersion ?? DEFAULT_RCODE_VERSION,
...
/**
 * Runs argv following the `Rcode extension` prefix. The top-level CLI dispatcher
...
export async function runExtensionCommand(
  ...
  try {
    ...
    io.stderr.write(`Rcode extension: ${errorMessage(error)}\n`)
    io.stderr.write(RCODE_EXTENSION_CLI_USAGE)
    return ServeExitCode.usage
  }
  if (parsed.command === 'help' || parsed.flags.has('help')) {
    io.stdout.write(RCODE_EXTENSION_CLI_USAGE)
    return ServeExitCode.ok
  }
  if (!COMMANDS.has(parsed.command)) {
    io.stderr.write(`Rcode extension: unknown command: ${parsed.command}\n`)
    io.stderr.write(RCODE_EXTENSION_CLI_USAGE)
    return ServeExitCode.usage
  }
  ...
  try {
    ...
    const services = suppliedServices ?? createExtensionCliServices({
      dataDir: parsed.values.get('data-dir') ?? io.env?.RCODE_DATA_DIR,
    ...
    await ownedServices?.manager.shutdown().catch((error: unknown) => {
      io.stderr.write(`Rcode extension: shutdown failed: ${errorMessage(error)}\n`)
    })
  }
}
...
async function runValidate(
  ...
  if (details.isFile()) {
    const inspection = await inspectRcodexArchive(path, { compatibility: services.compatibility })
    writeResult(parsed, io, 'Extension archive is valid', projectInspection(inspection))
    return ServeExitCode.ok
  }
  if (!details.isDirectory()) throw new ExtensionError('EXTENSION_PACKAGE_SOURCE_INVALID', 'Validation path must be a directory or .Rcodex file')
  ...
  const temporary = await mkdtemp(join(tmpdir(), 'Rcode-extension-validate-'))
  try {
    const inspection = await packRcodex(path, join(temporary, 'validation.Rcodex'), {
...
async function runPack(
  ...
  const defaultName = `${extensionId(development.manifest)}-${development.manifest.version}.Rcodex`
  ...
  const inspection = await packRcodex(source, output, {
    compatibility: services.compatibility,
...
async function runInstall(
  ...
  const [input] = parsed.positionals
  if (input === undefined) throw usageError('install requires a .Rcodex path or --development/--index')
  const archivePath = resolveFromCwd(input, io)
  const inspection = await inspectRcodexArchive(archivePath, { compatibility: services.compatibility })
  const id = extensionId(inspection.manifest)
  requirePermissionAcceptance(parsed, io, id, inspection.manifest.permissions)
...
async function runList(
  ...
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: RCODE_EXTENSION_CLI_SCHEMA_VERSION, revision: registry.revision, extensions })
  } else if (extensions.length === 0) {
    io.stdout.write('No Rcode extensions installed.\n')
  } else {
...
async function runDoctor(
  ...
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: RCODE_EXTENSION_CLI_SCHEMA_VERSION, healthy, diagnostics })
...
async function runLogs(
  ...
  if (parsed.json) {
    writeJson(io.stdout, {
      schemaVersion: RCODE_EXTENSION_CLI_SCHEMA_VERSION,
      extensionId: id,
      logPath,
...
function projectInspection(inspection: Awaited<ReturnType<typeof inspectRcodexArchive>>) {
  return {
    ...
    enginesRcode: inspection.manifest.engines.Rcode,
...
function resolvePackOutput(output: string, defaultName: string): string {
  return extname(output).toLowerCase() === '.Rcodex' ? output : join(output, defaultName)
}

async function loadScaffolder(): Promise<NonNullable<ExtensionCliIo['scaffold']>> {
  const packageName = 'create-Rcode-extension'
  try {
    ...
    throw new ExtensionError(
      ...
      'create-Rcode-extension is not installed with this Rcode distribution',
      {},
      { cause: error }
    )
  }
...
function writeResult(parsed: ParsedArguments, io: ExtensionCliIo, message: string, result: unknown): void {
  if (parsed.json) {
    writeJson(io.stdout, { schemaVersion: RCODE_EXTENSION_CLI_SCHEMA_VERSION, result: redactSecrets(result) })
  } else {
...
function writeCommandError(parsed: ParsedArguments, io: ExtensionCliIo, error: unknown): void {
  const normalized = normalizeCliError(error)
  if (parsed.json) {
    io.stderr.write(`${JSON.stringify({ schemaVersion: RCODE_EXTENSION_CLI_SCHEMA_VERSION, error: normalized })}\n`)
  } else {
    io.stderr.write(`Rcode extension ${parsed.command}: ${normalized.code}: ${normalized.message}\n`)
