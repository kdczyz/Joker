...
import { RCODE_SYSTEM_PROMPT } from '../prompt/Rcode-system-prompt.js'
...
export type RcodeServeRuntimeOptions = {
  host: string
  port: number
  configPath?: string
  dataDir: string
  /** Product-owned catalog of default local .Rcodex packages. */
  ...
  capabilities?: RcodeCapabilitiesConfig
...
export type RcodeServeHandle = NodeHttpServerHandle & {
 ...
 */
export async function createRcodeServeRuntime(
  options: RcodeServeRuntimeOptions
): Promise<ServerRuntime> {
  await mkdir(options.dataDir, { recursive: true, mode: 0o700 })
  let activeOptions: RcodeServeRuntimeOptions = { ...options }
  ...
  let prefix = createImmutablePrefix({
    systemPrompt: RCODE_SYSTEM_PROMPT,
    pinnedConstraints: [
  ...
  const migrateLegacyProviderCredentials = async (): Promise<void> => {
    const sources = [
      ...(activeOptions.apiKey.trim() && !activeOptions.credentialSourceId ? [{
        ...
        providerId: 'default',
        providerName: 'Rcode default provider',
  ...
  const withBackgroundShellTools = (
    tools: LocalTool[],
    optionsForTools: RcodeServeRuntimeOptions = activeOptions
  ): LocalTool[] => {
  ...
  // subscription set as the main model). Rcode-process signals that via env so we
  // route default-provider turns to the SDK too, not just per-provider ones.
	  const defaultIsAgentSdk = process.env.RCODE_RUNTIME_PROVIDER_KIND === 'agent-sdk'
	  ...
	  if (agentSdkProviderIds.size > 0 || defaultIsAgentSdk) {
	    sdkRuntimeDeps = {
	      ...
	      ...(attachmentStore ? { attachmentStore } : {}),
	      ...(memoryStore ? { memoryStore } : {}),
	      ...(process.env.RCODE_CLAUDE_BINARY
	        ? { pathToClaudeCodeExecutable: process.env.RCODE_CLAUDE_BINARY }
	        : {})
	    }
	  }
  ...
  stopThreadAuxiliaryWork = async (threadId) => {
    ...
    nowIso,
	  ...
	  const extensionMediaProcesses = new ExtensionMediaProcessService({
	    ...
	    ...(process.env.RCODE_FFPROBE_PATH ? { ffprobePath: process.env.RCODE_FFPROBE_PATH } : {}),
	    ...(process.env.RCODE_FFMPEG_PATH ? { ffmpegPath: process.env.RCODE_FFMPEG_PATH } : {})
	  })
...
): Promise<RcodeServeRuntimeOptions> {
...
function agentSdkProviderIdsForOptions(options: RcodeServeRuntimeOptions): Set<string> {
...
function agentSdkProviderSignature(options: RcodeServeRuntimeOptions): string {
  return [...agentSdkProviderIdsForOptions(options)].sort().join('\n')
}

function mergeRuntimeConfigApplyOptions(
  current: RcodeServeRuntimeOptions,
  request: RuntimeConfigApplyRequest
): RcodeServeRuntimeOptions {
...
function tokenEconomyConfigForOptions(
  options: Pick<RcodeServeRuntimeOptions, 'tokenEconomyMode' | 'tokenEconomy'>
): TokenEconomyConfig {
  return {
...
function toolOutputLimitsForOptions(
  options: Pick<RcodeServeRuntimeOptions, 'toolOutputLimits'>
...
function builtinToolOptionsForOptions(options: RcodeServeRuntimeOptions) {
...
export async function startRcodeServe(
  options: RcodeServeRuntimeOptions
): Promise<RcodeServeHandle> {
  ...
  // progress" forever with nothing running (kdczyz/Rcode#370).
  void runtime.turnService
    .reconcileOrphanedTurns()
    .then(async (threadIds) => {
      if (threadIds.length > 0) {
        console.warn(`[Rcode] marked orphaned turn(s) on ${threadIds.length} thread(s) as failed after restart`)
      }
      if (threadIds.length > 0 && runtime.resumeInterruptedGoals) {
        const resumed = await runtime.resumeInterruptedGoals(threadIds)
        if (resumed > 0) {
          console.warn(`[Rcode] auto-resumed ${resumed} interrupted goal(s) after restart`)
        }
      }
    })
    .catch((error) => {
      console.warn('[Rcode] orphaned turn reconciliation failed:', error)
    })
  ...
  void runtime.delegationRuntime
    ...
    .then((count) => {
      if (count > 0) {
        console.warn(`[Rcode] marked ${count} orphaned subagent run(s) as failed after restart`)
      }
    })
    .catch((error) => {
      console.warn('[Rcode] orphaned child-run reconciliation failed:', error)
    })
  return {
    ...server,
    runtime,
