...
describe('resolveEventLoopStallThresholdMs', () => {
  ...
  it('honors a positive override', () => {
    expect(resolveEventLoopStallThresholdMs({ RCODE_EVENT_LOOP_STALL_LOG_MS: '5000' })).toBe(5_000)
  })

  it('ignores garbage and non-positive overrides', () => {
    expect(resolveEventLoopStallThresholdMs({ RCODE_EVENT_LOOP_STALL_LOG_MS: 'abc' })).toBe(2_000)
    expect(resolveEventLoopStallThresholdMs({ RCODE_EVENT_LOOP_STALL_LOG_MS: '0' })).toBe(2_000)
    expect(resolveEventLoopStallThresholdMs({ RCODE_EVENT_LOOP_STALL_LOG_MS: '-5' })).toBe(2_000)
  })
})
