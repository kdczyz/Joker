...
export const EXTENSION_SESSION_ID_HEADER = 'x-Rcode-extension-session-id'
export const EXTENSION_SESSION_NONCE_HEADER = 'x-Rcode-extension-session-nonce'
  ...
  const extensions = Object.values(registry.extensions)
    ...
    .flatMap((entry) => {
      ...
      const compatible = compatibility.api.compatible &&
        compatibility.RcodeEngine.compatible &&
...
class ProtectedMediaOperationTokenRegistry {
  ...
  consume(token: string): void {
    ...
    if (this.consumed.size >= 65_536) {
      throw new ExtensionBrokerError(
        'conflict',
        'Protected media operation capacity was reached; restart Rcode before retrying'
      )
    }
...
async function resolveViewTarget(
  ...
  try {
    ...
    throw new ExtensionViewSessionError('not_found', 'Extension view is incompatible with this Rcode version')
  }
