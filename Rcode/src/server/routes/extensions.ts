 ...
 *
 * This is deliberately separate from `buildRouter` so the Rcode composition root
...
async function inspectExtension(
  ...
  const inspection = await inspectRcodexArchive(body.data.path, runtime.validation)
  return jsonResponse({
    schemaVersion: 1,
