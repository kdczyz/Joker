...
import { parseExtensionManifest } from '@Rcode/extension-api'
import {
  ExtensionPaths,
  ExtensionRegistry,
...
describe('extension public routes', () => {
  ...
  it('derives a core-bound view identity and never accepts a forged or runtime guest credential', async () => {
    ...
    expect(created.body.src).toBe('Rcode-extension://acme.dashboard/webview/index.html')
...
async function createFixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'Rcode-extension-public-routes-'))
