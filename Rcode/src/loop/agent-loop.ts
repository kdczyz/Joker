...
import type { RolesConfig } from '../config/Rcode-config.js'
...
export type AgentLoopOptions = {
  ...
  /**
   * Tuning + test seams for goal auto-resume (kdczyz/Rcode#370). Defaults
   ...
   * Agent SDK instead of Rcode's own model loop, billing the user's Claude
   * subscription. Rcode's tools/persona/permissions are injected into the SDK.
   */
...
export class AgentLoop {
  ...
  */
  ...
  private async runTurnOwned(threadId: string, turnId: string): Promise<TurnExecutionStatus> {
    ...
    // the user's subscription; Rcode's brain is injected). All other providers
    // fall through to Rcode's native loop below.
    ...
    try {
      ...
      if (delegatedSdkRuntime) {
        // The delegated SDK owns its model stream and cannot consume Rcode's
      ...
      // instead of the bare "Rcode turn failed" string. See issue #26.
  ...
  static defaultPrefix(): ImmutablePrefix {
    return createImmutablePrefix({
      systemPrompt: 'You are Rcode, a careful and helpful assistant.',
