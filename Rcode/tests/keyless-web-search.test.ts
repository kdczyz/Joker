...
describe('KeylessSearchExecutor', () => {
  const ddgUrl = DUCKDUCKGO_ENGINE.requestUrl('rcode')
  const bingUrl = BING_ENGINE.requestUrl('rcode')
  ...
  it('serves repeat queries from the TTL cache', async () => {
    ...
    const request = { query: 'Rcode', limit: 5, timeoutMs: 1000, signal: new AbortController().signal }
