...
describe('Rcode agent CLI commands', () => {
  let dataDir = ''

  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'Rcode-cli-'))
  })
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitRcodeCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitRcodeCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitRcodeCliCommand(['nope']).error).toMatch(/unknown command/)
  })

  it('lists tools from Rcode exec with JSON output', async () => {
    const c = capture()
  ...
  it('invokes a direct tool through Rcode exec', async () => {
    const c = capture()
    const code = await runAgentCommand('exec', [
      '--data-dir',
      dataDir,
  ...
  it('lists dynamic runtime tools from Rcode exec', async () => {
  ...
  it('shares config loading between serve and agent commands', async () => {
    const configPath = join(dataDir, 'Rcode.config.json')
