...
describe('extension runtime compatibility admission', () => {
  it('negotiates current and earlier minors and rejects future minor, future major, and engine mismatches', () => {
    ...
    expect(() => assertManifestCompatible(
      manifestFor({ apiVersion: '1.2.0', enginesRcode: '>=0.2.0' }),
      minorPolicy
  ...
  it('re-admits installed versions before activation, selection, and rollback', async () => {
    const root = await temporaryRoot('Rcode-extension-compat-installed-')
    ...
    const v1Archive = join(root, 'v1.Rcodex')
    await packRcodex(source, v1Archive, { compatibility: installPolicy })
    ...
    const v2Archive = join(root, 'v2.Rcodex')
    await packRcodex(source, v2Archive, { compatibility: installPolicy })
  ...
  it('re-admits mutable development manifests against current API and engine policy', async () => {
    const root = await temporaryRoot('Rcode-extension-compat-development-')
    const paths = new ExtensionPaths({
      packageRoot: join(root, 'extensions'),
      dataRoot: join(root, 'data')
    })
    ...
    const newerRcode = new ExtensionPackageManager(paths, registry, {
      compatibility: policy({ apiVersion: '1.0.0', RcodeVersion: '0.2.0' })
    })
    await newerRcode.registerDevelopment(engineSource, { grantedPermissions: [] })
...
function policy(options: { apiVersion: string; RcodeVersion?: string }): ExtensionCompatibility {
  return {
    RcodeVersion: options.RcodeVersion ?? '0.1.0',
    supportedManifestVersions: [1],
...
function manifestFor(options: {
  ...
  return parseExtensionManifest({
    ...
    engines: { Rcode: options.enginesRcode ?? '*' },
    main: 'dist/main.mjs',
