...
describe('HTTP server', () => {
  let dataDir = ''
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'Rcode-http-'))
  })
  afterEach(async () => {
  ...
  it('returns 200 on /health without auth', async () => {
    ...
    expect(body).toEqual({ status: 'ok', service: 'Rcode', mode: 'serve' })
  })
    ...
    const decide = await dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_1', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: consent('allow')
        },
        body: JSON.stringify({ decision: 'allow' })
      })
    )
    ...
    const conflict = await dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_1', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: consent('deny')
        },
    ...
    const request = dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_audited', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: approvalConsent('appr_audited', 'allow')
        },
        body: JSON.stringify({ decision: 'allow' })
      })
    )
    ...
    await expect(dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_audit_failure', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: approvalConsent('appr_audit_failure', 'allow')
        },
        body: JSON.stringify({ decision: 'allow' })
      })
    ...
    const makeRequest = () => dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_same_decision', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: approvalConsent('appr_same_decision', 'allow')
        },
        body: JSON.stringify({ decision: 'allow' })
      })
    )
    ...
    const request = (decision: 'allow' | 'deny') => dispatchRequest(
      ...
      new Request('http://localhost/v1/approvals/appr_opposite_decision', {
        ...
        headers: {
          authorization: 'Bearer tok-1',
          'content-type': 'application/json',
          [RCODE_APPROVAL_CONSENT_HEADER]: approvalConsent('appr_opposite_decision', decision)
        },
  ...
  it('resumes a persisted session into a new Rcode thread', async () => {
    const h = buildHarness()
    await h.threadService.create(
      { workspace: '/tmp/project', model: 'deepseek-chat', mode: 'agent' },
