...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'Rcode-skills-'))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'Rcode-skill-real-'))
    try {
  ...
  it('rejects a skill entry symlinked outside its package', async (ctx) => {
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-skill-secret-'))
    try {
  ...
  it('rejects workspace-discovered skill packages symlinked outside the workspace', async (ctx) => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-skill-workspace-boundary-'))
    const external = await mkdtemp(join(tmpdir(), 'Rcode-skill-external-'))
    try {
      await writeSkillAt(external, 'outside', {
        id: 'outside', name: 'Outside', triggers: { commands: ['/outside'] }
      }, 'PRIVATE KEY MATERIAL')
      await mkdir(join(workspace, '.Rcode', 'skills'), { recursive: true })
      try {
        await symlink(join(external, 'outside'), join(workspace, '.Rcode', 'skills', 'outside'), 'dir')
      } catch {
        ctx.skip()
        return
      }
  ...
  it('scopes the dynamic skill catalog to the current workspace', async () => {
    const workspaceA = await mkdtemp(join(tmpdir(), 'Rcode-skill-workspace-a-'))
    const workspaceB = await mkdtemp(join(tmpdir(), 'Rcode-skill-workspace-b-'))
    try {
      ...
      const disabledConfig = RcodeCapabilitiesConfig.parse({
        skills: {
          enabled: true,
          roots: [rootA],
          workspaceRoots: [workspaceA, workspaceB],
  ...
  it('discovers workspace-local Rcode skills from .Rcode/skills', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-skill-workspace-Rcode-'))
    try {
      const skillRoot = join(workspace, '.Rcode', 'skills')
