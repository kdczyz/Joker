...
describe('Rcode built-in tools', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Rcode-tools-'))
    backgroundShellDataDir = await mkdtemp(join(tmpdir(), 'Rcode-bg-shell-data-'))
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello Rcode' },
    ...
    expect(editedDisk).toContain('hello Rcode')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello Rcode')
