    ...
    const todos = ThreadTodoListSchema.parse({
      ...
      items: [{
        ...
        source: {
          kind: 'plan',
          planId: 'plan_1',
          relativePath: '.Rcodesdd/plan/plan.md',
          ordinal: 0,
          contentHash: 'abc'
        },
        createdAt: '2026-06-03T00:00:00.000Z',
...
describe('cli', () => {
  ...
  it('accepts the product-owned bundled extension directory from CLI or environment', () => {
    expect(parseServeOptions([
      '--data-dir=/srv/ca',
      '--bundled-extensions-dir=/opt/Rcode/bundled-extensions'
    ]).bundledExtensionsDir).toBe('/opt/Rcode/bundled-extensions')
    expect(parseServeOptions(['--data-dir=/srv/ca'], {
      RCODE_BUNDLED_EXTENSIONS_DIR: '/Applications/Rcode/resources/bundled-extensions'
    }).bundledExtensionsDir).toBe('/Applications/Rcode/resources/bundled-extensions')
  })
  ...
  it('applies CLI and standard OTLP environment precedence over config', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'Rcode-observability-config-'))
    try {
      const configPath = join(dir, 'Rcode.config.json')
  ...
  it('loads serve and context compaction settings from an explicit config file', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'Rcode-config-'))
    try {
      const configPath = join(dir, 'Rcode.config.json')
      await writeFile(configPath, JSON.stringify({
        serve: {
  ...
  it('rejects a defaultProfile that is not defined in profiles', () => {
    expect(() => RcodeCapabilitiesConfig.parse({
  ...
  it('surfaces subagent profiles and policy in the runtime capability manifest', () => {
    const manifest = buildRuntimeCapabilityManifest({
      ...
      config: RcodeCapabilitiesConfig.parse({
        subagents: {
          enabled: true,
          maxParallel: 2,
          maxChildRuns: 6,
  ...
  it('loads config.json from the data dir when present', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'Rcode-data-'))
    try {
  ...
  it('exposes a usage string', () => {
    expect(SERVE_USAGE).toContain('Rcode serve')
  })
