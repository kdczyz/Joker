...
import { DEFAULT_RCODE_MODEL } from '../src/config/Rcode-config.js'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-todos-'))
    try {
      const relativePath = '.Rcodesdd/plan/demo.md'
  ...
  it('refuses to patch a plan path that escapes through a symlink', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'Rcode-todos-workspace-'))
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-todos-outside-'))
    try {
      const relativePath = '.Rcodesdd/plan/demo.md'
      const planDir = join(workspace, '.Rcodesdd', 'plan')
...
describe('ThreadService.resumeSession', () => {
  it('uses the Rcode default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_RCODE_MODEL)
  })
})
