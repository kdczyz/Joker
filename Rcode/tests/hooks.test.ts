...
describe('command hooks', () => {
  ...
  it('does not expose runtime credentials to command hooks', async () => {
    const previousRuntimeToken = process.env.RCODE_RUNTIME_TOKEN
    const previousApiKey = process.env.DEEPSEEK_API_KEY
    process.env.RCODE_RUNTIME_TOKEN = 'runtime-secret'
    process.env.DEEPSEEK_API_KEY = 'model-secret'
    try {
      const hooks = resolveConfiguredHooks([
        {
          phase: 'UserPromptSubmit',
          command: `node -e "console.log([process.env.RCODE_RUNTIME_TOKEN || 'missing', process.env.DEEPSEEK_API_KEY || 'missing'].join('|'))"`
        }
      ])
      const outcome = await runUserPromptSubmitHooks(hooks, {
        threadId: 'th',
      ...
      if (previousRuntimeToken === undefined) delete process.env.RCODE_RUNTIME_TOKEN
      else process.env.RCODE_RUNTIME_TOKEN = previousRuntimeToken
