...
describe('scanThreadStore replacement detection', () => {
  ...
  it('reports changed when the path is atomically replaced after handle fstat', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-race-'))
    roots.push(root)
  ...
  it('invalidates the report when an earlier artifact changes during a later read', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-stability-'))
    roots.push(root)
    const thread = createThreadRecord({
  ...
  it('invalidates the report when an inspected symlink becomes a regular file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-symlink-race-'))
    roots.push(root)
    const thread = createThreadRecord({
  ...
  it('tracks an unreadable file identity and detects its later replacement', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-unreadable-race-'))
    roots.push(root)
    const thread = createThreadRecord({
  ...
  it('ignores unrelated sibling data-directory activity during the scan', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-unrelated-'))
    roots.push(root)
    const thread = createThreadRecord({
  ...
  it('ignores normal background-shell, legacy, and unrelated attachment activity', async () => {
    const root = await mkdtemp(join(tmpdir(), 'Rcode-thread-store-doctor-normal-activity-'))
    roots.push(root)
    const thread = createThreadRecord({
