...
function buildGuiPlan(relativePath = '.Rcodesdd/plan/login.md', operation: 'draft' | 'refine' = 'draft') {
  return {
    operation,
...
describe('create_plan tool: path validation', () => {
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# nested',
        operation: 'draft',
        plan_relative_path: '.Rcodesdd/plan/nested/a.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.Rcodesdd/plan/nested/a.md')
      })
    )
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# not md',
        operation: 'draft',
        plan_relative_path: '.Rcodesdd/plan/login.txt'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.Rcodesdd/plan/login.txt')
      })
    )
    expect(result.isError).toBe(true)
  })
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# mismatch',
        operation: 'refine',
        plan_relative_path: '.Rcodesdd/plan/other.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: {
          operation: 'refine',
          workspaceRoot: '/tmp/ws',
          relativePath: '.Rcodesdd/plan/login.md',
          planId: 'plan_login'
        }
      })
    )
  ...
  it('rejects an operation that differs from the active GUI plan operation', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.Rcodesdd/plan/login.md', 'refine')
      })
    )
    expect(result.isError).toBe(true)
...
describe('create_plan tool: execution safety', () => {
  ...
  it('rejects a free-form legacy plan path as a new target', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(JSON.stringify(result.output)).toMatch(/\.Rcodesdd\/plan/)
  })
  ...
  it('still executes create_plan through the tool host during plan mode', async () => {
    ...
    expect(result.item.kind === 'tool_result'
      ...
      : ''
    ).toBe('.Rcodesdd/plan/safe-plan.md')
  })
})
...
describe('create_plan tool: success and atomic write', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Rcode-plan-'))
    ...
    await mkdir(join(workspace, '.Rcodesdd/plan'), { recursive: true })
    await writeFile(join(workspace, '.Rcodesdd/plan/login.md'), previousMarkdown, 'utf8')
  })
  ...
  it('writes a fresh plan to the reserved path and returns structured metadata', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          ...
          relativePath: '.Rcodesdd/plan/login.md',
          planId: `${workspace}:.Rcodesdd/plan/login.md`,
    ...
    expect(output.relative_path).toBe('.Rcodesdd/plan/login.md')
    expect(output.operation).toBe('draft')
    expect(output.summary).toContain('.Rcodesdd/plan/login.md')
    expect(output.content_hash).toMatch(/^[a-f0-9]{16}$/)
    expect(output.byte_size).toBe(Buffer.byteLength('# Login plan\n\n- step 1', 'utf8'))
    expect(output.absolute_path).toBe(join(workspace, '.Rcodesdd/plan/login.md'))
  ...
  it('rejects a reserved plan directory symlink that escapes the workspace', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-plan-outside-'))
    try {
      await rm(join(workspace, '.Rcodesdd'), { recursive: true, force: true })
      await symlink(outside, join(workspace, '.Rcodesdd'), process.platform === 'win32' ? 'junction' : 'dir')
      ...
      const result = await executeCreatePlanTool(
        ...
        buildContext({
          ...
          guiPlan: {
            operation: 'draft',
            workspaceRoot: workspace,
            relativePath: '.Rcodesdd/plan/escape.md',
            planId: 'plan_escape'
          }
        })
      )
  ...
  it('self-allocates a non-colliding path when plan mode has no reserved context', async () => {
    ...
    // `.Rcodesdd/plan/login.md` already exists in this workspace.
    expect(output.relative_path).toBe('.Rcodesdd/plan/login-2.md')
    const persisted = await readFile(output.absolute_path, 'utf8')
  ...
  it('leaves the previous plan untouched when the abort signal fires before rename', async () => {
    ...
    expect(result.isError).toBe(true)
    const persisted = await readFile(join(workspace, '.Rcodesdd/plan/login.md'), 'utf8')
