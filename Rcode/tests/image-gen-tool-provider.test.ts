...
function imageGenConfig(overrides: Record<string, unknown> = {}) {
  return RcodeCapabilitiesConfig.parse({
    imageGen: {
      enabled: true,
...
function attachmentStore(rootDir: string, overrides: Record<string, unknown> = {}) {
  return new FileAttachmentStore({
    rootDir,
    config: RcodeCapabilitiesConfig.parse({ attachments: { enabled: true, ...overrides } }).attachments,
...
describe('Image gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'Rcode-imagegen-'))
  })
  ...
  it('rejects generated-image writes through an escaping workspace symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-imagegen-outside-'))
    try {
  ...
  it('does not read a reference image through an escaping workspace symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'Rcode-imagegen-reference-outside-'))
    try {
